...
function attach2()
	{
	var name = "NULL";
	var inp = document.getElementById('fileElementId1');
...
<div>
 ...
 {{if $error}}<div align="center" style="color: #FF0000; font-size:15px;">{{$error}}</div>{{/if}}
    ...
    <td width="80%" style="padding-left:20px;">
      <table width="80%" cellpadding="0" cellspacing="0" id="tasklisttable">
        <tr><th>Variables: </th><td>{{foreach from=$email_temp_vars_buyers key="key" item="item" }}&nbsp;<input type="text" readonly="readonly" value="{{$item}}" />&nbsp;{{/foreach}}</td></tr>
         <tr><th>Subject: </th><td><input type="text" name="{{$TABLE}}[subject]" style="width:620px;" value="{{$subject}}" /></td></tr>
          <tr><th>Message: </th><td> <textarea cols="110" id="editor1"  name="{{$TABLE}}[email_body]" rows="10" class="mceEditor">{{$body}}</textarea> 
       <!--   <table id="none-table22"><tr><td>{{tinymce InstanceName="$TABLE[email_body]" class="email_body" Value=$body Cols="140"}}</td></tr></table>-->
         <!-- <table id="none-table22"><tr><td> <textarea cols="110" id="editor1"  name="$TABLE[email_body]" rows="10" class="mceEditor">{{$detail.es_signature}}</textarea> </td></tr></table>-->
          </td></tr> 
        <tr><th>Attachment: </th><td>
          <div id="emailattachment">
          		{{foreach from=$checklistbusdocs item="item" key="key"}}
                <div><input type="checkbox" name="{{$TABLE}}[attachments][]" value="{{$BASE_URL}}{{$FILE_PATH}}files/document_check_list_files/{{$item.doc_file_name}}" checked="checked" />&nbsp;
                <a href="javascript:parent.window.open('{{$BASE_URL}}{{$FILE_PATH}}files/document_check_list_files/{{$item.doc_file_name}}','mywindow');">{{$item.admin_doc_name}}</a></div>
                {{/foreach}} <br />
                {{foreach from=$customdocs item="item" key="key"}}
                <div><input type="checkbox" name="{{$TABLE}}[attachments][]" value="{{$BASE_URL}}{{$FILE_PATH}}files/document_check_list_files/{{$item.cdc_file_name}}" checked="checked" />
               &nbsp;<a href="javascript:parent.window.open('{{$BASE_URL}}{{$FILE_PATH}}files/document_check_list_files/{{$item.cdc_file_name}}','mywindow');" target="_top">{{$item.cdc_doc_name}}</a></div>
                {{/foreach}}
                 {{foreach from=$attached item="item" key="key"}}
                <div><input type="checkbox" name="{{$TABLE}}[attachments][]" value="{{$BASE_URL}}{{$FILE_PATH}}files/document_check_list_files/{{$item.cdc_doc_name}}" checked="checked" />
               &nbsp;<a href="javascript:parent.window.open('{{$BASE_URL}}{{$FILE_PATH}}files/document_check_list_files/{{$item.cdc_doc_name}}','mywindow');" target="_top">{{$item.cdc_doc_name}}</a></div>
                {{/foreach}}
      ...
      <tr> <th> 
    <!--  <input type="submit" value="Load Aproval Document" name="load_apr_doc" id ="load_apr_doc"  />-->
     ...
     <input type = "checkbox" name="{{$TABLE}}[attachments][]" value="{{$BASE_URL}}{{$FILE_PATH}}files/business_documents/{{$item.bpd_doc_name}}"  /> {{$item.bpd_type}} 
      ...
      {{/foreach}}
       ...
       <tr><th>Select File:</th><td><input type="file" name="eattach[]" id="fileElementId1" multiple />&nbsp;<input type="button" onclick="javascript:attach2();" value="Attach" name="attach" /></td></tr>       
