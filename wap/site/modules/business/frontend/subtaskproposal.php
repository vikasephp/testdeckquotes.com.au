<?php
$fwMainView = 'file:' . getcwd() . '/subtaskproposal.tpl';
...
$sql = "SELECT * FROM proposal_tasks_sub where bts_bsn_id = ".$bus_id. " And bts_task_id = ".$tid .  " And bts_task_list_number = ".$tn;
...
if(!empty($fwViewData['subtaskdata'])) {
 ...
 $sqlLp = "select proposal_task_list_subtask_lp.* from proposal_task_list_subtask_lp where bstlp_bst_id = ".$tid; 
...
if(!empty($submit))
{
	...
	$bustasksub = new Fw_Db_Table('proposal_tasks_sub');
