...
<tbody>
 ...
 <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
    <td>{{counter}}</td>
    <td>{{$item.qa_question|truncate:100:"...":true}}</td> 
    <td> {{$item.qa_answer|truncate:100:"...":true}} </td>
    <td>{{if $item.qa_attachment}}<a href = "{{$BASE_URL}}{{$FILE_PATH}}files/question_answer_doc/{{$item.qa_attachment}}" target="_blank">
