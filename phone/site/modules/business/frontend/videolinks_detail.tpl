...
{{if $msg != 0}}
<script type="text/javascript">
	parent.requestvideolinksInfo(1);
   </script>
<script type="text/javascript">
...
{{/if}}
   {{if $msg != 0}}
<script type="text/javascript">
	parent.toggleLayer('divvideolinks','addvideo');
	closeTop();
   </script>
{{/if}}
</head>
<table width="100%" id="none-table" style="background-color:#FFFFFF;">
  <form name="detail" method="post" action="{{$BASE_URL}}business.videolinks_detail" enctype="multipart/form-data" onsubmit="showTop();return false;">
    <tr>
    ...
    <input type="hidden" name="video[vl_bsn_id]" value="{{$bsn_id}}" id="edt_id"/>
      
          <tr>
            <td>Enter Video Link:</td>
            <th colspan="3">
            <input type="text" class="xxlrg" name="video[vl_video_link]" value="{{$detail.vl_video_link}}" id="dt_name" style="width:400px;"/>
            </th>
          </tr>
    ...
    <tr>
      <td colspan="2"><div align="left">
          <input type="submit" name="subAddDetail" value="Save" class="sml"/>&nbsp;
          <input type="button" name="cancel" value="Cancel" onclick="javascript:parent.add_edit_video_links();" class="sml"/>
          </div></td>
    </tr>
  </form>
</table>
