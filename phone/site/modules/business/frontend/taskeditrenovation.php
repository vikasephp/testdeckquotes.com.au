<?php
$fwMainView = 'file:' . getcwd() . '/taskeditrenovation.tpl';
$bus_id = $fwRequest->getparam('bus_id', 0);
$taskId = $fwRequest -> getParamget('taskId','');
$fwViewData['tn'] = $fwRequest -> getParamget('tn','');
...
{		
	...
	if($complete == '0')
	{
		$sql_update = "UPDATE renovation_tasks
							SET  bt_complete = 0,
							bt_completed_date=0,
							bt_completed_by=0,
							bt_comment=''
	...
	elseif($complete == '1')
	{
		...
		$sql_update = "UPDATE renovation_tasks SET bt_complete = 1,
	...
	$tablebtasks = new Fw_Db_Table('renovation_tasks');
	$tablebtasks->setWhere("bt_task_types_id > 0 AND bt_id = ".$taskId);
	if(!$tablebtasks->rowExists())
		{
		$sql_tasks = "	SELECT bt.*, bst.bst_task_name, bst.bst_document, bst.bst_instruction,
					bst.bst_id, cs.customer_name
		...
		$sql_tasks = "	SELECT 	bt.*, bs.pt_name, bst.bst_task_name,
					bst.bst_document, bst.bst_instruction,	bst.bst_id, cs.customer_name
					FROM renovation_tasks as bt
