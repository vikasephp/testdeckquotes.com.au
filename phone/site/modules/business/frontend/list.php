				...
				foreach($bcs_data as $k1 => $v1)
						{
							...
							if($v1 == end($bcs_data))
							{$sellers .= $v1['bcust_fname']." ".$v1['bcust_lname'];}else{
...
foreach($data_wsexport as $k => $v) {
		$listdata[]= '<table class="first" cellpadding="4" cellspacing="2">
...
foreach($data_wsexport as $key1 => $val1)
							...
							if($id == $val1['st_id'])
								{
								if($val1['task_cnt'] !== '0/0'){$task_cnt = $val1['task_cnt'];}
								if($val1['doc_cnt'] !== '0/0'){$doc_cnt = $val1['doc_cnt'];}
...
<tr><td colspan="5" class="second" width="662">&nbsp;</td></tr>
<tr>
...
if(!empty($submit))
{
  ...
  //if($detail['seller_name']){$string.= "bus_customers.bcust_fname LIKE '%".trim($detail['seller_name'])."%' OR bus_customers.bcust_lname LIKE '%".trim($detail['seller_name'])."%'";}
	...
	if($detail['seller_name']){ $seller_name = addslashes($detail['seller_name']); $string .= "bus_customers.bcust_fname LIKE '%".trim($seller_name)."%' AND ";}
	if($detail['bsn_name']){ $bsn_name = addslashes($detail['bsn_name']); $string.= "business.bsn_name LIKE '%".trim($bsn_name)."%' AND ";}
	if($detail['bsn_states']){$string.= $status_que;}
	if($detail['ind_codes']){$string.= $intr_que." AND ";}
	if($detail['bsn_readyforsale']){$string.= "business.bsn_readyforsale = ".trim($detail['bsn_readyforsale'])." AND "; 
	$fwViewData['bsn_readyforsale'] = $detail['bsn_readyforsale'];}
	if($detail['bsn_status']){$string.= $st_que." AND ";}
	if($detail['bsn_types']){$string.= $pt_que." AND ";}
	if($detail['bsn_cd_ask_price_from'] AND empty($detail['bsn_cd_ask_price_to'])){$string.= "business.bsn_cd_ask_price_from >= '".trim($detail['bsn_cd_ask_price_from'])."' AND ";}
	
	if($detail['bsn_sub_status3']){  $sdata[] = "'Open'"; }
	if($detail['bsn_sub_status1']){  $sdata[] = "'Lost'"; }	
	if($detail['bsn_sub_status2']){ $sdata[] = "'Hold'";}
	
	$sin = ($sdata)? implode(",",$sdata) : '';
	if($sin): $string.= " business.bsn_sub_status IN ($sin) AND "; endif;

	if(empty($detail['bsn_cd_ask_price_from']) AND $detail['bsn_cd_ask_price_to']){$string.= "business.bsn_cd_ask_price_to <= '".trim($detail['bsn_cd_ask_price_to'])."' AND ";}
	
	if($detail['bsn_cd_ask_price_from'] AND $detail['bsn_cd_ask_price_to']){$string.= "business.bsn_cd_ask_price_from >= '".trim($detail['bsn_cd_ask_price_from'])."' AND business.bsn_cd_ask_price_to <= '".trim($detail['bsn_cd_ask_price_to'])."' AND ";}

  if($detail['dpn_unique_id']) {  $string = " document_proposal_name.dpn_unique_id = " . $detail['dpn_unique_id'] . " AND "; }
...
$sql = "SELECT business.*, busness_status.*, project_type.*, business_sellers.*, bus_customers.bcust_id, bus_customers.bcust_fname, bus_customers.bcust_lname FROM business 
...
{
$sql = "SELECT  business.*, busness_status.*, project_type.*, business_sellers.* FROM business 
			LEFT JOIN busness_status ON business.bsn_status = busness_status.st_id 
			LEFT JOIN project_type ON business.bsn_type = project_type.pt_id 
			LEFT JOIN business_sellers ON business.bsn_id = business_sellers.bs_business_id
			"." ".$order;
...
if(empty($userData)){$fwViewData['search_msg'] = "No projects were found.";}
}
//db($userData);
//exit;
...
if($sql22){$sellerData = $fwDb->query($sql22);}
