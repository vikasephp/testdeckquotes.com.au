...
if(!empty($submit))
{
		...
		if(!$detail['bsn_addto_cpr']) { $detail['bsn_addto_cpr'] = 0 ; }
		if(!$detail['bsn_addto_complete']) { $detail['bsn_addto_complete'] = 0 ; }
		...
		if($detail['bsn_address_same']) { $detail['bsn_address_same'] = 1; } else { $detail['bsn_address_same'] = 0; }
		if($detail['bsn_unsubscribe_email']) { $detail['bsn_unsubscribe_email'] = 1; } else { $detail['bsn_unsubscribe_email'] = 0; }
		...
		if(empty($detail['bsn_auto_edossier'])){$detail['bsn_auto_edossier'] = 0;}

		/****Begin Change date format to be stored in database *****/
		if(!empty($detail['bsn_status_date'])){$detail['bsn_status_date'] = changedate_y_m_d($detail['bsn_status_date']); };
		if(!empty($detail['bsn_status_sys_date'])){$detail['bsn_status_sys_date'] = changedate_y_m_d($detail['bsn_status_sys_date']); };
		if(!empty($detail['bsn_date_listed_date'])){$detail['bsn_date_listed_date'] = changedate_y_m_d($detail['bsn_date_listed_date']); };
		if(!empty($detail['bsn_cd_date_appraised_date'])){$detail['bsn_cd_date_appraised_date'] = changedate_y_m_d($detail['bsn_cd_date_appraised_date']); };
		if(!empty($detail['bsn_cd_planing_sell_date'])){$detail['bsn_cd_planing_sell_date'] = changedate_y_m_d($detail['bsn_cd_planing_sell_date']); };
		if(!empty($detail['bsn_marketing_date'])){$detail['bsn_marketing_date'] = changedate_y_m_d($detail['bsn_marketing_date']); };
		if(!empty($detail['bsn_last_update_letter_date'])){$detail['bsn_last_update_letter_date'] = changedate_y_m_d($detail['bsn_last_update_letter_date']); };
	...
	if(!empty($table_id))
		{
			...
			if($detail['bsn_sub_status'] == 'Open')  { $detail['bsn_planning_moveto_old'] = 0; }
			if($detail['bsn_sub_status'] == 'Hold')  { $detail['bsn_planning_moveto_old'] = 1; }
			if($detail['bsn_sub_status'] == 'Lost')  { $detail['bsn_planning_moveto_old'] = 1; }
				...
				if(!empty($bsn_seller))
					{
						...
						if(!$tablecust_bus->rowExists()){ $temp = $tablecust_bus->insertRow($data); }					
					}
...
//
					...
					if(!$table_ppd_detail->rowExists())
					{
						foreach($payment_stage as $pk2 => $pv2)
						{
							...
							//$ppd['ppd_when_payrec'] = $pv2['ppd_when_payrec'];
...
if(!empty($delete_msg) AND $delete_msg>0){	$fwViewData['delete_msg'] = 'Project has been Deleted Successfully'; }
...
$totaled =0;
$totaled_hia = 0;
foreach($prog_val as $ppk => $ppv)
	{
		$v1 = str_replace("$", '',$ppv['ppd_value']);
		$v2 = str_replace(",", '',$v1);
		$totaled = $totaled + $v2;	
		
		$vh= str_replace("$", '',$ppv['ppd_hia_value']);
		$vh1 = str_replace(",", '',$vh);
		$totaled_hia = $totaled_hia + $vh1;
	}
...
if($detail['eml_ie_email'] == 'External')
	{
		if(send_email($toname,$to, $fromname, $from, $subject, $email_body, $attachmentsend))
		{
			...
			if(!empty($doclogattach)) { $logDetail['elog_doc_attached'] = implode(", ",$doclogattach); }
			if(!empty($templogattach)) { $logDetail['elog_temp_attached'] = implode(", ",$templogattach); }
...
if($sqlET){$userData = $fwDb->query($sqlET);}
...
if($sqlqalog){$qalogData = $fwDb->query($sqlqalog);}
...
$_SESSION['sqlqalog'] = $sqlqalog;
	...
	if (!(isset($pagenum)))
		...
		if($sql22){$fwViewData['listqalog']= $fwDb->query($sql22);}
		...
		if($_FILES['doc2_'.$kprm])
			{
					...
					$destination = BASE_DIR.'files/progress_payment_doc/'.$docfile_2;
...
if($bsn_id) {
	...
	if(!empty($taskdata)) {
		...
		$submit_nr = $fwRequest->getParam('not_required_save', '');
		...
		if(!empty($submit_nr))
		{
			...
			{
				$doc_id = $nrd['doc_id'];	
				if(!empty($nrd['doc_not_required'])) { $value1 =  1; } else { $value1 =  0; }
		...
		if(!empty($submit_nr))
		{
			...
			{		
				...
				{
					$cdc_id = $nrd2['cdc_id'];	
					if(!empty($nrd2['cdc_not_required'])) { $value1 =  1; } else { $value1 =  0; }
			...
			{		
				...
				foreach($bdf_data as $nrd)
				{
					...
					if($nrd['bd_cover_letter'] == 1) {
						...
						if(empty($bdf_data['bd_cover_user'])) { $nrdetail['bd_cover_user'] = $user_name; }
					}
					else {
					...
					if($nrd['bd_emailed_client'] == 1) {
							...
							if(empty($bdf_data['bd_emailed_user'])) { $nrdetail['bd_emailed_user'] = $user_name; }
					} else {
					...
					if($nrd['bd_posted_client'] == 1 ) {
							...
							if(empty($bdf_data['bd_posted_user'])) { $nrdetail['bd_posted_user'] = $user_name; }
					}
					else {
...
if($bsn_id > 0) { $fwViewData['bid'] = $bsn_id; } else { $fwViewData['bid'] = 0; }
...
if (!(isset($pagenum)))
 ...
 if(!empty($setdata)) { $k=$k+1;} else { $k = 0; }
