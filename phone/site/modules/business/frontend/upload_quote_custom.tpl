...
<div><br />
 ...
 {{else}}
  ...
  <tr>
  	<td><strong>Upload Quote : <input type="file" name="quote1" id="doc_file_name" /></strong>
       {{if $quote}} <a href = "{{$BASE_URL}}{{$FILE_PATH}}files/purchase_order_quote/{{$quote}}">PDF</a> {{/if}}</td>
       <td><select name = "quote[po_type]" >
        ...
        <td><input type="text" name = "quote[po_value]" value="{{$detail.value}}" /> </td>
        <td><input type="text" name = "quote[po_supplier]" value="{{$detail.supplier}}"/> </td>
 </tr>
