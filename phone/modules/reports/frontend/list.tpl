    ...
    <table id="doclist-table" class="nav-back" width="100%">
        <thead>
         <tr>	
                    ...
                    <th class="topmenu" align="center" valign="middle" width="4%">Date Create</th>
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
                ...
                <td><a href="">{{$item.btdl_desc|truncate:200:"...":true}}</a></td>
               <td>{{$item.btdl_creator_id}}</td>
               <td>{{$item.btdl_responcible_id}}</td>
                <td>{{$item.btdl_urgency}}</td>
                 <td>{{if $item.btdl_create_date !== '0000-00-00 00:00:00'}}{{$item.btdl_create_date|date_format:"%d/%m/%Y %H:%M"}}{{/if}}</td>
                  <td>{{$item.btdl_count_day}}</td>
                   <td>{{if $item.btdl_due_date !== '0000-00-00 00:00:00'}}{{$item.btdl_due_date|date_format:"%d/%m/%Y %H:%M"}}{{/if}}</td>
                    <td>{{$item.btdl_status}}</td>
