...
{{/if}}
   {{if $msg != 0}}
<script type="text/javascript">
	parent.toggleLayer('divproposalalert','addproposalalert');
...
<table width="100%" id="none-table" style="background-color:#FFFFFF;">
  <form name="detail" method="post" action="{{$XFA.proposalalert_detail}}" enctype="multipart/form-data" onsubmit="showTop();return false;">
   ...
   <tr>
      <td>
            <table width="100%" height="200" style="background-color:#FFFFFF; border:1px solid #c1dad7;">
              <tr>
                <td>Alert text:&nbsp;</td>
                <th colspan="3"> <textarea rows="4" cols="150" name="{{$TABLE}}[be_alert_text]" id="be_alert_text">{{$detail.be_alert_text}}</textarea></th>
              </tr>
              
              <tr>
              <td> Assign to </th>
              <th colspan="3">
              <select name="{{$TABLE}}[be_assign_to]"> 
               {{foreach from = $emailcontacts key="key" item="item" }}
               <option value="{{$item.ec_contact_name}} - {{$item.ec_email_id}}" {{if $detail.be_assign_to eq $item.ec_contact_name}} selected="selected" {{/if}}> {{$item.ec_contact_name}} - {{$item.ec_email_id}} </option>
               {{/foreach}}
      		  ...
      		  <td> Active: &nbsp; </td>
              <th>
               <select name="{{$TABLE}}[be_alert_active]"> 
                  <option value="Yes" {{if $detail.be_alert_active eq 'Yes'}} selected="selected" {{/if}}> Yes </option>
                  <option value="No" {{if $detail.be_alert_active eq 'No'}} selected="selected" {{/if}}> No </option>
              </select>
              </th>
              </td>	
            </table>
    ...
    <tr>
      <td colspan="2"><div align="left">
          ...
          <input type="button" name="cancel" value="Cancel" onclick="javascript:parent.add_edit_proposal_alerts();" class="sml"/>
          </div></td>
    </tr>
  </form>
</table>
