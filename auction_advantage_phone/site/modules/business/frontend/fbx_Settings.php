...
//            $flag = 0;
//            
//            if ($s3->putObjectFile($path . "/" . $zipfile, 'deckquote', 'files/delivery_doc/'.$zipfile, S3::ACL_PUBLIC_READ)) {
...
function upload($filename, $tmpname)
{
	...
	include_once $filetoinclude;
        ...
        try {
            ...
            if ($s3->putObjectFile($path . "/" . $zipfile, 'deckquote', 'files/document_check_list_files_roof/'.$zipfile, S3::ACL_PUBLIC_READ)) {
                $flag = 1; //die('in putobject');
            } else {
                sleep(10);
            if ($s3->putObjectFile($path . "/" . $zipfile, 'deckquote',  'files/document_check_list_files_roof/'.$zipfile, S3::ACL_PUBLIC_READ))
