<?php
            ...
            foreach ($data['pp_steps'] as $index => $step) {
                ...
                if(!empty($step['substeps']) && empty($step['contents_substeps'])) {
                    foreach($step['substeps'] as $substep) {
						...
						if(isset($substep['content'])) {
							$html .= ('<div>' . $substep['content'] . '</div>');
						}
						$html .= '</div>';
                    }
                }
                ...
                // if substeps is not empty and content_substeps is not empty, then it should be added in index table
                if(!empty($step['substeps']) && !empty($step['contents_substeps'])) {
    ...
    $data = $aiObject->getProcedureDocuments($pd);
