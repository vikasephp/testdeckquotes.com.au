  ...
  <table id="doclist-table" class="nav-back" width="100%">
    ...
    <form name="mlist" method="post">
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
        ...
        <td class="tooltip">{{if $item.mm_doc_pdf}}<a href="{{$BASE_URL}}files/meeting_minutes_doc/{{$item.mm_doc_pdf}}?dummy={{$ran}}" target="_blank">{{$item.mm_doc_pdf|truncate:70:"...":true}}</a>
      <span class="tooltiptext">{{$item.mm_doc_pdf|stripslashes}}</span> {{/if}}</td>
        <td class="tooltip">{{if $item.mm_doc_word}}<a href="{{$BASE_URL}}files/meeting_minutes_doc/{{$item.mm_doc_word}}?dummy={{$ran}}"> {{$item.mm_doc_word|truncate:70:"...":true}} </a>
