...
<input type="button" name="Add New" value="Add Type Options" onclick="javascript:add_type();" /> <br />
...
<strong>Total : </strong> <input type="text" readonly="readonly" value="${{$total}}" style="width:90px;" />
...
<!--<a href="{{$BASE_URL}}files/uploads_2024/{{$updata.td_file_name}}" target="_blank" >Download</a> -->
  <a href="/30_days_rec_report.download_content_file?file_name={{$updata.td_file_name}}&module_name=30_days_rec_report.home" target="_blank"/>Download</a>

&nbsp;  {{$updata.td_user}} &nbsp; &nbsp; {{$updata.td_date}}{{/if}} </strong> <br />
 ...
 {{foreach from=$list key="key" item="item"}}
    ...
    <tr bgcolor="#FFFFFF">
      ...
      <a href="{{$BASE_URL}}30_days_rec_report.reason/td_id/{{$item.td_id}}" class="various">Read</a>
       ...
       {{else }}
       <td>
       <a href="{{$BASE_URL}}30_days_rec_report.colorbox/td_id/{{$item.td_id}}" class="various">Color</a></td>
       {{/if}}
       
         <td><input type="button" name="vr" value="View Reason" onclick="javascript:showreason({{$item.td_id}});" style="width:90px; height:20px; font-size:11px; line-height:17px !important; padding:0px !important" />
         {{if $item.td_reason}} {{$item.td_reason_date}}{{/if}}
         </td>
          
         <td>
         {{if $item.checklist173}}
         <a href="/30_days_rec_report.download_content_cou?file_name={{$item.checklist173}}&module_name=30_days_rec_report.home" >Download</a> 
        {{/if}}
         </td>
        ...
        <form name ="recmet3" method="post" action="">
     	<input type="hidden" name="dmp[{{$item.td_id}}]" value="0" />
  	    <label class="switch">
<input class="switch-input" type="checkbox"  name="dmp[{{$item.td_id}}]" value="1"  onclick="this.form.submit();" {{if $item.td_dmp eq 1 }} checked="checked" {{/if}} />
       ...
       <td>
        <form name ="recmet3" method="post" action="">
     	<input type="hidden" name="ws[{{$item.td_id}}]" value="0" />
  	    <label class="switch">
<input class="switch-input" type="checkbox"  name="ws[{{$item.td_id}}]" value="1"  onclick="this.form.submit();" {{if $item.td_weekly_statement  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       {{if $item.td_weekly_statement  eq 1}} {{$item.td_ws_date}} <br />{{$item.td_ws_user}} {{/if}}
        </form>  
       ...
       <!-- <a href="https://deckquote.s3.amazonaws.com/files/uploads/{{$item.td_document}}" target="_blank"/>-->
        <a href="/30_days_rec_report.download_content?file_name={{$item.td_document}}&module_name=30_days_rec_report.home" target="_blank"/>
        Download{{/if}}</td>
 	<td>
 <input type="text" name="email[{{$item.td_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo33_{{$item.td_id}}" value="{{$item.td_last_email_date}}" onfocus ="update_email({{$item.td_id}}, this.value);"  /> 
 ...
 <input type="text" name="spoken[{{$item.td_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo44_{{$item.td_id}}" value="{{$item.td_last_spoken_date}}" onfocus ="update_spoken({{$item.td_id}}, this.value);"  /> 
...
<!--  <a href="https://deckquote.s3.amazonaws.com/files/uploads/{{$item.td_correspondance}}" target="_blank"/>-->
   <a href="/30_days_rec_report.download_content?file_name={{$item.td_correspondance}}&module_name=30_days_rec_report.home" target="_blank"/>
  Download{{/if}}</td>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="hide[{{$item.td_id}}]" value="1"  onclick="this.form.submit();" {{if $item.td_hide eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
