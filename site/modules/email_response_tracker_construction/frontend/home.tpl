...
function add_status()
{
	var viewurl = "{{$BASE_URL}}email_response_tracker_construction.view_status"+ "/random/" + Math.random();	
...
function add_type()
{
	var viewurl = "{{$BASE_URL}}email_response_tracker_construction.view_type"+ "/random/" + Math.random();	
...
&nbsp;<strong> Filter By Type: </strong> 
      <select name="type_search"/>
        ...
        <option value="{{$item4.et_id}}" {{if $item4.et_id eq $status}} selected="selected" {{/if}}>{{$item4.et_option}}</option>
        {{/foreach}}
        </select> 
        &nbsp; &nbsp;<input type="submit" value="Filter" name="filter" />   
        &nbsp;<input type="submit" name="clear" value = "Clear Filter" />        
