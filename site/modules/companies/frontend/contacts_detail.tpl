...
<table id="list-table" width="100%">
   ...
   <tr>
     ...
     <datalist id='comp'>
         ...
         <input type="text" list = 'comp' name="contacts[cs_company]" style ="width:700px" value="{{$cname}}" />

    </tr>
    
    <tr>
     <th>Positions</th>
     <td colspan="3"><datalist id='pos'>
           {{foreach from=$posdetail  key="key" item="item"}}
            <option value="{{$item.to_id}} | {{$item.to_option}}">
            {{$item.to_option}}
            </option>
            {{/foreach}}
  	 </datalist>
         <input type="text" list = 'pos' name="contacts[cs_position]" style ="width:700px" value="{{$position}}" />
 </td>
    </tr>  
    ...
    <tr>
     <th>Landline</th>
     <td> <input type="text" name="contacts[cs_landline]" value="{{$detail.cs_landline}}" style="width:300px;"  /> </td>
   
     <th>Mobile</th>
     <td> <input type="text" name="contacts[cs_mobile]" value="{{$detail.cs_mobile}}" style="width:300px;"  /> </td>
    </tr>
    
    <tr>
    <th>Emergency</th>
     <td> <input type="text" name="contacts[cs_emergency]" value="{{$detail.cs_emergency}}" style="width:300px;"  /> </td>
	...
	<script type="text/javascript" language="javascript" >
        var frmvalidator  = new Validator("detail");
			...
			//frmvalidator.addValidation("contacts[cl_company_name]","req", "Please specify company name.");
			//frmvalidator.addValidation("contacts[cl_contact_name]","req", "Please specify contact name.");
    </script>
</div>
