...
<script type="text/javascript">
	    ...
	    var geturl= "{{$BASE_URL}}templatedoc.doclist?dhd_dh_id="; // The server-side script
		var getdelurl= "{{$BASE_URL}}templatedoc.docdelete?dhd_id="; // The server-side script
...
<h3 class="page-title">{{$title}}</h3>
 ...
 <input type="hidden" name="bm_listing[dh_id]" value="{{$detail.$ID}}" id="dh_id"/>
     <table id="viewdoc-table" width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
            <th> Summary :</th>
            <td><textarea rows="5" cols="90" name="bm_listing[dh_summery]">{{$detail.dh_summery}}</textarea> </td>
        </tr>   
         <tr>
       ...
       <tr>
            <th> Pdf Link:</th>
            <td> <input type="text" name="bm_listing[dh_link_pdf]" value="{{$detail.dh_link_pdf}}" size="160" /></td>
        </tr>
        
         <tr>
            <th> Word Link:</th>
            <td> <input type="text" name="bm_listing[dh_link_word]" value="{{$detail.dh_link_word}}" size="160" /></td>
        </tr> 
