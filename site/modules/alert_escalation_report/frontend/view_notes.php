...
$ord =  " ORDER BY if(  alert_escalation_notes.cn_date = ' '
         OR alert_escalation_notes.cn_date IS NULL , 1, 0 ) , STR_TO_DATE( alert_escalation_notes.cn_date, '%d-%m-%Y' ) DESC";
		 
$ord2 =  " ORDER BY if(  car_supplier_update.cu_date = ' '
         OR car_supplier_update.cu_date IS NULL , 1, 0 ) ,  car_supplier_update.cu_date DESC";		 

$sql = "Select * from alert_escalation_notes where cn_car_id = ".$car_id .' '.$ord;
$data = $fwDb->query($sql);

$sql2 = "Select count(*) as tot from alert_escalation_notes where cn_car_id = ".$car_id;
...
foreach($suData as $k=>$v)
{
  ...
  $sql_nt = "Select max(STR_TO_DATE(cn_date, '%d-%m-%Y' )) as notes_date from alert_escalation_notes where cn_car_id = ".$car_id;
