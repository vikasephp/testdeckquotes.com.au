			...
			<tbody>
				{{if $emails}}
					{{foreach from=$emails key="index" item="row"}}
						<tr>
							<td data-col="id">{{$row.aere_id}}</td>
							<td data-col="to">{{$row.aere_to}}</td>
							<td data-col="cc">{{$row.aere_cc}}</td>
							<td data-col="email_type">{{$row.email_type}}</td>
							<td data-col="send_type">{{$row.send_type}}</td>
							<td data-col="send_date">{{$row.send_date}}</td>
							<td data-col="action">
								<a href="{{$BASE_URL}}{{$BASEFOLDER}}.send_email/delete/{{$row.aere_id}}" onclick="javascript:if(!confirm('Are you sure you want to delete the record?')) return false;" title="Delete"><img style="height: 16px;; width:16px" src="{{$BASE_URL}}css/admin/images/deletecross.png" alt="" /></a>
							</td>
						</tr>
					{{/foreach}}
				{{else}}
