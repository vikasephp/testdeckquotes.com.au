...
<div style="float:left; width:100%; text-align:left; margin-left:5px;">
    ...
    <input type="button" name="People_Panel" value="People Management Panel"
        onclick="document.location.href='{{$BASE_URL}}people_management.list'" />
    &nbsp;
    <input type="button" name="Add_People" value="Add People"
        onclick="javascript:add_new('{{$BASE_URL}}people_management.detail');" />
...
<div style="float:right;">
    <input type="button" value="Export To XLS"
        onclick="document.location.href='{{$BASE_URL}}people_management.list/export/1'" /> &nbsp;
    <input type="button" name="People_Panel" value="Position Management Panel"
        onclick="document.location.href='{{$BASE_URL}}position.list'" />
</div>
...
<table id="list-table" class="nav-back" width="100%">
    <thead>
        <tr>
            ...
            <th class="topmenu" align="left" valign="middle" width="20%">
                <div align="left">Person:</div>
            </th>
            <th class="topmenu" align="left" valign="middle" width="5%">
                <div align="left">Active:</div>
            </th>
            <th class="topmenu" align="left" valign="middle" width="6%">
                <div align="left">Type</div>
            </th>
            <th class="topmenu" align="left" valign="middle" width="6%">
                <div align="left">Latest Letter of Offer</div>
            </th>
            <th class="topmenu" align="left" valign="middle" width="30%">
                <div align="left">Quick Details:</div>
            </th>
            <th class="topmenu" align="left" valign="middle">
                <div align="left">Employee Doc Folder Link:</div>
            </th>
            <th class="topmenu" align="left" valign="middle">
                <div align="left">Positions:</div>
            </th>
            <th class="topmenu" align="left" valign="middle">
                <div align="left">Type:</div>
            </th>
            ...
            <th class="topmenu" align="left" valign="middle" width="8%">
    ...
    <tbody>
        {{if $notfound}}
        <tr>
            <th colspan="10" s><strong>{{$notfound}}</strong>{{/if}}</th>
            {{foreach from=$list key="key" item="item"}}
        <tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
            <td>{{counter}}.</td>
            <td>
                <p><b>Name:&nbsp;</b>{{$item.user_name}}</p>
                <p><b>Company:&nbsp;</b>{{$item.customer_company}}</p>
            </td>
            ...
            <td>
                <form name="recmet3" method="post" action="">
                    ...
                    <label class="switch">
                        <input class="switch-input" type="checkbox" name="active[{{$item.user_id}}]" value="1"
                            onclick="update_active({{$item.user_id}},this.checked);" {{if $item.user_pm_active eq 1 }}
                            checked="checked" {{/if}} />
                ...
                <div id="cond{{$item.user_id}}">
                    {{if $item.user_pm_active eq 1}}{{$item.user_pm_active_user}}<br />{{$item.user_pm_active_date}}{{/if}}
                </div>
                <script>
            ...
            <td>
                <select name="{{$TABLE}}[user_pm_type]" onChange="update_type({{$item.user_id}}, this.value)">
                    ...
                    <option value="{{$item3.et_id}}" {{if $item.user_pm_type eq $item3.et_id}} selected="selected"
                        {{/if}}>{{$item3.et_option}}</option>
                    {{/foreach}}
                </select>
                <script>
            ...
            <td>{{if $item.customer_latest_letter}}<a href="{{$item.customer_latest_letter}}"
                    target="_blank">Link</a>{{/if}}</td>

            <td>
                <p><b>Mobile Phone:&nbsp;</b>{{$item.customer_phone}}</p>
                <p><b>Personal Email:&nbsp;</b>{{$item.user_email}}</p>
                <p><b>Business Phone:&nbsp;</b>{{$item.customer_phone_business}}</p>
                <p><b>Tax Number:&nbsp;</b>{{if $item.customer_tax_num}}{{$item.customer_tax_num}}
                    {{else}}NA{{/if}}
                </p>
                <p><b>Date of birth:&nbsp;</b>{{if $item.customer_dob !== '0000-00-00'}}
                    {{$item.customer_dob|date_format:"%d/%m/%Y"}}
                    {{else}} NA{{/if}}
                </p>
                <p><b>Health:&nbsp;</b>{{if $item.customer_health}}{{$item.customer_health}}
                    {{else}}NA{{/if}}
                </p>
            </td>
            <td data-title="Employee Doc Folder Link">
                {{ if $item.customer_ms_team_folder_link }}
                    <a href="{{ $item.customer_ms_team_folder_link }}" target="_blank">Link</a>
                {{ /if }}
            </td>
            <td>{{if $item.p_name}}
            ...
            <td>{{if $item.customer_status == 0}}Current
                {{else}}Previously{{/if}}
            </td>
            <td>
                <div align="center">
                    {{if $USER_GROUP == 'Administrators' OR $item.user_id == $cur_user}}
                    <a href="{{$BASE_URL}}{{$XFA.detail}}/user_id/{{$item.user_id}}" class="various" title="Edit"><img
                            style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/edit.png" /></a>
                    {{/if}}
                    &nbsp;<a href="{{$BASE_URL}}{{$XFA.view}}/user_id/{{$item.user_id}}" class="various"
                        title="View"><img style="height: 20px;; width:20px"
                            src="{{$BASE_URL}}css/admin/images/view.png" /></a>
                    {{if $USER_GROUP == 'Administrators'}}
                    &nbsp;<a href="{{$BASE_URL}}{{$XFA.delete}}/user_id/{{$item.user_id}}"
                        onclick="javascript:if(!confirm('Are you sure you want to delete.? It will loss all related data.')) return false;"
    ...
    <tr> {{if $list}}
        <td align="center">
            <div class='pagination'> {{if $pagenum == 1}} <span class='disabled'>� previous</span>
                {{else}} <a class="pagination" href="{{$BASE_URL}}{{$XFA.list}}/pagenum/{{math equation=" x - y"
                        x=$pagenum y=1}}" title="Previous">� previous</a> {{/if}}

                    {{if $pagenum != 1}} <span class="paginate"><a class="pagination"
                            href="{{$BASE_URL}}{{$XFA.list}}/pagenum/1" title="{{$page_num}} Page">1</a></span>
                    {{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span class="paginate"><a
                                class="pagination" href="{{$BASE_URL}}{{$XFA.list}}/pagenum/2"
