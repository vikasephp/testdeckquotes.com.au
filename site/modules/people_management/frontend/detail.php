...
if (!empty($submit)) {
	...
	if ($detail['user_positions']) {
		$detail['customer_positions'] = implode("|", $detail['user_positions']);
	}
	if ($detail['customer_dob']) {
		$detail['customer_dob'] = changedate_y_m_d($detail['customer_dob']);
	}
...
if ($user_id > 0) {
	...
	if (!empty($pages['customer_positions'])) {
		$pages['customer_positions'] = explode("|", $pages['customer_positions']);
	}
