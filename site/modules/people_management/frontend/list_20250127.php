...
$fwViewData['cur_user'] = $_SESSION['user']['user_id'];
   ...
   if($active_filter):
	...
	if($active_filter == 2) { $active_filter = 0; }
  ...
  if($clearall) {  $where = '';  unset($_SESSION['searchkey']);  unset($_SESSION['type']); unset($_SESSION['active_filter']);}
...
foreach($list as $k=>$v)
{
	...
	if($v['user_pm_active'] == 1) { $active = "Yes" ; } else { $active = "No"; }
