...
if ($papm) {
	...
	if ($papm[$ky] == 1) {
		...
		$detail_5['bt_completed_by'] = $_SESSION['user']['user_id'];
		$table->setWhere('bt_bsn_id = ' . $ky . ' and bt_task_id = 166 ');
		$detail = $table->updateRow($detail_5);
	} else {
		$detail_5['bt_complete'] = 0;
		$detail_5['bt_completed_date'] = '0000-00-00 00:00:00';
		$detail_5['bt_completed_by'] = '';
		$table->setWhere('bt_bsn_id = ' . $ky .  ' and bt_task_id = 166 ');
		$detail = $table->updateRow($detail_5);
	}
}
...
if (!empty($pbd)) {
    $ord = " ORDER BY if(business.bsn_pcp_pbd = '' OR business.bsn_pcp_pbd IS NULL, 1, 0), STR_TO_DATE(business.bsn_pcp_pbd, '%d-%m-%Y')";
}
...
$sql = "select document_proposal_name.*, business.bsn_pcp_pbd, pt.bt_complete, pt.bt_completed_date, pt.bt_completed_by, users.user_name from document_proposal_name LEFT JOIN business ON business.bsn_id = document_proposal_name.dpn_bsn_id LEFT JOIN proposal_tasks pt ON pt.bt_bsn_id = document_proposal_name.dpn_bsn_id AND pt.bt_task_id = 166 AND pt.bt_task_list_number = 1 Left Join users on users.user_id = pt.bt_completed_by $where $ord ";
...
$sql_count_proposal_booked = "select count(business.bsn_pcp_pbd) as total_proposal_booked from document_proposal_name LEFT JOIN business ON business.bsn_id = document_proposal_name.dpn_bsn_id LEFT JOIN proposal_tasks pt ON pt.bt_bsn_id = document_proposal_name.dpn_bsn_id AND pt.bt_task_id = 166 AND pt.bt_task_list_number = 1 WHERE document_proposal_name.dpn_pcp_show = 1 and document_proposal_name.dpn_pcp_hide = 0 and (business.bsn_pcp_pbd != '' and business.bsn_pcp_pbd is not null)";
$data_count_proposal_booked = $fwDb->queryOne($sql_count_proposal_booked);
$fwViewData['proposal_booked'] = $data_count_proposal_booked['total_proposal_booked'];

$sql_count_proposal_not_booked = "select count(business.bsn_pcp_pbd) as total_proposal_not_booked from document_proposal_name LEFT JOIN business ON business.bsn_id = document_proposal_name.dpn_bsn_id LEFT JOIN proposal_tasks pt ON pt.bt_bsn_id = document_proposal_name.dpn_bsn_id AND pt.bt_task_id = 166 AND pt.bt_task_list_number = 1 WHERE document_proposal_name.dpn_pcp_show = 1 and document_proposal_name.dpn_pcp_hide = 0 and (business.bsn_pcp_pbd = '' or business.bsn_pcp_pbd is null)";
$data_count_proposal_not_booked = $fwDb->queryOne($sql_count_proposal_not_booked);
$fwViewData['proposal_not_booked'] = $data_count_proposal_not_booked['total_proposal_not_booked'];
