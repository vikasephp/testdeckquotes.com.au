...
<div style="float:left; width:100%;">
    <table id="doclist-table" class="nav-back" width="100%">
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
            	<td>{{$countn}}.</td>
                <td>{{$item.bsd_title}}</td>
                <td><a href="#">{{$item.bsd_summery|truncate:200:"...":true}}</a></td>
               <td>{{$item.bsd_bus_streem}}</td>
               <td>{{if $item.bsd_access == 1}}All Access{{else}}Users{{/if}}</td>
                <td>{{$item.user_name}}</td>
                <td>{{if $item.bsd_last_update !== '0000-00-00 00:00:00'}}{{$item.bsd_last_update|date_format:"%d/%m/%Y %H:%M"}}{{else}}
                  {{$item.bsd_insert_date|date_format:"%d/%m/%Y %H:%M"}}{{/if}}</td>
                 <td>
                 ...
                 {{foreach from=$item.doc_files key="keyfiles" item="itemfiles"}}
                    ...
                    <li><img src="{{$itemfiles.iconpath}}" width="20" /> <br />
                      ...
                      <ul class="small-menu">
                        ...
                        <!--<a href="{{$BASE_URL}}{{$FILE_PATH}}files/business_documents_hub/{{$itemfiles.dhd_name}}" title="{{$itemfiles.dhd_name}}" target="_blank" style="color:#FFFFFF;">-->
   <a href="/bustrategydocs.download_content?file_name={{$itemfiles.dhd_name}}&module_name=bustrategydocs.home" target="_blank"/>
 ...
 <a href="/bustrategydocs.download_content?file_name={{$itemfiles.dhd_name}}&module_name=bustrategydocs.home" target="_blank"/>                        
     ...
     <a href="{{$BASE_URL}}{{$XFA.delete}}/{{$ID}}/{{$item.$ID}}" onclick="javascript:if(!confirm('Are you sure want to delete the business document? Because it will delete all the document related to this listing.')) return false;" title="Delete"><img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/delete.png"/></a>
  <!--   {{/if}}-->
     </td>
            </tr>
