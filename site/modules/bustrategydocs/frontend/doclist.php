<?php
$dhd_bsd_id = $fwRequest->getParamget('dhd_bsd_id', 0);
if($dhd_bsd_id > 0)
	{
$thisTable = new Fw_Db_Table('business_strategy_docs');
$thisTable->setWhere("dhd_bsd_id = $dhd_bsd_id");
	$docData = $thisTable->getRows();
		...
		if($docData)
		{
			...
			foreach($docData as $k => $v)
					...
					$iconpath = get_icon_path($v['dhd_type'], $v['dhd_name'], 'business_documents_hub');
					if($count % 5 == 0)
							{
							$data.='</tr><tr>';
