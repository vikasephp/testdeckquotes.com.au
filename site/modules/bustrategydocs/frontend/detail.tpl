...
<script type="text/javascript">
	    ...
	    var geturl= "{{$BASE_URL}}bustrategydocs.doclist?dhd_bsd_id="; // The server-side script
		var getdelurl= "{{$BASE_URL}}bustrategydocs.docdelete?dhd_id="; // The server-side script
	...
	function handleHttpAllBusinessResponse() {   
        ...
        function show_doclist() { 
			var dochubid = document.getElementById('bsd_id').value;
...
<div align="center">
  ...
  <form name="detail" method="post" action="{{$XFA.detail}}" enctype="multipart/form-data">
    <input type="hidden" name="bm_listing[bsd_id]" value="{{$detail.$ID}}" id="bsd_id"/>
    <table id="viewdoc-table" width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <th > Name:</th>
        <td><input type="text" name="bm_listing[bsd_title]" value="{{$detail.bsd_title}}" style="width:300px;"/></td>
      </tr>
      <tr>
        <th> Summary :</th>
        <td><textarea rows="8" cols="60" name="bm_listing[bsd_summery]">{{$detail.bsd_summery}}</textarea>
        </td>
      </tr>
      <tr>
        <th> Business Stream:</th>
        <td><input type="text" name="bm_listing[bsd_bus_streem]" value="{{$detail.bsd_bus_streem}}" size="35" /></td>
      </tr>
      <tr>
        <th > Access:</th>
        <td><select name="bm_listing[bsd_access]" class="lrg" style="width:220px;">
            <option value="1" {{if $detail.bsd_access == 1}} selected="selected" {{/if}}> All Users</option>
            <option value="2" {{if $detail.bsd_access == 2}} selected="selected" {{/if}}> Users</option>
          </select>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <th colspan="2"><div id="doc_list" style="height:auto; padding-bottom:10px; min-height:40px;"></div></th>
      </tr>
      <tr>
...
<script type="text/javascript" language="javascript" >
    ...
    frmvalidator.addValidation("bm_listing[bsd_title]","req", "Please specify title.");
</script>
{{/if}}