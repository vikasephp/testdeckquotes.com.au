...
<script type="text/javascript">
	...
	function view_commencement_appoint_complete() {
		var viewurl = "{{$BASE_URL}}construction_pipeline_report.view_commencement_appoint_complete" + "/random/" + Math.random();
...
<form name="mlist" method="post" enctype="multipart/form-data">
	...
	<div style="float:left; text-align:left;">
		...
		&nbsp;<input type="submit" name="sortby_csd" value="Sort by Could Start Build Date" />
		&nbsp;<input type="button" name="commencement_appoint" value="Commencement Appointment Completed" onclick="javascript:view_commencement_appoint_complete();"  /><br />
		...
		<table id="doclist-table" class="nav-back" width="100%">
			<thead>
				<tr>
                    ...
                    <th class="topmenu" align="center" valign="middle" width="8%">Smartsheet Calendar Created</th>
						...
						{{else }}
							...
							<td data-col="commencement_notice">
								{{if $item.commencement_notice}}
									...
									<p style="margin: 0;">{{ $item.commencement_notice_user }}</p>
									{{ if $item.commencement_notice_date }}
										<p style="margin: 0;">{{ $item.commencement_notice_date|date_format:"%d-%m-%Y" }}</p>
									{{ /if }}
								{{/if}}
							</td>
							...
							<td data-col="change_log" style="white-space: nowrap;">
								<a href="{{$BASE_URL}}construction_pipeline_report.change_log/bsn_id/{{$item.bsn_id}}/bs_original_starting_date/{{$item.bs_original_starting_date}}" class="various">Change Log</a>
								<!-- <table>
									<tr>
										<th>Date Changed</th>
										<th>Onsite Day</th>
									...
									{{ assign var=last_index value=$item.total_change_log-1 }}
									...
									<tr>
										...
										<td>
											...
											{{ if $index >= $last_index }}
												{{ assign var=next_value value=$item.bs_original_starting_date|@strtotime }}
											{{ else }}
												{{ assign var=next_value value=$item.change_log[$post_index].cprol_onsite_date|@strtotime }}
											{{ /if }}									
											{{ if $next_value > 0 }}
											...
											<span style="display: none;">
												...
												<br>
												{{ $item.bs_original_starting_date }} - {{ $item.change_log[$post_index].cprol_onsite_date }} - {{ $next_value }}
												<br>
											</span>
										</td>
									</tr>
                            ...
                            <td {{if $item.bs_cpr_smartsheet eq 0}}style="background: #F00" {{elseif
								$item.bs_cpr_smartsheet eq 1 }}style="background:#00CC33" {{/if}}>
								<select name="smartsheet" Onchange="update_smartsheet({{$item.bsn_id}},this.value)">
									<option value="0" {{if $item.bs_cpr_smartsheet eq 0}} selected="selected" {{/if}}>No</option>
									<option value="1" {{if $item.bs_cpr_smartsheet eq 1}} selected="selected" {{/if}}>Yes</option>
								</select>

								<script>
									function update_smartsheet(id, value) {
										$.ajax({
											type: "GET",
											url: "{{$BASE_URL}}construction_pipeline_report.update_smartsheet/bsn_id/" + id + "/value/" + value,
											success: function (result) {
												window.location.reload();
											}
										});
