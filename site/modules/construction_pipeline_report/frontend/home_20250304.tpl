...
&nbsp; &nbsp; <input type="button" name="pm1" value="Project Management - Could Start" onclick="javascript:pm_could();" />
...
<input type="button" value="Export To XLS" onclick="document.location.href='{{$BASE_URL}}construction_pipeline_report.home/export/1'" /> &nbsp;
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="8%">Construction Commencement<br /> Letter</th>
        ...
        <th class="topmenu" align="center" valign="middle" width="3%">Could Start Build Date from the Planning Task Tracker</th>
        ...
        <th class="topmenu" align="center" valign="middle" width="8%">HIA Value</th>
        ...
        <th class="topmenu" align="center" valign="middle" width="8%">Move to Construction in Progress Report</th>
    ...
    <tbody>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="hide[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bs_cpr_hide eq 1 }} checked="checked" {{/if}} />
        ...
        <input type="text" name="bs_cpr_lik_hia_date[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo10_{{$item.bs_customers_id}}" value="{{$item.bs_cpr_lik_hia_date}}"  /> 
       <input type="submit" value="Set" name="likelyhia" class="set" />
        ...
        <select name="sn[{{$item.bsn_id}}]" Onchange = "update_sn({{$item.bsn_id}},this.value)" >
          ...
          <option value="{{$itemsn.sn_id}}"  {{if $itemsn.sn_id eq $item.bs_cpr_sn}} selected="selected" {{/if}}>{{$itemsn.sn_option}}</option>
          {{/foreach}}
          </select>
	 ...
	 <script>
        ...
        <td>{{$item.status}}</td>
        <td>
        <select name="proj_type" Onchange = "update_ptype({{$item.bsn_id}},this.value)" >
                <option value="">Please Select </option>
                {{foreach from = $typedata key="keyt" item="itemt"}}
                <option value="{{$itemt.to_id}}" {{if $itemt.to_id eq $item.bsn_cip_proj_type}} selected="selected" {{/if}}>{{$itemt.to_type_option}}</option>
                {{/foreach}}
        </select>
	  ...
	  <script>
       ...
       <input type="file" name="cpr_cc_letter[{{$item.bsn_id}}]" /><br />
        ...
        <!--<a href="{{$BASE_URL}}files/cma/{{$item.bs_cpr_cc_letter}}" target="_blank">Download</a>-->
         <a href="/construction_pipeline_report.download_content?file_name={{$item.bs_cpr_cc_letter}}&module_name=construction_pipeline_report.home" target="_blank"/>Download</a>
...
<input class="switch-input" type="checkbox"  name="plans[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bs_cpr_plans_audited eq 1 }} checked="checked" {{/if}} />
      ...
      {{if $item.bs_cpr_plans_audited eq 1 }} {{$item.bs_cpr_plans_audited_user}} <br /> {{$item.bs_cpr_plans_audited_date}} {{/if}}
       ...
       <td>
       <select name="sub" Onchange = "update_substatus({{$item.bsn_id}},this.value)">
       <option value="1" {{if $item.bs_cpr_sub_status eq 1}} selected="selected" {{/if}}>Commencement and warranty received</option>
       <option value="2" {{if $item.bs_cpr_sub_status eq 2}} selected="selected" {{/if}}>Warranty received, commencement applied for</option>
       <option value="3" {{if $item.bs_cpr_sub_status eq 3}} selected="selected" {{/if}}>Warranty applied for</option>
       <option value="4" {{if $item.bs_cpr_sub_status eq 4}} selected="selected" {{/if}}>No warranty applied for</option>
       </select>
        ...
        <td>{{$item.bsn_latest_csbd}} </td>
        
         {{ if $item.bs_traffic_light eq 1}}
       <td style="background:#00CC33";><br /><br />{{$item.bs_tl_user}}<br />{{$item.bs_tl_date}}<br />
       <a href="{{$BASE_URL}}construction_pipeline_report.reason/bsn_id/{{$item.bsn_id}}/a/1" class="various">Read</a><br />
       <a href="{{$BASE_URL}}construction_pipeline_report.show_update_notes/bsn_id/{{$item.bsn_id}}" class="various">Notes</a>
       </td>
       ...
       <input type="text" name="amt_depo_date[{{$item.bsn_id}}]" value="{{$item.bs_cpr_amt_depo_paid}}" onkeyup = "update_amt_depopaid({{$item.bsn_id}},this.value)" style="width:70px;"/>
       {{$item.bs_cpr_paid_user}}<br /> 
       <input type="text" name="bs_cpr_paid_date[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo4_{{$item.bs_customers_id}}" value="{{$item.bs_cpr_paid_date}}"  /> 
	   ...
	   function update_amt_depopaid(id,value)
        ...
        <td> 
         ...
         <input type="text" name="hia[{{$item.bcust_id}}]"  value="{{$item.bcust_cpr_hia_value}}" onkeyup = "update_hia({{$item.bcust_id}},this.value)" /> 
         </form>
	   ...
	   function update_hia(id,value)
        ...
        <td>
            <form name="original_starting_date" method="post" action="">
                <input type="text" name="bs_original_starting_date[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo5_{{$item.bs_customers_id}}" value="{{$item.bs_original_starting_date}}"  /> 
        ...
        <input type="text" name="bsn_starting_onsite_date[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo2_{{$item.bs_customers_id}}" value="{{$item.bsn_starting_onsite_date}}"  /> 
       ...
       <form name = "qaed" method="post" action="">
        <input type="text" name="bs_last_qaed[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo3_{{$item.bs_customers_id}}" value="{{$item.bs_last_qaed}}"  /> 
       ...
       <input type="submit" value="Set" name="owc" class="set" />
        ...
        <td {{if $item.bs_meeting_status eq 0}}style="background: #F00"{{elseif $item.bs_meeting_status eq 1 }}style="background:#00CC33"{{/if}}>
           <select name="meeting_status" Onchange = "update_meetingstatus({{$item.bsn_id}},this.value)">
               <option value="0" {{if $item.bs_meeting_status eq 0}} selected="selected" {{/if}}>No</option>
               <option value="1" {{if $item.bs_meeting_status eq 1}} selected="selected" {{/if}}>Yes</option>
           </select>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="move[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bs_con_pro_report eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
