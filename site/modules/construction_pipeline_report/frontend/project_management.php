<?php
$fwMainView = 'file:' . getcwd() . '/project_management.tpl';
...
$sql = "SELECT  business_sellers.bs_business_id,   business.bsn_latest_csbd, business_sellers.bs_cpr_onsite_date,
...
foreach($setData as $k => $v)
{
	if($v['bsn_latest_csbd']) 
	{
		$time=strtotime($v['bsn_latest_csbd']);
		$month=date("F",$time);
		$year=date("Y",$time);
		$count1=0;
		foreach($setData as $k2=>$v2)
		{
			$time2=strtotime($v2['bsn_latest_csbd']);
			$month2=date("F",$time2);
			$year2 =date("Y",$time2);
