...
if (!empty($onsite_date)) {
	...
	$onsiteTable = new Fw_Db_Table('construction_pipeline_report_onsite_log');
	...
	foreach ($save_od as $k => $v) {
		...
		$onsiteTable->insertRow([
			'cprol_bsn_id' => $k,
			'cprol_user' => $_SESSION['user']['user_name'],
			'cprol_onsite_date' => date('Y-m-d', strtotime($v)),
		]);
	}
}
...
if (!empty($original_onsite_date)) {
	...
	foreach ($save_original_od as $kd => $vd) {
		...
		// $oosdDetail['bs_original_starting_user'] = $_SESSION['user']['user_name'];
...
// $qry = 'SELECT COUNT(*) AS tot_com_notice FROM `document_check_list` DCL LEFT JOIN business_sellers AS BS ON BS.bs_business_id = DCL.doc_bsn_id WHERE DCL.`doc_name_id` = 235 AND DCL.doc_file_name != "" AND BS.bs_cpr_hide = 0';
// $qry = 'SELECT COUNT(*) AS tot_com_notice FROM `document_check_list` WHERE `doc_name_id` = 235 AND doc_file_name != ""';
// $res = $fwDb->queryOne($qry);
// $fwViewData['tot_com_notice'] = $res['tot_com_notice'];
...
$sql = "SELECT business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
	...
	business.bsn_status, business.bsn_address, business.bsn_orginal_csbd, business.bsn_latest_csbd, business.bsn_latest_csbd_date, business.bsn_latest_csbd_user, business.bsn_type, business_sellers.bs_cpr_hb_grant,
	...
	business.bsn_cip_proj_type, business.bsn_onsite_user , business_sellers.bs_cpr_sub_status, business_sellers.bs_meeting_status, business_sellers.bs_original_starting_date, business_sellers.bs_original_starting_user , business_sellers.bs_cpr_smartsheet
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			$sql_onsiteDate_updatedOn = "SELECT * FROM construction_pipeline_report_onsite_log where cprol_bsn_id = " . $v['bsn_id'] . " order by cprol_id desc LIMIT 1";
			$data_onsiteDate_updatedOn = $fwDb->queryOne($sql_onsiteDate_updatedOn);
			$setdata2[$k]['data_onsiteDate_updatedOn'] = $data_onsiteDate_updatedOn['cprol_log_date'];


			$qry = 'SELECT DCL.doc_file_name, DCL.doc_user_name, U.user_name FROM document_check_list AS DCL INNER JOIN users AS U ON U.user_id = DCL.doc_user_name WHERE DCL.doc_name_id = 457 AND DCL.doc_bsn_id = ' . $v['bsn_id'] . ';';
			$res = $fwDb->queryOne($qry);
			$setdata2[$k]['bs_original_starting_user'] = $res['user_name'];
			$setdata2[$k]['bs_original_starting_doc'] = $res['doc_file_name'];
			$setdata2[$k]['latest_inclusions_letter'] = $res['doc_file_name'];

			$qry = 'SELECT DCL.doc_file_name, DCL.doc_user_name, U.user_name FROM document_check_list AS DCL INNER JOIN users AS U ON U.user_id = DCL.doc_user_name WHERE DCL.doc_name_id = 456 AND DCL.doc_bsn_id = ' . $v['bsn_id'] . ';';
			$res = $fwDb->queryOne($qry);
			$setdata2[$k]['latest_planning_letter'] = $res['doc_file_name'];
			
			$sql_ptName = 'SELECT pt_name FROM project_type left Join business on business.bsn_type = project_type.pt_id where business.bsn_id = '. $v['bsn_id'];
			$data_ptName = $fwDb->queryOne($sql_ptName);
			$setdata2[$k]['real_project_type_name'] = $data_ptName['pt_name'];

			$qry = 'SELECT DCL.doc_file_name, DCL.doc_user_name, DCL.doc_date_uploaded, U.user_name FROM document_check_list AS DCL INNER JOIN users AS U ON U.user_id = DCL.doc_user_name WHERE DCL.doc_name_id = 235 AND DCL.doc_bsn_id = ' . $v['bsn_id'] . ';';
			$res = $fwDb->queryOne($qry);
			$setdata2[$k]['commencement_notice'] = $res['doc_file_name'];
			$setdata2[$k]['commencement_notice_user'] = $res['user_name'];
			$setdata2[$k]['commencement_notice_date'] = $res['doc_date_uploaded'];
			if ($res['doc_file_name']) {
				$tot_com_notice++;
			}

			// $qry = 'SELECT * FROM construction_pipeline_report_onsite_log WHERE cprol_bsn_id = ' . $v['bsn_id'] . ';';
			// $res = $fwDb->query($qry);
			// // db($res);
			// $setdata2[$k]['change_log'] = $res;
			// $setdata2[$k]['total_change_log'] = count($res);
			// die();
		}
		$fwViewData['tot_com_notice'] = $tot_com_notice;
