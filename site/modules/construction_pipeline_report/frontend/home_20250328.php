...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			$qry = 'SELECT DCL.doc_file_name, DCL.doc_user_name, U.user_name FROM document_check_list AS DCL INNER JOIN users AS U ON U.user_id = DCL.doc_user_name WHERE DCL.doc_name_id = 235 AND DCL.doc_bsn_id = ' . $v['bsn_id'] . ';';
			$res = $fwDb->queryOne($qry);
