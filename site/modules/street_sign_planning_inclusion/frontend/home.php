...
//Move the Record to Construction Code
if (!empty($_POST['move']) && (int)$_POST['move'] === 1 && !empty($_POST['move_ss_id'])) {

    $move_ss_id = (int)$_POST['move_ss_id'];
    $record = $fwDb->queryOne("SELECT * FROM street_sign_pl_in WHERE ss_id = $move_ss_id ");

    if ($record) {
        unset($record['ss_id']);
        $exists = $fwDb->queryOne("
            SELECT 1
            FROM street_sign_construction
        ...
        if (!$exists) {
            ...
            $fwDb->queryOne("
                INSERT INTO street_sign_construction (" . implode(',', $columns) . ")
                VALUES (" . implode(',', $values) . ")
            ");

            $fwDb->queryOne("DELETE FROM street_sign_pl_in WHERE ss_id = $move_ss_id ");
        }
    }
}
//End Move the Record to Construction Code
...
//$query = "SELECT bsn_status, bsn_id, bsn_name FROM business WHERE bsn_status LIKE '%|2|%' OR bsn_status LIKE '%|5|%' OR bsn_status LIKE '%|15|%' OR bsn_status LIKE '%|18|%'";
//$query = "SELECT bsn_id, bsn_name, bsn_status FROM business WHERE (bsn_status LIKE '%|2|%' OR bsn_status LIKE '%|5|%' OR bsn_status LIKE '%|18|%') AND bsn_status NOT LIKE '%|3|%' AND bsn_status NOT LIKE '%|4|%' AND bsn_status NOT LIKE '%|6|%' ";
...
$fwViewData['title'] = "CGFB Street Sign Report Planning And Inclusions";
...
if ($export > 0) {
	$sqlexport = "Select street_sign_pl_in.* from street_sign_pl_in";
	...
	header('Content-Disposition: attachment;filename="street_sign_pl_in.xls"');
...
if (!empty($submit)) {
	...
	if ($_SESSION['intensity_filter']) {
		$where .= " AND street_sign_pl_in.ss_intensity IN (" . $_SESSION['intensity_filter'] . ")";
	}

	if ($_SESSION['signaction_filter']) {
		$where .= " AND street_sign_pl_in.ss_sign_action LIKE '%" . $_SESSION['signaction_filter'] . "%'";
	}

	if ($_SESSION['banneraction_filter']) {
		$where .= " AND street_sign_pl_in.ss_banner_action LIKE '%" . $_SESSION['banneraction_filter'] . "%'";
	}

	$sssql = "SELECT * from street_sign_pl_in " . $where;
