...
{
	var viewurl = "{{$BASE_URL}}policy.view_procedure"+ "/random/" + Math.random();	
    ...
    <table id="doclist-table" class="nav-back" width="100%">
        <thead>
         <tr>	
            		...
            		<th class="topmenu" align="center" valign="middle" width="200">Supplier Name</th>
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
            	<td>{{$countn}}.</td>
                <td>{{$item.bp_title}}</td>
                <!--<td>{{$item.bp_summery|truncate:200:"...":true}}</td>-->
                ...
                <td>{{if $item.bp_last_update !== '0000-00-00 00:00:00'}}{{$item.bp_last_update|date_format:"%d/%m/%Y %H:%M"}}{{else}}
                 ...
                 {{foreach from=$item.doc_files key="keyfiles" item="itemfiles"}}
                    ...
                    <li><img src="{{$itemfiles.iconpath}}" width="20" /> <br />
                      ...
                      <ul class="small-menu">
<!--                        <li> {{if $itemfiles.bpd_docs_status == 0}}<a href="{{$BASE_URL}}{{$FILE_PATH}}files/{{$document_path}}/{{$itemfiles.bpd_name}}?dummy={{$ran}}" title="{{$itemfiles.dhd_name}}" target="_blank" style="color:#FFFFFF;">{{else}}<a href="{{$BASE_URL}}dochub.open/fileid/{{$itemfiles.bpd_docs_path}}?dummy={{$ran}}"  class="shofile" style="color:#FFFFFF;" >{{/if}}View/Edit</a></li>-->
...
<!-- <a href="{{$BASE_URL}}{{$FILE_PATH}}files/{{$document_path}}/{{$itemfiles.bpd_name}}?dummy={{$ran}}" title="{{$itemfiles.dhd_name}}" target="_blank" style="color:#FFFFFF;">-->
  <a href="/policy.download_content?file_name={{$itemfiles.bpd_name}}&module_name=policy.home" target="_blank"/>
