	...
	<table id="list-table" class="nav-back" width="100%">
		...
		<tbody>
			...
			<tr bgcolor="#FFFFFF">
				...
				<td>
					<span class="lm_doc_date_uploaded__ff" id="r__id_{{$item.lm_id}}">{{$item.lm_doc_date_uploaded}}</span>
				...
				<td>
					<form name="syn" method="post" action="" style="float: left;">
						...
						<input type="submit" name="sync" value="Sync" onclick="javascript:if(!confirm('Are you sure want to Synchronize This plan to Proposal Checklist?')) return false;">
						<br>
						{{$item.lm_sync_date}}
						<br>
						{{$item.lm_sync_user}}
					</form>
					<a style="margin-left: 10px;" href="{{$BASE_URL}}{{$XFA.delete}}/{{$ID}}/{{$item.$ID}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img style="height:20px; width:20px;" src="{{$BASE_URL}}css/admin/images/delete.png"></a>
				</td>
