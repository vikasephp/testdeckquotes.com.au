...
<form name="sort" method="post" action="">
  <div style="float:left; text-align:left;">
    ...
    <input type="text" name="{{$TABLE}}[keyword]" value="{{$keyword}}" style="width:300px;" placeholder="Enter project name after dash(-)"/>
    &nbsp;
...
<div id="">
 ...
 {{foreach from=$list key="key" item="item"}}
    ...
    <tr bgcolor="#FFFFFF">
        ...
        <td><span class="lm_doc_date_uploaded__ff" id="r__id_{{$item.lm_id}}"> {{$item.lm_doc_date_uploaded}} </span>
        ...
        <a href="/latest_merge_plan.download_content?file_name={{$item.lm_doc_file_name}}&module_name=latest_merge_plan.home" target="_blank">Download</a>
       {{/if}}
        </td>
        ...
        <td>
          <form name="syn" method="post" action="" style="float: left;">
            ...
            <input type="submit" name="sync" value="Sync" onclick="javascript:if(!confirm('Are you sure want to Synchronize This plan to Proposal Checklist?')) return false;" /> <br />
            {{$item.lm_sync_date}} <br /> {{$item.lm_sync_user}}
          </form>
          <a style="margin-left: 10px;" href="{{$BASE_URL}}{{$XFA.delete}}/{{$ID}}/{{$item.$ID}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete">
