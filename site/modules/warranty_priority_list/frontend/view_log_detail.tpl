...
<div align="center" style="min-height:350px; padding-top: 40px;">
    ...
    <table cellpadding="0" cellspacing="0" class="table" id="list-table" style="margin-bottom: 30px; width: 900px;">
		...
		<tbody>
			...
			<tr>
				<td {{if $data_log.wa_attention_req eq 'Yes'}}style="background:#F00;"{{/if}}>Urgent Attention Required</td>
				<td style="display:flex;flex-wrap:wrap;align-items:baseline;">
					<select name="[wa_attention_req]" Onchange="update_attention_req({{$data_log.wa_id}}, this.value)">
						<option value="No" {{if $data_log.wa_attention_req == 'No'}}selected{{/if}}>No</option>
						<option value="Yes" {{if $data_log.wa_attention_req == 'Yes'}}selected{{/if}}>Yes</option>
					</select>
					{{if $data_log.wa_attention_req eq 'Yes'}}
					<div class="ml-20">
						<span>{{$data_log.wa_attention_req_by}}</span>
						<span class="ml-20">{{$data_log.wa_attention_req_at|date_format:"%d-%m-%Y"}}</span>
					</div>
					{{/if}}
					<script>
						function update_attention_req(id, value) {
							$.ajax({
								type: "GET",
								url: "{{$BASE_URL}}warranty_log2.update_attention_req/wa_id/" + id + "/value/" + encodeURIComponent(value),
								success: function (result) {
									window.location.reload();
								}
							});
			...
			<tr>
				<td>Lockbox Status</td>
				<td>{{$lockbox_status}}</td>
			</tr>
			<tr>
					...
					<div style="display: flex; gap: 8px;">
						<div style="display: flex; flex-direction: column; gap: 5px;">
							...
							<div>
								...
								<br>
								<span>{{$data_log.wa_flag_date|date_format:"%d-%m-%Y"}}</span>
							</div>
							<script>
						...
						<div style="display: flex; flex-direction: column; gap: 8px;">
							<form method="POST" style="display: flex; flex-direction: row; gap: 8px; align-items: end; margin: 0;">
								<textarea rows="4" style="width: 350px;" name="elsn_notes" required></textarea>
								<input type="submit" value="Submit" name="update_elsn_notes">
							</form>
							{{if $warranty_log_escalation_notes}}
							<div style="display: flex; flex-direction: column; gap: 4px;">
								<span>{{$warranty_log_escalation_notes.elsn_notes}}</span>
								<span>{{$warranty_log_escalation_notes.user}}</span>
								<span>{{$warranty_log_escalation_notes.date}}</span>
							</div>
							{{/if}}
							<div>
								<a class="various" href="{{$BASE_URL}}warranty_log2.escalation_notes_view/wa_id/{{$data_log.wa_id}}">View all notes</a>
							</div>
						</div>
					</div>
				</td>
			...
			<tr>
				...
				<td>
					...
					<!-- {{if $data_log.wa_is_inspection_fee_required_accepted == 0 && $data_log.wa_is_inspection_fee_required_scheduled == 0 && $data_log.wa_is_inspection_fee_required_inspection_completed == 0}}{{/if}} -->
					{{if $data_log.wa_is_inspection_fee_required_inspection_completed == 0}}
						<p>
							<div style="display: flex; gap: 10px;">
								<form style="margin: 0;" method="POST">
									<label for="">Select Warranty Inspectors</label>
									<div class="bsearch_scroll_div" style="margin-bottom: 8px;">
										<input type="hidden" name="wi_id_array[]" value="1">
										<ul style="margin: 0; padding: 0;">
											{{foreach from=$warranty_inspectors key="index" item="row"}}
												{{ assign var=wi_id value=$row.wi_id }}
												<li>
													<input id="wi_id_array{{$row.wi_id}}" type="checkbox" name="wi_id_array[]" value="{{$row.wi_id}}" {{if $row.wi_is_admin == 1}}checked disabled title="You cannot uncheck this"{{/if}} {{if $warranty_log_inspectors.$wi_id}}checked{{/if}}>
													<label for="wi_id_array{{$row.wi_id}}">{{$row.wi_name}} ({{$row.wi_email}})</label>
												</li>
											{{/foreach}}
										</ul>
									</div>
									{{if $data_log.wa_is_inspection_fee_required && $data_log.wa_inspection_proposed_fees == 75 && !$data_log.wa_is_inspection_fee_required_cancelled}}
										<input type="submit" name="paid_inspection[insepection_fee]" value="$75" style="background: green;" onclick="return confirm('Are you sure you want to remove this log from inspection logs?')">
										<input type="hidden" name="paid_inspection[wa_is_inspection_fee_required]" value="0">
										<input type="hidden" name="paid_inspection[wa_inspection_proposed_fees]" value="">
										<input type="hidden" name="paid_inspection[wa_wlis_id]" value="0">
									{{else}}
										<input type="submit" name="paid_inspection[insepection_fee]" value="$75" onclick="return confirm('Are you sure you want to add this log for inspection for $75 inspection fees?')">
										<input type="hidden" name="paid_inspection[wa_is_inspection_fee_required]" value="1">
										<input type="hidden" name="paid_inspection[wa_inspection_proposed_fees]" value="75">
										<input type="hidden" name="paid_inspection[wa_wlis_id]" value="1">
									{{/if}}
									<input type="button" name="a" value="A">
									{{if $data_log.wa_is_inspection_fee_required && $data_log.wa_inspection_proposed_fees == 0 && !$data_log.wa_is_inspection_fee_required_cancelled}}
										<input type="submit" name="complimentary_inspection[insepection_fee]" value="Complimentary" style="background: green;" onclick="return confirm('Are you sure you want to remove this log from inspection logs?')">
										<input type="hidden" name="complimentary_inspection[wa_is_inspection_fee_required]" value="0">
										<input type="hidden" name="complimentary_inspection[wa_inspection_proposed_fees]" value="">
										<input type="hidden" name="complimentary_inspection[wa_wlis_id]" value="0">
									{{else}}
										<input type="submit" name="complimentary_inspection[insepection_fee]" value="Complimentary" onclick="return confirm('Are you sure you want to add this log for inspection for complimentary inspection fees?')">
										<input type="hidden" name="complimentary_inspection[wa_is_inspection_fee_required]" value="1">
										<input type="hidden" name="complimentary_inspection[wa_inspection_proposed_fees]" value="0">
										<input type="hidden" name="complimentary_inspection[wa_wlis_id]" value="1">
									{{/if}}
									<input type="button" name="a" value="A">
								</form>
							</div>
					...
					{{else}}
						<div>
							<p><strong>Warranty Inspectors:</strong></p>
							<ol style="padding-left: 15px; margin-top: 0;">
								{{foreach from=$warranty_inspectors key="index" item="row"}}
									{{ assign var=wi_id value=$row.wi_id }}
									{{if $warranty_log_inspectors.$wi_id}}
										<li>{{$row.wi_name}} ({{$row.wi_email}})</li>
									{{/if}}
								{{/foreach}}
							</ol>
						</div>
					...
					{{if $data_log.wa_is_inspection_fee_required}}
						<p style="margin-top: 10px;">
							...
							{{$data_log.inspection_status}}
						</p>
						<p style="margin-top: 10px;">
							<strong>Inspection Type:</strong>
							{{if $data_log.wa_inspection_proposed_fees == 75}}$75{{else}}Complimentary{{/if}}
						</p>
						<div style="height: 200px; overflow: auto; width: 405px;">
							<table style="width: 400px;">
								<tr>
									<th>Status</th>
									<th>Date</th>
								</tr>
								{{foreach from=$inspection_status_logs key="index" item="row"}}
									<tr>
										<td>{{$row.status}}</td>
										<td>{{$row.date}}</td>
									</tr>
					...
					<div style="margin-top: 10px;">
						<p id="view_status_main{{$data_log.wa_id}}">
							<a
								href="javascript:void(0);"
								onclick="
									let view_status_at{{$data_log.wa_id}} = document.getElementById('view_status_at{{$data_log.wa_id}}');
									if(view_status_at{{$data_log.wa_id}}.style.display == 'none') {
										view_status_at{{$data_log.wa_id}}.style.display = 'block';
									}
									else {
										view_status_at{{$data_log.wa_id}}.style.display = 'none';
									}
									
								"
							>Status History</a>
						</p>
						<table id="view_status_at{{$data_log.wa_id}}" style="display: none; width: 100%;">
							{{$data_log.status_history_table}}
						</table>
					</div>
				</td>
				<script>
			...
			<!-- <tr>
				<td>Inpsection Photos</td>
				<td>
					<div style="display: flex; flex-direction: column;">
						{{assign var=sn value=1}}
						{{foreach from=$inspection_photos key="index" item="row"}}
							<div>
								Photo {{$sn}} : <a href="{{$row.url}}" target="_blank">Download</a>
							</div>
							{{assign var=sn value=$sn+1}}
