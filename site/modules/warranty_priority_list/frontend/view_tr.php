...
$sqls = "SELECT se_id, se_wa_id, se_supplier, se_first_name, se_surname, se_email, se_auto_email, SUM(CASE WHEN se_status = 'Open' THEN se_no_of_alerts ELSE 0 END) AS open_alerts, SUM(CASE WHEN se_status = 'Pending' THEN se_no_of_alerts ELSE 0 END) AS pending_alerts, (SELECT GROUP_CONCAT(cs_mobile SEPARATOR ', ') FROM contacts WHERE cs_primary_email = supplier_email_warranty.se_email LIMIT 1) AS se_number FROM supplier_email_warranty WHERE se_status = 'Open' or se_status = 'Pending' GROUP BY se_email ORDER BY se_supplier";
