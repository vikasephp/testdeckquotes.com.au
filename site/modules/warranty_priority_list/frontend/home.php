...
$where = " WHERE ( wa_status = 'Open' Or wa_status = 'Pending' ) AND wa_attention_req = 'Yes' ";
...
if (!empty($submit)) {
	$where = "WHERE 1=1 AND wa_attention_req = 'Yes' ";
...
if (!empty($submitclosed)) {
	$where = "WHERE wa_attention_req = 'Yes' AND wa_status = 'Closed'";
...
$search_by_scheduling = $fwRequest->getParam('search_by_scheduling', '');
if ($search_by_scheduling) {
	//echo "<pre>"; print_R($_POST); exit();

    switch ($search_by_scheduling) {

        case 'Scheduled Today':
			$where .= " AND wa_schedule_date <> '' AND STR_TO_DATE(wa_schedule_date, '%d-%m-%Y') = CURDATE()";
			break;

        case 'Unscheduled Logs':
            $where .= " AND (wa_schedule_date = '' OR wa_schedule_date IS NULL)";
            break;

        case 'Overdue Scheduled Logs':
            $today = date('Y-m-d');
            $where .= " AND wa_schedule_date <> '' AND STR_TO_DATE(wa_schedule_date, '%d-%m-%Y') < '$today'";
            break;

        case 'Specific Scheduled Date':
			$specific_schedule_date = $fwRequest->getParam('specific_schedule_date', '');
            if (!empty($specific_schedule_date)) {
				$specific_schedule_date = date('d-m-Y', strtotime($specific_schedule_date));
				$where .= " AND wa_schedule_date = '$specific_schedule_date'";
				$fwViewData['specific_schedule_date'] = $specific_schedule_date;
			}
            break;
    }

    $_SESSION['search_by_scheduling'] = $search_by_scheduling;
    $fwViewData['search_by_scheduling'] = $search_by_scheduling;
    $_SESSION['where'] = $where;

} elseif (!empty($_SESSION['search_by_scheduling'])) {

    $search_by_scheduling = $_SESSION['search_by_scheduling'];
...
if ($clear) {
	...
	$fwViewData['search_by_scheduling'] = '';
	unset($_SESSION['search_by_scheduling']);
}
...
if (!empty($submit)) {
	...
	$where = " WHERE (wa_status like '%Open%' OR wa_status like '%Pending%') AND wa_attention_req = 'Yes' ";
...
if (!empty($submit7)) {
	...
	if ($oc == 1) {
		$where2 = " AND wa_status = 'Open' AND wa_attention_req = 'Yes' ";
	} elseif ($oc == 2) {
		$where2 = " AND  wa_status = 'Closed' AND wa_attention_req = 'Yes' ";
	}
	$fwViewData['oc'] = $oc;
...
WHERE wl.wa_attention_req = 'Yes' AND wl.wa_status = 'Open'
  AND (
    NOT EXISTS (
      SELECT 1
...
$query = 'SELECT wa_project, COUNT(*) AS total FROM warranty_log WHERE wa_status IN ("Open", "Pending") AND wa_attention_req = "Yes" GROUP BY wa_project';
$result = $fwDb->query($query);
...
$query = 'SELECT wa_type, COUNT(*) AS total FROM `warranty_log` WHERE wa_attention_req = "Yes" AND wa_id IN (SELECT wa_id FROM warranty_log WHERE wa_status = "Open" OR wa_status = "Pending") GROUP BY wa_type;';
$result = $fwDb->query($query);
foreach ($result as $row) {
...
//CGFB Count
$sql_cgfb_count = "SELECT COUNT(*) AS total_open_or_pending, SUM(CASE WHEN wa_cgfb_resp = 1 THEN 1 ELSE 0 END) AS cgfb_resp_1_count, SUM(CASE WHEN wa_cgfb_resp = 2 THEN 1 ELSE 0 END) AS cgfb_resp_2_count, SUM(CASE WHEN wa_cgfb_resp = 3 THEN 1 ELSE 0 END) AS cgfb_resp_3_count, SUM(CASE WHEN wa_cgfb_resp NOT IN (1, 2, 3) THEN 1 ELSE 0 END) AS cgfb_resp_please_select FROM warranty_log WHERE (wa_status = 'Open' OR wa_status = 'Pending') AND wa_attention_req = 'Yes' ";
...
//WorkmanShip Count
$sql_workmanship_count = "SELECT COUNT(*) AS total_workmanship, SUM(CASE WHEN wa_wlwd_id = 3 THEN 1 ELSE 0 END) AS workmanship_minor, SUM(CASE WHEN wa_wlwd_id = 4 THEN 1 ELSE 0 END) AS workmanship_major, SUM(CASE WHEN wa_wlwd_id = 5 THEN 1 ELSE 0 END) AS workmanship_equipment, SUM(CASE WHEN wa_wlwd_id NOT IN (3, 4, 5) THEN 1 ELSE 0 END) AS workmanship_please_select FROM warranty_log WHERE (wa_status = 'Open' OR wa_status = 'Pending') AND wa_attention_req = 'Yes' ";
