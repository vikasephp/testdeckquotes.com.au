...
//Search by Workmanship Defect
$workmanship_defect = $fwRequest->getParam('search_by_workmanship_defect', '');
if ($workmanship_defect):

	if ($workmanship_defect == 'NIL') {
		$where .= " And wa_wlwd_id = 0";
	} else {
		$where .= " And wa_wlwd_id = " . $workmanship_defect;
	}
	$_SESSION['workmanship_defect'] = $workmanship_defect;
	$fwViewData['workmanship_defect'] = $_SESSION['workmanship_defect'];
	$_SESSION['where'] = $where;

elseif ($_SESSION['workmanship_defect']):

	$where .= " And wa_wlwd_id = " . $_SESSION['workmanship_defect'];
	$fwViewData['workmanship_defect'] = $_SESSION['workmanship_defect'];
	$_SESSION['where'] = $where;

endif;

//Start Allocated Supplier 
$alloc_supplier = $fwRequest->getParam('alloc_supplier', '');
if ($alloc_supplier):

	$where .= " AND wa_include_supplier LIKE '%" . $alloc_supplier . "%'  ";
	$_SESSION['alloc_supplier'] = $alloc_supplier;
	$fwViewData['alloc_supplier'] = $_SESSION['alloc_supplier'];

elseif ($_SESSION['alloc_supplier'] && $pagenum > 0):

	$where .= " AND wa_include_supplier LIKE  '%" . $_SESSION['alloc_supplier'] . "%' ";
	$fwViewData['alloc_supplier'] = $_SESSION['alloc_supplier'];

endif;
//End Allocated Supplier
...
if ($clear) {
	...
	unset($_SESSION['workmanship_defect']);
	$fwViewData['workmanship_defect'] = '';
	unset($_SESSION['sortNotesDueDate']);
	$fwViewData['alloc_supplier'] = '';
	unset($_SESSION['alloc_supplier']);
}
...
if (!empty($submit)) {
	...
	foreach ($waData as $k => $m) {
		...
		$link = '';
		if ($m['wa_image_1']) {
			$link .= '<a target="_blank" href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_1"] . '&module_name=warranty_log.home">Link of Image 1 </a><br>';
		}
		if ($m['wa_image_2']) {
			$link .= '<a target="_blank" href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_2"] . '&module_name=warranty_log.home">Link of Image 2</a> <br>';
		}

		if ($m['wa_image_3']) {
			$link .= '<a target="_blank" href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_3"] . '&module_name=warranty_log.home">Link of Image 3</a> <br>';
		}

		if ($m['wa_image_4']) {
			$link .= '<a target="_blank" href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_4"] . '&module_name=warranty_log.home">Link of Image 4</a><br>';
		}

		if ($m['wa_image_5']) {
			$link .= '<a target="_blank" href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_5"] . '&module_name=warranty_log.home">Link of Image 5</a>';
		}
...
if (!empty($submit)) {
	...
	foreach ($waData as $k => $m) {
		...
		if ($m['wa_image_1']) {
			...
			if ($file !== false) {
				$link .= '<img src="' . $file . '" alt="Image 1" style="max-width:150px; max-height:150px; margin:5px;" /><br>';
			} else {
				$link .= '';
			}
			//$link .= '/warranty_log.download_content?file_name=' . $m["wa_image_1"] . '&module_name=warranty_log.home';
		}
		if ($m['wa_image_2']) {
			$folder_path = "files/warranty_log/";
			...
			$file = get_file_raw_data($folder_path, $object_name);
			if ($file !== false) {
				$link .= '<img src="' . $file . '" alt="Image 2" style="max-width:150px; max-height:150px; margin:5px;" /><br>';
			} else {
				$link .= '';
			}
			//$link .= '/warranty_log.download_content?file_name=' . $m["wa_image_2"] . '&module_name=warranty_log.home';
		}
		...
		if ($m['wa_image_3']) {
			...
			$file = get_file_raw_data($folder_path, $object_name);
			if ($file !== false) {
				$link .= '<img src="' . $file . '" alt="Image 3" style="max-width:150px; max-height:150px; margin:5px;" /><br>';
			} else {
				$link .= '';
			}
			//$link .= '/warranty_log.download_content?file_name=' . $m["wa_image_3"] . '&module_name=warranty_log.home';
		}
		...
		if ($m['wa_image_4']) {
			...
			$file = get_file_raw_data($folder_path, $object_name);
			if ($file !== false) {
				$link .= '<img src="' . $file . '" alt="Image 4" style="max-width:150px; max-height:150px; margin:5px;" /><br>';
			} else {
				$link .= '';
			}
			//$link .= '/warranty_log.download_content?file_name=' . $m["wa_image_4"] . '&module_name=warranty_log.home';
		}
		...
		if ($m['wa_image_5']) {
			...
			$file = get_file_raw_data($folder_path, $object_name);
			if ($file !== false) {
				$link .= '<img src="' . $file . '" alt="Image 1" style="max-width:150px; max-height:150px; margin:5px;" />';
			} else {
				$link .= '';
			}
			//$link .= '/warranty_log.download_content?file_name=' . $m["wa_image_5"] . '&module_name=warranty_log.home';
		}
		...
		if (!empty($notesData[$m['wa_id']])) {
			foreach ($notesData[$m['wa_id']] as $kn => $vn) {
				...
				$notes .= '<td>';
				if (!empty($vn['wn_attachment'])) {
					//$notes .= '<a href="' . BASE_URL . "files/uploads/" . $vn['wn_attachment'] . '">Link - Issue</a>';
					$fileUrl = BASE_URL . "files/uploads/" . $vn['wn_attachment'];
					...
					if ($headers && strpos($headers[0], '200') !== false) {
						...
						if (in_array(strtolower($contentType), $allowedImageTypes) || in_array($ext, $allowedImageExtensions)) {
							$notes .= '<img src="' . $fileUrl . '" alt="Attachment" style="max-width:150px; max-height:150px; margin:5px;" />';
						} else {
							$notes .= '<a href="' . $fileUrl . '" target="_blank">Download Attachment</a>';
						}
					} else {
						$notes .= '';
					}
		...
		if (!empty($suData[$m['wa_id']])) {
			foreach ($suData[$m['wa_id']] as $kn2 => $vn2) {
				...
				$notes .= '<td>';
				if (!empty($vn2['ws_attachment'])) {
					//$notes .= '<img src="https://warrantyreport.com.au/public/global_files/uploads/warranty_issue_comments_files/' . $vn2['ws_attachment']. '" alt="Image 5" style="max-width:150px; max-height:150px; margin:5px;" />';
					$imageUrl = 'https://warrantyreport.com.au/public/global_files/uploads/warranty_issue_comments_files/' . $vn2['ws_attachment'];

					$headers = @get_headers($imageUrl, 1);
					$ext = strtolower(pathinfo($imageUrl, PATHINFO_EXTENSION)); // Get file extension
					...
					if ($headers && strpos($headers[0], '200') !== false) {
						...
						if (is_array($contentType)) {
							$contentType = $contentType[0]; // Handle if multiple headers
						}

						// Check valid image
						if (in_array(strtolower($contentType), $allowedImageTypes) || in_array($ext, $allowedImageExtensions)) {
							$notes .= '<img src="' . $imageUrl . '" alt="Attachment" style="max-width:150px; max-height:150px; margin:5px;" />';
						} else {
							$notes .= '<a href="' . $imageUrl . '" target="_blank">Download Attachment</a>';
						}
					} else {
						// File not accessible or 404
						$notes .= '<span>Attachment Unavailable</span>';
					}
				}
				$notes .= '</td>';
				$notes .= '</tr>';
...
if (!empty($submit)) {
	...
	foreach ($waData as $k => $m) {
		...
		$link = '';
		if ($m['wa_image_1']) {
			$link .= '<a href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_1"] . '&module_name=warranty_log.home" target="_blank">Link of Image 1 </a><br>';
		}
		if ($m['wa_image_2']) {
			$link .= '<a href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_2"] . '&module_name=warranty_log.home" target="_blank">Link of Image 2</a> <br>';
		}

		if ($m['wa_image_3']) {
			$link .= '<a href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_3"] . '&module_name=warranty_log.home" target="_blank">Link of Image 3</a> <br>';
		}

		if ($m['wa_image_4']) {
			$link .= '<a href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_4"] . '&module_name=warranty_log.home" target="_blank">Link of Image 4</a><br>';
		}

		if ($m['wa_image_5']) {
			$link .= '<a href="' . BASE_URL . 'documents.download_content?file_name=' . $m["wa_image_5"] . '&module_name=warranty_log.home" target="_blank">Link of Image 5</a>';
		}
		...
		if (!empty($notesData[$m['wa_id']])) {
			foreach ($notesData[$m['wa_id']] as $kn => $vn) {
				...
				if (isset($vn['wn_id'])) {
					...
					$notes .= '<td>';
					if (!empty($vn['wn_attachment'])) {
						$notes .= '<a target="_blank" href="' . BASE_URL . "files/uploads/" . $vn['wn_attachment'] . '">Link</a><br/>';
					}
					if (!empty($vn['attachment1'])) {
						$notes .= '<a target="_blank" href="' . BASE_URL . "files/uploads/" . $vn['attachment1'] . '">Link</a><br/>';
					}
					if (!empty($vn['attachment2'])) {
						$notes .= '<a target="_blank" href="' . BASE_URL . "files/uploads/" . $vn['attachment2'] . '">Link</a><br/>';
					}
					if (!empty($vn['attachment3'])) {
						$notes .= '<a target="_blank" href="' . BASE_URL . "files/uploads/" . $vn['attachment3'] . '">Link</a><br/>';
					}
					if (!empty($vn['attachment4'])) {
						$notes .= '<a target="_blank" href="' . BASE_URL . "files/uploads/" . $vn['attachment4'] . '">Link</a><br/>';
					}
					if (!empty($vn['attachment5'])) {
						$notes .= '<a target="_blank" href="' . BASE_URL . "files/uploads/" . $vn['attachment5'] . '">Link</a><br/>';
					}
					$notes .= '</td>';
					...
					$notes .= '<td>';
					if (!empty($vn['ws_attachment'])) {
						$notes .= '<a target="_blank" href="https://www.warrantyreport.com.au/public/global_files/uploads/warranty_issue_comments_files/' . $vn['ws_attachment'] . '">Link</a><br/>';
					}
					if (!empty($vn['ws_attachment_2'])) {
						$notes .= '<a target="_blank" href="https://www.warrantyreport.com.au/public/global_files/uploads/warranty_issue_comments_files/' . $vn['ws_attachment_2'] . '">Link</a><br/>';
					}
					if (!empty($vn['ws_attachment_3'])) {
						$notes .= '<a target="_blank" href="https://www.warrantyreport.com.au/public/global_files/uploads/warranty_issue_comments_files/' . $vn['ws_attachment_3'] . '">Link</a><br/>';
					}
					if (!empty($vn['ws_attachment_4'])) {
						$notes .= '<a target="_blank" href="https://www.warrantyreport.com.au/public/global_files/uploads/warranty_issue_comments_files/' . $vn['ws_attachment_4'] . '">Link</a><br/>';
					}
					if (!empty($vn['ws_attachment_5'])) {
						$notes .= '<a target="_blank" href="https://www.warrantyreport.com.au/public/global_files/uploads/warranty_issue_comments_files/' . $vn['ws_attachment_5'] . '">Link</a><br/>';
					}
					$notes .= '</td>';
				}
				...
				$notes .= '<td>';
				if (!empty($vn['wn_attachment'])) {
					$notes .= '<a href="' . BASE_URL . "files/uploads/" . $vn['wn_attachment'] . '">Link</a><br/>';
				}
				if (!empty($vn['attachment1'])) {
					$notes .= '<a href="' . BASE_URL . "files/uploads/" . $vn['attachment1'] . '">Link</a><br/>';
				}
				if (!empty($vn['attachment2'])) {
					$notes .= '<a href="' . BASE_URL . "files/uploads/" . $vn['attachment2'] . '">Link</a><br/>';
				}
				if (!empty($vn['attachment3'])) {
					$notes .= '<a href="' . BASE_URL . "files/uploads/" . $vn['attachment3'] . '">Link</a><br/>';
				}
				if (!empty($vn['attachment4'])) {
					$notes .= '<a href="' . BASE_URL . "files/uploads/" . $vn['attachment4'] . '">Link</a><br/>';
				}
				if (!empty($vn['attachment5'])) {
					$notes .= '<a href="' . BASE_URL . "files/uploads/" . $vn['attachment5'] . '">Link</a><br/>';
				}
				$notes .= '</td>';*/
				$notes .= '</tr>';
			}
...
$query = 'SELECT wa_project, COUNT(*) AS total FROM warranty_log WHERE wa_status IN ("Open", "Pending") GROUP BY wa_project;';
...
if (!empty($setdata2)) {
	foreach ($setdata2 as $k => $v2) {
		...
		if (!empty($data['bs_business_id'])) {
			...
			$warranty_link = BASE_URL . 'sales_phase_logon.warranty/bsn_id/' . $data['bs_business_id'];
...
$workmanshipTable = new Fw_Db_Table("warranty_log_workmanship_defect");
$fwViewData['workmanshipDefect'] = $workmanshipTable->getAllRows();
...
//CGFB Count
$sql_cgfb_count = "SELECT COUNT(*) AS total_open_or_pending, SUM(CASE WHEN wa_cgfb_resp = 1 THEN 1 ELSE 0 END) AS cgfb_resp_1_count, SUM(CASE WHEN wa_cgfb_resp = 2 THEN 1 ELSE 0 END) AS cgfb_resp_2_count, SUM(CASE WHEN wa_cgfb_resp = 3 THEN 1 ELSE 0 END) AS cgfb_resp_3_count, SUM(CASE WHEN wa_cgfb_resp NOT IN (1, 2, 3) THEN 1 ELSE 0 END) AS cgfb_resp_please_select FROM warranty_log WHERE wa_status = 'Open' OR wa_status = 'Pending' ";
$result_cgfb_count = $fwDb->query($sql_cgfb_count);
//echo "<pre>"; print_r($result_cgfb_count); exit();
$fwViewData['cgfb_resp_1_count'] = $result_cgfb_count[0]['cgfb_resp_1_count'];
$fwViewData['cgfb_resp_2_count'] = $result_cgfb_count[0]['cgfb_resp_2_count'];
$fwViewData['cgfb_resp_3_count'] = $result_cgfb_count[0]['cgfb_resp_3_count'];
$fwViewData['total_open_or_pending'] = $result_cgfb_count[0]['total_open_or_pending'];
$fwViewData['cgfb_resp_please_select'] = $result_cgfb_count[0]['cgfb_resp_please_select'];

//WorkmanShip Count
$sql_workmanship_count = "SELECT COUNT(*) AS total_workmanship, SUM(CASE WHEN wa_wlwd_id = 3 THEN 1 ELSE 0 END) AS workmanship_minor, SUM(CASE WHEN wa_wlwd_id = 4 THEN 1 ELSE 0 END) AS workmanship_major, SUM(CASE WHEN wa_wlwd_id = 5 THEN 1 ELSE 0 END) AS workmanship_equipment, SUM(CASE WHEN wa_wlwd_id NOT IN (3, 4, 5) THEN 1 ELSE 0 END) AS workmanship_please_select FROM warranty_log WHERE wa_status = 'Open' OR wa_status = 'Pending' ";
$result_workmanship_count = $fwDb->query($sql_workmanship_count);
$fwViewData['total_workmanship'] = $result_workmanship_count[0]['total_workmanship'];
$fwViewData['workmanship_minor'] = $result_workmanship_count[0]['workmanship_minor'];
$fwViewData['workmanship_major'] = $result_workmanship_count[0]['workmanship_major'];
$fwViewData['workmanship_equipment'] = $result_workmanship_count[0]['workmanship_equipment'];
$fwViewData['workmanship_please_select'] = $result_workmanship_count[0]['workmanship_please_select'];
