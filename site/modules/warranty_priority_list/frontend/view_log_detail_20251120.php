...
$wa_is_inspection_fee_required = $fwRequest->getParam('wa_is_inspection_fee_required', '');
if($wa_is_inspection_fee_required != '' && $id != '') {
	$table->setWhere('wa_id = ' . $id);
	if($table->rowExists()) {
        $wa_wlis_id = 0;
        $wa_inspection_proposed_fees = 0;
        if($record['wa_is_inspection_fee_required'] == 0) {
            $wa_wlis_id = 1;
            $wa_inspection_proposed_fees = 75;
        }

		$record = $table->getRow();
		$table->updateRow([
			'wa_is_inspection_fee_required' => !$record['wa_is_inspection_fee_required'],
