...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
		...
		business_sellers.bc_incl_letter 
		...
		. $where. " And  business.bsn_status like '%|5|%' Order By business.bsn_status_sys_date DESC";


//$userData = $fwDb->query($sql);

//$sql = "Select business.bsn_id, business.bsn_address, person_responsible_detail.prd_name, business.bsn_status_sys_date from business 
//		Right JOIN person_responsible_detail ON business.bsn_id = person_responsible_detail.prd_bsn_id And 
//		person_responsible_detail.prd_prm_id = 10
//        where business.bsn_status like '%|5|%'  Order By business.bsn_status_sys_date Desc"   ;   
...
foreach($setdata as $k => $v)
{
 ...
 foreach ($setdata2 as $key => $row) {
	...
	$report = new Fw_Db_Table('report_inclusion');
	$report->setWhere("1=1");
	$report->deleteRows();
...
$fwViewData['title'] = "Inclusions Report";
