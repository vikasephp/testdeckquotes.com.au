...
<script type="text/javascript">
	...
	function add_new_checklist() {
		var viewurl = "{{$BASE_URL}}designer_tracker.add_checklist" + "/random/" + Math.random();
...
<style>
	...
	.checklist_tab th, .checklist_tab td {
		border: 1px solid black;
	}
	.checklist_tab tr th, .checklist_tab tr td {
		padding: 5px 10px;
	}
</style>
...
<form name="sort" method="post" action="">
	<div style="float: left; text-align: left; margin-left: 5px;">
		<!--<div style="margin-bottom: 5px;display:none;">
			<div style="display: inline-block; padding: 10px 10px; background: orange; color: red">
				<span style="font-weight: bold;">Revision Procedure Link:</span> {{$revision_procedure_link}}
			</div>
		</div>-->
		<div>
			<strong>Search: </strong>
			<input type="text" name="{{$TABLE}}[keyword]" value="{{$keyword}}" style="width:400px;" placeholder="Search for Task Description, Or Assigner Or Task Type">
		...
		<div>
			...
			<strong> Hide Complete </strong>
			<input type="checkbox" name="hidecomplete" onchange="this.form.submit();" {{if $val eq 1 }} checked="checked" {{/if}}>
		</div>
	</div>
	
	<div class="" style="float: left;margin-left: 16%;">
		<table class="checklist_tab">
			<thead>
				<tr>
					<th colspan="2">CHECKLIST</th>
					<th><input type="button" name="Add New" value="Add Checklist" onclick="javascript:add_new_checklist();" />	</th>
				</tr>
			</thead>
			<tbody>
				{{foreach from=$d_checklist key="key_check" item="item_check"}}
				<tr>
					<td>{{$item_check.dtc_name}}</td>
					<td>
						{{if $item_check.dtc_link}}
							<a href="{{$item_check.dtc_link}}" target="_blank">Link</a>
						{{/if}}
					</td>
					<td>
						<a href="{{$BASE_URL}}designer_tracker.add_checklist/dtc_id/{{$item_check.dtc_id}}" class="various"
							title="Edit"><img style="height:20px; width:20px;" title="Edit" src="{{$BASE_URL}}css/admin/images/edit.png" /></a>&nbsp;&nbsp;<a href="{{$BASE_URL}}designer_tracker.delete_checklist/dtc_id/{{$item_check.dtc_id}}" onclick="javascript:if(!confirm('Are you sure want to delete this Checklist?')) return false;" title="Delete"><img style="height: 16px;; width:16px" src="{{$BASE_URL}}css/admin/images/deletecross.png"/></a>
					</td>
				</tr>
				{{/foreach}}
			</tbody>
...
<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
	<div style="float:right;">
		<input type="button" name="Add New" value="Add New Task" onclick="javascript:add_new();" />		
	</div>
</form>
		...
		<div>
			<table id="doclist-table" class="nav-back" width="100%">
				...
				<tbody>
					...
					<tr {{if $item.tdt_status=='COMPLETE' }}style="background:#008000; color: #fff;"{{else}}bgcolor="#ffffff"{{/if}}>
						<td>{{$item.tdt_id}}</td>
						...
						<td>
							<select name="{{$TABLE}}[dt_priority]"
								Onchange="update_priority({{$item.tdt_id}},this.value)">
								<option value="-1" {{if $item.tdt_priority eq -1}} selected="selected" {{/if}}>Please
									Select </option>
								{{foreach from = $prdetail key="key4" item="item4"}}
								<option value="{{$item4.pr_id}}" {{if $item4.pr_id eq $item.tdt_priority}}
						...
						<td>
							...
							<option value="IN PROGRESS" {{if $item.tdt_status=='IN PROGRESS' }}selected{{/if}}>IN
								PROGRESS</option>
							<option value="AWAITING" {{if $item.tdt_status=='AWAITING' }}selected{{/if}}>AWAITING
							</option>
							<option value="FEEDBACK" {{if $item.tdt_status=='FEEDBACK' }}selected{{/if}}>FEEDBACK
							</option>
							<option value="COMPLETE" {{if $item.tdt_status=='COMPLETE' }}selected{{/if}}>COMPLETE
							</option>
							</select>
						...
						<td>
							<a {{if $item.tdt_status=='COMPLETE' }}style="color: #fff;"{{/if}} href="{{$BASE_URL}}designer_tracker.view_comment/tc_task_id/{{$item.tdt_id}}/tdt_dt_id/{{$item.tdt_dt_id}}"
								class="various" title="View">View</a>
						</td>


						<td> <a href="{{$BASE_URL}}designer_tracker.new_task/tdt_id/{{$item.tdt_id}}" class="various"
								title="Edit">
...
{{if $notask }}
<div style="margin-top:20px; font-size:20px;">{{$notask}}</div> {{/if}}