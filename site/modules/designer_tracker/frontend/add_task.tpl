...
<table id="list-table" width="100%">
   ...
   <tr>
    ...
    <tr>
     ...
     <td>
      {{if $tdt_id}} 
	     <input type="text" name="task[tdt_task_assigner]" value="{{$detail.tdt_task_assigner}}" style ="width:180px" />
     {{else}}
      <input type="text" name="task[tdt_task_assigner]" value="{{$user_name}}" style ="width:180px" />
     {{/if}}
   ...
   <tr>
     ...
     <th>Task Type</th>
     <td>
      <select name="task[tdt_task_type]">
          <option value="">Please Select</option>
         {{foreach from=$tasktype key="keyt" item="itemt"}}
         <option value="{{$itemt.tt_task_type}}" {{if $itemt.tt_task_type  eq $detail.tdt_task_type}} selected="selected" {{/if}}> 
             {{$itemt.tt_task_type}}
            </option>
         {{/foreach}}
       </select>
     
<!--     <select name="task[tdt_task_type]" style="width:300px;"/>
