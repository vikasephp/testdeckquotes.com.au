...
<form name="sort" method="post" action="">
	...
	<div style="float: left; text-align: left; margin-left: 5px;">
		...
		<div>
			...
			<input type="text" list="filter_project_list" name="{{$TABLE}}[keyword_project]" value="{{$keyword_project}}" style="width: 300px;" placeholder="Search for Project" />
			<datalist id="filter_project_list">
				{{foreach from=$filter_project_list key="index" item="item"}}
					<option>{{$index}}</option>
				{{/foreach}}
			</datalist>
		</div>
		<div>
			...
			<input type="text" list="filter_customer_list" name="{{$TABLE}}[keyword_customer]" value="{{$keyword_customer}}" style="width: 300px;" placeholder="Search for Customer" />
			<datalist id="filter_customer_list">
				{{foreach from=$filter_customer_list key="index" item="item"}}
					<option>{{$index}}</option>
				{{/foreach}}
			</datalist>
		</div>
		<div>
			<strong>Checklist No.:</strong>
			<input type="text" list="filter_checklist_no_list" name="{{$TABLE}}[keyword_checklist]" value="{{$keyword_checklist}}" style="width: 300px;" placeholder="Search for Checklist No." />
			<datalist id="filter_checklist_no_list">
				{{foreach from=$filter_checklist_no_list key="index" item="item"}}
					<option>{{$item}}</option>
				{{/foreach}}
			</datalist>
		</div>
		<div>
			<strong>Customer Designer:</strong>
			<input type="text" list="filter_designer_list" name="{{$TABLE}}[keyword_designer]" value="{{$keyword_designer}}" style="width: 300px;" placeholder="Search for Customer Designer" />
			<datalist id="filter_designer_list">
				{{foreach from=$filter_designer_list key="index" item="item"}}
					<option>{{$item}}</option>
				{{/foreach}}
			</datalist>
		</div>
		<div>
			...
			<input type="text" list="filter_architect_list" name="{{$TABLE}}[keyword_architect]" value="{{$keyword_architect}}" style="width: 300px;" placeholder="Search for Architect" />
			<datalist id="filter_architect_list">
				{{foreach from=$filter_architect_list key="index" item="item"}}
					<option>{{$item}}</option>
				{{/foreach}}
			</datalist>
			<input type="submit" value="Search" name="search" />
			<input type="submit" value="Clear All Search" name="clear" />
...
<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
	<div style="float:right;">
		...
		<input type="button" name="Add Person" value="Add Person"
			onclick="javascript:add_new('{{$BASE_URL}}{{$XFA.design_team}}');" />
		<input type="button" name="Edit Team" value="Edit Team"
			onclick="javascript:add_new('{{$BASE_URL}}{{$XFA.edit_team}}');" />
		<input type="button" name="Add Project" value="Add Project"
		...
		<input type="submit" value="Show All Project" name="showall" /> <br/>
		<input type="button" name="export_to_excel" value="Export to Excel"
			onclick="javascript:add_new('{{$BASE_URL}}designer_tracker.export_status');" />
	</div>
</form>
			...
			<table id="doclist-table" class="nav-back" width="100%">
				...
				<tbody>
					...
					<tr {{if $item.bsn_per_complete ge 90}} bgcolor="#66FF66" {{else}} bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}} {{/if}}">
						...
						<td><input type="button" name="viewtask" value="View" onclick="javascript:view_task({{$item.dt_id}});" style="width:90px; height:20px; font-size:11px; line-height:17px !important; padding:0px !important" />
						</td>
						...
						<td>
							<form name="recmet3" method="post" action="">
								...
								<label class="switch">
									<input class="switch-input" type="checkbox" name="completed[{{$item.dt_id}}]"
										value="1" onclick="this.form.submit();" {{if $item.dt_complete eq 1 }}
										checked="checked" {{/if}}>
