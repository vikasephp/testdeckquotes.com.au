...
<h3 class="page-title">Task List Report</h3>
<br />

<form name="sort" method="post" action="">
<div style="float:left; text-align:left;">
<span>&nbsp;&nbsp;<strong>Search By Assigner :</strong>&nbsp;&nbsp;</span>
 <select name="{{$TABLE}}[assigner]">
          <option value="">Please Select</option>
         {{foreach from=$assignerdata key="keyi" item="itemi"}}
            <option value="{{$itemi.tdt_task_assigner}}" > 
             {{$itemi.tdt_task_assigner}}
            </option>
 	 {{/foreach}}
 </select>
 <input type="submit" value="Search" name="search_assigner"  />
<br />

<span>&nbsp;&nbsp;<strong>Search By Assignee :</strong>&nbsp;&nbsp;</span>
 <select name="{{$TABLE}}[keyword]">
          <option value="">Please Select</option>
...
<!--<input type="text" name="{{$TABLE}}[keyword]" value="{{$keyword}}" style="width:400px;" placeholder="Search for Assignee"/>-->
&nbsp; 
...
<form name="taskc" method="post" action="">
<input type="submit" value="Show Current Task Only" name="current"  />
<input type="submit" value="Show Complete Task" name="complete"  />
</form>
</div>
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="6%">Priority</th>
...
<!--        <td> <a href="{{$BASE_URL}}designer_tracker.new_task/tdt_id/{{$item.tdt_id}}" class="various" title="Edit"> 
  ...
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
    <tr> {{if $list}}
      <td align="center"><div class='pagination'> {{if $pagenum == 1}} <span class='disabled'>« previous</span> {{else}} <a class="pagination" href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/{{math equation="x - y" x=$pagenum y=1}}" title="Previous">« previous</a> {{/if}}
          
          {{if $pagenum != 1}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/1" title="{{$page_num}} Page">1</a></span> {{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/2" title="{{$page_num}} Page">2</a></span> {{else}}<span class='current'>2</span> {{/if}}
          {{if $paginateprev.0 > 3}}
          ...
          {{/if}}
          ...
          {{else}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/{{$page_num}}" title="{{$page_num}} Page">{{$page_num}}</a></span> {{/if}}
          {{/foreach}}
          ...
          {{elseif $page_num2 == 2}}
          {{else}}
          {{if $page_num2 == $pagenum OR $page_num2 <= 0}} <span class='current'> {{$page_num2}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/{{$page_num2}}" title="{{$page_num2}} Page">{{$page_num2}}</a> {{/if}}
          {{/if}}     
          {{/if}}
          {{/foreach}}
          ...
          {{elseif $lastone == 2}}
          {{else}}
          {{if $pagenum == $lastone }} <span class='current'>{{$lastone}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/{{$lastone}}" title="{{$lastone}} Page">{{$lastone}}</a> {{/if}}
          {{/if}}
          {{if $last == 1}}
          {{elseif $last == 2}}
          {{else}}
          {{if $pagenum == $last}} <span class='current'>{{$last}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/{{$last}}" title="{{$last}} Page">{{$last}}</a> {{/if}}
          {{/if}}
          {{if $pagenum == $last}} <span class='disabled'>next »</span> {{else}} <a class="pagination" href="{{$BASE_URL}}designer_tracker.tasklist_report/pagenum/{{math equation="x + y" x=$pagenum y=1}}" title="Next">next »</a></div>
        {{/if}} </td>
      {{else}}
