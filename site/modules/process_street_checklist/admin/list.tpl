    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
            		...
            		<th class="topmenu" align="center" valign="middle" width="20%">Sales Path Way</th> 
                    	<th class="topmenu" align="center" valign="middle" width="70%">Link</th> 
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
             <td>{{counter}}</td>
                <td>{{$item.pst_sales_pathway|stripslashes}}</td>
                <td><a href="{{$item.pst_link}}" target="_blank">{{$item.pst_link}}</a></td>
