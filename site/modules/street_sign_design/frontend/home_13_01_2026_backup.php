...
//Move the Record to Planning Code
if (!empty($_POST['move']) && (int)$_POST['move'] === 1 && !empty($_POST['move_ss_id'])) {

    $move_ss_id = (int)$_POST['move_ss_id'];
    $record = $fwDb->queryOne("SELECT * FROM street_sign_design WHERE ss_id = $move_ss_id ");

    if ($record) {
        unset($record['ss_id']);
        $exists = $fwDb->queryOne("
            SELECT 1
            FROM street_sign_pl_in
            WHERE ss_hide =  0 AND ss_bsn_id = '{$record['ss_bsn_id']}'
              AND ss_address = '{$record['ss_address']}'
        ");
        ...
        if (!$exists) {
            ...
            $fwDb->queryOne("
                INSERT INTO street_sign_pl_in (" . implode(',', $columns) . ")
                VALUES (" . implode(',', $values) . ")
            ");

            $fwDb->queryOne("DELETE FROM street_sign_design WHERE ss_id = $move_ss_id ");
        }
    }
}
//End Move the Record to Planning Code
...
//$query = "SELECT bsn_status, bsn_id, bsn_name FROM business WHERE bsn_status LIKE '%|1|%' OR bsn_status LIKE '%|12|%' OR bsn_status LIKE '%|15|%' ";
//$query = "SELECT bsn_id, bsn_name, bsn_status FROM business WHERE (bsn_status LIKE '%|1|%' OR bsn_status LIKE '%|12|%') AND bsn_status NOT LIKE '%|2|%' AND bsn_status NOT LIKE '%|5|%' AND bsn_status NOT LIKE '%|18|%' AND bsn_status NOT LIKE '%|3|%' AND bsn_status NOT LIKE '%|4|%' AND bsn_status NOT LIKE '%|6|%' ";
...
if (!empty($setdata2)) {
	foreach ($setdata2 as $k => $v) {
		...
		// // $sqls = "SELECT bsn_status, bsn_id FROM business WHERE bsn_name LIKE '%" . stripslashes(trim($v['ss_address'])) . "%'";
		...
		$setdata2[$k]['ss_id'] = $v['ss_id'];
		$setdata2[$k]['bsn_id'] = $v['ss_bsn_id'];
		$setdata2[$k]['ss_address'] = $data['bsn_name'];
		// if($data['bsn_id']) {
		// 	echo $sql = 'UPDATE street_sign_design SET ss_bsn_id = ' . $data['bsn_id'] . ' WHERE ss_id = ' . $v['ss_id'];
		// 	echo '<br>';
...
$fwViewData['title'] = "CGFB Street Sign Report Designs";
...
$sql10 = "Select * from street_sign_type_design";
$fwViewData['typedetail'] = $fwDb->query($sql10);

$sqlact = "Select * from street_sign_action_design";
$fwViewData['actiondetail'] = $fwDb->query($sqlact);

$sqli = "Select * from street_sign_intensity_design";
$fwViewData['intensitydetail'] = $fwDb->query($sqli);

$sqlba = "Select * from street_sign_banner_design";
...
if ($export > 0) {
	$sqlexport = "Select street_sign_design.* from street_sign_design";
	...
	// Rename worksheet
	$objPHPExcel->getActiveSheet()->setTitle('Street Sign');
	$objPHPExcel->setActiveSheetIndex(0);

	// Redirect output to a client’s web browser (Excel5)
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename="street_sign_design.xls"');
...
if (!empty($submit)) {
	...
	if ($_SESSION['intensity_filter']) {
		$where .= " AND street_sign_design.ss_intensity IN (" . $_SESSION['intensity_filter'] . ")";
	}

	if ($_SESSION['signaction_filter']) {
		$where .= " AND street_sign_design.ss_sign_action LIKE '%" . $_SESSION['signaction_filter'] . "%'";
	}

	if ($_SESSION['banneraction_filter']) {
		$where .= " AND street_sign_design.ss_banner_action LIKE '%" . $_SESSION['banneraction_filter'] . "%'";
	}

	$sssql = "SELECT * from street_sign_design " . $where;
