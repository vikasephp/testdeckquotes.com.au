...
if (!empty($submit)) {
	...
	$query = "SELECT bsn_id FROM business WHERE bsn_name LIKE '" . stripslashes(trim($detail['ss_address'])) . "'";
	$result = $fwDb->queryOne($query);
	$detail['ss_bsn_id'] = $result['bsn_id'];
...
$sql = "SELECT bsn_id, bsn_name, bsn_address from business where 1 = 1 ";
