...
<script type="text/javascript">
	function viewWhereWeAreAt(pbnsr_checklist, pbnsr_bsn_id) {
		var viewurl = "{{$BASE_URL}}presented_but_not_signed_report.where_we_are_at" + "/bsn_id/" + pbnsr_bsn_id + "/checklist/" + pbnsr_checklist;
		$(document).ready(function () {
			$.fancybox({
				'width': '90%',
				'height': '99%',
...
<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
	...
	<div style="float:right;">
		<input type="button" value="Export To XLS"
			onclick="document.location.href='{{$BASE_URL}}presented_but_not_signed_report.home/export/1'" /> &nbsp;
...
<div style="border:1px solid black;width:auto;height:auto;overflow-y:hidden;overflow-x:scroll;">
	<div class="div2 tableFixHead">
				...
				<form name="mlist" method="post">
					...
					{{foreach from=$list key="key" item="item"}}
						<tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
							...
							<td>
								<input type="text" name="letter[{{$item.bsn_id}}]" value="{{$item.bs_pbns_letter_text}}"
									onkeyup="update_letter({{$item.bsn_id}},this.value)" style="width:60px;" />
							...
							<td>
								<select name="status[{{$item.bsn_id}}]"
									Onchange="update_status({{$item.bsn_id}},this.value)">
									<option value="">Please Select </option>
									{{foreach from=$statusData key="keym" item="itemm"}}
									<option value="{{$itemm.st_status}}" {{if $item.bs_pbns_status eq $itemm.st_status}}
										selected="selected" {{/if}}>{{$itemm.st_status}}</option>
									{{/foreach}}
								</select>
								<script>
							...
							<td>
								<textarea rows="4" cols="40" class="tinymce2" id="pbnsr_where_we_are_at_{{ $item.dpn_unique_id }}" style="overflow-y: scroll;"></textarea>
								<input type="button" value="Save" onclick="addWhereWeAreAt({{ $item.dpn_unique_id }}, {{ $item.bsn_id }})">
								<input type="button" value="View" onclick="viewWhereWeAreAt({{ $item.dpn_unique_id }}, {{ $item.bsn_id }})">
							</td>
							<td style="text-align:center">
								<a href="{{$BASE_URL}}presented_but_not_signed_report.view_alerts/bsn_id/{{$item.bs_business_id}}"
							...
							<td>
								...
								<a href="/presented_but_not_signed_report.download_content?file_name={{$item.checklist144}}&module_name=presented_but_not_signed_report.home"
									...
									<form name="recmet2" method="post" action="">
										...
										<label class="switch">
											<input class="switch-input" type="checkbox"
												name="task72[{{$item.dpn_unique_id}}]" value="1"
												onclick="this.form.submit();" {{if $item.bt_comp_72 eq 1 }}
												checked="checked" {{/if}} />
							...
							<td>
								...
								{{else}}
									...
									<form name="recmet2" method="post" action="">
										...
										<label class="switch">
											<input class="switch-input" type="checkbox"
												name="task33[{{$item.dpn_unique_id}}]" value="1"
												onclick="this.form.submit();" {{if $item.bt_comp_33 eq 1 }}
												checked="checked" {{/if}} />
											<span class="switch-label" data-on="Yes" data-off="No"></span>
											<span class="switch-handle"></span> <br />
										</label>
							...
							<td>
								<form name="spoken" method="post" action="">
									<input type="text" name="hia_signed[{{$item.bsn_id}}]"
										class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo2_{{$item.bsn_id}}"
										value="{{$item.bs_pbns_hia_signed}}" />
									<input type="submit" value="Set" name="pbd" class="set" />
								</form>
							</td>

							<td><!--{{$item.bt_complete_70}}-->
								<a href="{{$BASE_URL}}presented_but_not_signed_report.view_downloads/bsn_id/{{$item.bsn_id}}/dpn_unique_id/{{$item.dpn_unique_id}}"
									class="various">Downloads </a>
		...
		<td>m2{{$item.bt_complete_53}}</td>-->
							<td><a href="{{$BASE_URL}}presented_but_not_signed_report.view_spl_cond/bsn_id/{{$item.bsn_id}}"
								...
								<form name="recmet" method="post" action="">
									...
									<label class="switch">
										<input class="switch-input" type="checkbox" name="hide[{{$item.dpn_unique_id}}]"
											value="0" onclick="this.form.submit();" {{if $item.dpn_pbns_hide eq 0 }}
											checked="checked" {{/if}} />
										<span class="switch-label" data-on="Yes" data-off="No"></span>
										<span class="switch-handle"></span> <br />
									</label>
...
<script>
	function addWhereWeAreAt(pbnsr_checklist, pbnsr_bsn_id) {
		...
		const formData = {
            ...
            action: "addWhereWeAreAt"
        };
		$.ajax({
            type: "POST",
            url: "{{$BASE_URL}}presented_but_not_signed_report.ajax_handler",
            data: formData,
            dataType: "json",
            encode: true,
