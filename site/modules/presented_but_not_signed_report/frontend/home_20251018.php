...
$query = 'SELECT pbnsr_id, pbnsr_checklist, pbnsr_bsn_id, pbnsr_changes FROM pbnsr_where_we_are_at AS P1 WHERE pbnsr_id = (SELECT MAX(pbnsr_id) FROM pbnsr_where_we_are_at AS P2 WHERE P2.pbnsr_checklist = P1.pbnsr_checklist) GROUP BY pbnsr_checklist;';
$result = $fwDb->query($query);
$whereWeAreAtData = [];
foreach($result as $row) {
	$whereWeAreAtData[$row['pbnsr_checklist']] = $row['pbnsr_changes'];
}
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			$setdata2[$k]['changes'] = $whereWeAreAtData[$v['dpn_unique_id']] ?? '';
