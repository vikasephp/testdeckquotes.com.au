	...
	$(document).ready(function () {
		...
		$(".show-read-more").each(function () {
			...
			if ($.trim(myStr).length > maxLength) {
				...
				//var pendingStr = myStr.substring(maxLength, $.trim(myStr).length);
				//var pendingStr = myStr.substring(maxLength);
				var pendingStr = myStr.substring(maxLength, myStr.length);
				$(this).empty().html(newStr);
				$(this).append('<a href="javascript:void(0);" class="read-more">Read More...</a>');
				$(this).append('<span class="more-text">' + pendingStr + '</span>');
			}
		});
...
<style>
	...
	.mistake_log_c select,
	.mistake_log_c input,
	.mistake_log_c textarea {
		width: 100%;
				...
				<td>
					...
					{{if $item.ml_image}}
						<div style="margin-top: 5px;">
							<a href="/mistake_log.download_content?file_name={{$item.ml_image}}" style="font-weight: bold; font-size: 11px;" target="_blank">Download</a>
						</div>
					{{/if}}
				</td>
		...
		<tr>
			...
			<td align="center">
				<div class='pagination'>
					...
					{{if $page_num == $pagenum OR $page_num <= 2}} {{elseif $page_num==$lastone}} {{else}} <span class="paginate"><a class="pagination" href="{{$BASE_URL}}{{$XFA.home}}/pagenum/{{$page_num}}" title="{{$page_num}} Page">{{$page_num}}</a></span>
						{{/if}}
						{{/foreach}}

						{{foreach from=$paginatenext key="page_key" item="page_num2"}}
						{{if $page_num2 < $last AND $page_num2 !=$lastone}} {{if $page_num2==1}} {{elseif $page_num2==2}} {{else}} {{if $page_num2==$pagenum OR $page_num2 <=0}} <span class='current'> {{$page_num2}}</span>
							{{else}}
							...
							{{/if}}
							{{/foreach}}
							{{if $page_num2 < $lasttow}} ... {{/if}} {{if $lastone==1}} {{elseif $lastone==2}} {{else}} {{if $pagenum==$lastone }} <span class='current'>{{$lastone}}</span>
								{{else}}
