		...
		<table id="list-table" width="100%">
			...
			<tr>
				...
				<td>
					...
					<br />
					<!--<input type="button" value="AI Rewrite - TBD" name=""  />-->
					<input type="button" value="AI Rewrite" id="aiRewriteBtn" />
				</td>
			</tr>
			<script type="text/javascript">
				document.getElementById('aiRewriteBtn').addEventListener('click', function () {
					...
					this.disabled = true;
					this.value = 'Rewriting...';

					// AJAX call to your PHP endpoint
					var xhr = new XMLHttpRequest();
					xhr.open('POST', '{{$BASE_URL}}mistake_log.ai_rewrite', true);
					xhr.setRequestHeader('Content-Type', 'application/json');

					xhr.onreadystatechange = function () {
						if (xhr.readyState === 4) {
							document.getElementById('aiRewriteBtn').disabled = false;
							document.getElementById('aiRewriteBtn').value = 'AI Rewrite';
							...
							if (xhr.status === 200) {
								try {
									...
									if (response.status === 'success') {
										...
										//alert(response.message || 'Rewrite failed.');
			...
			<tr>
				<th>Upload Image</th>
				<td>
					<input type="file" name="ml_image" id="ml_image" style="width:700px;" />
					{{ if $detail.ml_image }}
						<a href="{{$BASE_URL}}mistake_log.download_content?file_name={{$detail.ml_image}}" style="font-weight: bold; font-size: 11px;" target="_blank">Download</a>
						<a href="{{$BASE_URL}}mistake_log.delete_form_files/{{$ID}}/{{$detail.$ID}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img src="{{$BASE_URL}}/images/delete_icon.png" width="20" /></a>
					{{/if}}
				</td>
			</tr>
