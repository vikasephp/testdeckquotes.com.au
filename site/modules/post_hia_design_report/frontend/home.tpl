...
<form method="POST" action="{{$BASE_URL}}{{$XFA.home}}" style="display:none;">
    <input type="submit" name="sort_by_day" value="Sort By Date" />
</form>
...
<input type="text" list = 'project' name="project_name" style ="width:350px" value="{{$search_project}}" /> 
<input type="submit" name="search" value = "Search" /> 
<input type="submit" id="triggerClear" name="clear" value = "Clear Search" /> 
&nbsp;&nbsp;<input type="submit" value="Show Hidden" name="unhide_phdr"> <br />
...
<input class="switch-input" type="checkbox"  name="signed[{{$item.ph_id}}]" value="1"  onclick="this.form.submit();" {{if $item.ph_client_signed eq 1 }} checked="checked" {{/if}} />
       ...
       <input type="file" class="upload-doc-file" data-ph-id="{{$item.ph_id}}" />
       <button class="upload-doc-file-btn" data-ph-id="{{$item.ph_id}}" data-ph-bsn-id="{{$item.bsn_id}}">Upload</button>
...
<script>
	...
	slider.oninput = function() {
      ...
      <textarea name="wherewe" rows="5" onkeyup="update_where_we({{$item.ph_id}},this.value)" style="width:200px">{{$item.ph_where_we_are}}</textarea>
         <script>
       ...
       <td> {{$item.ph_request}}<br />{{$item.ph_addto_user}}<br />{{$item.ph_addto_date}}
        ...
        <td>
            <form name="phdrHide" method="post" action="">
                <label class="switch" for="ph_phdr_hide_{{ $item.ph_id }}" data-value="{{ $item.ph_phdr_hide }}">
                    <input id="ph_phdr_hide_{{ $item.ph_id }}" class="switch-input" {{ if $item.ph_phdr_hide eq 1
                        }} checked {{ /if }} onclick="phdrDisplayToggle('{{$item.ph_id}}', this.checked)"
                        type="checkbox" />
                    <span class="switch-label" data-on="Yes" data-off="No"></span>
                    <span class="switch-handle"></span> <br />
                </label>
            ...
            <script>
			function phdrDisplayToggle(id,value)
			{
			    var val;
		        if(value == false) { val = 0; } else { val = 1; }
					...
					$.ajax({
					   type: "GET",
					   url: "{{$BASE_URL}}post_hia_design_report.update_phdr_toggle/ph_id/"+id+"/value/"+val,
						     success: function(result){
							if (val == 1){
							    //alert("Status is changed to Hide");
                  //$("input[name='unhide_phdr']").val("");                
                  //location.reload();
...
<script>
var $newJQ = $.noConflict(true);
$newJQ(document).ready(function() {
    $newJQ(document).on('click', '.upload-doc-file-btn', function(event) {
        event.preventDefault();
        var $uploadBtn = $newJQ(this);
        const phId = $newJQ(this).data('ph-id');
        const phBsnId = $newJQ(this).data('ph-bsn-id');
        const fileInput = $newJQ(`input[data-ph-id="${phId}"]`);
        const file = fileInput[0].files[0];

        if (file) {
            const formData = new FormData();
            formData.append('upload_doc_file', file);
            formData.append('ph_id', phId);
            formData.append('ph_bsn_id', phBsnId);
