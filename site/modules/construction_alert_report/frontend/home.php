...
if ($clear) {
	...
	unset($_SESSION['filter_car_created_by']);
	unset($_SESSION['noContactAssigned']);
	unset($_SESSION['noDueDate']);
	//unset($_SESSION['noRevDueDate']);
	$fwViewData['noContactAssigned'] = '';
} else {
	$filter_car_created_by = $fwRequest->getParam('filter_car_created_by', '');
}
...
$sqlp = "SELECT business.bsn_name, business.bsn_id from business";

$projdetail = $fwDb->query($sqlp);
...
foreach ($projdetail as $project) {
	$projectIdArr[$project['bsn_id']] = $project['bsn_name'];
	$projectNameArr[$project['bsn_name']] = $project['bsn_id'];
}


$fwViewData['projectIdArr'] = $projectIdArr;
$fwViewData['projectNameArr'] = $projectNameArr;
...
if ($include) {
	...
	$sqlp = "select car_project, car_status, car_bsn_id, car_id from construction_alert_report where car_id = " . $ky_2;
	$projData = $fwDb->queryOne($sqlp);


	if ($projData['car_status'] == 'Open') {

		$sq1cust = "SELECT business.bsn_name, business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname, bus_customers.bcust_misc_moble, bcust_misc_email1 from business_sellers
		  Inner Join bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id
		  Inner Join business ON business_sellers.bs_business_id = business.bsn_id
		  where business.bsn_id = '" . $projData['car_bsn_id'] . "'";
		...
		if (!$tableic->rowExists() && $val_2 == 'Yes') {
			...
			$icDetail['ic_car_id'] = $projData['car_id'];
			$icDetail['ic_bsn_id'] = $projData['car_bsn_id'];
			$icDetail['ic_project'] = $custdata['bsn_name'];
	...
	if ($val_2 == 'Yes') {
		...
		$html .= '<tr><td>Project</td><td>' . $projectIdArr[$Data_ic['car_bsn_id']] . '</td></tr>';
		...
		if ($Data_ic['car_image1']) {
			$link .= ($Data_ic['car_image1_newdesign'] ? '<a href="' . showUrl($Data_ic['car_image1']) . '">Link to Attachment1</a><br>' : '<a href="' . BASE_URL . "files/uploads/" . $Data_ic['car_image1'] . '">Link to Attachment1</a><br>');
		}
		if ($Data_ic['car_image2']) {
			$link .= ($Data_ic['car_image2_newdesign'] ? '<a href="' . showUrl($Data_ic['car_image2']) . '">Link to Attachment1</a><br>' : '<a href="' . BASE_URL . "files/uploads/" . $Data_ic['car_image2'] . '">Link to Attachment2</a><br>');
		}

		if ($Data_ic['car_image3']) {
			$link .= ($Data_ic['car_image3_newdesign'] ? '<a href="' . showUrl($Data_ic['car_image3']) . '">Link to Attachment1</a><br>' : '<a href="' . BASE_URL . "files/uploads/" . $Data_ic['car_image3'] . '">Link to Attachment3</a><br>');
		}

		if ($Data_ic['car_image4']) {
			$link .= ($Data_ic['car_image4_newdesign'] ? '<a href="' . showUrl($Data_ic['car_image4']) . '">Link to Attachment1</a><br>' : '<a href="' . BASE_URL . "files/uploads/" . $Data_ic['car_image4'] . '">Link to Attachment4</a><br>');
		}

		if ($Data_ic['car_image5']) {
			$link .= ($Data_ic['car_image5_newdesign'] ? '<a href="' . showUrl($Data_ic['car_image5']) . '">Link to Attachment1</a><br>' : '<a href="' . BASE_URL . "files/uploads/" . $v2['car_image5'] . '">Link to Attachment5</a><br>');
		}
		if ($link) {
		...
		$sql_3 = "Select ic_name, ic_email from include_customers where ic_project = '" . $projectIdArr[$Data_ic['car_bsn_id']] . "'";
		...
		// send_email($toname, 'amans@ephpsolutions.com', 'CGFB', 'operations@cgfb.com.au', $subject, $message, $attachmentsend);
		// send_email($toname, 'supportmanager@cgfb.com.au', 'CGFB', 'operations@cgfb.com.au', $subject, $message, $attachmentsend);
	}
}
...
if ($project):
	...
	$where .= " AND  construction_alert_report.car_bsn_id  = " . $projectNameArr[$project] . " ";
	$_SESSION['project'] = $project;
	$fwViewData['project'] = $_SESSION['project'];

elseif ($_SESSION['project'] && $pagenum >= 0):

	$where .= " AND  construction_alert_report.car_bsn_id = " . $projectNameArr[$_SESSION['project']] . " ";
	$fwViewData['project'] = $_SESSION['project'];
...
if ($filter_car_created_by):
	$where = " where construction_alert_report.car_created_by LIKE '%" . $filter_car_created_by . "%'  ";
	$_SESSION['filter_car_created_by'] = $filter_car_created_by;
	$fwViewData['filter_car_created_by'] = $_SESSION['filter_car_created_by'];
elseif ($_SESSION['filter_car_created_by'] && $pagenum >= 0):
	$where = " where  construction_alert_report.car_created_by LIKE '%" . $_SESSION['filter_car_created_by'] . "%'   ";
	$fwViewData['filter_car_created_by'] = $_SESSION['filter_car_created_by'];
else:
	$fwViewData['filter_car_created_by'] = '';
endif;

$noContactAssigned = $fwRequest->getParam('noContactAssigned', '');
if (!empty($noContactAssigned)):
	//$where .= " AND car_include_supplier IS NOT NULL AND car_include_supplier <> '' AND ( car_incl_supp_email IS NULL OR car_incl_supp_email = '' )";
	$where .= " AND ( car_incl_supp_email IS NULL OR car_incl_supp_email = '' )";
	$_SESSION['noContactAssigned'] = $noContactAssigned;
	$fwViewData['noContactAssigned'] = $_SESSION['noContactAssigned'];
elseif ($_SESSION['noContactAssigned'] && $pagenum >= 0):
	//$where .= " AND car_include_supplier IS NOT NULL AND car_include_supplier <> '' AND ( car_incl_supp_email IS NULL OR car_incl_supp_email = '' )";
	$where .= " AND ( car_incl_supp_email IS NULL OR car_incl_supp_email = '' )";
	$fwViewData['noContactAssigned'] = $_SESSION['noContactAssigned'];
else:
...
$noDueDate = $fwRequest->getParam('noDueDate', '');
if (!empty($noDueDate)):
	$where .= " AND car_new_date = ''";
	$_SESSION['noDueDate'] = $noDueDate;
	$fwViewData['noDueDate'] = $_SESSION['noDueDate'];
elseif ($_SESSION['noDueDate'] && $pagenum >= 0):
	$where .= " AND car_new_date = ''";
	$fwViewData['noDueDate'] = $_SESSION['noDueDate'];
else:
	unset($_SESSION['noDueDate']);
	$fwViewData['noDueDate'] = '';
endif;

// No Due Date Ends

//$noRevDueDate = $fwRequest->getParam('noRevDueDate', '');
//if (!empty($noRevDueDate)):
//	$where .= " AND car_revised_new_date = ''";
//	$_SESSION['noRevDueDate'] = $noRevDueDate;
//	$fwViewData['noRevDueDate'] = $_SESSION['noRevDueDate'];
//elseif ($_SESSION['noRevDueDate'] && $pagenum >= 0):
//	$where .= " AND car_revised_new_date = ''";
//	$fwViewData['noRevDueDate'] = $_SESSION['noRevDueDate'];
//else:
//	unset($_SESSION['noRevDueDate']);
//	$fwViewData['noRevDueDate'] = '';
//endif;
...
$countQuery = "SELECT COUNT(*) AS total FROM " . $TABLE . " " . $where . " " . $ord . "";
$total_rows = $fwDb->queryOne($countQuery)['total'];

$query = "SELECT DISTINCT " . $TABLE . ".car_id AS distinct_id, (SELECT MAX(STR_TO_DATE(cn_date, '%d-%m-%Y' )) FROM car_notes WHERE cn_car_id = " . $TABLE . ".car_id) AS notes_date, (SELECT MAX(cu_date) FROM car_supplier_update WHERE cu_car_id = " . $TABLE . ".car_id) AS notes_date, (SELECT COUNT(*) FROM car_supplier_update WHERE cu_car_id = " . $TABLE . ".car_id AND cu_alert_resolved = 1) AS cuar, (SELECT pr_priority FROM car_priority WHERE pr_id = " . $TABLE . ".car_urgency) AS pr_priority, (SELECT COUNT(*) FROM car_supplier_update WHERE car_supplier_update.cu_car_id = " . $TABLE . ".car_id) AS tot, " . $TABLE . ".*, business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname, bus_customers.bcust_misc_moble, business.bsn_name FROM " . $TABLE . " INNER JOIN business ON business.bsn_id = " . $TABLE . ".car_bsn_id INNER JOIN business_sellers ON business_sellers.bs_business_id = business.bsn_id INNER JOIN bus_customers ON bus_customers.bcust_id = business_sellers.bs_customers_id " . $where . " GROUP BY construction_alert_report.car_id " . $ord . "";
// $startTime = microtime(true);
// $endTime = microtime(true);
// $executionTime = $endTime - $startTime;
// echo "END SINGLE QUERY & START LOOP:";
// db($executionTime);
...
if ($total_rows > 0) {
	...
	$listsnew = $fwDb->query($sql2);

	foreach ($listsnew as $i => $v) {
		if (!empty($listsnew[$i])) {
			if ($listsnew[$i]['tot'] > 0) {
				$listsnew[$i]['supplier_upadate'] = true;
			}
			$listsnew[$i]['car_project'] = stripslashes($listsnew[$i]['bsn_name']);

			$listsnew[$i]['urgency'] = $listsnew[$i]['pr_priority'];

			if (count($listsnew[$i]['cuar']) >= 1) {
				$listsnew[$i]['resolved'] = 1;
			} else {
				$listsnew[$i]['resolved'] = 0;
			}
			
			if ($listsnew[$i]['car_include_supplier'] != '' && $listsnew[$i]['car_incl_supp_email'] == ''){
				$listsnew[$i]['no_contact_assigned'] = 1;
			} else {
				$listsnew[$i]['no_contact_assigned'] = 0;
			}

			$listsnew[$i]['notes_date']  = changedate_d_m_y(strtotime($listsnew[$i]['notes_date']) > strtotime($listsnew[$i]['notes_date2']) ? $listsnew[$i]['notes_date'] : $listsnew[$i]['notes_date2']);
...
// db(ini_get('memory_limit'));
// db(ini_get('max_execution_time'));
...
/**
 * CRON
 * include_customers_email
 * supplier_email
 * weekly_car_email
 * car_email_report
 * car_staff_email
 * car_action_step_email
 */

//  echo showUrl('test1.jpg');
