...
$tableSupemail = new Fw_Db_Table('supplier_email');
...
$car_id = $fwRequest->getParam('car_id', '');
	...
	$matsql = "SELECT contacts.* , companies.co_company_name, type_options.to_option  from contacts 
	  ...
	  foreach($list as $ck=>$vc)
	 {
		...
		$sqlc = "Select * from supplier_email where se_email = '".$vc['cs_primary_email']."' and se_car_id = ".$car_id;
...
if(!empty($add_emails))
{
	...
	$sqlAdd = "Select se_email from supplier_email where se_car_id = ".$car_id;
	...
	$detail['car_incl_supp_email'] = $em2;
