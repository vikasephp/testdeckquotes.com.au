...
if($email_report && empty($er))
{
	 ...
	 $ky = $keys[0];
	 $val = $email_report[$ky];
	 if($val == 'on' ) { $edetail['ic_auto_email'] = 1; } else { $edetail['ic_auto_email'] = 0; }
...
$sqlc = "select * from include_customers";

$custData = $fwDb->query($sqlc);

foreach($custData as $k => $v)
{
	$sql = " Select count(*) as tot from construction_alert_report  where car_project like '%".$v['ic_project']."%' 
		 and car_status = 'Open' and car_include_cust = 'Yes'";
	$totalert = $fwDb->queryOne($sql);
	$custData[$k]['tot_alert'] = $totalert['tot'];	 	
}
