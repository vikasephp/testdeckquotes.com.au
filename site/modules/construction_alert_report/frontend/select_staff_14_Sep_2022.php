...
$tableStaff = new Fw_Db_Table('include_resp_staff');
...
$car_id = $fwRequest->getParam('car_id', '');
	...
	$matsql = "SELECT contacts.* , companies.co_company_name, type_options.to_option  from contacts 
		  ...
		  where companies.co_id = 2 order by contacts.cs_is_primary desc ";
	 $list = $fwDb->query($matsql);
	 foreach($list as $ck=>$vc)
	 {
		
		$sqlc = "Select * from include_resp_staff where ir_email = '".$vc['cs_primary_email']."' and ir_car_id = ".$car_id;
...
if(!empty($add_emails))
{
	...
	foreach($emails as $k => $v)
	{
		...
		if($v =='on') {
		 ...
		 $tableStaff->setWhere("ir_email = '".$email."' and ir_car_id =".$car_id);
	 	 if(!$tableStaff->rowExists())
	         {
			if($v == 'on') {
				$irDetail['ir_car_id'] = $car_id;
