...
<div align="center" style="min-height:350px;">
	...
	<form name="detail" method="post" action="{{$XFA.detail}}" enctype="multipart/form-data">
		<table id="list-table" width="100%">
			...
			<tr>
				...
				<td>
					...
					<input type="text" list='project' name="{{$TABLE}}[car_project]" style="width:700px"
						value="{{$detail.car_project}}" />
				</td>
			</tr>

			<tr>
				<th>Date</th>
				<td><input type="text" name="{{$TABLE}}[car_date]" value="{{$detail.car_date}}" id="demo1"
						class="w16em dateformat-d-ds-m-ds-Y dtpic" /> </td>
			</tr>

			<tr>
				<th>Alert</th>
				<td>
					<!--<input  type="text" name="{{$TABLE}}[car_alert]"  value="{{$detail.car_alert}}" style="width:700px;" /> -->
					<textarea cols="180" rows="5" name="{{$TABLE}}[car_alert]" id="editor1"
						class="ckeditor">{{$detail.car_alert}}</textarea>
			...
			<tr>
				...
				<td>
					...
					<div style="text-align:left;">
						{{ if $detail.car_image1_newdesign }}
							Photo 1 : <a href="/{{ $BASEFOLDER }}.download_content?file_name={{$detail.car_image1}}&module_name={{ $BASEFOLDER }}.home" target="_blank">Preview</a>
						{{ else }}
							Photo 1 : <a href="{{$BASE_URL}}files/uploads/{{$detail.car_image1}}" target="_blank">Preview</a>
						{{ /if }}
					...
					<div style="text-align:left;">
						{{ if $detail.car_image2_newdesign }}
							Photo 2 : <a href="/{{ $BASEFOLDER }}.download_content?file_name={{$detail.car_image2}}&module_name={{ $BASEFOLDER }}.home" target="_blank">Preview</a>
						{{ else }}
							Photo 2 : <a href="{{$BASE_URL}}files/uploads/{{$detail.car_image2}}" target="_blank">Preview</a>
						{{ /if }}
					...
					<div style="text-align:left;">
						{{ if $detail.car_image3_newdesign }}
							Photo 3 : <a href="/{{ $BASEFOLDER }}.download_content?file_name={{$detail.car_image3}}&module_name={{ $BASEFOLDER }}.home" target="_blank">Preview</a>
						{{ else }}
							Photo 3 : <a href="{{$BASE_URL}}files/uploads/{{$detail.car_image3}}" target="_blank">Preview</a>
						{{ /if }}
					...
					<div style="text-align:left;">
						{{ if $detail.car_image4_newdesign }}
							Photo 4 : <a href="/{{ $BASEFOLDER }}.download_content?file_name={{$detail.car_image4}}&module_name={{ $BASEFOLDER }}.home" target="_blank">Preview</a>
						{{ else }}
							Photo 4 : <a href="{{$BASE_URL}}files/uploads/{{$detail.car_image4}}" target="_blank">Preview</a>
						{{ /if }}
					...
					<div style="text-align:left;">
						{{ if $detail.car_image5_newdesign }}
							Photo 5 : <a href="/{{ $BASEFOLDER }}.download_content?file_name={{$detail.car_image5}}&module_name={{ $BASEFOLDER }}.home" target="_blank">Preview</a>
						{{ else }}
							Photo 5 : <a href="{{$BASE_URL}}files/uploads/{{$detail.car_image5}}" target="_blank">Preview</a>
						{{ /if }}
          ...
          {{/foreach}}
					...
					<div class="multiselect">
						...
						<div id="checkboxes">
							...
							<label for="one{{$item.po_id}}">
								<input type="checkbox" id="one_{{$item.car_id}}" name="{{$TABLE}}[car_type][]"
									value="{{$item4.cp_type}}" />{{$item4.cp_type}}</label>
							{{/foreach}}
						</div>
					</div>
			...
			<tr>
				...
				<td><select name="{{$TABLE}}[car_urgency]" style="width:300px;" />
					...
					<option value="{{$item3.pr_id}}" {{if $detail.car_urgency==$item3.pr_id}}selected{{/if}}>
						{{$item3.pr_priority}}
					</option>
					{{/foreach}}
					</select>
			...
			<tr>
				<th>Due Date</th>
				<td><input type="text" name="{{$TABLE}}[car_new_date]" value="{{$detail.car_new_date}}" id="demo2"
						class="w16em dateformat-d-ds-m-ds-Y dtpic" /> </td>
			</tr>
     ...
     <td> 
			...
			<tr>
				...
				<td><select name="{{$TABLE}}[car_status]" style="width:300px;" />
                 ...
                 <option value="Closed" {{if  $detail.car_status == 'Closed'}}selected{{/if}}>Closed</option>-->
					...
					<option value="{{$item5.to_option}}" {{if $item5.to_option eq $detail.car_status}}
						selected="selected" {{/if}}>{{$item5.to_option}}</option>
					{{/foreach}}
					</select>
				</td>
			</tr>
       ...
       -->
					<option value="{{$item4.po_name_eml}}" {{if
						$detail.car_created_by==$item4.po_name_eml}}selected{{/if}}>
						{{$item4.po_name_eml}}
					</option>
