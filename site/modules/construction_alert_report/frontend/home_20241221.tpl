...
<div id="">
 ...
 {{foreach from=$list key="key" item="item"}}
     ...
     <tr {{if $item.car_status eq 'Pending'}} bgcolor="#f7b900" {{else}} bgcolor="#FFFFFF" {{/if}}>
        ...
        Photo 1 :  <a href="{{$BASE_URL}}files/uploads/{{$item.car_image1}}" target="_blank">
            <img src="{{$BASE_URL}}files/uploads/{{$item.car_image1}}" >
            Download</a>
        </div>
        {{/if}}
        ...
        <td>
         ...
         <option value="">Please Select</option>
          {{foreach from=$typedetail key="key6" item="item6"}}
          <option value="{{$item6.cp_type}}" {{if $item.car_type eq $item6.cp_type}} selected="selected" {{/if}}>{{$item6.cp_type}}</option>
	      {{/foreach}}
	      </select> 
        ...
        <td>
        <input type="text" name="car_new_date[{{$item.car_id}}]" class="w16em dateformat-d-ds-m-ds-Y" id="demo_{{$item.car_id}}" value="{{$item.car_new_date}}" onfocus ="update_two({{$item.car_id}}, this.value);" style="width:70px;"  /> 
...
<input class="switch-input" type="checkbox"  name="include[{{$item.car_id}}]" value="Yes"  onclick="this.form.submit();" {{if $item.car_include_cust eq 'Yes' }} checked="checked" {{/if}} />
        ...
        <td> 
         {{if $item.car_status eq 'Pending'}}
       		 <a href="{{$BASE_URL}}construction_alert_report.status_approve/car_id/{{$item.car_id}}" class="various">{{$item.car_status}}
        {{else}}
        	{{$item.car_status}}
