			...
			<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
				...
				<div>
					...
					<div style="display: inline-block; margin-left: 5px;">
						Total Amount Owed <input type="text" value="{{$totaAmountOwed}}" style="width: 100px;" readonly>
					</div>
				</div>				
			</form>
		</td>
	...
	<table id="list-table" class="nav-back" width="100%">
		<thead>
			<tr>
				...
				<th class="topmenu" align="center" valign="middle" width="6%">Invoice Amount</th>
				<th class="topmenu" align="center" valign="middle" width="6%" data-col="amount_owed">Amount Owed</th>
				...
				<th class="topmenu" align="center" valign="middle" width="6%">Total Interest</th>
				...
				<th class="topmenu" align="center" valign="middle" width="5%">Alerts</th>
			</tr>
		</thead>
		<tbody>
			...
			<tr bgcolor="#FFFFFF">
				...
				<td data-hia_value="{{$item.hia_value}}" data-ppd_value="{{$item.ppd_value}}" data-amount_owed="{{$item.amount_owed}}" data-interest_amount="{{$item.interest_amount}}" data-total_amount_calculated="{{$item.total_amount_calculated}}">
					{{$item.ppd_hia_value}}
				</td>
				<td data-col="amount_owed">
					{{if $item.ppd_hia_value && $item.ppd_value }}
						{{assign var="hia" value=$item.ppd_hia_value|replace:"$":""|replace:",":""}}
						{{assign var="val" value=$item.ppd_value|replace:"$":""|replace:",":""}}
						{{math assign="amount_owed" equation="hia - (val)" hia=$hia val=$val}}
						${{$amount_owed|number_format:2:".":","}}
					{{/if}}
				</td>
				<td>14%</td>
				<td data-col="annual">
					{{if $item.ppd_hia_value && $item.ppd_value }}
						{{assign var="hia" value=$item.ppd_hia_value|replace:"$":""|replace:",":""}}
						{{assign var="val" value=$item.ppd_value|replace:"$":""|replace:",":""}}
						{{math assign="amount_owed" equation="(hia - (val)) * 0.14" hia=$hia val=$val}}
						${{$amount_owed|number_format:2:".":","}}
					{{else}}
						${{$item.annual}}
					{{/if}}
				...
				<td data-col="daily">
					{{if $item.ppd_hia_value && $item.ppd_value }}
						{{assign var="hia" value=$item.ppd_hia_value|replace:"$":""|replace:",":""}}
						{{assign var="val" value=$item.ppd_value|replace:"$":""|replace:",":""}}
						{{math assign="amount_owed" equation="(hia - val) * 0.14 / 365" hia=$hia val=$val}}
						${{$amount_owed|number_format:2:".":","}}
					{{else}}
						{{$item.daily}}
					{{/if}}
				...
				<td data-col="total_interest">
					{{if $item.ppd_hia_value && $item.ppd_value }}
						{{assign var="hia" value=$item.ppd_hia_value|replace:"$":""|replace:",":""}}
						{{assign var="val" value=$item.ppd_value|replace:"$":""|replace:",":""}}
						{{assign var="days_late" value=$item.days_late}}
						{{math assign="amount_owed" equation="(((hia - val) * 0.14) / 365) * days_late" hia=$hia val=$val days_late=$days_late}}
						${{$amount_owed|number_format:2:".":","}}
					{{else}}
						${{$item.total}}
					{{/if}}
				</td>
				<td data-col="total_amount">
					{{if $item.ppd_hia_value && $item.ppd_value }}
						{{assign var="hia" value=$item.ppd_hia_value|replace:"$":""|replace:",":""}}
						{{assign var="val" value=$item.ppd_value|replace:"$":""|replace:",":""}}
						{{assign var="days_late" value=$item.days_late}}
						{{math assign="amount_owed" equation="((((hia - val) * 0.14) / 365) * days_late) + (hia - val)" hia=$hia val=$val days_late=$days_late}}
						${{$amount_owed|number_format:2:".":","}}
					{{else}}
						${{$item.total_amount}}
					{{/if}}
				...
				<td bgcolor="{{ if $item.ppd_notes }}#f00{{ /if }}"">
					<a href="{{$BASE_URL}}lateletter_report.notes/ppd_id/{{$item.ppd_id}}" class="various">Alerts</a>
				</td>
			</tr>
