...
$where1 = " And PPD.ppd_default_int = 1";
...
if ($keyword):
	$where2 .= " AND B.bsn_name LIKE '%" . $keyword . "%'";
	$_SESSION['keyword'] = $keyword;
elseif ($_SESSION['keyword']):
	$where2 .= " AND B.bsn_name LIKE '%" . $_SESSION['keyword'] . "%' ";
endif;


$matsql = getLateLetterReportQuery($where1 . " " . $where2, ' AND days_late >= 10') . " ORDER BY days_late";
/* $matsql = "SELECT progress_payment_detail.*, business.bsn_name, business.bsn_status from progress_payment_detail
		   ...
		   (business.bsn_status LIKE '%|2|%' OR business.bsn_status LIKE '%|3|%' OR business.bsn_status LIKE '%|4|%' OR business.bsn_status LIKE '%|6|%')" . $where1 . " " . $where2 . "
...
foreach ($listsnew as $k1 => $v1) {
	...
	$sqlrec = "SELECT count(*) as rec from progress_payment_detail where ppd_bsn_id = " . $v1['ppd_bsn_id'] . " And (length(ppd_when_payrec)<=0 AND ppd_invoice_sent != '' AND ppd_inv_due_date != '')";
	...
	$ppd_value = 0;
	if($v1['ppd_hia_value']) {
		$ppd_hia_value = $v1['ppd_hia_value'];
		$ppd_hia_value = str_replace('$', '', $ppd_hia_value);
		$ppd_hia_value = str_replace(',', '', $ppd_hia_value);		
	}
	if($v1['ppd_value']) {
		$ppd_value = $v1['ppd_value'];
		$ppd_value = str_replace('$', '', $ppd_value);
		$ppd_value = str_replace(',', '', $ppd_value);
	}
	$amount_owed = $ppd_hia_value - ($ppd_value);
	$interest_amount = ($amount_owed * 0.14) * $dayscount / 365;
	$total_amount_calculated = $amount_owed + $interest_amount;
	$listsnew[$k1]['hia_value'] = $ppd_hia_value;
	$listsnew[$k1]['ppd_value'] = $ppd_value;
	$listsnew[$k1]['amount_owed'] = $amount_owed;
	$listsnew[$k1]['interest_amount'] = $interest_amount;
	$listsnew[$k1]['total_amount_calculated'] = $total_amount_calculated;
	$totaAmountOwed += ( $total_amount_calculated );
}

$fwViewData['totaAmountOwed'] = '$' . number_format($totaAmountOwed, 2);
$fwViewData['list'] = $listsnew;
