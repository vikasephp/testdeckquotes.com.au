    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
                   ...
                   <th class="topmenu" align="center" valign="middle" width="6%">Amount</th> 
                   <th class="topmenu" align="center" valign="middle" width="3%">Interest</th> 
                   <th class="topmenu" align="center" valign="middle" width="5%">Annaul</th> 
                   <th class="topmenu" align="center" valign="middle" width="3%">Days</th> 
                   <th class="topmenu" align="center" valign="middle" width="3%">Daily</th> 
 ...
 {{foreach from=$list key="key" item="item"}}
    ...
    <tr bgcolor="#FFFFFF">
        ...
        <td>{{if $item.ppd_invoice_sent}}
         <a href="/business.download_content_pp?file_name={{$item.ppd_invoice_sent}}&module_name=business.home" target="_blank" >Download </a>
          {{/if}}
        </td>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="send[{{$item.ppd_id}}]" value="1"  onclick="this.form.submit();" {{if $item.ppd_default_int eq 1 }} checked="checked" {{/if}} />
       ...
       {{$item.ppd_send_user}} <br /> {{$item.ppd_send_date}}
        ...
        <td>
             {{if $item.ppd_letter_to_be}} 
 				<a href="/business.download_content_pp?file_name={{$item.ppd_letter_to_be}}&module_name=business.home" target="_blank" >Download </a>
             {{/if}}
             
       </td>
      <td>  {{$item.ppd_email_date}}</td>
        
         <td><a href="{{$BASE_URL}}lateletter_report.notes/ppd_id/{{$item.ppd_id}}" class="various" style="color: #0C6;">Notes</a></td>
    </tr>  
