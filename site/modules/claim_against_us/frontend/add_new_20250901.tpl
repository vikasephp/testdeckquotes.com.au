	...
	<form name="detail" method="post" action="" enctype="multipart/form-data">
		<table id="list-table" width="100%">
            ...
            <tr>
				...
				<td>
					<select name="{{$TABLE}}[ldd_type_id]">
						<option value="0">Please Select</option>
                        {{ foreach from=$typedata key="key1" item="item1" }}
                        <option value="{{ $item1.lddt_id }}" {{if $detail.ldd_type_id eq $item1.lddt_id}} selected="selected" {{/if}}>{{ $item1.lddt_option }}</option>
                        {{ /foreach }}
					</select>
				</td>
			</tr>
            ...
            <tr>
				...
				<td>
					<select name="{{$TABLE}}[ldd_status_id]">
                        ...
                        <option value="{{ $item2.ldds_id }}" {{if $detail.ldd_status_id eq $item2.ldds_id}} selected="selected" {{/if}}>{{ $item2.ldds_option }}</option>
                        {{ /foreach }}
					</select>
				</td>
			</tr>
