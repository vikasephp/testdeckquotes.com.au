...
{{if !$opr}}
 ...
 <input type="hidden" name="upload[{{$ID}}]" value="{{$detail.$ID}}"/>
 <table id="viewcust-table" width="450" cellpadding="0" cellspacing="0">   
 <tr><th >Name:</th><td> {{$detail.btdl_title}}</div></td></tr>
  <tr>
...
<tr> 
     ...
     {{else}}<div style="font-size:14px; height:29px; padding-top:50px;">{{$opr}}</div> {{/if}}
</div>
   {{if !$opr}}
