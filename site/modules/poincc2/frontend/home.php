...
$table_poincc2_merge_po_files = new Fw_Db_Table('poincc2_merge_po_files');
...
if ($showalert || isset($_SESSION['showalert'])):
	//$where .= "AND length(".$TABLE.".po_alert_text) > 0";
	$_SESSION['showalert'] = $showalert;
	$where .= "  AND poincc2.po_supplier like '" . $_SESSION['keyword'] . "%' AND length(" . $TABLE . ".po_alert_text) > 0";
...
if (strtotime($dt1) and strtotime($dt2)) {
	$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$dt1','%d-%m-%Y') AND STR_TO_DATE('$dt2','%d-%m-%Y') ";
	...
	$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') BETWEEN
					STR_TO_DATE('" . $_SESSION['dt1'] . "','%d-%m-%Y') AND STR_TO_DATE('" . $_SESSION['dt2'] . "','%d-%m-%Y') ";
} else if (strtotime($dt1) and !strtotime($dt2)) {
	$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') >= STR_TO_DATE('$dt1','%d-%m-%Y')";
	$_SESSION['dt1'] = $dt1;
	$fwViewData['dt_date1_ss'] = $dt1;
	$_SESSION['dt2'] = $dt2;
} else if (strtotime($_SESSION['dt1']) and !strtotime($_SESSION['dt2'])) {
	$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') >= STR_TO_DATE('" . $_SESSION['dt1'] . "','%d-%m-%Y')";
} else if (!strtotime($dt1) and strtotime($dt2)) {
	$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') <= STR_TO_DATE('$dt2','%d-%m-%Y')";
	$_SESSION['dt1'] = $dt1;
	$fwViewData['dt_date2_ss'] = $dt2;
	$_SESSION['dt2'] = $dt2;
} else if (!strtotime($_SESSION['dt1']) and strtotime($_SESSION['dt2'])) {
	$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') <= STR_TO_DATE('" . $_SESSION['dt2'] . "','%d-%m-%Y')";
}


// date search ends

// DUE date search starts

if (!$clearall) {
	$due_dt1 = $fwRequest->getParam('dt_due_date1', '');
	$due_dt2 = $fwRequest->getParam('dt_due_date2', '');
}

if (strtotime($due_dt1) and strtotime($due_dt2)) {
	$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$due_dt1','%d-%m-%Y') AND STR_TO_DATE('$due_dt2','%d-%m-%Y') ";
	$_SESSION['due_dt1'] = $fwViewData['due_dt1_ss'] = $due_dt1;
	$_SESSION['due_dt2'] = $fwViewData['due_dt2_ss'] = $due_dt2;
} else if (strtotime($_SESSION['due_dt1']) and strtotime($_SESSION['due_dt2'])) {

	$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') BETWEEN
					STR_TO_DATE('" . $_SESSION['due_dt1'] . "','%d-%m-%Y') AND STR_TO_DATE('" . $_SESSION['due_dt2'] . "','%d-%m-%Y') ";
} else if (strtotime($due_dt1) and !strtotime($due_dt2)) {
	$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') >= STR_TO_DATE('$due_dt1','%d-%m-%Y')";
	$_SESSION['due_dt1'] = $due_dt1;
	$fwViewData['due_dt1_ss'] = $due_dt1;
	$_SESSION['due_dt2'] = $due_dt2;
} else if (strtotime($_SESSION['due_dt1']) and !strtotime($_SESSION['due_dt2'])) {
	$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') >= STR_TO_DATE('" . $_SESSION['due_dt1'] . "','%d-%m-%Y')";
} else if (!strtotime($due_dt1) and strtotime($due_dt2)) {
	$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') <= STR_TO_DATE('$due_dt2','%d-%m-%Y')";
	$_SESSION['due_dt1'] = $due_dt1;
	$_SESSION['due_dt2'] = $due_dt2;
	$fwViewData['due_dt2_ss'] = $due_dt2;
} else if (!strtotime($_SESSION['due_dt1']) and strtotime($_SESSION['due_dt2'])) {
	$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') <= STR_TO_DATE('" . $_SESSION['due_dt2'] . "','%d-%m-%Y')";
}
// DUe date search ends
...
if ($clearall):
	...
	unset($_SESSION['dt1']);
	unset($_SESSION['dt2']);
	unset($_SESSION['due_dt1']);
	unset($_SESSION['due_dt2']);
	unset($_SESSION['showalert']);
	unset($fwViewData['inv_stg']);
	...
	$fwViewData['dt_date1_ss'] = "";
	$fwViewData['dt_date2_ss'] = "";
...
$sql3 = "SELECT DISTINCT po_project_number AS project_number FROM poincc2
UNION
SELECT DISTINCT po_project_number2 FROM poincc2
UNION
SELECT DISTINCT po_project_number3 FROM poincc2
UNION
SELECT DISTINCT po_project_number4 FROM poincc2
UNION
SELECT DISTINCT po_project_number5 FROM poincc2
UNION
SELECT DISTINCT po_project_number6 FROM poincc2
UNION
SELECT DISTINCT po_project_number7 FROM poincc2
UNION
SELECT DISTINCT po_project_number8 FROM poincc2
UNION
SELECT DISTINCT po_project_number9 FROM poincc2
UNION
SELECT DISTINCT po_project_number10 FROM poincc2
...
if (!empty($startDownloadAndAssignPoNumber)) {
	...
	$tableCronJobs->setWhere("status in ('pending', 'in_progress') AND module = 'poincc2'");
	...
	if ($tableCronJobs->rowExists()) {
		...
		$insertCronJobData 	= [
			...
			'payload' => json_encode(array(
				...
				'po_invoice_date' => array(
					'from' => $_SESSION['dt1'],
					'to' => $_SESSION['dt2']
				),
				'po_invoice_due_date' => array(
					'from' => $_SESSION['due_dt1'],
					'to' => $_SESSION['due_dt2']
				),
...
if (!empty($clear_database_for_PO_files)) {
	$thisTable = new Fw_Db_Table("poincc2_merge_po_files");
	$thisTable->setWhere("`is_downloaded` = 'YES'");
	$thisTable->deleteRows();
	// loop through the files one by one
	$files_list = glob($_SERVER['DOCUMENT_ROOT'] . "/files/po_files_list/*");
	...
	$fileName = 'po_pdf_list_' . date("d-m-y") . '.zip';
	$zip_file_path = $_SERVER['DOCUMENT_ROOT'] . '/site/modules/poincc2/frontend/' . $fileName;
...
$fetch_po_r = "SELECT SQL_CALC_FOUND_ROWS null as row_name, poincc2.* FROM poincc2  " . $where . " ORDER BY STR_TO_DATE(poincc2.po_invoice_date,'%d-%m-%Y') DESC";
...
$data = $fwDb->query($sqlmerge);*/ // echo "<pre>";print_r($data); //exit;
	$start = 0; /*Change made on 24-05-2025*/
	foreach ($data_to_assign_po as $k => $v) {

		if ($start >= 80) {
			break;
		} /*Change made on 24-05-2025*/

		$myFetchQ = "SELECT * FROM `poincc2_merge_po_files` WHERE `file_name` LIKE '" . $v['po_invoice_doc'] . "'";
		$data = $fwDb->query($myFetchQ);
		if (!empty($data) && count($data) > 0) {
			// echo "<br>data exists";
		} else {
			$folder_path = "files/purchase_order_invoice/";
			$source = fetch_file_raw_data_2($folder_path, $v['po_invoice_doc']);
			if ($source) {
				...
				$start++; /*Change made on 24-05-2025*/
			}
		}
	}
}
...
if (!empty($download_all_files_as_zip)) {
	...
	$pathdir = $_SERVER['DOCUMENT_ROOT'] . "/files/po_files_list/";
	$dir = opendir($pathdir);
	...
	if (count(glob("$pathdir/*")) > 0) {
		...
		$myFetchQ = "SELECT * FROM `poincc2_merge_po_files` WHERE is_po_number_assigned='YES'";
		...
		$zip->close();
		echo $zip_file_path = $_SERVER['DOCUMENT_ROOT'] . '/site/modules/poincc2/frontend/' . $fileName;
...
if (!empty($assign_po_number)) {
	...
	foreach ($data as $k => $v) {
		...
		$myFetchQ = "SELECT * FROM `poincc2_merge_po_files` WHERE `file_name` = '" . $v['po_invoice_doc'] . "' and is_po_number_assigned='NO'";
		$data_21 = $fwDb->query($myFetchQ);

		if (!empty($data_21) && count($data_21) > 0) {
			//echo "<br> count ::".$postart++;
			$postart++; /*Change made on 24-05-2025*/
			if (!empty($v['po_purchase_order_no'])) {
				$print_text = "HAVE PO  - " . $v['po_purchase_order_no'];
			} else {
				$print_text = "NO PO";
			}
			$source = $_SERVER['DOCUMENT_ROOT'] . '/files/po_files_list/' . $v['po_invoice_doc'];
			...
			if ($response->status == 200 || $response->status == "200") {
				...
				$tablerm = new Fw_Db_Table('poincc2_merge_po_files');
...
$myFetchQ = "SELECT * FROM `poincc2_merge_po_files` WHERE `is_downloaded` = 'YES'";
$data = $fwDb->query($myFetchQ);
$fwViewData['total_po_downloaded_files'] = count($data);

// COUNT TOTAL DOWNLOADED FILES having PO NUMBERS
$myFetchQ = "SELECT * FROM `poincc2_merge_po_files` WHERE `is_downloaded` = 'YES'  and is_po_number_assigned='YES' ";
$data = $fwDb->query($myFetchQ);
$fwViewData['total_po_number_files'] = count($data);

if (!empty($assign_po_number)) {
	if (count($data_to_assign_po) == count($data)) {
...
->addPDF('path/to/source/file2.pdf', 'all', 'horizontal');*/
	$fn = "merged_poincc2_PO_MATCH" . "_" . $dt . ".pdf";
	$fn = "just_checking.pdf";
	//$lsql = $fwRequest->getParam('sql', '');  $sqlmerge = stripslashes($lsql);
	$sqlmerge = "SELECT * FROM `poincc2_merge_po_files`";
	$data = $fwDb->query($sqlmerge); // echo "<pre>";print_r($data); //exit;
	foreach ($data as $k => $v) {
	...
	echo BASE_DIR . 'files/merged_poincc2/' . $fn;
...
$sql_approver = "SELECT * FROM poincc2_invoice_approver";
...
if (!empty($export)) {
	...
	$my_excel_data = [];
	$title =  ['URN', 'Type', 'P Or NP', 'Project', 'Email Date', 'Invoice No', 'Supplier', 'Invoice Date', 'Invoice Due Date', 'Invoice Value', 'US Invoice Value', 'Invoice Stage', 'MYOBID', 'PO Number', 'Value of PO', 'Chart of Acc', 'Alert'];
	...
	$xlsx = Shuchkin\SimpleXLSXGen::fromArray($my_excel_data)->downloadAs('poincc2_file.xlsx');;
