        ...
        <tr>
           ...
           {{foreach from=$busdetail key="key2" item="item2"}}
			...
			<div id="projectFields">
             <input class="project-input" type="text" name="poincc[po_project_number][]" list='address' value="{{$detail.po_project_number}}" style="width:300px;"/>
			  </div>
			 
			 <img id="addProjectAddress" style="height:16px; width:16px" src="{{$BASE_URL}}css/admin/images/addnew.png"/>
            
            </td>
			<script>
				document.getElementById('addProjectAddress').addEventListener('click', function() {

				var inputs = document.querySelectorAll('.project-input');
				var maxFields = 10;
				...
				if (lastInput.value.trim() === '') {
					alert('Please fill the current address first.');
					return;
				}
				...
				newInput.type = 'text';
				newInput.name = 'poincc[po_project_number][]';
				newInput.setAttribute('list', 'address');
				newInput.className = 'project-input';
				newInput.style.width = '300px';
				newInput.style.marginBottom = '5px';

				document.getElementById('projectFields').appendChild(newInput);
