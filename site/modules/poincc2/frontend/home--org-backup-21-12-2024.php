...
$sqlvar = 0;
 ...
 if($clearall):
	...
	$fwViewData['due_dt1_ss'] = ""; $fwViewData['due_dt2_ss'] = "";
	$fwViewData['dt_date1_ss'] = ""; $fwViewData['dt_date2_ss'] = "";
  ...
  elseif($_SESSION['invoice_no']):
    ...
    if(!$clearall){
		...
		if(strtotime($dt1) and strtotime($dt2)) {
			...
			$_SESSION['dt1'] = $dt1;  $fwViewData['dt_date1_ss'] = $dt1;
			$_SESSION['dt2'] = $dt2;
			...
			$_SESSION['dt1'] = $dt1;  $fwViewData['dt_date2_ss'] = $dt2;  
			$_SESSION['dt2'] = $dt2;
	...
	// DUE date search starts
       ...
       if(!$clearall){
		...
		$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$due_dt1','%d-%m-%Y') AND STR_TO_DATE('$due_dt2','%d-%m-%Y') ";
			...
			$_SESSION['due_dt1'] = $due_dt1; $fwViewData['due_dt1_ss'] = $due_dt1;
			...
			$_SESSION['due_dt2'] = $due_dt2; $fwViewData['due_dt2_ss'] = $due_dt2;
...
{
   $thisTable = new Fw_Db_Table("poincc2_merge_po_files");	$thisTable->setWhere("`is_downloaded` = 'YES'");
...
{
    $zip = new ZipArchive; $fileName = 'po_pdf_list_'.date("d-m-y").'.zip'; 
    $zip->open($fileName, ZipArchive::CREATE);  //echo "<pre>"; print_r($files_list);
    $pathdir = $_SERVER['DOCUMENT_ROOT']."/files/po_files_list/";  $dir = opendir($pathdir); 
    ...
    if( count(glob("$pathdir/*")) > 0 )
    {
        ...
        $zip->close(); $zip_file_path = $_SERVER['DOCUMENT_ROOT'].'/site/modules/poincc2/frontend/'.$fileName;
        ...
        readfile($zip_file_path);   exit; ob_start ();
    }
    else
    {
...
if(!empty($assign_po_number))
{
  ...
  $data = $fwDb->query($sqlmerge);// echo "<pre>";print_r($data); //exit;
  	foreach($data as $k => $v)
	{
	    $myFetchQ = "SELECT * FROM `poincc2_merge_po_files` WHERE `file_name` LIKE '".$v['po_invoice_doc']."' and is_po_number_assigned='NO'";
...
/*$pdf_jurosh->addPDF('path/to/source/file.pdf', 'all', 'vertical')
  ...
  $fn = "merged_poincc2_PO_MATCH"."_".$dt.".pdf";  $fn = "just_checking.pdf";
   //$lsql = $fwRequest->getParam('sql', '');  $sqlmerge = stripslashes($lsql);
