<?php
$fwMainView = 'file:' . getcwd() . '/add_financial_report.tpl';

$bsn_id = (int)$fwRequest->getParam('bsn_id', 0);
$fwViewData['bsn_id'] = $bsn_id;

$tableTask = new Fw_Db_Table('prospect_list_financial_report');

$user_id = $_SESSION['user']['user_id'];
$submit = $fwRequest->getParam('subAddDetail', '');
if (!empty($submit)) {	
	$detail = $fwRequest->getParam('task', array());
	$this_id = (int)$detail['plfr_id'];
	unset($detail['plfr_id']);

    if($this_id > 0){
		unset($detail['plfr_id']);
		$tableTask->setWhere("plfr_id = $this_id");
