...
$pstable = new Fw_Db_Table('prospect_list');
...
if($hide)
{
	 ...
	 foreach($hide as $k=>$v)
	 {
	  ...
	  $detail['pl_hide'] = $v;
	  $pstable->setWhere("pl_id = ".$id);
	  if($pstable->rowExists())
	 {
		 $this_id = $pstable->updateRow($detail);
	  }
}


$keyword = $listdata['keyword'];
$where = "WHERE pl_hide = 0 ";
...
$statusTable = new Fw_Db_Table($ATTRIBUTES['status']['table']);
$fwViewData['status_data'] = $status_data = $statusTable->getAllRows();
$serviceTable = new Fw_Db_Table($ATTRIBUTES['service_type']['table']);
$fwViewData['service_data'] = $service_data = $serviceTable->getAllRows();
