...
<div class="table-heading-content">
	<div class="left">
		<h2 class="tab-title text-left inner-tab-title">Invoices</h2>
	</div>
	<div class="right">
		<input type="button" name="Add New Invoices" value="Add New Invoices" onclick="javascript:open_url('add_invoices_detail');" />
	</div>
...
<table id="list-table">
	<thead>
		<tr>
			...
			<th class="topmenu" align="center" valign="middle">Paid/Settled</th>
	...
	<tbody>
		...
		{{foreach from=$invoices_list key="key" item="item"}}
		<tr bgcolor='{{ cycle values=" #D3E8D6,WHITE" advance=true }}'>
			<td>{{$inv_count}}</td>
			<td>{{$item.plir_date_period}}</td>
			<td>
				{{if $item.plir_report_link}}
					<a href="{{$item.plir_report_link}}" target="_blank">Link</a>
				{{/if}}
			</td>
			<td>{{$item.plir_email_date|date_format:"%d-%m-%Y"}}</td>
			<td>
				<form name="recmet3" method="post" action="">
					<input type="hidden" name="plir_paid[{{$item.plir_id}}]" value="0">
					<input type="hidden" name="plir_paid[bsn_id]" value="{{$bsn_id}}">
					<label class="switch">
						<input class="switch-input" type="checkbox" name="plir_paid[{{$item.plir_id}}]" value="1" onclick="this.form.submit();" {{if $item.plir_paid eq 1}} checked="checked" {{/if}} />
						<span class="switch-label" data-on="Yes" data-off="No"></span>
						<span class="switch-handle"></span> <br />
					</label>
				</form>
				{{if $item.plir_paid eq 1}}
				{{$item.plir_paid_user}}
				<br/>{{$item.plir_paid_date|date_format:"%d-%m-%Y"}}
				{{/if}}
			</td>
			<td>
				<a href="{{$BASE_URL}}{{$BASEFOLDER}}.add_invoices_report/bsn_id/{{$bsn_id}}/plir_id/{{$item.plir_id}}" class="various" title="Edit"><img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/edit.png"></a><a href="{{$BASE_URL}}{{$BASEFOLDER}}.delete_invoice/bsn_id/{{$bsn_id}}/plir_id/{{$item.plir_id}}" onclick="javascript:if(!confirm('Are you sure want to delete this record?')) return false;" title="Delete"><img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/delete.png"></a>
			</td>
		</tr>
