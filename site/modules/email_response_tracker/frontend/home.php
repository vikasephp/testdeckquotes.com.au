...
$table = new Fw_Db_Table('email_response_tracker');
...
if(!empty($status_search)) {
	$where = " where email_response_tracker.er_response_status = ".$status_search;
	$_SESSION['status_search'] = $status_search;
}
else if(!empty($_SESSION['status_search'])) {
	$where = " where email_response_tracker.er_response_status = ".$_SESSION['status_search'];
}

$type_search = $fwRequest->getParam('type_search', '');
if(!empty($type_search)) {
	$where = " where email_response_tracker.er_response_type = ".$type_search;
	$_SESSION['type_search'] = $type_search;
}
else if(!empty($_SESSION['type_search'])) {
	$where = " where email_response_tracker.er_response_type = ".$_SESSION['type_search'];
}


$priority = $fwRequest->getParam('priority', '');
if(!empty($priority)) {
	$where = " where email_response_tracker.er_priority = '".$priority."'";
	$_SESSION['priority'] = $priority;
}
else if(!empty($_SESSION['priority'])) {
	$where = " where email_response_tracker.er_priority = '".$_SESSION['priority']."'";
}
...
if(!empty($printreport)) {
	$heading = "Email Response Tracker Accounts Report";
	$fname='ERT_Accounts';
