...
&nbsp; &nbsp;<input type="button" name="component" value="Edit Component" onclick="javascript:add_component();" /> <br />
    ...
    <tr bgcolor="#FFFFFF">
       ...
       <!-- <td><strong>{{$item.customer}}</strong><br /><br />
        ...
        <form name ="recmet2" method="post" action="">
       	<select name="{{$TABLE}}[qm_component]"  onchange="update_component({{$item.qm_id}}, this.value)" >
      		 ...
      		 {{foreach from = $cdetail key="key2" item="item2"}}
       		 <option value="{{$item2.qb_id}}" {{if $item2.qb_id eq $item.qm_component }} selected="selected" {{/if}}>{{$item2.qb_component}}</option>
       		  {{/foreach}}
        </select>
        </form>
