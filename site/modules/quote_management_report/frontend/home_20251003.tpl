...
<script type="text/javascript">
	...
	function add_procedure() {
		var viewurl = "{{$BASE_URL}}quote_management_report.view_procedure" + "/random/" + Math.random();
	...
	function add_component() {
		var viewurl = "{{$BASE_URL}}quote_management_report.view_component" + "/random/" + Math.random();
	...
	function edit_component() {
		var viewurl = "{{$BASE_URL}}quote_management_report.view_edit_project" + "/random/" + Math.random();
...
<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
	<div style="float:left; text-align:left;">
		...
		&nbsp; &nbsp;<input type="button" name="component" value="Edit Component"
			onclick="javascript:add_component();" /> 
		
		&nbsp; &nbsp;<input type="button" name="edit_project" value="Edit Project"
			onclick="javascript:edit_component();" /> <br />
			
		<datalist id='project'>
			{{foreach from=$project_address key="key" item="item_pro"}}
			<option value="{{$item_pro.qm_project}}">
				{{$item_pro.qm_project}}
			</option>
			{{/foreach}}
		</datalist>
		&nbsp; &nbsp;<input type="text" list="project" name="address" style="width:350px;" placeholder="Search for Address" value="{{$address}}" />&nbsp;<br />
		
		<datalist id='position_list'>
			{{foreach from=$position_list key="key" item="item_posi"}}
			<option value="{{$item_posi.qb_sup_position}}">
				{{$item_posi.qb_sup_position}}
			</option>
			{{/foreach}}
		</datalist>
		&nbsp; &nbsp;<input type="text" list="position_list" name="position" style="width:350px;" placeholder="Search for Position" value="{{$position}}" />&nbsp; <br />
		...
		<tbody>
			...
			<tr bgcolor="#FFFFFF">
				...
				<td>
					...
					<!--<form name="recmet2" method="post" action="">
						<select name="{{$TABLE}}[qm_component]"
							onchange="update_component({{$item.qm_id}}, this.value)">
							<option value="0" {{if $item.qm_component eq 0}} selected="selected" {{/if}}>Please Select
							</option>
							{{foreach from = $cdetail key="key2" item="item2"}}
							<option value="{{$item2.qb_id}}" {{if $item2.qb_id eq $item.qm_component }}
								selected="selected" {{/if}}>{{$item2.qb_component}}</option>
							{{/foreach}}
						</select>
					</form>

					<script>
						function update_component(id, value) {
							$.ajax({
								type: "GET",
								url: "{{$BASE_URL}}quote_management_report.update_component/qm_id/" + id + "/value/" + value,
				...
				<td>
					...
					<a href="/quote_management_report.download_content?file_name={{$item.qm_attachment_5}}&module_name=quote_management_report.home">Download</a>
					{{/if}}
				</td>
				...
				<td
					...
					{{if $item.qm_qmrs_id == $approved_status_id}}
					style="background-color: green;"
					{{elseif $item.qm_qmrs_id == $rejected_status_id}}
					style="background-color: red;"
					{{/if}}
				>
					<select name="qm_qmrs_id{{ $item.$ID }}" onchange="updateRecord('{{ $item.$ID }}', 'qm_qmrs_id', this.value, true)">
					{{foreach from=$quote_management_report_status key="index" item="row"}}
						<option value="{{$row.qmrs_id}}" {{if $row.qmrs_id == $item.qm_qmrs_id}}selected{{/if}}>{{$row.qmrs_name}}</option>
					{{/foreach}}
					</select>
				</td>
                ...
                if (data.success == true) {
                    if(updateRecord){
						if(column == 'qm_qmrs_id') {
							...
							const selectedText = $('select[name="' + uniqueID + '"] option:selected').text();
							console.log(uniqueID, selectedText);
							let bgcolor = 'white';
