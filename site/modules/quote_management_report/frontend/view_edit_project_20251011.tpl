...
<div align="center" style="min-height:350px; z-index:9999999;">
	...
	<form name="detail" method="post" action="" enctype="multipart/form-data">
		<div
			style="display:none; float:right; margin-right:10px; background: #09F; color:#FFF !important; padding:6px; margin-bottom:5px;">
			<a href="{{$BASE_URL}}quote_management_report.add_component/qb_id/{{$qb_id}}" class="various" title="Edit"
				style="color:#FFF; text-decoration:none; font-size:14px;"> Add New Edit Project </a>
		</div>
		...
		<table id="list-table2" width="99%">
			<tr>
				...
				<td width="70%">
					...
					<input type="text" id="projectIn" list='project' name="qmr[qm_project]" style="width:100%;"
						value="{{$detail.qm_project}}" />
				</td>
			</tr>
		</table><br />
			...
			<tr pro="{{$project_get}}">
				...
				<td>
					...
					{{foreach from=$qmrData item="qmr"}}
						...
						{{/if}}
					{{/foreach}}
					<select data-check="{{$detail.qm_project}}" name="qmr[components][{{$item.qb_id}}]" onchange="showTotalQuotesNeeded({{$item.qb_id}}, this.value)">
					...
					<div id="showTotalQuotesNeeded{{$item.qb_id}}" style="display: inline-block;">
