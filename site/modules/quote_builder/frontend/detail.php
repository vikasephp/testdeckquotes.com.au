...
if(!empty($submit2))
{
	...
	$prda['qb_supplier_email'] = "|" .implode("|",$prda['cont'])."|";
...
$show_contacts = $fwRequest->getParam('show_contacts', '');
if(!empty($show_contacts))
{
	$detail = $fwRequest->getParam($TABLE, array());
	$supplier = $detail['qb_supplier'];
	$fwViewData['show'] = 1;
	  
	$consql = "SELECT contacts.* from contacts 
                     Left join companies ON contacts.cs_company = companies.co_id 
	             where companies.co_company_name = '".$supplier."' and length(contacts.cs_primary_email) > 0";
...
if(!empty($submit))
{
	...
	if(!empty($detail['cont'])) {
		$se = '|';
		foreach($detail['cont'] as $k=>$v)
		{
			$se .= $v ."|";	
		}
		
		$detail['qb_supplier_email'] = $se;
	}
