            ...
            <tr bgcolor="{{ cycle values=" #D3E8D6,WHITE" advance=true }}">
                ...
                <td data-col="invoice">
                    <form name="cal" method="post" enctype="multipart/form-data">
                        ...
                        <input type="file" name="ldd_evidence_brief_link" style="width: 175px;" />
                        <input type="submit" name="save_ldd_evidence_brief_link" title="Save" value="Upload" class="set2">
                    </form>
                    {{ if $item.ldd_evidence_brief_link }}
                    <a href="/{{ $BASEFOLDER }}.download_content?file_name={{$item.ldd_evidence_brief_link}}&module_name={{ $BASEFOLDER }}.home" target="_blank">Download</a>
                    <a href="/{{ $BASEFOLDER }}.delete_ldd_evidence_brief_link/{{$ID}}/{{ $item.$ID }}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img src="{{$BASE_URL}}/images/delete_icon.png" width="20" /></a>
                    {{/if}}
                    <!-- {{ if $item.ldd_evidence_brief_link }}
                        <p style="margin-bottom: 10px;"><a href="{{ $item.ldd_evidence_brief_link }}" class="various">View</a></p>
                    {{ /if }}
                    <p>
