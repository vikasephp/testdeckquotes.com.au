...
$save_ldd_invoice_file = $fwRequest->getParam('save_ldd_invoice_file', '');
if ($save_ldd_invoice_file && !empty($_FILES['ldd_invoice_file']['name']) && $this_id = (int)$fwRequest->getParam($ID, 0)) {
	$file = $_FILES['ldd_invoice_file'];
	$docfile_1 = $file['name'];
	$docfile_1 =  preg_replace('/[^A-Z0-9._]/i', '_', $docfile_1);
	$temp_name_1 = $file['tmp_name'];
	$fileUploaded = upload($docfile_1, $temp_name_1);
	$record['ldd_invoice_file'] = $docfile_1;
...
if (!empty($resultData)) {
	...
	if ($sql2) {
		...
		foreach ($lists as $list):
			...
			$sql_na = "SELECT count(*) as total_notes FROM `construction_invoice_disputes_notes` where cidn_ldd_id = ".$list['ldd_id'];
			$data_na= $fwDb->queryOne($sql_na);
			$list['total_notes'] = $data_na['total_notes'];
...
$fwViewData['invoice_type_data'] = $invoice_type_data = $thisTable->getAllRows();
