...
$tablePoincc2MergePoFiles = new Fw_Db_Table('poincc2_merge_po_files');
$tableCronJobs 		= new Fw_Db_Table('cron_jobs');
$tableCronJobs->setWhere("status = 'pending' AND module = 'poincc2'");
$pendingJobs 		= $tableCronJobs->getRows();

if(!count($pendingJobs)) {
	echo 'poincc2 : No pending jobs found!'; exit;	
}
...
function clearDatabaseForPoFiles($job)
{
	...
	$files_list = glob(BASE_DIR . "/files/po_files_list/*");
	...
	$zip_file_path = BASE_DIR . '/site/modules/poincc2/frontend/' . $fileName;
...
{
	global $cronFWDB;
	global $tablePoincc2MergePoFiles;
	...
	foreach ($records as $key => $row) {
		...
		$mergeFetchQuery = "SELECT * FROM `poincc2_merge_po_files` WHERE `file_name` LIKE '" . $row['po_invoice_doc'] . "'";
		...
		if (!empty($data) && count($data) > 0) {
			...
			if ($source) {
				...
				$tablePoincc2MergePoFiles->insertRow($insertData);
			}
		}
	}
}		
...
function assignPoNumber($job)
{
	...
	foreach ($records as $key => $row) {
		...
		$mergeFetchQuery= "SELECT * FROM `poincc2_merge_po_files` WHERE `file_name` = '" . $row['po_invoice_doc'] . "' and is_po_number_assigned='NO'";
		$data 			= $cronFWDB->query($mergeFetchQuery);
		...
		if (!empty($data) && count($data) > 0) {
			...
			$poValue = '';
			if (!empty($row['po_po_value'])) {
				
				$color = 'FF0000'; // default red
				if (
					isset($row['po_invoice_value'], $row['po_po_value']) &&
					$row['po_invoice_value'] !== '' && $row['po_po_value'] !== '' &&
					$row['po_invoice_value'] === $row['po_po_value']
				) {
					$color = '008000'; // green
				}
				
				$poValue = "PO value - " .$row['po_po_value'];
			}
			
			$source 	= BASE_DIR . '/files/po_files_list/' . $row['po_invoice_doc'];
			$response 	= assignPoNumberToPdfApi($row['po_invoice_doc'], $printText, $poValue, $color);
			...
			if ($response->status == 200 || $response->status == "200") {
				...
				$tablePoincc2MergePoFiles->setWhere("`file_name` LIKE '" . $row['po_invoice_doc'] . "' and is_po_number_assigned='NO'");
				$tablePoincc2MergePoFiles->updateRow($updatePoData);
			}
		}
	}
}
...
function fetchPoRecords($job)
{
	...
	// invoice date search starts
	$poInvoiceDateFrom 	= $payload->po_invoice_date->from ? $payload->po_invoice_date->from : null;
	$poInvoiceDateTo 	= $payload->po_invoice_date->to ? $payload->po_invoice_date->to : null;
	if (strtotime($poInvoiceDateFrom) and strtotime($poInvoiceDateTo)) {
		$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$poInvoiceDateFrom','%d-%m-%Y') AND STR_TO_DATE('$poInvoiceDateTo','%d-%m-%Y') ";
	} else if (strtotime($poInvoiceDateFrom) and !strtotime($poInvoiceDateTo)) {
		$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') >= STR_TO_DATE('$poInvoiceDateFrom','%d-%m-%Y')";
	} else if (!strtotime($poInvoiceDateFrom) and strtotime($poInvoiceDateTo)) {
		$where .= " AND STR_TO_DATE(po_invoice_date, '%d-%m-%Y') <= STR_TO_DATE('$poInvoiceDateTo','%d-%m-%Y')";
	}
	// invoice date search ends
	
	// invoice due date search starts
	$poInvoiceDueDateFrom 	= $payload->po_invoice_due_date->from ? $payload->po_invoice_due_date->from : null;
	$poInvoiceDueDateTo 	= $payload->po_invoice_due_date->to ? $payload->po_invoice_due_date->to : null;
	if (strtotime($poInvoiceDueDateFrom) and strtotime($poInvoiceDueDateTo)) {
		$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$poInvoiceDueDateFrom','%d-%m-%Y') AND STR_TO_DATE('$poInvoiceDueDateTo','%d-%m-%Y') ";
	} else if (strtotime($poInvoiceDueDateFrom) and !strtotime($poInvoiceDueDateTo)) {
		$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') >= STR_TO_DATE('$poInvoiceDueDateFrom','%d-%m-%Y')";
	} else if (!strtotime($poInvoiceDueDateFrom) and strtotime($poInvoiceDueDateTo)) {
		$where .= " AND STR_TO_DATE(po_invoice_due_date, '%d-%m-%Y') <= STR_TO_DATE('$poInvoiceDueDateTo','%d-%m-%Y')";
	}
	// invoice due date search ends
	
	$fetchQuery = "SELECT SQL_CALC_FOUND_ROWS null as row_name, poincc2.* FROM poincc2 " . $where . " ORDER BY STR_TO_DATE(poincc2.po_invoice_date,'%d-%m-%Y') DESC";
...
function fetchS3FileAsRawData($folderPath,$objectName)
{
    ...
    try {
		...
		file_put_contents(BASE_DIR.'/files/po_files_list/'.$file, $file_raw_data);
		return BASE_DIR.'/files/po_files_list/'.$file;
...
function assignPoNumberToPdfApi($po_invoice_doc_name, $print_text, $po_value, $color)
{
    $curl = curl_init();
	...
	curl_setopt_array($curl, array(
		...
		CURLOPT_POSTFIELDS =>'{
			...
			"url": "https://testdeckquotes.com.au/files/po_files_list/'.$po_invoice_doc_name.'",
			"annotationsString": "430;5;0-;'.$print_text.';12+bold;Arial;FF0000;|430;25;0-;'.$po_value.';12+bold;Arial;'.$color.';",
			"imagesString": "",
			"fieldsString": ""
		}',
