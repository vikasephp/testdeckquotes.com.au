...
$tableqa = new Fw_Db_Table('construction_calendar_enquiries');

$submit = $fwRequest->getParam('subAddDetail', '');
if(!empty($submit))
{
    $detail = $fwRequest->getParam($TABLE, array());
	$this_id = (int)$detail['question_id'];
  	...
  	unset($detail['id']);
		...
		if($_FILES['ans_attach_1']['name'])
		{
				...
				$detail['attachment_ans_1'] = $docfile_1;
		...
		if($_FILES['ans_attach_2']['name'])
		{
				...
				$detail['attachment_ans_2'] = $docfile_2;
	    ...
	    if($_FILES['ans_attach_3']['name'])
		{
				...
				$detail['attachment_ans_3'] = $docfile_3;
	    ...
	    if($_FILES['ans_attach_4']['name'])
		{
				...
				$detail['attachment_ans_4'] = $docfile_4;
	    ...
	    if($_FILES['ans_attach_5']['name'])
		{
				...
				$detail['attachment_ans_5'] = $docfile_5;
	   ...
	   if($this_id > 0)
    	{
		     $detail['answer_user'] = $_SESSION['user']['user_name'];
			 $detail['answer_date'] = date('d-m-Y');
	...
	$sql = "Select business.bsn_name, construction_calendar_enquiries.enquiry, construction_calendar_enquiries.solution, 
	        construction_calendar_enquiries.created_at, construction_calendar_enquiries.user_id from business 
	        inner join construction_calendar_enquiries on business.bsn_id = construction_calendar_enquiries.project_id
			where construction_calendar_enquiries.id = ".$this_id;
	
	$data1 = $fwDb->queryOne($sql);
	
	$sql2 = "select bcust_fname,bcust_lname, bcust_misc_email1, bcust_misc_moble from bus_customers where bcust_id = ".$data1['user_id'];
	...
	$html .= "<p><strong>Your Question :</strong>".$data1['enquiry']."</p>";
	$html .= "<p><strong>Date of Question :</strong>".$data1['created_at']."</p>";
	$html .= "<p><strong>Answer :</strong>".$data1['solution']."</p>";
	...
	$html .= "<p>To view the enquiry and answer in detail, visit the <a href='www.constructioncalendar.com.au'>Construction Calendar Website</a> and log in by your phone and email address";
	...
	$message .= "Enquiry :".$data1['enquiry'].chr(10);
	$message .= "Answer :".$data1['solution'].chr(10);
	$message .= "You may proceed to the www.constructioncalendar.com.au Website to view in detail".chr(10); 
