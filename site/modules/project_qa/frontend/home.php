...
$table = new Fw_Db_Table('construction_calendar_enquiries');
...
if($merge)
{
	 ...
	 $sql2 = "SELECT construction_calendar_enquiries.*,business.bsn_name,business_sellers.bs_customers_id,construction_calendar_events.text as 		event_text,construction_calendar_events.start_date,construction_calendar_events.end_date FROM `construction_calendar_enquiries` LEFT JOIN construction_calendar_events ON construction_calendar_enquiries.event_id=construction_calendar_events.id LEFT JOIN business ON construction_calendar_enquiries.project_id=business.bsn_id INNER JOIN business_sellers ON construction_calendar_enquiries.project_id=business_sellers.bs_business_id where construction_calendar_enquiries.id = ".$ky_2; 
...
$where  = " where length(construction_calendar_enquiries.solution) IS NULL ";
...
$where .= " AND construction_calendar_enquiries.project_id = ".$bsn_id;
...
elseif($_SESSION['keyword'] ):
	...
	$where .= " AND construction_calendar_enquiries.project_id  = ".$_SESSION['bsn_id'];
	$fwViewData['keyword']=$_SESSION['keyword'];
endif;
...
//$sql = "SELECT construction_calendar_enquiries.*,business.bsn_name,business_sellers.bs_customers_id FROM `construction_calendar_enquiries` 
//LEFT JOIN business ON construction_calendar_enquiries.project_id=business.bsn_id 
//INNER JOIN business_sellers ON construction_calendar_enquiries.project_id=business_sellers.bs_business_id " .$where ."
//order by  construction_calendar_enquiries.solution IS NULL DESC, construction_calendar_enquiries.created_at ASC";

$sql = " SELECT construction_calendar_enquiries.*, business.bsn_name FROM `construction_calendar_enquiries`
         Inner JOIN business ON construction_calendar_enquiries.project_id=business.bsn_id "
		  .$where ." order by  construction_calendar_enquiries.solution IS NULL DESC, construction_calendar_enquiries.created_at ASC";
