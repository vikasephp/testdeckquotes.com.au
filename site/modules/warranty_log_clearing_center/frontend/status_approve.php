...
if (!empty($submit)) {
	...
	if ($status == 'Yes') {
		...
		if ($data['wf_is_tented'] == 1) {
			$wlDetail['wa_tenanted'] = 'Yes';
			$wlDetail['wa_tenant_name'] = $data['wf_tenant_name'];
			$wlDetail['wa_tenant_number'] = $data['wf_tenant_number'];
		} else {
		...
		$fwViewData['opr'] = $wa_id = $table->insertRow($wlDetail);

		$query = 'SELECT * FROM warranty_form_attachments WHERE wfa_wf_id = ' . $wf_id . ';';
		$result = $fwDb->query($query);
		foreach($result as $row) {
			$newData = [
				'wla_wa_id' => $wa_id,
				'wla_file' => $row['wfa_file'],
				'wla_type' => $row['wfa_type'],
			];
			$warranty_log_attachments = new Fw_Db_Table('warranty_log_attachments');
			$warranty_log_attachments->insertRow($newData);
		}
		...
		$photos = $serviceClass->setLogPhotosIdWise($wf_id);
		$images = '';
		foreach($photos as $index => $file) {
			$images .= ('Photo ' . ($index+1) . ' :  <a href="https://deckquotes.com.au/' . $file['url'] . '" target="_blank">Download</a><br>');
		}
		...
		if (!empty($fwViewData['opr'])) {
			$message2 .= '<table border="1" cellpadding="10" cellspacing="0" style="border-collapse: collapse;">
						...
						<tr>
						<td>' . $data_wl['wa_id'] . '</td><td>' . $data_wl['wa_project'] . '</td><td>' . $data_wl['wa_problem'] . '</td><td>' . $images . '</td>
						</tr>
						</table><br>';
		}
