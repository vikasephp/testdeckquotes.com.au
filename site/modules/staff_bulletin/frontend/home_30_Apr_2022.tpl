  ...
  <table id="doclist-table" class="nav-back" width="100%">
    ...
    <form name="mlist" method="post">
      ...
      <td>{{$countn}} </td>
       <td>
           {{if $item.sb_document}}
            <a href="{{$BASE_URL}}files/meeting_minutes_doc/{{$item.sb_document}}?dummy={{$ran}}" target="_blank" style="padding-bottom:2px;" onclick="javascript:counter({{$item.sb_id}})">{{$item.sb_document|truncate:70:"...":true}}</a>
           {{/if}} 
       </td>
