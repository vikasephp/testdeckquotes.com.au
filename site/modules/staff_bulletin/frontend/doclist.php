<?php
$da_id= $fwRequest->getParamget('sb_id', '');
if($sb_id > 0)
{
	
$thisTable = new Fw_Db_Table('staff_bulletin');
$thisTable->setWhere("sb_id = $sb_id");
...
if($docData)
{
	...
	if(!empty($docData['sb_document'])) 
	{
	
	$delid = $da_id ;
		
	$da_merge_plans = $docData['sb_document'];
