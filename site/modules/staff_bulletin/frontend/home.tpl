...
{
	var viewurl = "{{$BASE_URL}}staff_bulletin.view_procedure"+ "/random/" + Math.random();	
...
&nbsp &nbsp; <input type="button" name="proc_panel" value="Procedure Panel" onclick="javascript:add_procedure();" />
</div>
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    ...
    <form name="mlist" method="post">
      ...
      <td>{{$countn}} </td>
       <td>
           {{if $item.sb_document}}
           <!-- <a href="https://deckquote.s3.amazonaws.com/files/meeting_minutes_doc/{{$item.sb_document}}?dummy={{$ran}}" target="_blank" style="padding-bottom:2px;" onclick="javascript:counter({{$item.sb_id}})">{{$item.sb_document|truncate:70:"...":true}}</a>-->
           
           <a href="/staff_bulletin.download_content?file_name={{$item.sb_document}}&module_name=staff_bulletin.home
" target="_blank" onclick="javascript:counter({{$item.sb_id}})">{{$item.sb_document|truncate:70:"...":true}}</a>
