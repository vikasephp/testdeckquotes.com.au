...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
        ...
        bus_customers.bcust_id, bus_customers.bcust_mr_pl_type, bus_customers.bcust_mr_surv_rev, bus_customers.bcust_mr_surv_impl,
        ...
        bus_customers.bcust_mr_pp_nick, bus_customers.bcust_mr_ppsigned, bus_customers.bcust_mr_ent_lodg, business.bsn_ptt_d_type,
        bus_customers.bcust_mr_goal_date, bus_customers.bcust_mr_da_lodged, bus_customers.bcust_mr_likely_da_dt,
		bus_customers.bcust_mr_ba_lodged,  bus_customers.bcust_mr_csbd_accu, business_sellers.bs_traffic_light, business_sellers.bs_tl_user,
...
if(!empty($submit))
{
	...
	$html .= '<tr><th style="width:9%";>Project Details</th><th>Hia Signed Date</th><th style="width:4%";>Planning Manager</th><th style="width:3%";>Days</th>';
	$html .= "<th>Planning Type</th><th>Survey Received</th><th>Survey Implemented</th><th>Project Plans Request Sent</th>";
	$html .= "<th>Ready for Nick to Check</th><th>PP Ready for customer sign off?</th>";
	$html .= "<th>Project Plans checked by Nick?</th><th>Project Plans signed off by Client</th><th>Entities Lodged</th>";
	$html .= "<th>Goal Date for DA to be lodged</th><th>Da lodged</th><th>Likley DA Approval Date</th>";
	$html .= "<th>Ba lodged</th><th>CSBD (Day of BA)</th><th>Is CSBD accurate?</th>";
	$html .= "</tr>";
	foreach($setdata2 as $kp => $vp)
	{
		
		if($vp['bcust_mr_surv_rev'] == "No") { $b1 = 'style="background-color:#F00;"'; } else {$b1 =''; }
		if($vp['bcust_mr_surv_impl'] == "No") { $b2 = 'style="background-color:#F00;"'; } else {$b2 =''; }
		if($vp['bcust_mr_pp_req'] == "No") { $b3 = 'style="background-color:#F00;"'; } else {$b3 =''; }
		...
		if($vp['bcust_mr_ent_lodg'] == "No") { $b6 = 'style="background-color:#F00;"'; } else {$b6 =''; }
		if($vp['bcust_mr_da_lodged'] == "No") { $b7 = 'style="background-color:#F00;"'; } else {$b7 =''; }
		
		if($vp['bcust_mr_ba_lodged'] == "No") { $b8 = 'style="background-color:#F00;"'; } else {$b8 =''; }
		if($vp['bcust_mr_csbd_accu'] == "No") { $b9 = 'style="background-color:#F00;"'; } else {$b9 =''; }
		
		$html .= "<tr>";
		//$html .= "<td>".$vp['bcust_fname']." ".stripslashes($vp['bcust_lname']). "<br>".stripslashes($vp['bsn_name'])."</td>";
		$html .= "<td>".stripslashes($vp['bsn_address'])."</td>";
		
		$html .= "<td>".$vp['hia_signed_date']."</td>";
		$html .= "<td>".$vp['bcust_mr_pl_manager']."</td>";
		$html .= "<td>".$vp['dayscount']."</td>";
		$html .= "<td>".$vp['bcust_mr_pl_type']."</td>";
		$html .= '<td '. $b1.'>'.$vp["bcust_mr_surv_rev"].'</td>';
		$html .= "<td ". $b2.">".$vp['bcust_mr_surv_impl']."</td>";
		$html .= "<td ". $b3.">".$vp['bcust_mr_pp_req']."</td>";
		$html .= "<td ". $b4.">".$vp['bcust_mr_reday_nick']."</td>";
		$html .= "<td ". $b5.">".$vp['bcust_mr_pr_readyfor']."</td>";
		$html .= "<td>".$vp['bcust_mr_pp_nick']."</td>";
		$html .= "<td >".$vp['bcust_mr_ppsigned']."</td>";
		$html .= "<td ". $b6.">".$vp['bcust_mr_ent_lodg']."</td>";
		$html .= "<td>".$vp['bcust_mr_goal_date']."</td>";
		$html .= "<td ". $b7.">".$vp['bcust_mr_da_lodged']."</td>";
		$html .= "<td>".$vp['bcust_mr_likely_da_dt']."</td>";
		$html .= "<td ". $b8.">".$vp['bcust_mr_ba_lodged']."</td>";
		$html .= "<td>".$vp['bsn_latest_csbd']."</td>";
		$html .= "<td ". $b9.">".$vp['bcust_mr_csbd_accu']."</td>";
