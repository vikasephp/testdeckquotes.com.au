...
&nbsp;&nbsp;<input type="button" name="Add New" value="Add New Letter Code" onclick="javascript:add_new('{{$BASE_URL}}timeline_report.add_letter_code');" />  &nbsp;
<input type="button" name="edit_cat" value="Edit Letter Code" onclick="javascript:add_new('{{$BASE_URL}}timeline_report.edit_letter_code');" />&nbsp;
<input type="button" name="view_letter" value="View Letter" onclick="javascript:add_new('{{$BASE_URL}}timeline_report.view_letter');" />
</div>

<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
<div style="float:right;"> 
<input type="submit" name="openclose" value ="Open and Closed Status" />
 ...
 {{foreach from=$list key="key" item="item"}}
    ...
    <tr bgcolor="#FFFFFF">
        ...
        <div style="text-align:center;"> 
         ...
         <a href="/timeline_report.download_content?file_name={{$item.tc_letter_upload}}&module_name=timeline_report.home" target="_blank">Download</a>
        </div>
        {{/if}}
        
        </td>
        <td>{{$item.tc_letter_email}} {{if $item.tc_letter_email}} <br /> By : {{$item.tc_email_user}}{{/if}} </td>
        <td>{{if $item.tc_notes}}
             <a href ="{{$BASE_URL}}timeline_report.view_notes/tc_id/{{$item.tc_id}}" class="various">View Notes</a>
            {{/if}}
        </td>
         <td>
        {{if $item.tc_evidence}}
        ...
        <a href="/timeline_report.download_content?file_name={{$item.tc_evidence}}&module_name=timeline_report.home" target="_blank">Download</a>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="sync" value="{{$item.tc_id}}"  onclick="this.form.submit();" {{if $item.tc_sync eq 1 }} checked="checked" {{/if}} />
