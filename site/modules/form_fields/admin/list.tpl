    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
		            ...
		            <th class="topmenu" align="center" valign="middle" width="15%">Project Type</th>  
                    <th class="topmenu" align="center" valign="middle" width="35%">Question</th>  
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
             <td>{{counter}}</td>
    			<td>{{$item.pt_name}} </td>
                <td>{{$item.ff_question}} </td>
                <td>{{$item.ff_que_status}}</td>
                <td><a href="{{$BASE_URL}}{{$XFA.detail}}/{{$ID}}/{{$item.$ID}}">
                <img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/edit.png"/></a>&nbsp;&nbsp;
                <a href="{{$BASE_URL}}{{$XFA.delete}}/{{$ID}}/{{$item.$ID}}" onclick="javascript:if(!confirm('Are you sure you want to delete the Question?')) return false;"><img style="height: 20px; width:20px" src="{{$BASE_URL}}css/admin/images/delete.png"/></a>
                </td>
            </tr>
