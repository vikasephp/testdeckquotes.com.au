...
<script type="text/javascript">
	    ...
	    var geturl= "{{$BASE_URL}}availability_list.doclist/mid/"; // The server-side script
		var getdelurl= "{{$BASE_URL}}availability_list.docdelete/mid/"; // The server-side script
...
<h3 class="page-title">{{$title}}</h3>
 ...
 <input type="hidden" name="mmin[al_id]" value="{{$detail.$ID}}" id="dh_id"/>
     <table id="viewdoc-table" width="100%" cellpadding="0" cellspacing="0">
     
        <tr>
            <th> Document Name:</th>
            <td> <input type="text" name="mmin[al_doc_name]" value="{{$detail.al_doc_name}}" style="width:500px;"/></td>
        </tr>
        <tr>
...
<script type="text/javascript" language="javascript" >
    ...
    frmvalidator.addValidation("mmin[al_doc_name]","req", "Please specify document name.");
</script>
{{/if}}