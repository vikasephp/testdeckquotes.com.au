...
$trackingtable = new Fw_Db_Table('planning_assessment_tracking');

$searchaddr = $fwRequest->getParam('searchaddr', '');

$submitmoveto = $fwRequest->getParam('moveto', '');
if (!empty($submitmoveto)) {
	$movetoData = $fwRequest->getParam('new', '');

	foreach ($movetoData as $mtk => $mtv) {
		$moveDetail['bsn_planning_moveto_old'] = 0;
		$tablebusiness->setWhere('bsn_id = ' . $mtk);
		$this_id = $tablebusiness->updateRow($moveDetail);
		
		$trackData['pat_bsn_id'] = $mtk;
		$trackData['pat_movedto_quickreport'] = 1;
		$trackData['pat_moved_by'] = $_SESSION['user']['user_name'];;
		$trackingtable->insertRow($trackData);
	}
...
//unset($_SESSION['keyword']);
//$where = "WHERE business.bsn_planning_moveto_old =1 ";
$where = "WHERE ((bsn_sub_status IN ('Hold', 'Lost') AND bsn_planning_moveto_old <> 0 )  OR ( bsn_sub_status = 'Open' AND bsn_planning_moveto_old = 1)) ";
...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
		...
		business.bsn_sub_status, business.bsn_status_sys_date, business.bsn_address , business.bsn_rating
...
$qry = "SELECT qrm_bsn_id, qrm_meet from planning_ass_meet";
$res = $fwDb->query($qry);
$meetdata = [];
foreach($res as $row) {
	$meetdata[$row['qrm_bsn_id']] = $row;
}

$qry = "SELECT bt_bsn_id, bt_task_id, bt_complete, bt_completed_date from business_tasks WHERE bt_task_id IN (555,474,312,301,330,369,340,302,137,206)";
$res = $fwDb->query($qry);
$busTaskData = [];
foreach($res as $row) {
	if(!isset($busTaskData[$row['bt_bsn_id']])) {
		$busTaskData[$row['bt_bsn_id']] = [];
	}
	$busTaskData[$row['bt_bsn_id']][$row['bt_task_id']] = $row;
}

$qry = "SELECT doc_bsn_id, doc_name_id, doc_file_name from document_check_list WHERE doc_name_id IN (44,64,45,2,412)";
$res = $fwDb->query($qry);
$docChkLstData = [];
foreach($res as $row) {
...
$qry = 'SELECT sf_pid, count(*) as tot, max(sf_date) as last_followup_date from sales_followup GROUP BY sf_pid;';
$res = $fwDb->query($qry);
$salesData = [];
foreach($res as $row) {
	$salesData[$row['sf_pid']] = $row;
}

$qry = "SELECT bt_bsn_id, count(*) as totpltasks from business_tasks where bt_task_status_id = 2 GROUP BY bt_bsn_id;";
$res = $fwDb->query($qry);
$planingtasks = [];
foreach($res as $row) {
	$planingtasks[$row['bt_bsn_id']] = $row;
}

$qry = "SELECT bt_bsn_id, count(*) as pltaskcomp from business_tasks where bt_task_status_id = 2 AND bt_complete = 1 GROUP BY bt_bsn_id;";
$res = $fwDb->query($qry);
$taskpldone = [];
foreach($res as $row) {
	$taskpldone[$row['bt_bsn_id']] = $row;
}
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata as $k => $v) {
			...
			// Uid 412 Third Party Designs

			// $sql412 = "Select document_check_list.* from document_check_list
			//          where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 412";
			// $data412 = $fwDb->queryOne($sql412);

			// Uid 412 Third Party Designs ends
			...
			if (!count($taskpldone[$v['bsn_id']]) > 0) {
				$taskpldone[$v['bsn_id']]['pltaskcomp'] = 0;
			}
			...
			$setdata2[$k]['meet'] = $meetdata[$v['bsn_id']]['qrm_meet'];


			$setdata2[$k]['uid555complete'] = $busTaskData[$v['bsn_id']][555]['bt_complete'];
			$setdata2[$k]['uid555'] = $busTaskData[$v['bsn_id']][555]['bt_completed_date'];


			$setdata2[$k]['uid474complete'] = $busTaskData[$v['bsn_id']][474]['bt_complete'];
			$setdata2[$k]['uid474'] = $busTaskData[$v['bsn_id']][474]['bt_completed_date'];

			$setdata2[$k]['uid312complete'] = $busTaskData[$v['bsn_id']][312]['bt_complete'];
			$setdata2[$k]['uid312'] = $busTaskData[$v['bsn_id']][312]['bt_completed_date'];

			$setdata2[$k]['uid301complete'] = $busTaskData[$v['bsn_id']][301]['bt_complete'];
			$setdata2[$k]['uid301'] = $busTaskData[$v['bsn_id']][301]['bt_completed_date'];


			$setdata2[$k]['uid330complete'] = $busTaskData[$v['bsn_id']][330]['bt_complete'];
			$setdata2[$k]['uid330'] = $busTaskData[$v['bsn_id']][330]['bt_completed_date'];


			$setdata2[$k]['uid369complete'] = $busTaskData[$v['bsn_id']][369]['bt_complete'];
			$setdata2[$k]['uid369'] = $busTaskData[$v['bsn_id']][369]['bt_completed_date'];

			$setdata2[$k]['uid340complete'] = $busTaskData[$v['bsn_id']][340]['bt_complete'];
			$setdata2[$k]['uid340'] = $busTaskData[$v['bsn_id']][340]['bt_completed_date'];


			$setdata2[$k]['checklist44'] = $docChkLstData[$v['bsn_id']][44]['doc_file_name'];

			$setdata2[$k]['salesfolloup'] = $salesData[$v['bsn_id']]['tot'];
			$setdata2[$k]['lastfoldt'] = $salesData[$v['bsn_id']]['last_followup_date'];

			$setdata2[$k]['checklist45'] = $docChkLstData[$v['bsn_id']][45]['doc_file_name'];

			$setdata2[$k]['checklist2'] = $docChkLstData[$v['bsn_id']][2]['doc_file_name'];
			$setdata2[$k]['checklist412'] = $docChkLstData[$v['bsn_id']][412]['doc_file_name'];

			if ($busTaskData[$v['bsn_id']][302]['bt_complete'] == 1) {
				$setdata2[$k]['boxsent'] = $busTaskData[$v['bsn_id']][302]['bt_completed_date'];
			}

			$setdata2[$k]['uid137complete'] = $busTaskData[$v['bsn_id']][137]['bt_complete'];
			$setdata2[$k]['uid137'] = $busTaskData[$v['bsn_id']][137]['bt_completed_date'];


			$setdata2[$k]['uid206complete'] = $busTaskData[$v['bsn_id']][206]['bt_complete'];
			$setdata2[$k]['uid206'] = $busTaskData[$v['bsn_id']][206]['bt_completed_date'];
			...
			$setdata2[$k]['total_pl_task'] = $planingtasks[$v['bsn_id']]['totpltasks'];
			$setdata2[$k]['task_pl_done'] = $taskpldone[$v['bsn_id']]['pltaskcomp'];
		}
...
$fwViewData['title'] = "Planning Assesment Old Report";
...
if ($submitdownload) {
	...
	foreach ($userDataP as $k => $v) {
		...
		$setdataP[$k]['uid555complete'] = $busTaskData[$v['bsn_id']][555]['bt_complete'];
		$setdataP[$k]['uid555'] = $busTaskData[$v['bsn_id']][555]['bt_completed_date'];

		$setdataP[$k]['uid474complete'] = $busTaskData[$v['bsn_id']][474]['bt_complete'];
		$setdataP[$k]['uid474'] = $busTaskData[$v['bsn_id']][474]['bt_completed_date'];

		$setdataP[$k]['uid312complete'] = $busTaskData[$v['bsn_id']][312]['bt_complete'];
		$setdataP[$k]['uid312'] = $busTaskData[$v['bsn_id']][312]['bt_completed_date'];

		$setdataP[$k]['uid301complete'] = $busTaskData[$v['bsn_id']][301]['bt_complete'];
		$setdataP[$k]['uid301'] = $busTaskData[$v['bsn_id']][301]['bt_completed_date'];


		$setdataP[$k]['uid330complete'] = $busTaskData[$v['bsn_id']][330]['bt_complete'];
		$setdataP[$k]['uid330'] = $busTaskData[$v['bsn_id']][330]['bt_completed_date'];


		$setdataP[$k]['uid369complete'] = $busTaskData[$v['bsn_id']][369]['bt_complete'];
		$setdataP[$k]['uid369'] = $busTaskData[$v['bsn_id']][369]['bt_completed_date'];

		$setdataP[$k]['uid340complete'] = $busTaskData[$v['bsn_id']][340]['bt_complete'];
		$setdataP[$k]['uid340'] = $busTaskData[$v['bsn_id']][340]['bt_completed_date'];

		$setdataP[$k]['checklist44'] = $docChkLstData[$v['bsn_id']][44]['doc_file_name'];

		$setdataP[$k]['salesfolloup'] = $salesData[$v['bsn_id']]['tot'];
		$setdataP[$k]['lastfoldt'] = $salesData[$v['bsn_id']]['last_followup_date'];

		$setdataP[$k]['checklist45'] = $docChkLstData[$v['bsn_id']][45]['doc_file_name'];

		$setdataP[$k]['checklist2'] = $docChkLstData[$v['bsn_id']][2]['doc_file_name'];

		if ($busTaskData[$v['bsn_id']][302]['bt_complete'] == 1) {
			$setdataP[$k]['boxsent'] = $busTaskData[$v['bsn_id']][302]['bt_completed_date'];
		}

		$setdataP[$k]['uid137complete'] = $busTaskData[$v['bsn_id']][137]['bt_complete'];
		$setdataP[$k]['uid137'] = $busTaskData[$v['bsn_id']][137]['bt_completed_date'];


		$setdataP[$k]['uid206complete'] = $busTaskData[$v['bsn_id']][206]['bt_complete'];
		$setdataP[$k]['uid206'] = $busTaskData[$v['bsn_id']][206]['bt_completed_date'];
