                ...
                <div style="float: left; margin-left: 5px;">
                    <div>
                        ...
                        <span style="font-size: 18px;">
                            <a href="https://recruitments.deckquotes.com.au/login" target="_blank" rel="noopener">Click Here To Open Recruitment Website</a>
                        </span>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="re_position_apply">
                            <option value="">Show All</option>
                            {{foreach from=$get_positions_data item="item_posi" key="key_posi"}}
                                <option value="{{$item_posi.pm_position}}" {{if $item_posi.pm_position eq $re_position_apply}}selected{{/if}}>{{$item_posi.p_name}}</option>
                            {{/foreach}}
                        </select>
                        <input type="submit" value="Search" name="filter_position" />
    ...
    <table id="doclist-table" class="nav-back" width="100%">
        <thead>
            <tr>
                ...
                <th class="topmenu" align="center" valign="middle" width="3%" data-col="re_is_login_enabled">Login
                    Enabled</th>
                <th class="topmenu" align="center" valign="middle" width="3%" data-col="recruitment_list_log">Download
                    Doc Log</th>
                ...
                <th class="topmenu" align="center" valign="middle" width="6%" data-col="positions_apply">Position Apply</th>
				<th class="topmenu" align="center" valign="middle" width="3%">Email 1 Sent</th>
            ...
            <form name="mlist" method="post">
                ...
                <tr bgcolor="{{cycle values='#D3E8D6,WHITE' advance=true}}">
                        ...
                        <form name="recmet2" method="post" action="">
                            ...
                            <label class="switch">
                                <input class="switch-input" type="checkbox" name="login[{{$item.re_id}}]" value="1"
                                    onclick="this.form.submit();" {{if $item.re_is_login_enabled eq 1 }}
                                    checked="checked" {{/if}}>
                    ...
                    <td data-col="recruitment_list_log">
                        <input type="button" name="proc_panel" value="LOG"
                            onclick="javascript:viewLog({{$item.re_id}});" />
                    </td>
                    <td>{{$item.re_date}}</td>
                    <td>{{$item.re_position}}</td>
                    <td data-col="positions_apply">
                        {{if $get_positions_data}}
                        <select class="select" name="position_apply"
                            onchange="updateRecord('{{ $item.re_id }}', 're_position_apply', this.value)">
                            <option value="">Select Position Apply</option>
                            {{foreach from=$get_positions_data item="item_posi" key="key_posi"}}
                            <option value="{{$item_posi.pm_position}}" {{if $item_posi.pm_position eq
                                $item.re_position_apply}}selected{{/if}}>{{$item_posi.p_name}}</option>
                            {{/foreach}}
                        </select>
                        {{/if}}
                    </td>
					<td>
                        <form name="send_email_form" method="post" action="">
                            <!--<input type="hidden" name="email_cust[{{$item.re_id}}]" value="0" />-->
							<input type="hidden" name="re_id" value="{{$item.re_id}}">
							<input type="hidden" name="send_customer_email" value="1">
                            <input type="submit" value="Send Email">
                        </form>
                        {{if $item.re_cust_email_sent eq 1 }} {{$item.re_cust_email_user}}<br />{{$item.re_cust_email_date}}
                        {{/if}}
                    </td>
                    <td>
                        <div class="tooltip">
                        ...
                        <form name="recmet2" method="post" action="">
                            ...
                            <label class="switch">
                                <input class="switch-input" type="checkbox" name="hide[{{$item.re_id}}]" value="1"
                                    onclick="this.form.submit();" {{if $item.re_hide eq 1 }} checked="checked" {{/if}}>
                                <span class="switch-label" data-on="Yes" data-off="No"></span>
                                <span class="switch-handle"></span> <br>
...
<script>
    function updateRecord(primaryID, column, value, updateRecord = false) {
        ...
        $.ajax({
            type: "POST",
            url: "{{$BASE_URL}}recruitment_list.ajax_handler",
            data: formData,
            dataType: "json",
            encode: true,
