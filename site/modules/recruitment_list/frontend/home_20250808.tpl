...
<input type="text" name="{{$TABLE}}[keyword]" value="{{$keyword}}" style="width:400px;" placeholder="Search by Name, Position"/>
&nbsp;&nbsp;&nbsp;&nbsp;
...
<!--            <a href="https://deckquote.s3.amazonaws.com/files/meeting_minutes_doc/{{$item.re_survey_link}}?dummy={{$ran}}" target="_blank" style="padding-bottom:2px;">{{$item.re_survey_link|truncate:70:"...":true}}</a>-->
            ...
            <a href="/recruitment_list.download_content?file_name={{$item.re_survey_link}}&module_name=recruitment_list.home" target="_blank" style="padding-bottom:2px;">{{$item.re_survey_link|truncate:70:"...":true}}</a>
...
<!--              <a href="https://deckquote.s3.amazonaws.com/files/meeting_minutes_doc/{{$item.re_doc2}}?dummy={{$ran}}" target="_blank" style="padding-bottom:2px;">{{$item.re_doc2|truncate:70:"...":true}}</a>-->
              ...
              <a href="/recruitment_list.download_content?file_name={{$item.re_doc2}}&module_name=recruitment_list.home" target="_blank" style="padding-bottom:2px;">{{$item.re_doc2|truncate:70:"...":true}}</a>  
...
<!--              <a href="https://deckquote.s3.amazonaws.com/files/meeting_minutes_doc/{{$item.re_doc3}}?dummy={{$ran}}" target="_blank">{{$item.re_doc3|truncate:70:"...":true}}</a>-->
               ...
               <a href="/recruitment_list.download_content?file_name={{$item.re_doc3}}&module_name=recruitment_list.home" target="_blank">{{$item.re_doc3|truncate:70:"...":true}}</a>
        ...
        <a href="/recruitment_list.download_content?file_name={{$item.re_survey_result}}&module_name=recruitment_list.home" target="_blank">{{$item.re_survey_result}}</a>
          {{/if}} 
         </td>
        <td>{{$item.re_notes}}</td>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="email_1[{{$item.re_id}}]" value="1"  onclick="this.form.submit();" {{if $item.re_email_1_sent eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
         </label> 
         </form>
         {{if $item.re_email_1_sent eq 1 }} {{$item.re_email_1_user}}<br />{{$item.re_email_1_date}} {{/if}}
          </td>
        <td> <form name ="recmet2" method="post" action="">
     	<input type="hidden" name="email_2[{{$item.re_id}}]" value="0" />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="email_2[{{$item.re_id}}]" value="1"  onclick="this.form.submit();" {{if $item.re_email_2_sent eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       
        </label> 
         </form>
         {{if $item.re_email_2_sent eq 1 }} {{$item.re_email_2_user}}<br />{{$item.re_email_2_date}} {{/if}}
          </td>
        
        <td>
         {{if $item.re_recruitment_checklist}} <a href="{{$item.re_recruitment_checklist}}" target="_blank"> Link </a> <a href="{{$BASE_URL}}recruitment_list.delete_link/rc_link/{{$item.re_id}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img src="{{$BASE_URL}}css/admin/images/deletecross.png" width="16" /></a>
          {{else}}
         <form name="rec_check" action="" method="post" enctype="multipart/form-data">
            <input type="hidden" name="re_id" value="{{$item.re_id}}" />
            <input type="text" name="re_recruitment_checklist" required="required" style="width:100px;" />
       ...
       {{/if}}
         ...
         <td>
         {{if $item.re_onboarding_checklist}} <a href="{{$item.re_onboarding_checklist}}" target="_blank"> Link </a> <a href="{{$BASE_URL}}recruitment_list.delete_link/oc_link/{{$item.re_id}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img src="{{$BASE_URL}}css/admin/images/deletecross.png" width="16" /></a>
          {{else}}
         <form name="rec_check" action="" method="post" enctype="multipart/form-data">
            <input type="hidden" name="re_id" value="{{$item.re_id}}" />
            <input type="text" name="re_onboarding_checklist" required="required" style="width:100px;" />
       ...
       {{/if}}
        ...
        <td>{{if $item.re_qed eq 1}} Yes {{else}} No {{/if}}</td>
        <td>
        <form name ="recmet2" method="post" action="">
     	<input type="hidden" name="hide[{{$item.re_id}}]" value="0" />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="hide[{{$item.re_id}}]" value="1"  onclick="this.form.submit();" {{if $item.re_hide eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
