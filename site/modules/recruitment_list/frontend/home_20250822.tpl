            ...
            <form name="mlist" method="post">
                ...
                <tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
					...
					<td data-col="positions_apply">
						...
						<select class="select" name="position_apply" onchange="updateRecord('{{ $item.re_id }}', 're_position_apply', this.value)">
							<option value="">Select Position Apply</option>
							{{foreach from=$get_positions_data item="item_posi" key="key_posi"}}
							<option value="{{$item_posi.pm_position}}" {{if $item_posi.pm_position eq $item.re_position_apply}}selected{{/if}}>{{$item_posi.p_name}}</option>
							{{/foreach}}
						</select>
						{{/if}}
					</td>
