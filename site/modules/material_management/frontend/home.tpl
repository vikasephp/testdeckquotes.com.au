...
<div style="float:left;"><select name="material_short" onchange="submit();">
					<option value="0">--Select Material--</option>
	 				{{foreach from=$marerials key="key" item="item"}}
                    <option value="{{$item.mr_id}}" {{if $item.mr_id == $mid}}selected{{/if}}>{{$item.mr_name}}</option>
                    {{/foreach}}
</select> </div>
</form>
    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
                    ...
                    <th class="topmenu" align="center" valign="middle" width="5%">Image</th>   
            		<th class="topmenu" align="center" valign="middle" width="35%">Title</th>  
                    <th class="topmenu" align="center" valign="middle" width="25%">Weblink</th>  
                    <th class="topmenu" align="center" valign="middle" width="5%">Supplier</th> 
                    <th class="topmenu" align="center" valign="middle" width="5%">Brand</th>
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
                <td>{{counter}}</td>
                <td>{{if $item.ms_image}}<a href="{{$BASE_URL}}files/suppliers/{{$item.ms_image}}" class="view" title="View">
                <img src="{{$BASE_URL}}files/suppliers/{{$item.ms_image}}" width="80"></a>{{/if}}</td>
                <td>{{$item.mr_name}}</td>
                <td><a href="{{$item.ms_link}}" target="_blank">{{$item.ms_link}}</a></td>
                <td>{{if $item.sp_logo}}<img src="{{$BASE_URL}}files/suppliers/logo/{{$item.sp_logo}}" width="100">{{/if}}<div style="padding-top:10px;" align="center">{{$item.sp_name}}</div></td>
                <td>{{if $item.br_logo}}<img src="{{$BASE_URL}}files/suppliers/brandslogo/{{$item.br_logo}}" width="100">{{/if}}<div style="padding-top:10px;" align="center">{{$item.br_name}}</div></td>
