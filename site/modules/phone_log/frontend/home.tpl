...
<input type="text" name="keyword" value="{{$keyword}}" style="width:400px;" placeholder="Search for Number, Name, Company, Email" />
&nbsp;
...
<div id="">
 ...
 {{foreach from=$list key="key" item="item"}}
    ...
    <tr bgcolor="#FFFFFF">
		...
		<td>
		{{$item.ph_address}} 
		{{if $item.ph_bsn_id > 0 && $item.ph_bsn_id != ''}}
			-	<a href="{{$BASE_URL}}business.detail/bsn_id/{{$item.ph_bsn_id}}" target="_blank">Link</a> 
				<a href="{{$BASE_URL}}phone_log.delete_address/ph_id/{{$item.ph_id}}" onclick="javascript:if(!confirm('Are you sure want to Remove this Address?')) return false;" title="Remove Address">     <img style="height: 16px;; width:16px" src="{{$BASE_URL}}css/admin/images/deletecross.png"/></a>
		{{/if}}
		<br/><br/>

		<datalist id="address_{{$item.ph_id}}"></datalist>

		<input type="text" id="address_input_{{$item.ph_id}}" list="address_{{$item.ph_id}}" style="width:300px" placeholder="Enter Address" />
		<input type="hidden" id="ph_bsn_id_{{$item.ph_id}}" name="ph_bsn_id" value="{{$item.ph_bsn_id}}" />

		<button id="submit_btn_{{$item.ph_id}}" disabled>Save Address</button>
		<script type="text/javascript">
			var addressMap_{{$item.ph_id}} = {};

			$(document).ready(function(){

				$('#address_input_{{$item.ph_id}}').bind('keyup', function () {
					var q = $(this).val();
					if (q.length < 2) return;
					
					if (addressMap_{{$item.ph_id}}[q]) {
						return;
					}

					$.ajax({
						url: "{{$BASE_URL}}phone_log.search_address",
						type: "GET",
						data: { q: q },
						dataType: 'json',
						success: function (res) {
							var list = '';
							addressMap_{{$item.ph_id}} = {};

							for (var i = 0; i < res.length; i++) {
								//list += '<option value="' + res[i].address + '"></option>';
								//addressMap_{{$item.ph_id}}[res[i].address] = res[i].bsn_id;
								
								var label = res[i].address + ' (ID:' + res[i].bsn_id + ')';
								list += '<option value="' + label + '"></option>';
								addressMap_{{$item.ph_id}}[label] = res[i].bsn_id;
							}

							$('#address_{{$item.ph_id}}').html(list);
						}
					});
				});

				$('#address_input_{{$item.ph_id}}').bind('change', function () {
					var val = $(this).val();
					var bsn_id = addressMap_{{$item.ph_id}}[val] || '';
					$('#ph_bsn_id_{{$item.ph_id}}').val(bsn_id);

					if (bsn_id) {
						$('#submit_btn_{{$item.ph_id}}').removeAttr('disabled');
					} else {
						$('#submit_btn_{{$item.ph_id}}').attr('disabled', 'disabled');
					}
				});

				$('#submit_btn_{{$item.ph_id}}').bind('click', function () {
					var ph_bsn_id = $('#ph_bsn_id_{{$item.ph_id}}').val();
					if (!ph_bsn_id) {
					...
					$.ajax({
						url: "{{$BASE_URL}}phone_log.save_address",
						type: "POST",
						data: {
							ph_id: {{$item.ph_id}},
							ph_bsn_id: ph_bsn_id
