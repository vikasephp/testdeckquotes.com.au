...
<script type="text/javascript">
	...
	function add_procedure() {
		var viewurl = "{{$BASE_URL}}last_meeting_minutes.view_procedure" + "/random/" + Math.random();
	...
	<table id="list-table" class="nav-back" width="100%">
		<thead>
			<tr>
				...
				<th class="topmenu" align="center" valign="middle" width="15%">Date of Meeting</th>
				<th class="topmenu" align="center" valign="middle" width="15%">Status</th>
				<th class="topmenu" align="center" valign="middle" width="7%">Total Meeting Minutes</th>
				<th class="topmenu" align="center" valign="middle" width="7%">Link To Meeting Minutes</th>
		...
		<tbody>
			...
			<tr bgcolor="#FFFFFF">
				<td>{{$item.lmm_bsn_name}}</td>
				<td>
					<span class="lm_doc_date_uploaded__ff" id="r__id_{{$item.lmm_id}}">{{$item.latest_date}}</span>
					{{if $item.latest_date != "Latest"}}          
						<div id="update_elemnt_date_block">
							<button class="update_elemnt_date" data-id="" type="button">Update Date</button>
							<input style="display:none;" onchange="update_date('{{$item.lmm_id}}',this)" type="date" />
						</div>    
					{{/if}} 
				</td>
				<td>{{$item.status}}</td>
				<td>{{$item.tot_mm}}</td>
				<td>
					{{if $item.lmm_doc_file_name}}
					<a href="/last_meeting_minutes.download_content?file_name={{$item.lmm_doc_file_name}}&module_name=last_meeting_minutes.home"
...
<script>
	...
	function update_date(record_id, t) {
		...
		var formData = {
			r_ID: record_id,
			date: currentDate,
			action: "update_meetingminute_date"
		};

		$.ajax({
			type: "POST",
			url: "{{$BASE_URL}}latest_meeting_minutes.ajax_handler",
			data: formData,
			dataType: "json",
			encode: true,
