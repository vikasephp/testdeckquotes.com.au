...
$sql = '
    SELECT
        CASE
            WHEN TDT.tdt_task_assignee != "" AND TDT.tdt_due_date != "" THEN 1
            WHEN TDT.tdt_due_date != "" THEN 2
            WHEN TDT.tdt_task_assignee != "" THEN 3
            ELSE 4
        END AS final_order,
        IF(TDT.tdt_task_assignee = "", 0, 1) AS format_tdt_task_assignee,
        STR_TO_DATE(TDT.tdt_due_date, "%d-%m-%Y") AS format_tdt_due_date,
        TDT.tdt_id, TDT.tdt_dt_id, TDT.tdt_task_desc, TDT.tdt_task_assignee, TDT.tdt_task_assigner, TDT.tdt_due_date, TDT.tdt_status, DT.dt_checklist_no
    FROM task_designer_tracker AS TDT
        LEFT JOIN
            designer_tracker AS DT
        ON DT.dt_id = TDT.tdt_dt_id
    WHERE
        TDT.tdt_status != "COMPLETE"
        AND DT.dt_checklist_no IS NOT NULL
        AND STR_TO_DATE(TDT.tdt_due_date, "%d-%m-%Y") < "' . date('Y-m-d') . '"
    ORDER BY
        final_order ASC,
        format_tdt_task_assignee DESC,
        TDT.tdt_task_assignee ASC,
        format_tdt_due_date ASC,
        TDT.tdt_id ASC;
';
...
if (!empty($result)) {
    ...
    $query = "SELECT * FROM email_signature where es_id = 217";
    ...
    foreach ($result as $row) {
        ...
        $html .= '
                <tr>
                    ...
                    <td style="white-space: nowrap;">' . (strtotime($row['format_tdt_due_date']) > 0 ? date('d-M-Y', strtotime($row['format_tdt_due_date'])) : '') . '</td>
    ...
    $emailObj->subject = 'Design Task Report - Project Passed Due Date';
    ...
    $emailObj->addTo('Planning@cgfb.com.au', 'Planning CGFB');
    $emailObj->addTo('inclusions@cgfb.com.au', 'Inclusions CGFB');
