...
$query = getLateLetterReportQuery() . ' AND PPD.ppd_default_int = 1 AND DATEDIFF(CURRENT_DATE(), STR_TO_DATE(PPD.ppd_inv_due_date, "%d-%m-%Y")) = 1';
...
foreach ($result as $row) {
    ...
    $d1 = date('Y-m-d');
    $d2 = changedate_y_m_d($row['ppd_inv_due_date']);
    $d3 = changedate_y_m_d($row['ppd_when_payrec']);
    $days_late = 0;
    if (!empty($d2) && empty($d3)) {
        $days_late = daysDifference($d1, $d2);
    }
    ...
    if (!empty($row['ppd_hia_value'])) {
        ...
        $annual = ($value * 14) / 100;
        $daily = $annual / 365;
        $interest_calculated = $daily * $days_late;
