...
$query = getLateLetterReportQuery(' AND PPD.ppd_default_int = 1', ' AND days_late >= 10');
...
foreach ($result as $row) {
    ...
    if (!empty($row['ppd_hia_value']) && !empty($row['ppd_value'])) {
        $hia_value_raw = $row['ppd_hia_value'];
        $hia_value = str_replace(['$', ','], '', $hia_value_raw);

        $ppd_value_raw = $row['ppd_value'];
        $ppd_value = str_replace(['$', ','], '', $ppd_value_raw);

        $value = $hia_value - $ppd_value;

        $annual = ($value * 14) / 100;
        $daily = $annual / 365;
        $interest_calculated = $daily * $row['days_late'];
        $total_amount = $value + $interest_calculated;
    } elseif (!empty($row['ppd_hia_value'])) {
...
foreach ($records as $record) {
    foreach ($customers[$record['bsn_id']] as $customer) {
        ...
        $html .= '<br>';
        $html .= '<p>This email is to notify you that your progress payments have been made late. The invoice is as follows:</p>';
        $html .= '<br>';
        ...
        $html .= '<br>';
        $html .= '<p>Please Find Link of HIA FAQs</p>';
        $html .= '<p>' . $faqHIA . '</p>';
        $html .= '<br>';
        ...
        $html .= '<p>Please respond within 7 days or we will consider taking legal action</p>';
        $html .= '<br>';
        ...
        // $emailObj->addTo('supportfinance@cgfb.com.au', 'Support');
