...
$query = getLateLetterReportQuery(' AND PPD.ppd_default_int = 1', ' AND days_late = 11');
...
foreach ($result as $row) {
	...
	$amnt_owed = 0;
	$hia_value = !empty($row['hia_value']) ? (float)str_replace(['$', ','], '', $row['hia_value']) : 0;
	$paid_value = !empty($row['paid_value']) ? (float)str_replace(['$', ','], '', $row['paid_value']) : 0;

	if ($paid_value > 0) {
		$amnt_owed = $hia_value - $paid_value;
	...
	if ($amnt_owed > 0) {
		$annual = ($amnt_owed * 14) / 100;
		$daily = $annual / 365;
		$interest_calculated = $daily * $row['days_late'];
		$total_amount = $amnt_owed + $interest_calculated;
	}
    ...
    $records[$row['bsn_id']][] = [
		...
		'amount' => '$' . number_format(($amnt_owed > 0) ? $amnt_owed : $hia_value, 2, '.', ','), // Outstanding Amount
...
foreach ($records as $bsn_id => $payment_records) {
    ...
    foreach ($customers[$bsn_id] as $customer) {
        ...
        foreach ($payment_records as $record) {
            ...
            $html .= '<h4 style="font-weight: bold; font-size: 20px; margin-bottom: 0;">Invoice Summary:</h4>';
			...
			$html .= '<p>Also please find attached the invoice.</p>';
			...
			echo $html; //exit('Checking HTML');

            $emailObj->subject = $subject;
            $emailObj->message = $html;
            $emailObj->addFrom($signatureRes['es_reply_to'], $signatureRes['es_name']);
            //$emailObj->addTo($customer['bcust_misc_email1'], $customer['bcust_fname']);
            $emailObj->addCC('accounts@cgfb.com.au', 'Accounts');
            $emailObj->addCC('contracts@cgfb.com.au', 'Contracts');
            ...
            if (!$response['success']) {
                ...
                $ppd_id = $record['ppd_id'];
                $fwDb->queryOne("UPDATE progress_payment_detail SET ppd_email_date='$todayDate' WHERE ppd_id=$ppd_id");
            }
