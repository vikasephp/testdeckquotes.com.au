...
//Inner Join document_proposal_name On business.bsn_id = document_proposal_name.dpn_bsn_id Order by document_proposal_name.dpn_create_date DESC
...
$message .= "<tr><td>SrNo</td><td>Customer Name</td><td>Project Address</td><td>Link</td><td>Total</td>
			...
			//$sqldrop = "DROP table IF EXISTS document_proposal_name_temp";
			//$viewdrop = $fwDb->execute($sqldrop);
		
			//$sqlview = "Create table document_proposal_name_temp as select * from document_proposal_name order by dpn_create_date DESC ";
			//$viewData = $fwDb->execute($sqlview);
	...
	foreach($paData as $k => $v)
	{
			...
			$sqlpro = "select document_proposal_name.* from document_proposal_name where dpn_bsn_id =" .$bsn_id. " and dpn_include_in_report = 1 ";
				
			//$sqlpro = "select document_proposal_name_temp.* from document_proposal_name_temp where dpn_bsn_id =" .$bsn_id. " and dpn_include_in_report = 1 ";
			
			$proData = $fwDb->query($sqlpro);
	
			if(!empty($proData))
			{
			    foreach($proData as $pv)
			    {
						...
						$Statusdate = changedate_y_m_d($pv['dpn_create_date']);
						$curdate_y_m_d = date('Y-m-d');
						$days_at_status = daysDifference($curdate_y_m_d, $Statusdate);
						
						if($pv['dpn_design_type'] == 1) { $dt = "Concept Design" ; }
						elseif($pv['dpn_design_type'] == 2) { $dt = "Modification Design" ; }
						elseif($pv['dpn_design_type'] == 3) { $dt = "Existing Design" ; }
						elseif($pv['dpn_design_type'] == 4) { $dt = "External Design" ; }
						...
						$upsql_4 = "select document_check_list_mini.doc_name_id,  document_check_list_mini.doc_file_name, document_check_list_mini.doc_date_uploaded from 	document_check_list_mini
								   where document_check_list_mini.doc_checklist_number = ".$pv['dpn_proposal_number']. "
						...
						$sql_uid10 = "select proposal_tasks.bt_completed_date,  proposal_tasks.bt_complete  from proposal_tasks
						     	   where proposal_tasks.bt_task_list_number  = ".$pv['dpn_proposal_number']. "
						...
						$message .= "<td>".$pv['dpn_unique_id']."</td>";
						
						$message .= "<td>".changedate_d_m_Y($pv['dpn_create_date'])."</td>";
						
						if($pv['dpn_create_date'] != '0000-00-00 00:00:00') {
