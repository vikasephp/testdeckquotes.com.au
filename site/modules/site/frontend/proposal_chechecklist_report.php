...
$message .= "<tr><td>SrNo</td><td>Customer Name</td><td>Project Address</td><td>Link</td><td>Total</td>
			<td>ID</td> <td>Create Date</td><td>Day Count</td><td>Design Type</td><td>Customer Completed - Signed Customer Design Authority</td>
			<td>Customer Designer - Merged Final<br>Approved Plans - Signed By Customer -PDF</td>
	...
	foreach($paData as $k => $v)
	{
			$bsn_id =  $v['bsn_id'];
					  ...
					  if(!empty($bsn_id)) {
						...
						$prosql = "select document_check_list.doc_name_id,  document_check_list.doc_file_name, document_check_list.doc_date_uploaded from document_check_list
								   where document_check_list.doc_name_id = 45 and document_check_list.doc_bsn_id = ". $bsn_id;
						
						$data_pro = $fwDb->queryOne($prosql);
						...
						$message .= "<td>".$dt."</td>";
						$message .= "<td>".changedate_d_m_Y($data_pro['doc_date_uploaded'])."</td>";
...
$message .= "</table></body></html>";
		...
		$sqle = "Select ar_receving_email from automated_reports where ar_id = 3";
		...
		if(!empty($emails)) {
			...
			{
				send_email('Frank Walmsley', trim($ve), 'CGFB', 'info@cgfb.com.au', 'Proposal Checklist Report', $message, $attachment='');	
			}
		}
