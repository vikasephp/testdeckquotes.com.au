        ...
        <table id="list-table" width="100%">
           ...
           {{if $error}} <tr><th colspan="2" style="color:#FF0000;">{{$error}}</th></tr>{{/if}}
            ...
            <tr>
                ...
                <textarea cols="100" name="{{$TABLE}}[notes]" rows="5" class="tinymce">{{$detail.notes|stripslashes}}</textarea>    
                </td></tr></table>
                </td>
            </tr>
             <tr>
            ...
            <tr>
                ...
                <td> <!--<div style="width:650px;">
                        ...
                        {{foreach from=$formulatypes key="key" item="item"}}
                            ...
                            <input type="checkbox" name="{{$TABLE}}[tags][]" value="{{$item.sft_id}}" {{if $detail.tags}} {{if $item.sft_id|in_array:$detail.tags}} checked="checked" {{/if}} {{/if}}>&nbsp;{{$item.sft_title}}</li>
                        {{/foreach}}
                        </ul>
  					</div>-->
                   <input type="text" name = "{{$TABLE}}[tags]" size="140" value="{{$detail.tags}}"  />
                </td>
            </tr>
			<tr>
