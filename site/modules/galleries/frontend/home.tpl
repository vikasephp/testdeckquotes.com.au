...
<tbody>
 ...
 <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
    ...
    <td>
    <div align="center">
 <!--   {{if $item.file}}<a href="{{$BASE_URL}}files/galleries/{{$item.file}}" class="various2"><img src="{{$BASE_URL}}files/galleries/250_{{$item.file}}" border="2" /></a>{{/if}}</div>
-->   
 {{if $item.file}}<a href="https://deckquote.s3.amazonaws.com/files/galleries/{{$item.file}}" class="various2">
 <img data-img-name="{{$item.file}}" data-img-id="{{$item.id}}" width ="250" src="https://deckquote.s3.amazonaws.com/files/galleries/{{$item.file}}" border="2" /></a>{{/if}}</div>
...
<script type="text/javascript">
   ...
   function get_img_raw_data(myKeyVals){
      $.ajax({
              type: 'POST',
              url: '/galleries.get_image_raw_data',
              data: myKeyVals,
              dataType: "json",
    ...
    $(document).ready(function(){
        $("#list-table tr img").each(function(){
