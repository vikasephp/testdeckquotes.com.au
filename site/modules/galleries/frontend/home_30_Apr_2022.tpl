...
<tbody>
 ...
 <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
    ...
    <td>{{$item.id}}</td>
    <td>
    <div align="center">{{if $item.file}}<a href="{{$BASE_URL}}files/galleries/{{$item.file}}" class="various2"><img src="{{$BASE_URL}}files/galleries/250_{{$item.file}}" border="2" /></a>{{/if}}</div></td>
    <td>{{$item.title}}</td> 
    <td> {{$item.tags}} 
