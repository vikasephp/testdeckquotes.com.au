...
<script src="{{$BASE_URL}}js/fancybox/jquery-1.4.3.min.js"></script>
...
<div align="center">
<h3 class="page-title">{{$title}} - {{$bsn_name}}</h3>
...
<form name="detail" method="post" action="{{$XFA.home}}" enctype="multipart/form-data">
    ...
    <table id="list-table" width="90%" cellpadding="0" cellspacing="0" height="350" style="border:1px solid #CCC;">
		...
		{{if $wlai}}
			...
			<tr>
				...
				<td>
					<input type="text" value="{{$item.wlai_who}}" onkeyup="updateRecord('{{ $item.wlai_id }}', 'wlai_who', this.value)">
				</td>
				<td>
	...
	<div style="text-align: center; margin-top: 15px;">
		<input type="button" value="Merge And Email" onclick="mergeAndEmail({{$wlai_bsn_id}})">
	</div>	
</form>
...
<script>
	function updateRecord(primaryID, column, value, updateRecord = false) {
        ...
        $.ajax({
            type: "POST",
            url: "{{$BASE_URL}}watch_list_report.action_items_ajax",
            data: formData,
            dataType: "json",
            encode: true,
	...
	function mergeAndEmail(wlai_bsn_id) {
        const formData = {
            wlai_bsn_id: wlai_bsn_id,
            action: "mergeAndEmail"
        };
        $.ajax({
            type: "POST",
            url: "{{$BASE_URL}}watch_list_report.action_items_ajax",
