...
<form name="mlist" method="post">
	...
	<div style="float:left; text-align:left;">
		...
		&nbsp;&nbsp;<span style="border:1px solid #0CF; padding:3px;"> 7 Days Follow-Up Booked : {{$followup_data_weekly.weekly_total}}</span>&nbsp;&nbsp;<span style="border:1px solid #0CF; padding:3px;"> YTD Follow-Up Booked : {{$followup_data_ytd.ytd_total}}</span>
	</div>
		...
		<table id="doclist-table" class="nav-back" width="100%">
			<thead>
				<tr>
					...
					<th class="topmenu" align="center" valign="middle" width="4%">Action Items</th>
					<th class="topmenu" align="center" valign="middle" width="4%">Type</th>

					<th class="topmenu" align="center" valign="middle" width="6%">Add To Real State Report</th>
					...
					<th class="topmenu" align="center" valign="middle" width="4%">Planning Assessment Client Minutes
					</th>
					...
					<th class="topmenu" align="center" valign="middle" width="4%">Follow-Up Booked</th>
					...
					<th class="topmenu" align="center" valign="middle" width="4%">Hide From Report</th>
			...
			<tbody>
				...
				<tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
					...
					<td><strong>{{$item.bcust_fname}} &nbsp; {{$item.bcust_lname}}</strong> <br />{{$item.bsn_address}}
					</td>
					...
					<td>
						<a href="{{$BASE_URL}}watch_list_report.action_items/wlai_bsn_id/{{$item.bsn_id}}" class="various" title="Add">
						{{if $item.total_action_items}}
							{{$item.complete_action_items}}/{{$item.total_action_items}} <br/>
						{{/if}}
						Add
						</a>
					</td>
					...
					<td>
						<select name="type[{{$item.bsn_id}}]" Onchange="update_type({{$item.bsn_id}},this.value)">
							...
							<option value="{{$item6.wt_id}}" {{if $item6.wt_id eq $item.bsn_wlr_type}}
								selected="selected" {{/if}}>{{$item6.wt_type}}</option>
							{{/foreach}}
						</select>
					...
					<td check="{{$item.bsn_addto_rer}}">
						<form name="recmetT" method="post" action="">
							<input type="hidden" name="realestate_report[{{$item.bsn_id}}]" value="0" />
							<label class="switch">
								<input class="switch-input" type="checkbox" name="realestate_report[{{$item.bsn_id}}]"
									onclick="update_addto_real_estate_report({{$item.bsn_id}},this.checked)" {{if
									$item.bsn_addto_rer eq 1 }} checked="checked" {{/if}} />
								<span class="switch-label" data-on="Yes" data-off="No"></span>
								<span class="switch-handle"></span> <br />
							</label>
						</form>
						{{if $item.bsn_addto_rer eq 1}}
							{{$item.bsn_addto_rer_user}}<br />{{$item.bsn_addto_rer_date|date_format:"%d-%m-%Y"}}
						{{/if}}
						<script>
							function update_addto_real_estate_report(id, value) {
								...
								$.ajax({
									type: "GET",
									url: "{{$BASE_URL}}watch_list_report.update_addto_real_estate_report/bsn_id/" + id + "/value/" + value,
									success: function (result) {
										location.reload();
									}
								});
					...
					<td>{{ if $item.checklist485}}
						...
						<a href="/business.download_content?file_name={{$item.doc_485}}&module_name=business.home"
							target="_blank">Download</a><br />{{$item.checklist485}} {{/if}}
					...
					<td>
						<form name="recmetT" method="post" action="">
							...
							<label class="switch">
								<input class="switch-input" type="checkbox" name="docsign_sent[{{$item.bsn_id}}]"
									onclick="update_doc_sent({{$item.bsn_id}},this.checked)" {{if
									$item.bsn_wlr_docsign_sent eq 1 }} checked="checked" {{/if}} />
								<span class="switch-label" data-on="Yes" data-off="No"></span>
								<span class="switch-handle"></span> <br />
							</label>
						</form>
						{{if $item.bsn_wlr_docsign_sent eq 1}}
						{{$item.bsn_wlr_docsign_date}}<br />{{$item.bsn_wlr_docsign_user}} {{/if}}
					...
					<td>
						<form name="pbd" method="post" action="">
							<input type="text" name="bs_wlr_latest_css[{{$item.bsn_id}}]"
								class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo2_{{$item.bsn_id}}"
								value="{{$item.bs_wlr_latest_css}}" />
							<input type="submit" value="Set" name="hia" class="set" /> <br />
							{{if $item.bs_wlr_latest_css}}{{$item.bs_wlr_latest_css_user}} {{/if}}
						</form>
					</td>
          ...
          {{if $item.bsn_wlr_sunday_appt eq 1}} {{$item.bsn_wlr_sunappt_user}}<br />{{$item.bsn_wlr_sunappt_date}} {{/if}}
					...
					<td>
						<form name="recmetT2" method="post" action="">
							...
							<label class="switch">
								<input class="switch-input" type="checkbox" name="sunbooked[{{$item.bsn_id}}]" value="1"
									onclick="this.form.submit();" {{if $item.bsn_wlr_sunbook eq 1 }} checked="checked"
									{{/if}} />
								<span class="switch-label" data-on="Yes" data-off="No"></span>
								<span class="switch-handle"></span> <br />
							</label>
						</form>
						{{if $item.bsn_wlr_sunbook eq 1}}
						{{$item.bsn_wlr_sunbook_user}}<br />{{$item.bsn_wlr_sunbook_date}} {{/if}}
					...
					<td>
						<form name="recmet32" method="post" action="">
							...
							<label class="switch">
								<input class="switch-input" type="checkbox" name="couriered[{{$item.bsn_id}}]" value="1"
									onclick="this.form.submit();" {{if $item.bs_wlr_par_couriered eq 1 }}
									checked="checked" {{/if}} />
								<span class="switch-label" data-on="Yes" data-off="No"></span>
								<span class="switch-handle"></span> <br />
							</label>
						</form>
						{{if $item.bs_wlr_par_couriered eq 1}}
						{{$item.bs_wlr_par_cour_date}}<br />{{$item.bs_wlr_par_cour_user}} {{/if}}
					</td>
...
-->
						<a
							href="/watch_list_report.download_content?file_name={{$item.doc_614}}&module_name=watch_list_report.home">Download</a>
					...
					<td><input type="button" name="email" value="Email"
							onclick='javascript:sendemail("{{$item.bcust_misc_email1}}","{{$item.bcust_fname}}","{{$item.bsn_address}}");'
							style="width:54px; height:20px; font-size:11px; line-height:17px !important; padding:0px !important" />
					</td>
					<td> <!--<input type="checkbox" name="watch[{{$item.bsn_id}}]" value="2" />-->
						...
						<!--<form name="recmetT" method="post" action="">
							...
							<label class="switch">
								<input class="switch-input" type="checkbox" name="watch[{{$item.bsn_id}}]" value="2"
									onclick="this.form.submit();" {{if $item.bsn_watch_list eq 1 }} checked="checked"
									{{/if}} />
								<span class="switch-label" data-on="Yes" data-off="No"></span>
								<span class="switch-handle"></span> <br />
							</label>
						...
						<form name="recmetH" method="post" action="">
                            <input type="hidden" name="hide[{{$item.bsn_id}}]" value="0">
                            <label class="switch">
                                <input class="switch-input" type="checkbox" name="hide[{{$item.bsn_id}}]" value="1"
                                    onclick="this.form.submit();" {{if $item.bsn_watch_list_report_hidden eq 1 }} checked="checked" {{/if}}>
                                <span class="switch-label" data-on="Yes" data-off="No"></span>
                                <span class="switch-handle"></span> <br>
