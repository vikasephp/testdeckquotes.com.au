    ...
    <table id="list-table" class="nav-back" width="100%" >
        <thead>
        	...
        	<th class="topmenu" align="center" valign="middle" width="3%">Active</th>
                <th class="topmenu" align="center" valign="middle" width="3%">No Price</th>
                ...
                <th class="topmenu" align="center" valign="middle" width="5%">New Site</th>
                <th class="topmenu" align="center" valign="middle" width="5%">New Planning</th>
                <th class="topmenu" align="center" valign="middle" width="5%">New Total</th>
...
<input class="switch-input" type="checkbox"  name="online[{{$item.di_id}}]" value="1"  onclick="this.form.submit();" {{if $item.di_online eq 1 }} checked="checked" {{/if}} />
      ...
      <form name ="recmet10" method="post" action="">
     	<input type="hidden" name="noprice[{{$item.di_id}}]" value="0" />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="noprice[{{$item.di_id}}]" value="1"  onclick="this.form.submit();" {{if $item.di_no_price eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form> 
       {{if $item.di_no_price eq 1 }} {{$item.di_no_price_user}}<br> {{$item.di_no_price_date}}{{/if}} 
     </td>
    ...
    <td>
     ...
     $ <input type="text" name="new_build[{{$item.di_id}}]"  value="{{$item.di_new_build}}" id="nbc{{$item.di_id}}" style="width:90px" onkeyup = "update_new_build({{$item.di_id}},this.value)"  /> 
     </form> 
     
    <div id="nb{{$item.di_id}}">{{if $item.di_new_build}}{{$item.di_new_build_date}}<br />{{$item.di_new_build_user}}{{/if}}</div>
     <script>
    ...
    <td>
    <form name ="recmet" method="post" action="">
     $ <input type="text" name="ns[{{$item.di_id}}]"  value="{{$item.di_new_site}}" id="nbc{{$item.di_id}}" style="width:90px" onkeyup = "update_new_site({{$item.di_id}},this.value)"  /> 
     </form> 
     
    <div id="ns{{$item.di_id}}">{{if $item.di_new_site}}{{$item.di_new_site_date}}<br />{{$item.di_new_site_user}}{{/if}}</div>
     <script>
       
         function update_new_site(id,value)
	 {
		var tana15 = "#ns"+id;
		
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_pricing_update_report.update_new_site/di_id/"+id+"/value/"+value,
				   success: function(result){
					 $(tana15).html(result);  
			   }
			 });
    ...
    <td>
    <form name ="recmet" method="post" action="">
     $ <input type="text" name="np[{{$item.di_id}}]"  value="{{$item.di_new_planning}}" id="npa{{$item.di_id}}" style="width:90px" onkeyup = "update_new_planning({{$item.di_id}},this.value)"  /> 
     </form> 
     
    <div id="np{{$item.di_id}}">{{if $item.di_new_planning}}{{$item.di_new_planning_user}}<br />{{$item.di_new_planning_date}}{{/if}}</div>
     <script>
       
         function update_new_planning(id,value)
	 {
		var tana16 = "#np"+id;
		
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_pricing_update_report.update_new_planning/di_id/"+id+"/value/"+value,
				   success: function(result){
					 $(tana16).html(result);  
			   }
			 });
    ...
    <form name ="recmet2" method="post" action="">
     $ <input type="text" name="nt[{{$item.di_id}}]"  value="{{$item.di_new_total}}" id="nta{{$item.di_id}}" style="width:90px" onkeyup = "update_new_total({{$item.di_id}},this.value)"  /> 
     </form> 
     
    <div id="nt{{$item.di_id}}">{{if $item.di_new_total}}{{$item.di_new_total_user}}<br />{{$item.di_new_total_date}}{{/if}}</div>
     <script>
       
         function update_new_total(id,value)
	 {
		var tana17 = "#nt"+id;
		
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_pricing_update_report.update_new_total/di_id/"+id+"/value/"+value,
				   success: function(result){
					 $(tana17).html(result);  
			   }
			 });
