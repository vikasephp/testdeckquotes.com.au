...
<form name="mlist" method="post" enctype="multipart/form-data">
    <div style="float:left; text-align:left;">
		...
		<div>
			<strong> &nbsp;&nbsp;Search by Who is Managing: {{$managing}}</strong>
			<select name="search_by_who_is_managing">
				...
				{{ foreach from=$managedata key="index" item="row" }}
					<option value="{{ $row.$option_id }}" {{ if $row.$option_id eq $managing }} selected="selected" {{/if}}>{{ $row.$option_col }}</option>
				{{ /foreach }}
			</select>
    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
				...
				<th class="topmenu" align="center" valign="middle" data-col="outstanding_debt">Outstanding Debt</th>
                <th class="topmenu" align="center" valign="middle" data-col="outstanding_debt">Stop Statements</th>
				...
				<th class="topmenu" align="center" valign="middle" data-col="traffic_light">Traffic Light</th>
                ...
                <th class="topmenu" align="center" valign="middle" data-col="payment_date">Payment Date</th>
                <th class="topmenu" align="center" valign="middle" data-col="amount_paid">Amount Paid</th>
                <th class="topmenu" align="center" valign="middle" data-col="payment_status">Payment Status</th><th class="topmenu" align="center" valign="middle" data-col="action">Action</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr bgcolor="{{ cycle values=" #D3E8D6,WHITE" advance=true }}">
                ...
                <td data-col="dispute_date" style="width: 105px;">
					...
					{{if $item.ldd_dispute_date != ''}}
					{{$item.ldd_dispute_date_updated_by}}
					<br/>{{$item.ldd_dispute_date_updated_at|date_format:"%d-%m-%Y"}}
					{{/if}}
                </td>
                <td data-col="number_of_claims">
					...
					{{if $item.ldd_claim_id neq 0}}
					{{$item.ldd_claim_id_by}}
					<br/>{{$item.ldd_claim_id_date|date_format:"%d-%m-%Y"}}
					{{/if}}
                </td>
				...
				<td data-col="outstanding_debt">{{$item.outdebt}}</td>
                
                <td> 
                
                <form name ="recmet3" method="post" action="">
                    <input type="hidden" name="stop[{{$item.ldd_id }}]" value="0" />
                <label class="switch">
                <input class="switch-input" type="checkbox"  name="stop[{{$item.ldd_id }}]" value="1"  onclick="this.form.submit();" {{if $item.ldd_stop_stm eq 1 }} checked="checked" {{/if}} />
                    <span class="switch-label" data-on="Yes" data-off="No"></span> 
                    <span class="switch-handle"></span> <br />
                   </label> 
                 </form>  <br />
                 {{if $item.ldd_stop_stm eq 1}} {{$item.ldd_stop_stm_user}}<br />{{$item.ldd_stop_stm_date}} {{/if}}
                ...
                <!--<td data-col="evidence_brief">
                    {{ if $item.ldd_evidence_brief_link }}
                        <p style="margin-bottom: 10px;"><a href="{{ $item.ldd_evidence_brief_link }}" target="_blank">View</a></p>
                    {{ /if }}
                    <p>
                        <a href="{{$BASE_URL}}{{$BASEFOLDER}}.update_evidence_brief/{{$ID}}/{{$item.$ID}}" class="various">Edit</a>
                    </p>
                </td>-->
				<td data-col="traffic_light" style="font-weight:600;background:{{$item.ldd_traffic_color}};">
					<a href="{{$BASE_URL}}{{$BASEFOLDER}}.colorbox/{{$ID}}/{{$item.$ID}}"
								class="various">Color</a>
					<br /><br />{{$item.ldd_traffic_color_updated_by}}
					<br />{{$item.ldd_traffic_color_updated_date|date_format:"%d-%m-%Y"}}<br /><br />
					<a href="{{$BASE_URL}}{{$BASEFOLDER}}.read_reason/{{$ID}}/{{$item.$ID}}"
                ...
                <td data-col="who_is_managing">
					...
					{{$item.ldd_manage_id_by}}<br/>
					{{$item.ldd_manage_id_date}}
                </td>
                <td data-col="severity">
					...
					{{$item.ldd_severity_id_by}}<br/>
					{{$item.ldd_severity_id_date}}
                </td>
                <td data-col="status">
					...
					{{$item.ldd_status_id_by}}<br/>
					{{$item.ldd_status_id_date}}
                </td>
				<td data-col="note">
                ...
                <td data-col="payment_date" style="width: 120px;">
                    <form name="payment_date_form" method="post" action="">
                        <input type="text" name="ldd_payment_date[{{$item.$ID}}]" class="w16em dateformat-d-ds-m-ds-Y dtpic" 
                               id="payment_date_{{ $item.$ID }}" value="{{ $item.ldd_payment_date|date_format:'%d-%m-%Y' }}" />
                        <input type="submit" value="Set" name="set_ldd_payment_date" class="set" />
                    </form>
					{{if $item.ldd_payment_date != ''}}
					{{$item.ldd_payment_date_updated_by}}
					<br/>{{$item.ldd_payment_date_updated_at|date_format:'%d-%m-%Y'}}
					{{/if}}
                </td>
                <td data-col="amount_paid" style="width: 100px;">
                    <div class="input-group">
                        <!-- <span class="input-group-text">$</span> -->
                        <input type="text" class="form-control" onkeyup="updateRecord('{{ $item.$ID }}', 'ldd_amount_paid', this.value)" 
                               value="{{ $item.ldd_amount_paid }}" />
                    </div>
                </td>
                <td data-col="payment_status">
                    <select name="{{$TABLE}}[ldd_payment_status]" onchange="updateRecord('{{ $item.$ID }}', 'ldd_payment_status', this.value)">
                        <option value="">Select Status</option>
                        <option value="Not Paid" {{ if $item.ldd_payment_status eq 'Not Paid' }} selected="selected" {{/if}}>Not Paid</option>
                        <option value="Partially Paid" {{ if $item.ldd_payment_status eq 'Partially Paid' }} selected="selected" {{/if}}>Partially Paid</option>
                        <option value="Fully Paid" {{ if $item.ldd_payment_status eq 'Fully Paid' }} selected="selected" {{/if}}>Fully Paid</option>
                    </select>
					{{if $item.ldd_payment_status eq 'Not Paid' or $item.ldd_payment_status eq 'Partially Paid' or $item.ldd_payment_status eq 'Fully Paid' }}
						{{$item.ldd_payment_status_updated_by}}					
					<br/>{{$item.ldd_payment_status_updated_at|date_format:"%d-%m-%Y"}}
					{{/if}}
                </td>
                <td data-col="action">
