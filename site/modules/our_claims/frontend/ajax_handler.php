<?php
    ...
    if (!empty($postedData['action']) &&  $postedData['action'] == 'updateRecord') {
        ...
        if (!empty($primaryID)) {
			...
			if ($column === 'ldd_claim_id') {
				$update_data['ldd_claim_id_by'] = $user;
				$update_data['ldd_claim_id_date'] = $dt;
			}
			
			if ($column === 'ldd_payment_status') {
				$update_data['ldd_payment_status_updated_by'] = $user;
				$update_data['ldd_payment_status_updated_at'] = $dt;
			}
