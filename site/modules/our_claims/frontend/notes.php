<?php
$fwMainView = 'file:' . getcwd() . '/notes.tpl';
$table = new Fw_Db_Table($TABLE);
$add_notes = $fwRequest->getParam('add_notes', '');
$dm_id = $fwRequest->getParam($ID, 0);
$fwViewData['dm_id'] = $dm_id;
if ($add_notes) {
    $dm_id = $fwRequest->getParam($ID, '');
    $ldd_where_are_at = $fwRequest->getParam('ldd_where_are_at', '');
    $table->setWhere($ID . ' = ' . $dm_id);
    if ($table->rowExists()) {
        $notes['ldd_where_are_at'] = $ldd_where_are_at;
        $detail = $table->updateRow($notes);
    }
    $fwViewData['opr'] = "Close";
}
$sqlnotes = "SELECT ldd_where_are_at FROM $TABLE where $ID = " . $dm_id;
$notesData = $fwDb->queryOne($sqlnotes);
$fwViewData['ldd_where_are_at'] = $notesData['ldd_where_are_at'];
