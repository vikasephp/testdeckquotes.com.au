...
<h3 class="page-title">Designs Report - INACTIVE</h3>
<br />
...
<form name="mlist" method="post">
	<div style="float:right;">
		...
		<input type="button" value="Export To XLS"
			onclick="document.location.href='{{$BASE_URL}}design_report.home_inactive/export/1'" /> &nbsp;
			...
			<table id="doclist-table" class="nav-back" width="100%">
				<thead>
					<tr>
						...
						<th class="topmenu" align="center" valign="middle" width="4%">Project Address <br />Same
							As<br /> Customer Address</th>
						...
						<th class="topmenu" align="center" valign="middle" width="12%">Add To Construction Pipeline
							Report</th>
				...
				<tbody>
					...
					<tr {{if $item.dpn_dr_per_complete eq 100}} bgcolor="#99CC99" {{else}} bgcolor="{{cycle values="
						...
						<td> <strong>{{$item.bcust_fname}} &nbsp; {{$item.bcust_lname}}</strong>
							<br />{{$item.bsn_address}}</td>
...
-->
						...
						<td>
							<form name="surdet" method="post" action="">
								Surveyor :<input type="text" name="surveyor[{{$item.chknoData}}]"
									value="{{$item.dpn_dr_surveyor}}" style="width:70px;"
						...
						<td>{{if $item.survey_uploaded}}
							...
							<a href="/design_report.download_content?file_name={{$item.survey_uploaded}}&module_name=design_report.home_inactive"
						...
						<td>
							...
							<a href="/design_report.download_content?file_name={{$item.checklist179_file}}&module_name=design_report.home_inactive"
								target="_blank">Link</a>
							{{/if}}
						</td>
						...
						<td>
							...
							<select name="design_type[{{$item.chknoData}}]"
								...
								<option value="SR" {{if $item.dpn_dr_what eq 'SR' }} selected="selected" {{/if}}> SR
								</option>
								<option value="FPE" {{if $item.dpn_dr_what eq 'FPE' }} selected="selected" {{/if}}> FPE
								</option>
								<option value="Other" {{if $item.dpn_dr_what eq 'Other' }} selected="selected" {{/if}}>
									Other </option>
							</select>
 ...
 --> <input type="text" name="whereare[{{$item.chknoData}}]" value="{{$item.dpn_dr_wherewe}}"
   ...
   --> <input type="text" name="letertext[{{$item.chknoData}}]" value="{{$item.dpn_dr_letter_text}}"
						...
						<td>
							...
							<a href="{{$BASE_URL}}design_report.txtmsg_ec/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}"
								class="various">
								{{if $item.dpn_dr_ebc_comm}}<img src="{{$BASE_URL}}images/textmsg.png" />
								{{else}}<img src="{{$BASE_URL}}images/textmsgred.png" />{{/if}}

							</a>

							<input type="text" name="estimate[{{$item.chknoData}}]"
								value="{{$item.dpn_dr_estimate_cost}}"
						...
						<td>
							...
							<a href="{{$BASE_URL}}design_report.txtmsg_lsv/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}"
								class="various">
								{{if $item.dpn_dr_likey_comm}}<img src="{{$BASE_URL}}images/textmsg.png" />
								{{else}}<img src="{{$BASE_URL}}images/textmsgred.png" />{{/if}}
...
-->
							<input type="text" name="likelyvalue[{{$item.chknoData}}]"
								value="{{$item.dpn_dr_likely_value}}"
						...
						<td>
							...
							<a href="{{$BASE_URL}}design_report.txtmsg_erc/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}"
								class="various">
								{{if $item.dpn_dr_erc_comm}}<img src="{{$BASE_URL}}images/textmsg.png" />
								{{else}}<img src="{{$BASE_URL}}images/textmsgred.png" />{{/if}}
							...
							<input type="text" name="reno_cost[{{$item.chknoData}}]" value="{{$item.dpn_dr_erc}}"
						...
						<td>
							...
							<a href="{{$BASE_URL}}design_report.txtmsg_re/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}"
								class="various">
								{{if $item.dpn_dr_re_comm}}<img src="{{$BASE_URL}}images/textmsg.png" />
								{{else}}<img src="{{$BASE_URL}}images/textmsgred.png" />{{/if}}
						...
						<td>
							...
							<input type="text" name="alerts[{{$item.chknoData}}]" value="{{$item.dpn_dr_alerts}}"
 ...
 --> <input type="text" name="cust_design_hours[{{$item.chknoData}}]" value="{{$item.dpn_dr_cust_des_hour}}"
								onkeyup="update_cdh({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px" />
  ...
  --> <input type="text" name="overseas_hours[{{$item.chknoData}}]" value="{{$item.dpn_dr_overhour}}"
								onkeyup="update_oah({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px" />
  ...
  -->
							<input type="text" name="total_hours[{{$item.chknoData}}]"
								value="{{$item.dpn_dr_total_hours}}"
								onkeyup="update_tot_hours({{$item.chknoData|ltrim:'0'}},this.value)"
								style="width:40px" />
						...
						{{else }}
							...
							<td>
								{{if $item.lmpData.doc_file_name}}
									<a href="/design_report.download_content?file_name={{$item.lmpData.doc_file_name}}&module_name=design_report.home_inactive" target="_blank">Download-{{$item.chknoData}}</a>
								{{/if}}
							</td>
							<td>
								...
								<form name="pw" method="post" enctype="multipart/form-data">
									...
									<div style="text-align:center;">
										...
										<a href="/design_report.download_content_in?file_name={{$item.dpn_dr_latest_letter}}&module_name=design_report.home_inactive"
							...
							<td>
								...
								<input type="hidden" name="hide[{{$item.chknoData}}_{{$item.bs_customers_id}}]"
									value="0" />
								<label class="switch">

									<input class="switch-input" type="checkbox"
										name="hide[{{$item.chknoData}}_{{$item.bs_customers_id}}]" />
							...
							<td>
								<form name="showincpr" method="post" action="">
									...
									<label class="switch">
										<input class="switch-input" type="checkbox" name="incpr[{{$item.bsn_id}}]"
											value="1" onclick="this.form.submit();" {{if $item.bsn_showin_cpr eq 1 }}
											checked="checked" {{/if}} />
										<span class="switch-label" data-on="Yes" data-off="No"></span>
										<span class="switch-handle"></span> <br />
	...
	<tr> {{if $list}}
		<td align="center">
			<div class='pagination'> {{if $pagenum == 1}} <span class='disabled'>� previous</span>
				{{else}} <a class="pagination" href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{math equation=" x -
						y" x=$pagenum y=1}}" title="Previous">� previous</a> {{/if}}

					{{if $pagenum != 1}} <span class="paginate"><a class="pagination"
							href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/1" title="{{$page_num}} Page">1</a></span>
					{{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span class="paginate"><a
								class="pagination" href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/2"
						...
						{{else}}<span class='current'>2</span> {{/if}}
							...
							{{else}} <span class="paginate"><a class="pagination"
										href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$page_num}}"
								...
								{{else}}
									...
									{{else}} <a class="pagination"
											href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$page_num2}}"
										...
										{{else}}
											...
											{{else}} <a class="pagination"
													href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$lastone}}"
												...
												{{else}}
													...
													{{else}} <a class="pagination"
															href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$last}}"
															title="{{$last}} Page">{{$last}}</a> {{/if}}
														{{/if}}
														{{if $pagenum == $last}} <span class='disabled'>next �</span>
														{{else}} <a class="pagination"
																href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{math equation="
