...
.wrapper2{ }
.div1 {width:5490px; height: 20px; }
.div2 {width:5790px;  overflow: auto;}   
...
<input type="text" name="searchkey" value="{{$searchkey}}" style="width:400px;" placeholder="Search By Checklist Number"/>
&nbsp;&nbsp;&nbsp;&nbsp;
...
<span style="display:inline-flex; font-weight:bold; vertical-align:middle;"> &nbsp; &nbsp; &nbsp; &nbsp; Last Audit : &nbsp;
...
<input type="button" value="Export To XLS" onclick="document.location.href='{{$BASE_URL}}design_report.home_active/export/1'" /> &nbsp;
...
<!--<div class="wrapper2">-->
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="7%">General Hours</th>
	...
	function update_rating(id,value)
        ...
        <td> <select name="type[{{$item.chknoData}}]" Onchange = "update_type_new({{$item.chknoData|ltrim:'0'}},this.value)" >
	...
	function update_type_new(id,value)
        ...
        <td><input type="checkbox"  {{if $item.bsn_address_same eq 1}} checked="checked" {{/if}} readonly="readonly" /> </td>
	 ...
	 <input type="text" name="desagr[{{$item.chknoData}}]" value="{{$item.dpn_dr_deagree}}" onkeyup = "update_da_currency({{$item.chknoData|ltrim:'0'}},this.value)" style="width:50px;"/>
       ...
       <option value="" {{if $item.dpn_dr_building_file eq ''}} selected="selected" {{/if}}>  </option>
       <option value="Yes" {{if $item.dpn_dr_building_file eq 'Yes'}} selected="selected" {{/if}}> Yes </option>
       <option value="Ordered" {{if $item.dpn_dr_building_file eq 'Ordered'}} selected="selected" {{/if}}> Ordered </option>
 ...
 <input type="text" name="dpn_dr_surveydt[{{$item.chknoData}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo33_{{$item.chknoData}}" value="{{$item.dpn_dr_surveydt}}" onfocus ="update_two({{$item.chknoData|ltrim:'0'}}, this.value);"  /> 
        ...
        Surveyor :<input type="text" name="surveyor[{{$item.chknoData}}]" value="{{$item.dpn_dr_surveyor|urldecode}}"  style="width:70px;" onkeyup = "update_surveyor({{$item.chknoData|ltrim:'0'}},this.value)"/>
	...
	function update_who(id,value)
      ...
      <a href="/design_report.download_content?file_name={{$item.survey_uploaded}}&module_name=design_report.home_active" target="_blank">Link</a>
      {{/if}}</td>
      <td>  
     ...
     <!-- <a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.checklist179_file}}" target="_blank">Link</a>-->
      <a href="/design_report.download_content?file_name={{$item.checklist179_file}}&module_name=design_report.home_active" target="_blank">Link</a>
      {{/if}}
      </td>
...
<input class="switch-input" type="checkbox"  name="renoonly[{{$item.chknoData}}]" value="1"  onclick="this.form.submit();" {{if $item.dpn_dr_renoonly  eq 1 }} checked="checked" {{/if}} />
       ...
       <option value="DO" {{if $item.dpn_dr_type eq 'DO'}} selected="selected" {{/if}}> DO </option>
      ...
      <td> 
       ...
       <td> {{if $item.checklist53_file}}
        <!--<a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.checklist53_file}}" target="_blank">Download</a>-->
         <a href="/design_report.download_content?file_name={{$item.checklist53_file}}&module_name=design_report.home_active" target="_blank">Download</a>
        <br />
        {{$item.checklist53}}
        {{/if}} 
       </td>
 ...
 <td>  <input type="text" name="whereare[{{$item.chknoData}}]" value="{{$item.dpn_dr_wherewe|urldecode}}" onkeyup = "update_where_are({{$item.chknoData|ltrim:'0'}},this.value)"/>
...
<input class="switch-input" type="checkbox"  name="plan[{{$item.chknoData}}]" value="1"  onclick="this.form.submit();" {{if $item.dpn_dr_plan_attached  eq 1 }} checked="checked" {{/if}} />
        ...
        <td><form name ="recmet10" method="post" action="">
     	<input type="hidden" name="salesletter[{{$item.chknoData}}]" value="0" />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="salesletter[{{$item.chknoData}}]" value="1"  onclick="this.form.submit();" {{if $item.dpn_dr_sales_letter  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form> </td>
...
<input type="text" name="sqm[{{$item.chknoData}}]" value="{{$item.dpn_dr_sqm}}" onkeyup = "update_sqm2({{$item.chknoData|ltrim:'0'}},this.value)" style="width:50px"/>
	...
	function update_sqm2(id,value)
      ...
      <a href="{{$BASE_URL}}design_report.txtmsg_ec/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
       {{if $item.dpn_dr_ebc_comm}}<img src="{{$BASE_URL}}images/textmsgyellow.png" />{{else}}<img src="{{$BASE_URL}}images/textmsgblue.png" />{{/if}}
       
       </a>
      
         <input type="text" name="estimate[{{$item.chknoData}}]" value="{{$item.dpn_dr_estimate_cost|urldecode}}" onkeyup = "update_estimate_cost2({{$item.chknoData|ltrim:'0'}},this.value)" style="width:150px"/>
         
               <input type="text" name="estimate_cmt[{{$item.chknoData}}]" value="{{$item.dpn_dr_ec_cmt|urldecode}}" onkeyup = "update_estimate_cmt({{$item.chknoData|ltrim:'0'}},this.value)" style="width: 120px;"/> <span title="planningapprovalcanberra">www</span> <br />
      {{$item.dpn_dr_ec_date}}
	...
	function update_estimate_cost2(id,value)
        ...
        <td> 
         ...
         {{if $item.dpn_dr_likey_comm}}<img src="{{$BASE_URL}}images/textmsgyellow.png" />{{else}}<img src="{{$BASE_URL}}images/textmsgblue.png" />{{/if}}
          
          </a>
 
 
<input type="text" name="likelyvalue[{{$item.chknoData}}]" value="{{$item.dpn_dr_likely_value|urldecode}}" onkeyup = "update_likely_value2({{$item.chknoData|ltrim:'0'}},this.value)" style="width:150px !important;"/>
      
 <input type="text" name="likely_com[{{$item.chknoData}}]" value="{{$item.dpn_dr_likely_cmt|urldecode}}" onkeyup = "update_likely_cmt({{$item.chknoData|ltrim:'0'}},this.value)" style="width: 120px;"/> <span title="planningapprovalcanberra">www</span> <br />
      {{$item.dpn_dr_likely_date}}
       ...
       <a href="{{$BASE_URL}}design_report.txtmsg_erc/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
          {{if $item.dpn_dr_erc_comm}}<img src="{{$BASE_URL}}images/textmsgyellow.png" />{{else}}<img src="{{$BASE_URL}}images/textmsgblue.png" />{{/if}}
      </a>

      
    <input type ="text" name ="reno_cost[{{$item.chknoData}}]" value="{{$item.dpn_dr_erc|urldecode}}" onkeyup = "update_reno_cost({{$item.chknoData|ltrim:'0'}},this.value)" style="width:150px;" />  
    
    <input type="text" name="reno_com[{{$item.chknoData}}]" value="{{$item.dpn_dr_erc_cmt|urldecode}}" onkeyup = "update_reno_cmt({{$item.chknoData|ltrim:'0'}},this.value)" style="width: 120px;"/> <span title="planningapprovalcanberra">www</span> <br />
  ...
  <a href="{{$BASE_URL}}design_report.txtmsg_re/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
         {{if $item.dpn_dr_re_comm}}<img src="{{$BASE_URL}}images/textmsgyellow.png" />{{else}}<img src="{{$BASE_URL}}images/textmsgblue.png" />{{/if}}
 ...
 <input type="text" name="reno_extn[{{$item.chknoData}}]" value="{{$item.dpn_dr_r_plus_e}}" onkeyup = "update_reno_extn2({{$item.chknoData|ltrim:'0'}},this.value)" style="width:150px"/>
  ...
  <input type="text" name="alerts[{{$item.chknoData}}]" value="{{$item.dpn_dr_alerts|urldecode}}" onkeyup = "update_alerts({{$item.chknoData|ltrim:'0'}},this.value)"/>
 ...
 -->    <input type="text" name="ssod[{{$item.chknoData}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo200_{{$item.chknoData}}" value="{{$item.dpn_dr_tdso_dt}}" onfocus = "update_tar_sign({{$item.chknoData|ltrim:'0'}}, this.value);" /> 
      ...
      <script>
       ...
       <td> 
       <input type="text" name="general_hours[{{$item.chknoData}}]" id="h1{{$item.chknoData|ltrim:'0'}}" value="{{$item.dpn_dr_general_hours}}" onkeyup = "update_gen_hours({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
       <div id="gh{{$item.chknoData|ltrim:'0'}}">{{$item.dpn_dr_gen_hour_date}}<br />{{$item.dpn_dr_gen_hour_user}} </div>
  
       <script>
       
         function update_gen_hours(id,value)
		{
		...
		var tana10 = "#gh"+id;
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report.update_general_hours/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
					 $(tana10).html(result);  
       ...
       <td> 
<input type="text" name="cust_design_hours[{{$item.chknoData}}]" id="h2{{$item.chknoData|ltrim:'0'}}" value="{{$item.dpn_dr_cust_des_hour}}" onkeyup = "update_cdh({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
      
   <div id="usr{{$item.chknoData|ltrim:'0'}}">{{$item.dpn_dr_cdh_date}}<br />{{$item.dpn_dr_cdh_user}} </div>
       <script>
	   ...
	   function update_cdh(id,value)
		...
		var tana = "#usr"+id;
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report.update_cust_design_hours/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
					  $(tana).html(result);
			   }
			 });
 ...
 <input type="text" name="overseas_hours[{{$item.chknoData}}]" id="h3{{$item.chknoData|ltrim:'0'}}" value="{{$item.dpn_dr_overhour}}" onkeyup = "update_oah({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
 <div id="usr2{{$item.chknoData|ltrim:'0'}}">{{$item.dpn_dr_oah_date}}<br />{{$item.dpn_dr_oah_user}} </div>      
       <script>
	   ...
	   function update_oah(id,value)
		...
		var tana2 = "#usr2"+id;	
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report.update_over_archi_hours/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
				    $(tana2).html(result);	   
			   }
			 });
  ...
  -->  
     ...
     <input type="text" name="total_hours[{{$item.chknoData}}]" id="th{{$item.chknoData|ltrim:'0'}}" value="{{$item.total_hours}}" onchange= "update_tot_hours({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
	   ...
	   function update_tot_hours(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report.update_total_hours/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
      ...
      <!-- <a href = "https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.lmp_doc_file_name}}" target="_blank">Download</a>-->  
       <a href = "/design_report.download_content?file_name={{$item.lmp_doc_file_name}}&module_name=design_report.home_active" target="_blank">Download</a>           
       {{/if}}  
       ...
       <td>
        ...
        <input type="text" name="dpn_dr_last_spoken[{{$item.chknoData}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo22_{{$item.chknoData}}" value="{{$item.dpn_dr_last_spoken}}"  /> 
        <input type="submit" value="Set" name="last_spoken" class="set" /> 
        </form> 
             {{if $item.dpn_dr_last_spoken}} {{$item.dpn_dr_last_spoken_date}} <br /> {{$item.dpn_dr_last_spoken_user}} {{/if}}      
    
       </td>
         <td>
          <input type="text" name="total_hours[{{$item.chknoData}}]" id="th{{$item.chknoData|ltrim:'0'}}" value="{{$item.dpn_dr_plan_revno}}" onchange= "update_plan({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
     ...
     <a href="/design_report.download_content_up?file_name={{$item.dpn_dr_latest_letter}}&module_name=design_report.home_active" target="_blank">Download</a>
       
        <a href="{{$BASE_URL}}design_report.delete_w/dpn_unique_id/{{$item.chknoData}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img src="{{$BASE_URL}}/images/delete_icon.png" width="20" /></a>
       ...
       <form name="pw" method="post" enctype="multipart/form-data"> 
        ...
        <a href="/design_report.download_content_up?file_name={{$item.dpn_dr_cust_survey}}&module_name=design_report.home_active" target="_blank">Download</a>
        
       
        <a href="{{$BASE_URL}}design_report.delete_cs/dpn_unique_id/{{$item.chknoData}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img src="{{$BASE_URL}}/images/delete_icon.png" width="20" /></a>
   
        </div> {{/if}}
        <input type= "submit" name="upload_cu_survey" title="Save" value="Upload" class="set2"> &nbsp; 
        </form>
...
<script>
     ...
     <td><input type="text" name="dt"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo452_{{$item.chknoData}}{{$item.bsn_id}}"  onfocus = "update_next_date({{$item.chknoData|ltrim:'0'}}, this.value);"  value="{{$item.dpn_dr_next_date}}" /><br />
     ...
     <input type="text" name="tim" placeholder="Time" value="{{$item.dpn_dr_next_time}}" onkeyup="update_next_time({{$item.chknoData|ltrim:'0'}}, this.value);" /><br />
     <script>
     ...
     <input type="text" name="location"  placeholder="Location" value="{{$item.dpn_dr_next_location}}" onkeyup="update_next_location({{$item.chknoData|ltrim:'0'}}, this.value);" />
     ...
     <td> 
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="utility[{{$item.chknoData}}]" value="1"  onclick="this.form.submit();" {{if $item.dpn_err_add eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
     </td>
       <td>
...
<input class="switch-input" type="checkbox"  name="incpr[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bsn_showin_cpr eq 1 }} checked="checked" {{/if}} />
  ...
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
    <tr> {{if $list}}
      <td align="center"><div class='pagination'> {{if $pagenum == 1}} <span class='disabled'><< previous</span> {{else}} <a class="pagination" href="{{$BASE_URL}}{{$XFA.home_active}}/pagenum/{{math equation="x - y" x=$pagenum y=1}}" title="Previous"><< previous</a> {{/if}}
      
          {{if $pagenum != 1}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_active}}/pagenum/1" title="{{$page_num}} Page">1</a></span> {{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_active}}/pagenum/2" title="{{$page_num}} Page">2</a></span> {{else}}<span class='current'>2</span> {{/if}}
          {{if $paginateprev.0 > 3}}
          ...
          {{/if}}
          ...
          {{elseif $page_num2 == 2}}
          {{else}}
          {{if $page_num2 == $pagenum OR $page_num2 <= 0}} <span class='current'> {{$page_num2}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_active}}/pagenum/{{$page_num2}}" title="{{$page_num2}} Page">{{$page_num2}}</a> {{/if}}
          {{/if}}     
          {{/if}}
          {{/foreach}}
          ...
          {{elseif $lastone == 2}}
          {{else}}
          {{if $pagenum == $lastone }} <span class='current'>{{$lastone}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_active}}/pagenum/{{$lastone}}" title="{{$lastone}} Page">{{$lastone}}</a> {{/if}}
          {{/if}}
          {{if $last == 1}}
          {{elseif $last == 2}}
          {{else}}
          {{if $pagenum == $last}} <span class='current'>{{$last}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_active}}/pagenum/{{$last}}" title="{{$last}} Page">{{$last}}</a> {{/if}}
          {{/if}}
          {{if $pagenum == $last}} <span class='disabled'>next >></span> {{else}} <a class="pagination" href="{{$BASE_URL}}{{$XFA.home_active}}/pagenum/{{math equation="x + y" x=$pagenum y=1}}" title="Next">next >></a></div>
        {{/if}} </td>
      {{else}}
