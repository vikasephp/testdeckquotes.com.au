...
<!--<input type="submit" value="Percentage completed" name="percom"  />-->
...
--> <input type="text" name="desagr[{{$item.chknoData}}]" value="{{$item.dpn_dr_deagree}}" onkeyup = "update_da_currency({{$item.chknoData|ltrim:'0'}},this.value)" style="width:50px;"/>
	   ...
	   function update_da_currency(id,value)
		{
		 ...
		 $.ajax({
              ...
              <td> 
        <select name="bsn_building_file[{{$item.bsn_id}}]" Onchange = "update_building_file({{$item.bsn_id}},this.value)" >
       <option value="Yes" {{if $item.bsn_building_file eq 'Yes'}} selected="selected" {{/if}}> Yes </option>
       <option value="NA" {{if $item.bsn_building_file eq 'NA'}} selected="selected" {{/if}}> NA </option>
           </select>
       ...
       <script>
        ...
        function update_building_file(id,value)
		{
		 ...
		 $.ajax({
					   type: "GET",
					   url: "{{$BASE_URL}}/design_report.update_building_file/bsn_id/"+id+"/value/"+value,
						   success: function(result){
					   }
					 });
	...
	function update_who(id,value)
      ...
      <a href="{{$BASE_URL}}files/document_check_list_files/{{$item.checklist179_file}}" target="_blank">Link</a>{{/if}}
       ...
       <td> 
        ...
        <select name="renovation_report[{{$item.bsn_id}}]" Onchange = "rr_update({{$item.chknoData|ltrim:'0'}},this.value)" >
...
<input class="switch-input" type="checkbox"  name="renoonly[{{$item.chknoData}}]" value="1"  onclick="this.form.submit();" {{if $item.dpn_dr_renoonly  eq 1 }} checked="checked" {{/if}} />
      ...
      <td> 
       ...
       <td> 
       <input type="text" name="bsn_initial_meeting_date[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo1_{{$item.bsn_id}}" value="{{$item.bsn_initial_meeting_date}}" onfocus = "update_fmd({{$item.chknoData|ltrim:'0'}}, this.value);" /> 
	...
	function update_rating(id,value)
		{
		 ...
		 $.ajax({
					   type: "GET",
					   url: "{{$BASE_URL}}design_report.update_rating/bsn_id/"+id+"/value/"+value,
						   success: function(result){
					   }
					 });
...
/*	   function update_likely_value(id,value)
		{
			
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}/design_report.update_likely_value/bsn_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}*/
	   ...
	   function update_alerts(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}/design_report.update_alerts/bsn_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
 ...
 -->   <input type="text" name="cust_design_hours[{{$item.chknoData}}]" value="{{$item.dpn_dr_cust_des_hour}}" onkeyup = "update_cdh({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
  ...
  -->  
     <input type="text" name="total_hours[{{$item.chknoData}}]" value="{{$item.dpn_dr_total_hours}}" onkeyup = "update_tot_hours({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
       ...
       <td> 
        <input type="text" name="bsn_sign_off_sent_date[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo5_{{$item.bsn_id}}" value="{{$item.bsn_sign_off_sent_date}}"  /> 
