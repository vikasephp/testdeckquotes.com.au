...
window.location.href = "{{$BASE_URL}}design_report.viewtype";
</script>
{{/if}}
...
<table id="list-table" width="100%">
    <input type="hidden" name="type[dr_id]" value="{{$detail.dr_id}}" />
   ...
   {{if $error}} <tr><th colspan="2" style="color:#FF0000;">{{$error}}</th></tr>{{/if}}
    ...
    <tr>
     <th>Invoice Type </th>
     <td>
          <input type="text"  name="type[dr_type]" style ="width:700px" value="{{$detail.dr_type}}" />
      </td>
    </tr>
