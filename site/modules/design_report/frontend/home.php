...
//	}
//}
//End Initial meeting date

// Save initial meeting date
$bsn_initial_meeting_date = $fwRequest->getParam('bsn_initial_meeting_date', '');
if(!empty($bsn_initial_meeting_date)) {
	$saveimd = $fwRequest->getParam('bsn_initial_meeting_date', '');
	
	foreach($saveimd as $k=>$v)
	{
		$imdDetail['bsn_initial_meeting_date'] = $v;
		
		$tablebusiness->setWhere('bsn_id = '.$k);
		$detail = $tablebusiness->updateRow($imdDetail);		
	}
}
//End Initial meeting date

// Save Target Design Sign Off
$sign_0ff = $fwRequest->getParam('bsn_target_signoff_date', '');
if(!empty($sign_0ff)) {
	$savesod = $fwRequest->getParam('bsn_target_signoff_date', '');
	foreach($savesod as $k=>$v)
	{
		$sodDetail['bsn_target_signoff_date'] = $v;
		
		$tablebusiness->setWhere('bsn_id = '.$k);
		$detail = $tablebusiness->updateRow($sodDetail);		
	}
}
//End Target Design Sign Off


// Save Sign of sent date
$sign_0ff_sent = $fwRequest->getParam('bsn_sign_off_sent_date', '');
if(!empty($sign_0ff_sent)) {
	$sosd = $fwRequest->getParam('bsn_sign_off_sent_date', '');
	foreach($sosd as $k=>$v)
	{
		$sosdDetail['bsn_sign_off_sent_date'] = $v;
		
		$tablebusiness->setWhere('bsn_id = '.$k);
		$detail = $tablebusiness->updateRow($sosdDetail);		
	}
}
//End Sign of sent date
...
if($ascdsc == 1) { $ad = "ASC"; } elseif($ascdsc == 2) {  $ad = "DESC"; }
	...
	$ord = " Order By business.bsn_per_complete ". $ad;
	...
	elseif($_SESSION['ord'] && $pagenum > 0):
		...
		$ord = " Order By business.bsn_per_complete ". $_SESSION['ad'] ;
...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
	...
	bus_customers.bcust_misc_home,  business.bsn_id, business.bsn_name,  business.bsn_sub_status,
	document_check_list.doc_date_uploaded, business.bsn_status_sys_date, business.bsn_address, 
	business.bsn_type, business.bsn_per_complete, business.bsn_address_same, business.bsn_rating,
	business.bsn_initial_meeting_date, business.bsn_letter_text, business.bsn_target_signoff_date, 
	business.bsn_likely_value,  business.bsn_reno_report, business_sellers.bs_dr_survey_date,
	business.bsn_dr_hide, business.bsn_design_agreement, business.bsn_esti_reno_cost,
	business.bsn_where_are_we, business.bsn_alerts, business.bsn_dr_sqm, business_sellers.bs_dr_survey,
	business.bsn_cust_designer_hours, business.bsn_over_archi_hours, business.bsn_total_hours,
	business.bsn_sign_off_sent_date, business.bsn_estimate_cost, business.bsn_building_file,
        business.bsn_showin_cpr, business.bsn_dr_reno_extn
	...
	document_check_list.doc_name_id = 45 " . $where. ' and document_check_list.doc_date_uploaded != "0000-00-00 00:00:00"
...
foreach($setdata2 as $k2 => $v)
{
		...
		$hidedata = $fwDb->queryOne($sqlhide);
				...
				$Statusdate = changedate_y_m_d($v['doc_date_uploaded']);
		...
		if($showall == 0) 
		{
			  $sqlcn = "Select  dpn_bsn_id, dpn_unique_id, dpn_budget, dpn_dr_type, dpn_dr_what, dpn_dr_survey_imple,
			  ...
			  dpn_dr_hide, dpn_dr_latest_letter, dpn_dr_sqm, dpn_dr_last_audit, dpn_dr_last_audit_user,
			  dpn_dr_erc, dpn_dr_r_plus_e, dpn_dr_deagree, dpn_dr_wherewe, dpn_dr_cust_des_hour, dpn_dr_overhour,
			  dpn_dr_ebc_comm, dpn_dr_erc_comm, dpn_dr_likey_comm, dpn_dr_re_comm, dpn_dr_tdso_dt, dpn_dr_survey,
			  dpn_dr_surveydt, dpn_dr_total_hours
			  from document_proposal_name 
			  where  dpn_bsn_id = ". $v['bsn_id'] . ' and dpn_dr_hide = 0';
		} else {
		...
		$sqlcn_new = "Select  dpn_dr_first_meeting_date ,  dpn_unique_id from document_proposal_name 
			      where  dpn_bsn_id = ". $v['bsn_id'];
		$chknoData_new = $fwDb->query($sqlcn_new);
		...
		$sql_lcl = "Select document_check_list.doc_file_name from document_check_list
			   where document_check_list.doc_bsn_id = ". $v['bsn_id'] . " And document_check_list.doc_name_id = 458 ";
		...
		$sql_cwg = "Select document_check_list.doc_file_name, document_check_list.doc_date_uploaded from document_check_list
			    where document_check_list.doc_bsn_id = ". $v['bsn_id'] . " And document_check_list.doc_name_id = 180 ";
		...
		$sql_rcn = "Select document_renovation_name.dpn_unique_id from document_renovation_name
			    where document_renovation_name.dpn_bsn_id = ". $v['bsn_id'] ;
		$data_rcn = $fwDb->queryOne($sql_rcn);
				
		// Renovation Checklist Number End


		//if(!empty($des) &&  $dataDesig['prd_name'] == $des)
		//{
			//if($totchkno == 0)
//			{
//	
...
//				$setdata2[$k]['bsn_initial_meeting_date'] = $v['bsn_initial_meeting_date'];
//				$setdata2[$k]['bsn_target_signoff_date'] = $v['bsn_target_signoff_date'];
//				$setdata2[$k]['bsn_letter_text'] = $v['bsn_letter_text'];
//				$setdata2[$k]['bsn_sitescope_status'] = $v['bsn_sitescope_status'];
//				$setdata2[$k]['dpn_dr_what'] = '';
//				$setdata2[$k]['bsn_likely_value'] = $v['bsn_likely_value'];
//				$setdata2[$k]['bsn_hb_grant'] = $v['bsn_hb_grant'];
//				
//				$setdata2[$k]['reno_checklist_no'] = $data_rcn['dpn_unique_id'];
//				
//				$setdata2[$k]['dpn_dr_traffic_light'] = '';
//				$setdata2[$k]['dpn_dr_tl_user'] = '';
//				$setdata2[$k]['dpn_dr_tl_date'] = '';
//				$setdata2[$k]['dpn_dr_tl_reason'] = '';
//				$setdata2[$k]['dpn_dr_renoonly'] = '';
//				$setdata2[$k]['dpn_dr_scan'] = '';
//				$setdata2[$k]['dpn_dr_plan_attached'] = '';
//				
//				$setdata2[$k]['dpn_dr_first_meeting_date'] = $chknoData_new['dpn_dr_first_meeting_date'];
//				$setdata2[$k]['chknoData_new'] = $chknoData_new['dpn_unique_id'];
//				
//				
//				$setdata2[$k]['bsn_address_same'] = $v['bsn_address_same'];
//				$setdata2[$k]['link'] = $link;
//				$setdata2[$k]['bsn_design_agreement'] = $v['bsn_design_agreement'];
//				$setdata2[$k]['checklist53'] =  $cpData;
//				$setdata2[$k]['bsn_per_complete'] = $v['bsn_per_complete'];
//				$setdata2[$k]['bsn_where_are_we'] = $v['bsn_where_are_we'];
//				$setdata2[$k]['bsn_alerts'] = $v['bsn_alerts'];
//				$setdata2[$k]['bsn_site_scope_proposal'] = $v['bsn_site_scope_proposal'];
//				$setdata2[$k]['bsn_cust_designer_hours'] = $v['bsn_cust_designer_hours'];
//				$setdata2[$k]['bsn_over_archi_hours'] = $v['bsn_over_archi_hours'];
//				$setdata2[$k]['bsn_total_hours'] = $v['bsn_total_hours'];
//				$setdata2[$k]['bsn_sign_off_sent_date'] = $v['bsn_sign_off_sent_date'];
//				$setdata2[$k]['bsn_estimate_cost'] = $v['bsn_estimate_cost'];
//				$setdata2[$k]['bsn_site_scope_proposal'] = '';
//				
//				$setdata2[$k]['bsn_dr_reno_extn'] = $v['bsn_dr_reno_extn'];
//			
//				$setdata2[$k]['bsn_building_file'] = $v['bsn_building_file'];
//				$setdata2[$k]['bsn_esti_reno_cost'] = $v['bsn_esti_reno_cost'];
...
//	                        $setdata2[$k]['checklist45'] =  changedate_d_m_y($v['doc_date_uploaded']);
...
//			        $setdata2[$k]['dpn_dr_type'] = '';
//				$setdata2[$k]['dayscount'] = $dayscount;
//				$setdata2[$k]['bsn_dr_hide'] = $v['bsn_dr_hide'];
//				$setdata2[$k]['bsn_dr_sqm'] = $v['bsn_dr_sqm'];
//				$setdata2[$k]['bsn_showin_cpr'] = $v['bsn_showin_cpr'];
//				$setdata2[$k]['bs_dr_survey'] = $v['bs_dr_survey'];
//				$setdata2[$k]['bs_dr_survey_date'] = $v['bs_dr_survey_date'];
...
//			{
				...
				$setdata2[$k]['bsn_letter_text'] = $v['bsn_letter_text'];
				$setdata2[$k]['bsn_sitescope_status'] = $v['bsn_sitescope_status'];
				$setdata2[$k]['dpn_dr_what'] = $tv['dpn_dr_what'];
				$setdata2[$k]['bsn_likely_value'] = $v['bsn_likely_value'];
				$setdata2[$k]['bsn_hb_grant'] = $v['bsn_hb_grant'];
				$setdata2[$k]['bsn_reno_report'] = $v['bsn_reno_report'];
				$setdata2[$k]['bsn_address_same'] = $v['bsn_address_same'];
				
				$setdata2[$k]['reno_checklist_no'] = $data_rcn['dpn_unique_id'];
				
				$setdata2[$k]['dpn_dr_traffic_light'] = $tv['dpn_dr_traffic_light'];
				$setdata2[$k]['dpn_dr_tl_user'] = $tv['dpn_dr_tl_user'];
				$setdata2[$k]['dpn_dr_tl_date'] = $tv['dpn_dr_tl_date'];
				$setdata2[$k]['dpn_dr_tl_reason'] = $tv['dpn_dr_tl_reason'];
				$setdata2[$k]['dpn_dr_first_meeting_date'] = $chknoData_new['dpn_dr_first_meeting_date'];
				$setdata2[$k]['dpn_dr_estimate_cost'] = $tv['dpn_dr_estimate_cost'];
				$setdata2[$k]['dpn_dr_likely_value'] = $tv['dpn_dr_likely_value'];
				...
				$setdata2[$k]['bsn_building_file'] = $v['bsn_building_file'];
                                $setdata2[$k]['bsn_esti_reno_cost'] = $v['bsn_esti_reno_cost'];
                                $setdata2[$k]['dpn_dr_per_complete'] = $tv['dpn_dr_per_complete'];
				$setdata2[$k]['dpn_dr_letter_text'] = $tv['dpn_dr_letter_text'];
				$setdata2[$k]['dpn_dr_rr'] = $tv['dpn_dr_rr'];
			        $setdata2[$k]['chknoData'] = $tv['dpn_unique_id'];
				$setdata2[$k]['chknoData_new'] = $chknoData_new['dpn_unique_id'];
				$setdata2[$k]['dpn_dr_renoonly'] = $tv['dpn_dr_renoonly'];
				
				$setdata2[$k]['dpn_dr_scan'] = $tv['dpn_dr_scan'];
				$setdata2[$k]['dpn_dr_plan_attached'] = $tv['dpn_dr_plan_attached'];
				
				$setdata2[$k]['dpn_budget'] = $tv['dpn_budget'];
			        $setdata2[$k]['dpn_dr_survey_imple'] = $tv['dpn_dr_survey_imple'];
				$setdata2[$k]['dpn_dr_survey_imp_user'] = $tv['dpn_dr_survey_imp_user'];
				$setdata2[$k]['dpn_dr_survey_date'] = $tv['dpn_dr_survey_date'];
				$setdata2[$k]['dpn_dr_cust_budget'] = $tv['dpn_dr_cust_budget'];
				$setdata2[$k]['dpn_dr_budg_user'] = $tv['dpn_dr_budg_user'];
				$setdata2[$k]['dpn_dr_budg_date'] = $tv['dpn_dr_budg_date'];
				$setdata2[$k]['bcust_fname'] = $v['bcust_fname'];
				$setdata2[$k]['bcust_lname'] = $v['bcust_lname'];
				$setdata2[$k]['bcust_misc_email1'] = $v['bcust_misc_email1'];
				$setdata2[$k]['bcust_misc_email2'] = $v['bcust_misc_email2'];
				...
				$setdata2[$k]['prochklist'] = $datapc['dcnt_counter'];
				$setdata2[$k]['lmpData'] = $mpData;
			        $setdata2[$k]['dpn_dr_type'] = $tv['dpn_dr_type'];
				$setdata2[$k]['dayscount'] = $dayscount;
				$setdata2[$k]['bsn_dr_hide'] = $v['bsn_dr_hide'];
				$setdata2[$k]['bsn_dr_sqm'] = $v['bsn_dr_sqm'];
				$setdata2[$k]['bsn_showin_cpr'] = $v['bsn_showin_cpr'];
				...
				$setdata2[$k]['tasklist303'] =  changedate_d_m_y($data303['bt_completed_date']);
				$setdata2[$k]['checklist458'] = $data_lcl['doc_file_name'];
				$setdata2[$k]['dpn_dr_surveyor'] = $tv['dpn_dr_surveyor'];
				$setdata2[$k]['dpn_dr_sur_comp_date'] = $tv['dpn_dr_sur_comp_date'];
				$setdata2[$k]['dpn_dr_who_provided'] = $tv['dpn_dr_who_provided'];
				$setdata2[$k]['checklist179_file'] = $data_cwg['doc_file_name'];
				$setdata2[$k]['checklist179_date'] = changedate_d_m_y($data_cwg['bt_completed_date']);
				
				$setdata2[$k]['dpn_dr_hide'] = $tv['dpn_dr_hide'];
				$setdata2[$k]['dpn_dr_latest_letter'] = $tv['dpn_dr_latest_letter'];
				$setdata2[$k]['dpn_dr_sqm'] = $tv['dpn_dr_sqm'];
				$setdata2[$k]['dpn_dr_last_audit'] = $tv['dpn_dr_last_audit'];
				$setdata2[$k]['dpn_dr_last_audit_user'] = $tv['dpn_dr_last_audit_user'];
				$setdata2[$k]['dpn_dr_erc'] = $tv['dpn_dr_erc'];
				$setdata2[$k]['dpn_dr_r_plus_e'] = $tv['dpn_dr_r_plus_e'];
				$setdata2[$k]['dpn_dr_deagree'] = $tv['dpn_dr_deagree'];
				$setdata2[$k]['dpn_dr_wherewe'] = $tv['dpn_dr_wherewe'];
				$setdata2[$k]['dpn_dr_cust_des_hour'] = $tv['dpn_dr_cust_des_hour'];
				$setdata2[$k]['dpn_dr_overhour'] = $tv['dpn_dr_overhour'];
				$setdata2[$k]['dpn_dr_ebc_comm'] = $tv['dpn_dr_ebc_comm'];
				$setdata2[$k]['dpn_dr_erc_comm'] = $tv['dpn_dr_erc_comm'];
				
				$setdata2[$k]['dpn_dr_likey_comm'] = $tv['dpn_dr_likey_comm'];
				$setdata2[$k]['dpn_dr_re_comm'] = $tv['dpn_dr_re_comm'];
				$setdata2[$k]['dpn_dr_tdso_dt'] = $tv['dpn_dr_tdso_dt'];
				$setdata2[$k]['dpn_dr_survey'] = $tv['dpn_dr_survey'];
				$setdata2[$k]['dpn_dr_surveydt'] = $tv['dpn_dr_surveydt'];
				$setdata2[$k]['dpn_dr_total_hours'] = $tv['dpn_dr_total_hours'];
...
foreach($setdata2 as $k=>$v)
{
	...
	$objPHPExcel->getActiveSheet()
			...
			->setCellValue('C'.$row, $v['bsn_design_agreement'])
			->setCellValue('D'.$row, $v['bs_dr_survey'])
			...
			->setCellValue('K'.$row, $v['bsn_initial_meeting_date'])
			...
			->setCellValue('Q'.$row, $v['bsn_where_are_we'])
			->setCellValue('R'.$row, $v['dpn_dr_letter_text'])
		        ->setCellValue('S'.$row, $dt54)
			->setCellValue('T'.$row, $days)
			->setCellValue('U'.$row, $v['bsn_alerts'])
			->setCellValue('V'.$row, $v['bsn_target_signoff_date'])
			
			->setCellValue('W'.$row, $v['bsn_cust_designer_hours'])
			->setCellValue('X'.$row, $v['bsn_over_archi_hours'])
			->setCellValue('Y'.$row, $v['bsn_total_hours'])
			->setCellValue('Z'.$row, $v['bsn_sign_off_sent_date'])
			->setCellValue('AA'.$row, $v['dpn_dr_cust_budget'])
			->setCellValue('AB'.$row, $v['bsn_dr_sqm'])
			->setCellValue('AC'.$row, $v['bsn_estimate_cost'])
			->setCellValue('AD'.$row, $v['bsn_esti_reno_cost'])
			->setCellValue('AE'.$row, $v['bsn_likely_value'])
			
			->setCellValue('AF'.$row, $v['bsn_building_file'])
