...
<input type="button" value="Export To XLS" onclick="document.location.href='{{$BASE_URL}}design_report.home_inactive/export/1'" /> &nbsp;
 ...
 <input type="text" name="dpn_dr_surveydt[{{$item.chknoData}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo33_{{$item.chknoData}}" value="{{$item.dpn_dr_surveydt}}" onfocus ="update_two({{$item.chknoData|ltrim:'0'}}, this.value);"  /> 
	...
	function update_who(id,value)
     ...
     <!-- <a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.survey_uploaded}}" target="_blank">Link</a>-->
      <a href="/design_report.download_content?file_name={{$item.survey_uploaded}}&module_name=design_report.home_inactive" target="_blank">Link</a>
      {{/if}}</td>
      <td>  
      ...
      <!--<a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.checklist179_file}}" target="_blank">Link</a>-->
       <a href="/design_report.download_content?file_name={{$item.checklist179_file}}&module_name=design_report.home_inactive" target="_blank">Link</a>
      {{/if}}
      </td>
 ...
 -->   <input type="text" name="cust_design_hours[{{$item.chknoData}}]" value="{{$item.dpn_dr_cust_des_hour}}" onkeyup = "update_cdh({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
  ...
  -->  
       ...
       {{if $item.lmpData}}
        {{foreach from=$item.lmpData key="key3" item="item3"}}
        		...
        		<!-- <a href = "https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item3.doc_file_name}}" target="_blank">-->
                 <a href="/design_report.download_content?file_name={{$item3.doc_file_name}}&module_name=design_report.home_inactive" target="_blank">Link</a>
    ...
    <!--   <a href = "{{$BASE_URL}}files/document_check_list_files/{{$item.checklist458}}" target="_blank">Download</a>{{/if}} -->
      ...
      <a href="/design_report.download_content_in?file_name={{$item.dpn_dr_latest_letter}}&module_name=design_report.home_inactive" target="_blank">Download</a>
...
<input class="switch-input" type="checkbox"  name="incpr[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bsn_showin_cpr eq 1 }} checked="checked" {{/if}} />
  ...
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
    <tr> {{if $list}}
      <td align="center"><div class='pagination'> {{if $pagenum == 1}} <span class='disabled'>« previous</span> {{else}} <a class="pagination" href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{math equation="x - y" x=$pagenum y=1}}" title="Previous">« previous</a> {{/if}}
          
          {{if $pagenum != 1}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/1" title="{{$page_num}} Page">1</a></span> {{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/2" title="{{$page_num}} Page">2</a></span> {{else}}<span class='current'>2</span> {{/if}}
          {{if $paginateprev.0 > 3}}
          ...
          {{/if}}
          ...
          {{else}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$page_num}}" title="{{$page_num}} Page">{{$page_num}}</a></span> {{/if}}
          {{/foreach}}
          ...
          {{elseif $page_num2 == 2}}
          {{else}}
          {{if $page_num2 == $pagenum OR $page_num2 <= 0}} <span class='current'> {{$page_num2}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$page_num2}}" title="{{$page_num2}} Page">{{$page_num2}}</a> {{/if}}
          {{/if}}     
          {{/if}}
          {{/foreach}}
          ...
          {{elseif $lastone == 2}}
          {{else}}
          {{if $pagenum == $lastone }} <span class='current'>{{$lastone}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$lastone}}" title="{{$lastone}} Page">{{$lastone}}</a> {{/if}}
          {{/if}}
          {{if $last == 1}}
          {{elseif $last == 2}}
          {{else}}
          {{if $pagenum == $last}} <span class='current'>{{$last}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{$last}}" title="{{$last}} Page">{{$last}}</a> {{/if}}
          {{/if}}
          {{if $pagenum == $last}} <span class='disabled'>next »</span> {{else}} <a class="pagination" href="{{$BASE_URL}}{{$XFA.home_inactive}}/pagenum/{{math equation="x + y" x=$pagenum y=1}}" title="Next">next »</a></div>
        {{/if}} </td>
      {{else}}
