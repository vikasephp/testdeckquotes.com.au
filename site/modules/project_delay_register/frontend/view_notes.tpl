...
<div align="center" style="min-height:350px; z-index:9999999;">
	...
	<form name="detail" method="post" action="" enctype="multipart/form-data">
		<div style="float:right; margin-right:10px; background: #09F; color:#FFF !important; padding:6px; margin-bottom:5px;">
			<a href="{{$BASE_URL}}{{$BASEFOLDER}}.add_notes/docqa_id/{{$docqa_id}}" class="various" title="Edit" style="color:#FFF; text-decoration:none; font-size:14px;">Add New Notes</a>
		</div>
		<table id="list-table" width="99%">
			...
			{{if $data}}
				...
				<tr>
					<td>{{$en_count}}</td>
					<td>{{$itemN.doc_notes}}</td>
					<td>
						{{$itemN.user_name}}
					</td>
					<td>{{$itemN.doc_created_at|date_format:"%d-%m-%Y"}}</td>
					<td>
						<a href="{{$BASE_URL}}{{$BASEFOLDER}}.add_notes/doc_id/{{$itemN.doc_id}}/docqa_id/{{$docqa_id}}" class="various" title="Edit"><img style="height:20px; width:20px;" src="{{$BASE_URL}}css/admin/images/edit.png" /></a>&nbsp;&nbsp;
						<a href="{{$BASE_URL}}{{$BASEFOLDER}}.delete_notes/doc_id/{{$itemN.doc_id}}/docqa_id/{{$docqa_id}}"
