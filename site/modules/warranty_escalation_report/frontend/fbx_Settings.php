...
// module names
$MODULE_SINGULAR = 'Warranty Escalation Report';
$MODULE_PLURAL =   'Warranty Escalation Report';
...
$PPTABLE = 'pp_warranty_escalation_report';
$PPID = 'pr_id';
...
$ATTRIBUTES = [
	'action_type' => [
		'name' => 'Action Type',
		'table' => 'warranty_log_escalation_actions',
		'id' => 'wlea_id',
		'option_col' => 'wlea_option',
		'add_file' => 'action_type_add.tpl',
		'add_fle_name' => 'action_type_add',
		'view_file' => 'action_type_view.tpl',
		'view_fle_name' => 'action_type_view',
		'view_url' => BASE_URL . $BASEFOLDER . '.action_type_view',
		'delete_fle_name' => 'action_type_delete',
	]
];
$fwViewData['ATTRIBUTES'] = $ATTRIBUTES;

$BASEFOLDER = 'warranty_escalation_report';
	...
	$zipfile = $filename;
        ...
        try {
            if(move_uploaded_file($tmpname, $path .'/'.  basename($zipfile))) {  
				...
				if ($s3->putObjectFile($path . "/" . $zipfile, BUCKET_NAME, 'files/warranty_escalation_attachment/'.$zipfile, S3::ACL_PRIVATE)) {
...
function get_image_raw_data($zipfile)
{
    	...
    	$file_data = $s3->getObject($bucket_name,'files/warranty_log/'.$zipfile, $saveTo = false); 
