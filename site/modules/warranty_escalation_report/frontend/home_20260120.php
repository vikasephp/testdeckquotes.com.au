...
$total_escalation_logs = count($setdata2);
...
foreach($setdata2 as $row) {
	$total_escalation_logs_sum += $row['escalation_days_count'];
	if($row['escalation_days_count'] <= 7) {
		$total_last_7_days_escalation_logs++;
	}
	if($row['escalation_days_count'] > 30) {
...
$escalation_data_table = [
	...
	'Average time from when a log was added to escalation to when it was removed' => '',
