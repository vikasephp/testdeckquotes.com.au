...
<script type="text/javascript">
	    ...
	    var geturl= "{{$BASE_URL}}{{$XFA.doclist}}?{{$ID2}}="; // The server-side script
		var getdelurl= "{{$BASE_URL}}{{$XFA.docdelete}}?{{$ID1}}="; // The server-side script
...
<h3 class="page-title">{{$title}}</h3>
 ...
 <input type="hidden" name="{{$TABLE}}[{{$ID}}]" value="{{$detail.$ID}}" id="dh_id"/>
     <table id="viewdoc-table" width="100%" cellpadding="0" cellspacing="0">
        <tr>
            <th>Document Name:</th>
            <td> <input type="text" name="{{$TABLE}}[bp_title]" value="{{$detail.bp_title}}" style="width:500px;"/></td>
        </tr>
        <tr>
            <th>Document Summary :</th>
            <td><textarea rows="12" cols="60" name="{{$TABLE}}[bp_summery]" class="mceEditor">{{$detail.bp_summery}}</textarea> </td>
        </tr>   
        
        <tr>
        <th> Effective Date Legislation </th>
		<td> <input type="text" name="{{$TABLE}}[bp_effective_date]" value="{{$detail.bp_effective_date}}" size="35" class="w16em dateformat-d-ds-m-ds-Y"/> </td>
        </tr>
        
       <tr>
        <th>Link </th>
		<td> <input type="text" name="{{$TABLE}}[bp_link]" value="{{$detail.bp_link}}" size="120" /> </td>
        </tr>  
        
<!--         <tr>
            <th> Business Stream:</th>
            <td> <input type="text" name="{{$TABLE}}[bp_bus_streem]" value="{{$detail.bp_bus_streem}}" size="35" /></td>
        </tr>
        <tr>
            <th > Access:</th>
            <td> <select name="{{$TABLE}}[bp_access]" class="lrg" style="width:220px;">
            	<option value="1" {{if $detail.bp_access == 1}} selected="selected" {{/if}}> All Users</option>
          		<option value="2" {{if $detail.bp_access == 2}} selected="selected" {{/if}}> Users</option>    	    
  				 </select>
            </td>
        </tr>-->
        <tr>
...
<script type="text/javascript" language="javascript" >
    ...
    frmvalidator.addValidation("{{$TABLE}}[bp_title]","req", "Please specify title.");
</script>
{{/if}}