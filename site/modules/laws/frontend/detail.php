...
if(!empty($submit))
{
	...
	if($_FILES['docs']['size'] > 0)
		{		
					...
					upload($docfile, $src);
					$docs['bpd_name'] = $docfile;
					$docs['bpd_type'] = $file_type;
					$docs['bpd_size'] = $_FILES['docs']['size'];
					$docs['bpd_insert_date'] = date('Y-m-d H:i');
					
					
				//	$destination = BASE_DIR.'files/'.$Document_path."/".$docfile;
			...
			if($this_id)
				{
							...
							$detail['bp_last_update'] = date('Y-m-d H:i');
							$this_id = $tableDocuments->updateRow($detail);		
				}
			else
				{
							$detail['bp_insert_date'] = date('Y-m-d H:i');
				 			$this_id = $tableDocuments->insertRow($detail); 
							$docs['bpd_bp_id']= $this_id;         			
				}	

if($docfile){ $tabledoc->setWhere("bpd_name = '".$docfile."'");
