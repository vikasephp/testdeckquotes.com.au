    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
                    ...
                    <th class="topmenu" align="center" valign="middle" width="25%">Website</th> 
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
             <td>{{counter}}</td>
                <td>{{if $item.br_logo}}<img src="{{$BASE_URL}}files/suppliers/brandslogo/{{$item.br_logo}}">{{/if}}</td>
                <td>{{$item.br_name}}</td>
                <td>{{$item.br_website}}</td>
                <td>{{$item.br_status}}</td>
                 <td>{{$item.br_timestamp|date_format:"%m-%d-%Y"}}</td>
                <td>
