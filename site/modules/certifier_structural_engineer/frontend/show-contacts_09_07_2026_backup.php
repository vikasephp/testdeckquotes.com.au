...
$car_id = $fwRequest->getParam('car_id', '');
	...
	$matsql = "SELECT contacts.* , companies.co_company_name, type_options.to_option  from contacts 
	  ...
	  foreach($list as $ck=>$vc)
	 {
		...
		$sqlc = "Select * from  supplier_email_certifier where se_email = '".$vc['cs_primary_email']."' and se_car_id = ".$car_id;
	...
	//$sqlCl = "Select car_status from certifier_documents where car_id = ".$car_id;
...
if(!empty($add_emails))
{
	...
	foreach($emails as $k => $v)
	{
		...
		// {
			 $tableSupemail->setWhere("se_email = '".$emailA."' and se_car_id =".$car_id);
	...
	$sqlAdd = "Select se_email from  supplier_email_certifier where se_car_id = ".$car_id;
