...
<div align="center" style="min-height:350px; z-index:9999999;">
    <h3 class="page-title"><br />Footing Inspections - Required Documents</h3>    
...
<table id="list-table" width="99%">
        ...
        <select name="flooting_insp[cse_comm_notice]"  onChange="update_doc_checklist({{$bsn_id}}, this.value, 235)">
        ...
        <select name="flooting_insp[cse_flo_sepi2]" onChange="update_doc_checklist({{$bsn_id}}, this.value, 653)">
        ...
        <td style="width:40%"> 
		{{foreach from=$dataSupplier653 item=row653}}
			{{$row653.sa_include_supplier}}<br>
		{{/foreach}}
        <a href="{{$BASE_URL}}certifier_structural_engineer.include_suppliers/car_id/653/bsn_id/{{$bsn_id}}" class="various kill_others">Include Supplier</a><br><br>
        </td>
        ...
        <tr>
        <td style="width:40%">Ground Floor Floor Frame</td> 
        
        <td {{if  $data478.doc_not_required eq 1}} style="background:#999;"
            {{elseif $data478.doc_file_name}} style="background:#00CC33;"  {{else}} style="background:#F00;" {{/if}} id = "cond478">
           
            {{if $data478.doc_file_name}}
			<a href="/certifier_structural_engineer.download_content?file_name={{$data478.doc_file_name}}&module_name=certifier_structural_engineer.home">Download</a><br />
            {{/if}}
            
        <select name="flooting_insp[cse_flo_gfff]" onChange="update_doc_checklist({{$bsn_id}}, this.value, 478)" >
        <option value="0" {{if $data478.doc_not_required eq 0 }} selected="selected" {{/if}}>Required</option>
        <option value="1" {{if $data478.doc_not_required eq 1 }} selected="selected" {{/if}}>Not Required</option>
        </select>
        </td>
        
        <td style="width:40%"> 
		{{foreach from=$dataSupplier478 item=row478}}
			{{$row478.sa_include_supplier}}<br>
		{{/foreach}}
        <a href="{{$BASE_URL}}certifier_structural_engineer.include_suppliers/car_id/478/bsn_id/{{$bsn_id}}" class="various kill_others">Include Supplier</a><br><br>
        </td>
        ...
        <select name="flooting_insp[cse_se_design]" onChange="update_doc_checklist({{$bsn_id}}, this.value, 116)">
        ...
        <td style="width:40%"> 
		{{foreach from=$dataSupplier116 item=row116}}
			{{$row116.sa_include_supplier}}<br>
		{{/foreach}}
        <a href="{{$BASE_URL}}certifier_structural_engineer.include_suppliers/car_id/116/bsn_id/{{$bsn_id}}" class="various kill_others">Include Supplier</a><br><br>
        </td>
        </tr>
