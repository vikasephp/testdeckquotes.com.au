...
<div align="center">
<h3 class="page-title">Add Purchase Order</h3><br /><br />
   <form name="detail" method="post" action="{{$XFA.detail}}" enctype="multipart/form-data">							
   
   {{if !$opr}}
   {{if $msg }} <div style="font-weight:bold; font-size:15px; color:#F00;"> {{$msg }}</div> {{/if}}
 
 <input type="hidden" name="purchse_order[po_id]" value="{{$detail.po_id}}" id="dh_id"/>
     ...
     <table id="list-table" width="100%" cellpadding="0" cellspacing="0">
        ...
        <tr>
           ...
           {{foreach from=$projdetail key="key3" item="item3"}}
            ...
            {{/foreach}}
             <option value="000 | Non-project/Others" {{if $bsnid eq '0'}} selected="selected" {{/if}}>Non-project/Others</option>
	   </datalist>
           <input type="text" list = 'project' name="purchse_order[project_name]" style ="width:700px" value="{{$projname}}" />
        ...
        <tr>
        <th>Component:</th>
        <td> <datalist id='component'>
           {{foreach from=$compdetail key="key2" item="item2"}}
            <option value="{{$item2.pc_name}}" {{if $bsnid eq $item2.bsn_id}} selected="selected" {{/if}}>
            {{$item2.pc_name}}
            </option>
            {{/foreach}}
	   </datalist>
           <input type="text" list = 'component' name="purchse_order[po_component_name]" style ="width:700px" value="{{$detail.po_component_name}}" />
	</td>
        ...
        <tr>
        <th>Upload Linked Quote</th>
        <td>
        <input type="file" name="lq" />
        </td>
       </tr> 
        ...
        <input type="text" name="purchse_order[po_sent_date]" value="{{$detail.po_sent_date}}" class="w16em dateformat-d-ds-m-ds-Y" id="demo7" maxlength="25" />
         ...
         <tr>
            <th>PO Number:</th>
            <td> <input type="text" name="purchse_order[po_po_number]" value="{{$detail.po_po_number}}" style="width:200px;"/></td>
        </tr>
       
        <tr>
            <th>Quote Number:</th>
            <td> <input type="text" name="purchse_order[po_quote_number]" value="{{$detail.po_quote_number}}" style="width:200px;"/></td>
        </tr>
