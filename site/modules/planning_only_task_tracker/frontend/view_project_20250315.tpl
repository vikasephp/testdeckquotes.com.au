...
<div>
	<table cellpadding="5" cellspacing="0" border="1" width="60%">
		<tr>
			<th width="33%"><a href="{{$BASE_URL}}planning_task_tracker.view_project/bsn_id/{{$bsn_id}}#ppat">Project
					Planning Approval Tasks</a></th>
			<th width="33%"><a href="{{$BASE_URL}}planning_task_tracker.view_project/bsn_id/{{$bsn_id}}#ppd">Planning
					Project Documents</a></th>
			<th width="33%"><a href="{{$BASE_URL}}planning_task_tracker.view_project/bsn_id/{{$bsn_id}}#ut_re">Utilites
					And Referrals</a></th>
		</tr>
	</table>
</div>
...
<div id="ppd">
	<div style="padding:10px; text-align:center; margin-bottom:20px; font-size:18px; color:#09F"><strong>Planning
			Project Documents</strong> </div>
	...
	<script>
		...
		function update_record(id, col, value) {
			...
			$.ajax({
				type: "POST",
				url: "{{$BASE_URL}}planning_task_tracker.update_record",
				data: formData,
				dataType: "json",
				encode: true,
		...
		function update_da_number(id, value) {
			$.ajax({
				type: "GET",
				url: "{{$BASE_URL}}planning_task_tracker.update_da_number/bsn_id/" + id + "/value/" + value,
				success: function (result) {
				}
			});
		}
		function update_da_link(id, value) {
			$.ajax({
				type: "GET",
				url: "{{$BASE_URL}}planning_task_tracker.update_da_link/bsn_id/" + id + "/value/" + value,
				success: function (result) {
				}
			});
		}
		...
		<form name="nr" method="post" action="">
			...
			<tr {{if $item.pc_required eq 0}} style="background:#F00;" {{/if}} id="req_color{{$item.pc_id}}">
				...
				<td style="text-align:center;" data-sn="9">
					...
					<a href="{{$BASE_URL}}planning_task_tracker.breach_text/pc_doc_id/{{ $item.pc_doc_id }}/bsn_id/{{ $bsn_id }}" class="various">Link</a>
				</td>
