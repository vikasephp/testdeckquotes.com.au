...
$pottShowHidden = false;

$unhidden = $fwRequest->getParam('unhidden', '');
if (!empty($unhidden)) {
	unset($_SESSION['pott_show_hidden']);
	unset($_SESSION['unhide']);
}

$unhide = $fwRequest->getParam('unhide', '');
if (!empty($unhide)) {
	$_SESSION['pott_show_hidden'] = 1;
}
if (!empty($_SESSION['pott_show_hidden'])) {
	$pottShowHidden = true;
}

$pottHideClause = $pottShowHidden ? "business_sellers.bs_pott_hide = 1" : "business_sellers.bs_pott_hide = 0";
$where = " WHERE " . $pottHideClause . " ";
...
if ($savedata) {
	...
	if ($_FILES['pr_letter']) {
		...
		upload($doc1, $src);
		$letterData['bc_pott_letter'] = $doc1;
...
if (!empty($redlight)) {
	$where = "WHERE business_sellers.bs_traffic_light = 3 AND " . $pottHideClause . " ";
}

$amberlight = $fwRequest->getParam('amberlight', '');
if (!empty($amberlight)) {
	$where = "WHERE business_sellers.bs_traffic_light = 2 AND " . $pottHideClause . " ";
}


$greenlight = $fwRequest->getParam('greenlight', '');
if (!empty($greenlight)) {
	$where = "WHERE business_sellers.bs_traffic_light = 1 AND " . $pottHideClause . " ";
}
...
if (!empty($filter_ss)) {
	...
	$where = " WHERE business_sellers.bs_ptt_sub_status = " . $ss . " AND " . $pottHideClause . " ";
	$fwViewData['ss'] = $ss;
}
...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
		...
		business_sellers.bc_pott_letter, business_sellers.bc_planning_task_report, business_sellers.bs_traffic_light,
...
$qry = "SELECT doc_bsn_id, doc_name_id, doc_file_name, doc_date_uploaded from document_check_list WHERE doc_name_id IN (64,491,525,627,628,629,630,716,717)";
$res = $fwDb->query($qry);
$docChkLstData = [];
foreach ($res as $row) {
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			$sql_456 = "SELECT doc_file_name FROM document_check_list where doc_bsn_id = ". $v['bsn_id']. " and doc_name_id = 456";
			$data_456 = $fwDb->queryOne($sql_456);


			if (!empty($docChkLstData[$v['bsn_id']][525]['doc_file_name'])) {
				$ccdate = $docChkLstData[$v['bsn_id']][525]['doc_date_uploaded'];
			...
			$setdata2[$k]['bs_pott_hide'] = $v['bs_pott_hide'];
			...
			$setdata2[$k]['doc_file_name_525'] = $docChkLstData[$v['bsn_id']][525]['doc_file_name'];
			$setdata2[$k]['doc_date_uploaded_525'] = changedate_d_m_y($docChkLstData[$v['bsn_id']][525]['doc_date_uploaded']);
			...
			$setdata2[$k]['weekly_update_456'] = $data_456['doc_file_name'];
...
$fwViewData['title'] = "Planning Only Task Tracker";
...
if ($export > 0) {
	...
	$objPHPExcel->getProperties()->setCreator("Deckquotes")
		...
		->setCategory("Planning Only Task Tracker");
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		->setCellValue('I1', 'Original GCN')
		->setCellValue('J1', 'Planning Only proposal');
	...
	foreach ($setdata2 as $k => $v) {
		...
		$objPHPExcel->getActiveSheet()
			...
			->setCellValue('I' . $row, $v['bsn_orginal_csbd'])
			->setCellValue('J' . $row, $v['doc_date_uploaded_525']);
...
if ($bus_id > 0) {
	...
	$sql_A = "select bc_pott_letter from business_sellers where bs_business_id = " . $bus_id;
	$data_A = $fwDb->queryOne($sql_A);



	$folder_path = "files/cma/";
	$object_name = $data_A['bc_pott_letter'];
