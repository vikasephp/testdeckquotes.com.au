...
$sql = "Select bsn_name, bsn_da_number, bsn_da_link, bsn_da_link_2, bsn_da_link_3, bsn_da_link_4, bsn_da_link_5 from business where bsn_id = " . $bsn_id;
$data = $fwDb->queryOne($sql);

$fwViewData['bsn_name'] = $data['bsn_name'];
$fwViewData['bsn_da_number'] = $data['bsn_da_number'];
$fwViewData['bsn_da_link'] = $data['bsn_da_link'];
$fwViewData['bsn_da_link_2'] = $data['bsn_da_link_2'];
$fwViewData['bsn_da_link_3'] = $data['bsn_da_link_3'];
$fwViewData['bsn_da_link_4'] = $data['bsn_da_link_4'];
$fwViewData['bsn_da_link_5'] = $data['bsn_da_link_5'];
...
if (!empty($submit_nrt)) {
	...
	Location(BASE_URL . "planning_only_task_tracker.view_project/bsn_id/" . $bsn_id);
}
...
if (!empty($entities_referral_result)) {
	...
	foreach ($entities_referral_result as $key => $value) {
		...
		$sql3 = "SELECT * FROM `document_check_list` where doc_bsn_id= '" . $bsn_id . "' and doc_name_id = '" . $value['er_dc_uid'] . "'";
		...
		$sql4 = "SELECT count(dpn_bsn_id) as count FROM `document_proposal_name` where dpn_bsn_id = '" . $bsn_id . "' ";
		$result4 = $fwDb->query($sql4);
		//echo "<pre>"; print_r($result4); print_r($result4[0][count]); exit('checking');
		if ($result4[0][count] > 0) {
			$sql5 = "SELECT `dpn_unique_id`,`dpn_heritage_ref`,`dpn_water_ref`,`dpn_electric_ref`,`dpn_gas_ref`,`dpn_storm_ref` FROM `document_proposal_name` where dpn_bsn_id = '" . $bsn_id . "' ";
