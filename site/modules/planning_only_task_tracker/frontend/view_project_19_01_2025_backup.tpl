...
<table cellpadding="5" cellspacing="0" border="1"  width="60%">
  <tr>
    <th width="33%"><a href="{{$BASE_URL}}planning_task_tracker.view_project/bsn_id/{{$bsn_id}}#ppat">Project Planning Approval Tasks</a></th>
    <th width="33%"><a href="{{$BASE_URL}}planning_task_tracker.view_project/bsn_id/{{$bsn_id}}#ppd">Planning Project Documents</a></th>
    <th width="33%"><a href="{{$BASE_URL}}planning_task_tracker.view_project/bsn_id/{{$bsn_id}}#ut_re">Utilites And Referrals</a></th>
  </tr>
 </table>   

</div>

<div style="padding:10px; text-align:center; margin-bottom:20px; font-size:16px; color: #900"><strong>Project Name : {{$busdata.bsn_name}}</strong> </div>
<div id="ppat">
...
<img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/sub.png" title="Sub Task Edit"/></a> 
    ...
    <a href="javascript:modelwindow('{{$BASE_URL}}business.taskediturl/bus_id/{{$item.bt_bsn_id}}/taskId/{{$item.bt_id}}','90%','90%', 'yes');">
...
<img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/qa.png" title="Sub Task Edit"/></a> <br />
    ...
    <td align="center">{{if $item.bst_task_id == 0}}{{$item.bst_id}}{{else}}
    <a href ="{{$BASE_URL}}business_status_taskAdmin.detail/bst_id/{{$item.bst_id}}" target="_blank">{{$item.bst_task_id}}{{/if}}</td>
    <td> 
    ...
    <td align="center">{{if $item.bst_auto_email_text eq '1'}} Yes {{else}} No {{/if}} </td>
      
    <td> {{if $item.bst_auto_sms_text eq '1'}} Yes {{else}} No {{/if}} </td>  
  ...
  {{/foreach}}
   <tr> <td colspan="12" style="padding:5px;">

   <input type="hidden" name="bt_task_id[{{$item.bt_id}}]" value="{{$item.bt_task_id}}" />
   <input type="submit" name="not_required_task_save" value="Save Not Required" /></td>
</tr>
</table>
<br />
<br />
...
<table cellpadding="5" border="1" cellspacing="0" width="100%">
  <tr>
    ...
    <th width="5%">Type</th>
    {{if $ppdata.pd_minimum_doc}}
    <th width="5%">Minimum Document</th>
    {{/if}} 
    <th width="6%">Required</th>
    <th width="6%">Design Plans</th>
    <th width="6%">Operational Plans</th>
    <th width="6%">Planning Plans</th>
    <th width="6%">Document Checklist</th>

    <th width="7%">Customer Approved</th>
    <th width="8%">QA1</th>
    <th width="8%">QA2</th>
    <th width="8%">Ready For Upload Locked</th>
    <th width="8%">Department Uploaded</th>
    <th width="8%">Rules</th>
    <th width="8%">Sync WWW</th>
  ...
  {{foreach from=$print_data item=item key=key}}
    ...
    <td style="padding-left:5px;">{{$item.pc_document}}</td>
	<td>{{$item.pc_type}}</td>
    
    {{if $ppdata.pd_minimum_doc eq 'New Single Dwelling'}}
    <td width="5%">{{$item.ppr_new_single}} </td>
    {{elseif $ppdata.pd_minimum_doc eq 'Alterations To A Single Dwelling' }}
    <td width="5%">{{$item.ppr_alterations}}</td>
    {{elseif $ppdata.pd_minimum_doc eq 'Secondary Residence' }}
    <td width="5%">{{$item.ppr_secondary}} </td>
    {{elseif $ppdata.pd_minimum_doc eq 'Class 10 Structure' }}
    <td width="5%">{{$item.ppr_class10}}</td>
    {{elseif $ppdata.pd_minimum_doc eq 'Dual Occupancy' }}
    <td width="5%">{{$item.ppr_dual_occ}}</td>
    {{/if}} 
    ...
    <form name ="recmet3" method="post" action="">
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="required[{{$item.pc_id}}]"   onclick="update_required({{$item.pc_id}},this.checked);" {{if $item.pc_required  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
    <span id="req{{$item.pc_id}}">{{if $item.pc_required eq 0}}{{$item.pc_required_user}} <br />{{$item.pc_required_date}}{{/if}} </span>  
    </td>

 
    <td>{{if $item.pc_design_plan}}<a href="{{$item.pc_design_plan}}" target="_blank">Link</a>
       {{$item.pc_design_plan_user}}<br /> {{$item.pc_design_plan_date}}
       {{/if}} </td>
 
    <td> {{if $item.bsn_ptt_opi eq 1}} Yes <br />
             {{$item.bsn_ptt_opi_date}} <br /> {{$item.bsn_ptt_opi_user}} 
         {{else}} No {{/if}}
    </td>
    
    
    <td>{{if $item.pc_live_link}}<a href="{{$item.pc_live_link}}" target="_blank">Link</a>
        {{$item.pc_live_link_user}}<br /> {{$item.pc_live_link_date}}
        {{/if}} 
    </td>
    
   <td><!--{{if $item.pc_document_checklist}} <a href="{{$item.pc_document_checklist}}" target="_blank">Link</a>{{/if}}-->
      {{if $item.doc_file_name}}
      <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name}}&module_name=planning_task_tracker.view_project" target="_blank" style="text-decoration: none;">Download</a> <br />{{$item.user_name}}<br />{{$item.doc_date_uploaded}} {{/if}}
       
   </td>
    
	<td> 
    <form name ="recmet3" method="post" action="">
     	<input type="hidden" name="approve[{{$item.pc_id}}]"  />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="approve[{{$item.pc_id}}]"   onclick="update_pp({{$item.pc_id}},this.checked);" 
{{if $item.pc_cust_approve eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form> 
   
   <span id="app{{$item.pc_id}}">{{if $item.pc_cust_approve eq 1}}{{$item.pc_approve_user}} <br />{{$item.pc_approve_date}}{{/if}} </span> 
   </td>
   <td>
    ...
    <form name ="recmet3" method="post" action="">
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="qa1[{{$item.pc_id}}]"   onclick="update_qa1({{$item.pc_id}},this.checked);" {{if $item.pc_qa1 eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
    <span id="qa1{{$item.pc_id}}">{{if $item.pc_qa1 eq 1}}{{$item.pc_qa1_user}} <br />{{$item.pc_qa1_date}}{{/if}} </span>   
    </td> 

    <td>
    
    <form name ="recmet4" method="post" action="">
     	<input type="hidden" name="qa2[{{$item.pc_id}}]"  />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="qa2[{{$item.pc_id}}]"  onclick="update_qa2({{$item.pc_id}},this.checked);" {{if $item.pc_qa2 eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
    <span id="qa2{{$item.pc_id}}">{{if $item.pc_qa2 eq 1}}{{$item.pc_qa2_user}} <br />{{$item.pc_qa2_date}}{{/if}} </span>   
    </td> 
    
        <td>
    
    <form name ="recmet3" method="post" action="">
     	<input type="hidden" name="ready[{{$item.pc_id}}]"  />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="ready[{{$item.pc_id}}]"   onclick="update_ready({{$item.pc_id}},this.checked);" {{if $item.pc_ready_for eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
    ...
    <form name ="recmet5" method="post" action="">
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="depup[{{$item.pc_id}}]"  onclick="update_dep_uploaded({{$item.pc_id}},this.checked);" {{if $item.pc_dep_uploaded eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
    <span id="dep{{$item.pc_id}}">{{if $item.pc_dep_uploaded eq 1}}{{$item.pc_dep_upload_user}} <br />{{$item.pc_dep_upload_user}}{{/if}} </span>   
    </td> 
...
<strong>Utilites and referrals</strong> </div>

<table cellpadding="5" border="1" cellspacing="0" width="100%">
  <tr>
    <th width="4%"></th>
    <th width="20%">Required</th>
    <th width="5%">Date Applied</th>
    <th width="6%">Assessment Doc</th>
    <th width="6%">Status</th>
    <th width="6%">Condition Snapshot</th>
    <th width="7%">Add To The Design Change</th>
    <th width="8%">Action</th>
  </tr>
  
  <tr>
    <th width="10%">Evo – Electrical</th>
    <th width="20%"></th>
    <th width="5%"></th>
    <th width="6%"></th>
    <th width="6%"></th>
  ...
  <tr>
    <th width="10%">TCCS – Stormwater</th>
    <th width="20%"></th>
    <th width="5%"></th>
    <th width="6%"></th>
    <th width="6%"></th>
