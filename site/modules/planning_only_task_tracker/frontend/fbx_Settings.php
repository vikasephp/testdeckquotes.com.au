...
function create_planning_report($heading, $html, $bus_id)
		{
...
$nameoffile = 'planning_report';
$filename = $nameoffile."_".$bus_id.".pdf";
...
$pdf->SetTitle("CGFB");
$pdf->SetSubject('Planning Task Tracker');
...
$printdata['bc_planning_only_task_report'] = $filename;

$table = new Fw_Db_Table('business_sellers');
$table->setWhere('bs_business_id = '.$bus_id);
if($table->rowExists())
{
	
	$detail = $table->updateRow($printdata);	
}		
...
function create_management_report($heading, $html, $bus_id)
		{
...
function upload($filename, $tmpname)
{
	...
	$zipfile = $filename;
        ...
        try {
            ...
            if ($s3->putObjectFile($path . "/" . $zipfile, BUCKET_NAME, 'files/cma/'.$zipfile, S3::ACL_PRIVATE)) {
