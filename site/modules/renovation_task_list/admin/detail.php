...
$tablebusiness_tasks = new Fw_Db_Table('renovation_tasks');

$tablebusiness_tasks_order = new Fw_Db_Table('renovation_task_list_orders');
...
$tablebusiness_email_link = new Fw_Db_Table('renovation_task_list_email_link');
$tablelinked_procedure = new Fw_Db_Table('renovation_task_list_subtask_lp');
...
if($data['bst_business_types']){ $data['bst_business_types']= serialize($data['bst_business_types']); }
  ...
  if(!$bst_tasks_error){	
	...
	{
		$sql = "SELECT MAX(bst_order) as maxbsts FROM renovation_task_list" ;
		$maxdata = $fwDb->queryOne($sql);
		$data['bst_order'] = (int)$maxdata['maxbsts'] +1;
		
		$sql = "SELECT MAX(bst_id) as maxbstid FROM renovation_task_list";
		$maxdata = $fwDb->queryOne($sql);
		...
		$sql = "SELECT MAX(business_status_task_orders_tasks_oid) as maxord FROM renovation_task_list_orders WHERE business_status_task_orders_type_id = ".$key;
		$maxdata = $fwDb->queryOne($sql);
