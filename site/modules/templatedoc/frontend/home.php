...
if($_SESSION['sort_order']){$order = "ORDER BY ".$_SESSION['sort_order'];}else{$order = "ORDER BY ".$TABLE.".dh_uid ASC";}
				...
				if($doc_files)
					{
						foreach($doc_files as $kf => $vf)
								{
								if($vf['dhd_type']){$vf['iconpath'] = get_icon_path($vf['dhd_type'], $vf['dhd_name2'], 'template_documents_hub');}
								$filedata[] = $vf;
								}
					}
...
$fwViewData['title'] = "Template Document List";
