...
$sql10 = "Select * from street_sign_type";
$fwViewData['typedetail'] = $fwDb->query($sql10);

$sqlact = "Select * from street_sign_action";
$fwViewData['actiondetail'] = $fwDb->query($sqlact);

$sqli = "Select * from street_sign_intensity";
$fwViewData['intensitydetail'] = $fwDb->query($sqli);

$sqlba = "Select * from street_sign_banner_action";
...
if($export > 0)
    {
    $sqlexport = "Select street_sign.* from street_sign";
...
header('Content-Disposition: attachment;filename="street_sign.xls"');
...
if(!empty($submit))
{
	...
	if($_SESSION['intensity_filter']){
		$where .= " AND street_sign.ss_intensity IN (".$_SESSION['intensity_filter'].")";
	}
	
	if($_SESSION['signaction_filter']){
		$where .= " AND street_sign.ss_sign_action LIKE '%".$_SESSION['signaction_filter']."%'";
	}

	if($_SESSION['banneraction_filter']){
		$where .= " AND street_sign.ss_banner_action LIKE '%".$_SESSION['banneraction_filter']."%'";
	}

	$sssql = "SELECT * from street_sign ". $where;	
