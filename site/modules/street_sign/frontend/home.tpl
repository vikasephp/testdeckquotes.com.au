...
{
	var viewurl = "{{$BASE_URL}}street_sign.view_procedure"+ "/random/" + Math.random();	
...
function add_type()
{
	var viewurl = "{{$BASE_URL}}street_sign.view_type"+ "/random/" + Math.random();	
	$(document).ready(function () {
...
function add_action()
{
	var viewurl = "{{$BASE_URL}}street_sign.view_action"+ "/random/" + Math.random();	
	$(document).ready(function () {
...
{
	var viewurl = "{{$BASE_URL}}street_sign.view_intensity"+ "/random/" + Math.random();	
	$(document).ready(function () {
...
{
	var viewurl = "{{$BASE_URL}}street_sign.view_banner_action"+ "/random/" + Math.random();	
	$(document).ready(function () {
    ...
    <!--<form name="mlist" method="post">-->
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
		...
		function update_intensity(id,value)
		{
			 ...
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign.update_intensity/ss_id/"+id+"/value/"+value,
		...
		function update_sign_action(id,value)
		{
			 ...
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign.update_signaction/ss_id/"+id+"/value/"+value,
					   success: function(result){
						   $(tana).html(result);
					}
				});
		...
		function update_banner_action(id,value)
		{
			 ...
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign.update_banner_action/ss_id/"+id+"/value/"+value,
					   success: function(result){
						   $(tana2).html(result);
					}
				});
		...
		function update_sign_type(id,value)
		{
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign.update_signtype/ss_id/"+id+"/value/"+value,
					   success: function(result){
					}
				});
		}
         ...
         <a href="/street_sign.download_content?file_name={{$item.ss_actmapi_image}}&module_name=street_sign.home" >Download</a><br />
            {{$item.ss_actmapi_image_date}}
            {{/if}}
         </td>
         
         <td> {{if $item.ss_street_sign_image}}
         <a href="/street_sign.download_content?file_name={{$item.ss_street_sign_image}}&module_name=street_sign.home" >Download</a>
        <!-- <a href="https://deckquote.s3.amazonaws.com/files/street_sign/{{$item.ss_street_sign_image}}" target="_blank">Download</a> --><br />
            {{$item.ss_sign_image_date}}
            {{/if}}
         </td>
         <td>{{if $item.ss_banner_image}}
         <a href="/street_sign.download_content?file_name={{$item.ss_banner_image}}&module_name=street_sign.home" >Download</a>
       <br />
	...
	<td>
	   ...
	   function update_notes(id,value)
		...
		//alert(id);	
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}street_sign.update_notes/ss_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
