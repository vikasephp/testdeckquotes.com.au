...
function add_new()
 {
var viewurl = "{{$BASE_URL}}dochub.detail"+ "/random/" + Math.random();	
 $(document).ready(function () {
        $.fancybox({
				'width': '58%',
				'height': '70%',
    ...
    <table id="doclist-table" class="nav-back" width="100%">
        <thead>
         <tr>	
                    ...
                    <th class="topmenu" align="center" valign="middle" width="280">Download Counter </th>
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
            	<td>{{$countn}}.</td>
                <td>{{$item.dh_title}}</td>
                <td><a href="">{{$item.dh_summery|truncate:200:"...":true}}</a></td>
               ...
               <td>{{if $item.dh_access == 1}}All Access{{else}}Users{{/if}}</td>
                ...
                <td>{{if $item.dh_last_update !== '0000-00-00 00:00:00'}}{{$item.dh_last_update|date_format:"%d/%m/%Y %H:%M"}}{{else}}
                 ...
                 {{foreach from=$item.doc_files key="keyfiles" item="itemfiles"}}
                 	<td>
                       <ul  class="shofiles">
                    	<li><img src="{{$itemfiles.iconpath}}" width="20" />
                    <br /><a  class="shofilemenu">{{$itemfiles.dhd_name|truncate:"9"}}</a>
       <ul class="small-menu">
            <li> {{if $itemfiles.dhd_docs_status == 0}}
           <!-- <a href="{{$BASE_URL}}{{$FILE_PATH}}files/business_documents_hub/{{$itemfiles.dhd_name}}" title="{{$itemfiles.dhd_name}}" target="_blank" style="color:#FFFFFF;" onclick="javascript:counter({{$item.dh_id}})">-->
           
   <a href="/dochub.download_content?file_name={{$itemfiles.dhd_name}}&module_name=dochub.home" target="_blank" onclick="javascript:counter({{$item.dh_id}})"/>         
            
              {{else}}
            <a href="{{$BASE_URL}}{{$XFA.open}}/fileid/{{$itemfiles.dhd_path}}"  class="shofile" style="color:#FFFFFF;" >{{/if}}
            View/Edit</a></li>
<li>{{if $itemfiles.dhd_docs_status == 0}}
<!--<a href="{{$BASE_URL}}{{$FILE_PATH}}files/business_documents_hub/{{$itemfiles.dhd_name}}" title="{{$itemfiles.dhd_name}}" target="_blank" style="color:#FFFFFF;" onclick="javascript:counter({{$item.dh_id}})">-->
 <a href="/dochub.download_content?file_name={{$itemfiles.dhd_name}}&module_name=dochub.home" target="_blank" onclick="javascript:counter({{$item.dh_id}})"/> 
...
{{else}}
<a href="{{$BASE_URL}}{{$XFA.open}}/download/1/fileid/{{$itemfiles.dhd_path}}"  title="Start Download" style="color:#FFFFFF;">{{/if}}
Download</a></li>
        </ul>
        </li>
        </ul>
    ...
    <td> This Version :<input type="text" id="number1" value="{{$item.dh_thisversion}}" style="border:0px; background:inherit; width:20px;"/> <br />
    All Time : <input type="text" id="number2" value="{{$item.dh_alltime}}" style="border:0px; background:inherit;width:20px;"/> </td> 
   	...
   	function counter(dh_id)
	{
		...
		$.ajax({
			   type: "GET",
			   url: "http://www.deckquotes.com.au/dochub.counter/dh_id/"+dh_id,
...
<br /><br />
    <table id="doclist-table"  width="100%" bgcolor="#d3e8d6">
        <thead>
            <tr><th colspan="2"><div align="left">Look up and add procedure:</div></th></tr>
            <tr><th colspan="2"><div align="left">Procedure Summary Print:</div> </th></tr>
            <tr><th class="topmenu" valign="middle" ><div align="left">GCON Performance Reports:</div></th></tr>
