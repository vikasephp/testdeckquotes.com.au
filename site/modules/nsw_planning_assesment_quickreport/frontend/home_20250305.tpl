...
<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
   ...
   <div style="float:left; padding-top:8px;">
     ...
     &nbsp; &nbsp; &nbsp;<input type="button" name="alert_admin" value="Alert Admin" onclick="javascript:add_alert();" />
   </div> 
     <br />
   <div style="float:left; padding-top:8px;"> &nbsp;&nbsp;&nbsp;
...
&nbsp; &nbsp<strong> 7 Day Lead Count : {{$seven}} </strong> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;  <strong> 14 Day Lead Count : {{$fourteen}} </strong>
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    ...
    <tbody>
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
       ...
       <input name="same" type="checkbox" {{if $item.bsn_address_same eq 1}} checked="checked" {{/if}}  onclick="update_same({{$item.bsn_id}},this.checked)" />
        ...
        <td {{if $item.bs_paqr_alertoption gt 1}} style="background:#F00;" {{/if}}>
	...
	<td> 
       ...
       <select name="project_type[{{$item.bsn_id}}]" Onchange = "updatetype({{$item.bsn_id}},this.value)" >
        ...
        {{foreach from = $typedata key = "keya" item = "itema"}}
          <option value="{{$itema.pt_id}}" {{if $itema.pt_id eq $item.project_type}} selected="selected" {{/if}}> 
        		{{$itema.pt_name}}  
          </option>
        {{/foreach}}
       </select> 
	...
	function updatetype(id,value)
	{
	 $.ajax({
       ...
       <option value="7" {{if $item.bsn_proj_cust_type eq 7}} selected="selected" {{/if}}>Third Party Designs</option>
      </select>
	...
	function updatecusttype(id,value)
	{
	 $.ajax({
       ...
       <option value="Do Not Book" {{if $item.bs_qr_delivery_method eq 'Do Not Book' }} selected="selected" {{/if}}>Do Not Book </option>
  ...
  output{{$item.bsn_id}}.innerHTML = this.value;
	...
	function update(id,value)
	{
	 $.ajax({
        ...
        <input type="button" name="Alert" value="Hot Button" onclick="javascript:hot_button({{$item.bs_business_id}},{{$item.bs_customers_id}} );" style="width:90px; height:20px; font-size:11px; line-height:17px !important; padding:0px !important" />
              </td> 
