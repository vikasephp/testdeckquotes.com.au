...
$nsw_cpa_email = $fwRequest->getParam('nsw_cpa_email', '');
if ($nsw_cpa_email) {
	$keys_2 = array_keys($nsw_cpa_email);
	$ky_2 = $keys_2[0];
	$val_2 = $nsw_cpa_email[$ky_2];

	$detail['bs_cp_emailed'] = $val_2;
	$detail['bs_cp_emailed_user'] = $_SESSION['user']['user_name'];
	$detail['bs_cp_emailed_date'] = date('d-m-Y');
...
$where = "WHERE ((business.bsn_sub_status = 'Open' AND business.bsn_planning_moveto_old <> 1) OR (business.bsn_sub_status IN ('Hold', 'Lost') AND business.bsn_planning_moveto_old = 0) ) AND business.bsn_state = 'NSW' AND business.bsn_type <> 27 ";
...
if (!empty($dm) || isset($_SESSION['dm'])) {
	...
	$where = "WHERE ((bsn_sub_status = 'Open' AND bsn_planning_moveto_old <> 1) OR 
		 ...
		 business_sellers.bs_qr_delivery_method = 'To Be Determined' AND business.bsn_state = 'NSW' AND business.bsn_type <> 27 ";
	$_SESSION['dm'] = 1;
}
...
if (!empty($bbntp) || isset($_SESSION['bbntp'])) {
	...
	$where = "WHERE ((bsn_sub_status = 'Open' AND bsn_planning_moveto_old <> 1) OR 
         (bsn_sub_status IN ('Hold', 'Lost') AND bsn_planning_moveto_old =  0 ) ) AND
		 business_sellers.bs_qr_delivery_method = 'Booked But Not Taken Place' AND business.bsn_state = 'NSW' AND business.bsn_type <> 27";
	$_SESSION['bbntp'] = 1;
}
...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
	    ...
	    business_sellers.bs_qr_dm_date, business_sellers.bs_paqr_alertoption, business_sellers.bs_paqr_alertoption_at, business_sellers.bs_paqr_alertoption_by, business_sellers.bs_paqr_council, business_sellers.bs_paqr_council_by, business_sellers.bs_paqr_council_at, business_sellers.bs_paqr_sqm, business_sellers.bs_paqr_meeting_o,
		business_sellers.bs_paqr_meeting_o_user, business_sellers.bs_paqr_meeting_o_date, business_sellers.bs_cp_emailed, business_sellers.bs_cp_emailed_user, business_sellers.bs_cp_emailed_date
...
if ($plreport_not == 1 || $_SESSION['plreport_not'] == 1) {
	...
	$sql = "SELECT bs.bs_business_id, bs.bs_customers_id, bc.bcust_fname, bc.bcust_lname, bc.bcust_misc_email1, bc.bcust_misc_email2, bc.bcust_misc_moble, bc.bcust_misc_business, business.bsn_type, bc.bcust_misc_home, business.bsn_advert_text, business.bsn_id, business.bsn_name, business.bsn_sub_status, business.bsn_status_sys_date, business.bsn_address, business.bsn_rating, business.bsn_watch_list, business.bsn_proj_cust_type, bs.bs_qr_delivery_method, business.bsn_address_same, bs.bs_paqr_alertoption, bs.bs_paqr_alertoption_at, bs.bs_paqr_alertoption_by, business_sellers.bs_paqr_council, business_sellers.bs_paqr_council_by, business_sellers.bs_paqr_council_at, bs.bs_qr_dm_user, bs.bs_qr_dm_date, bs.bs_paqr_sqm, bs.bs_paqr_meeting_o, bs.bs_paqr_meeting_o_user, bs.bs_paqr_meeting_o_date FROM business_sellers bs JOIN bus_customers bc ON bs.bs_customers_id = bc.bcust_id JOIN business ON bs.bs_business_id = business.bsn_id " . $where . " AND NOT EXISTS ( SELECT 1 FROM document_check_list dcl WHERE dcl.doc_bsn_id = bs.bs_business_id AND dcl.doc_name_id = 44 AND TRIM(COALESCE(dcl.doc_file_name, '')) <> '' ) ORDER BY business.bsn_status_sys_date DESC";
}
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			//doc id 791
			$sql791 = "Select doc_file_name, doc_date_uploaded from document_check_list 
					where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 791";
			$data791 = $fwDb->queryOne($sql791);
			//doc id 791 end
		 ...
		 where bt_bsn_id = " . $v['bsn_id'] . " AND business_tasks.bt_task_status_id = 2 AND business_tasks.bt_complete = 1";
			...
			$setdata2[$k]['bs_paqr_council_by'] = '';
			if($v['bs_paqr_council_by']) {
				$setdata2[$k]['bs_paqr_council_by'] = $users[$v['bs_paqr_council_by']];
			}
			...
			$setdata2[$k]['checklist791'] = $data791['doc_file_name'];
			$setdata2[$k]['checklist791_date'] = changedate_d_m_y($data791['doc_date_uploaded']);
...
$fwViewData['title'] = "NSW Planning Assessment Quick Report";
...
if ($submitdownload) {
	...
	$sqlP = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
		...
		. $where . " Order By business.bsn_status_sys_date DESC";
	...
	$heading = "NSW Planning Assesment Quick Report";
...
$sqlAlert = "Select * from nsw_paqr_alert_admin";
$fwViewData['alertdata'] = $fwDb->query($sqlAlert);

$sqlCouncil = "Select * from nsw_paqr_council_admin";
$fwViewData['councildata'] = $fwDb->query($sqlCouncil);
