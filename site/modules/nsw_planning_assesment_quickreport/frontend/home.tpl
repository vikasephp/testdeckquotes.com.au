...
<script type="text/javascript">
	function addfeedback(bsn_id) {
		var viewurl = "{{$BASE_URL}}{{$BASEFOLDER}}.addfeedback" + "/bsn_id/" + bsn_id;
...
<script type="text/javascript">
	function viewfeedback(bsn_id) {
		var viewurl = "{{$BASE_URL}}{{$BASEFOLDER}}.viewfeedback" + "/bsn_id/" + bsn_id;
...
<script>
	...
	function add_alert() {
		var viewurl = "{{$BASE_URL}}{{$BASEFOLDER}}.view_alert" + "/random/" + Math.random();
	...
	function view_council() {
		var viewurl = "{{$BASE_URL}}{{$BASEFOLDER}}.view_council" + "/random/" + Math.random();
	...
	function hot_button(bsn_id, bcust_id) {
		...
		var viewurl = "{{$BASE_URL}}{{$BASEFOLDER}}.hot_button" + "/bcust_id/" + bcust_id + "/bsn_id/" + bsn_id;
...
<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
	...
	<div style="float:left; padding-top:8px;">
		...
		&nbsp; &nbsp; &nbsp;<input type="button" name="council_admin" value="Council Admin"
			onclick="javascript:view_council();" />
	</div>
	<br />
	<div style="float:left; padding-top:8px;"> &nbsp;&nbsp;&nbsp;
	...
	<div style="float:left;">
		<input type="submit" value="Booked but not taken place" name="bbntp" /> &nbsp;&nbsp;
...
<form name="mlist" method="post">
	<div style="float:right;">
		<!--<input type="submit" name="meetingoccur" value="MO" />
		<input type="submit" name="clearmo" value="Clear MO" />-->
	...
	<div style="float:left;"> &nbsp; &nbsp; <strong>Search:</strong>&nbsp;&nbsp;
		...
		&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<span style="border:1px solid #0CF; padding:3px;"> 7 Days PAR Booked :
			{{if $parbooked_data_weekly}}{{$parbooked_data_weekly.weekly_total}}{{/if}} </span>
		&nbsp;&nbsp;<span style="border:1px solid #0CF; padding:3px;"> YTD PAR Booked :
			{{if $parbooked_data_ytd}}{{$parbooked_data_ytd.ytd_total}}{{/if}} </span>
			...
			<table id="doclist-table" class="nav-back" width="100%">
				<thead>
					<tr>
						...
						<th class="topmenu" align="center" valign="middle" width="3%">Council</th>
						...
						<th class="topmenu" align="center" valign="middle" width="4%" data-col="alert">Alert</th>
						...
						<th class="topmenu" align="center" valign="middle" width="5%">NSW Client Planning Advice</th>
						<th class="topmenu" align="center" valign="middle" width="5%">NSW Client Planning Advice Emailed</th>
						<!--<th class="topmenu" align="center" valign="middle" width="5%">UT PAR</th>-->
						<th class="topmenu" align="center" valign="middle" width="5%" data-col="property_report">
							Property Report</th>
				...
				<tbody>
					...
					<tr bgcolor="{{cycle values='#D3E8D6,WHITE' advance=true}}">
						...
						<td style="text-align:center;">
							...
							<script>
								function update_same(id, value) {
									...
									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_same/bsn_id/" + id + "/value/" + val,
						...
						<td data-col="council">
							<select name="alert[{{$item.bsn_id}}]" onchange="update_council({{$item.bsn_id}}, this.value)">
								<option value="">  </option>
								{{foreach from=$councildata key="key_c" item="item_c"}}
								<option value="{{$item_c.npc_id}}" {{if $item_c.npc_id eq $item.bs_paqr_council}} selected="selected" {{/if}}>
									{{$item_c.npc_option}}
								</option>
								{{/foreach}}
							</select>
							<div id="update_council_option_{{$item.bsn_id}}">
								<p style="margin: 0; padding: 0;">{{$item.bs_paqr_council_by}}</p>
								<p style="margin: 0; padding: 0;">{{$item.bs_paqr_council_at}}</p>								
							</div>
							<script>
								function update_council(id, value) {
									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_council/bsn_id/" + id + "/value/" + value,
										success: function (result) {
											$('#update_council_option_' + id).html(result);
										}
									});
								}
							</script>
						...
						<td><a href="{{$item.project_link}}" target="blank">Link</a></td>
							...
							<form name="recmet3" method="post" action="">
								...
								<label class="switch">
									<input class="switch-input" type="checkbox"
										name="meeting_occured[{{$item.bs_business_id}}]" value="1"
										onclick="this.form.submit();" {{if $item.bs_paqr_meeting_o eq 1 }}
										checked="checked" {{/if}}>
						...
						<td style="text-align:center;">
							<input type="text" name="sqm[{{$item.bsn_id}}]" value="{{$item.bs_paqr_sqm}}" onchange="update_sqm({{$item.bsn_id}},this.value)">
							<script>
								function update_sqm(id, value) {
									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_sqm/bsn_id/" + id + "/value/" + value,
										success: function (result) {
										}
									});
								}
						...
						<td id="update_alert_option_td_{{$item.bsn_id}}" data-col="alert" {{if $item.bs_paqr_alertoption eq 2}} style="background:#F00;" {{/if}} {{if $item.bs_paqr_alertoption eq 6}} style="background:#0F0;" {{/if}}>
							...
							<div id="update_alert_option_{{$item.bsn_id}}">
								<p style="margin: 0; padding: 0;">{{$item.bs_paqr_alertoption_at}}</p>
								<p style="margin: 0; padding: 0;">{{$item.bs_paqr_alertoption_by}}</p>								
							</div>
							<script>
								function update_alert_option(id, value) {
									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_alert_option/bsn_id/" + id + "/value/" + value,
										success: function (result) {
											const td = $('#update_alert_option_td_' + id);
											$('#update_alert_option_' + id).html(result);
											if(value == 2) {
												td.css('background', '#F00');
											}
											else if(value == 6) {
												td.css('background', '#0F0');
											}
											else {
							...
							<a href="{{$BASE_URL}}{{$BASEFOLDER}}.notes/bsn_id/{{$item.bsn_id}}" class="various">Notes</a>
						</td>
						<td>
							...
							<script>
								...
								function updatetype(id, value) {
									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_project_type/bsn_id/" + id + "/value/" + value,
										success: function (result) {
										}
									});
						...
						<td>
							...
							<script>
								...
								function updatecusttype(id, value) {
									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_customer_type/bsn_id/" + id + "/value/" + value,
										success: function (result) {
										}
									});
								}
						...
						<td>
							...
							<select name="dm[{{$item.bsn_id}}]" Onchange="update_dm({{$item.bsn_id}},this.value)">
								...
								<option value="Booked But Not Taken Place" {{if $item.bs_qr_delivery_method
									eq 'Booked But Not Taken Place' }} selected="selected" {{/if}}>Booked But Not Taken
									Place</option>
							...
							<script>
								...
								function update_dm(id, value) {
									...
									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_delivery_method/bsn_id/" + id + "/value/" + value,
										success: function (result) {
											$(tana10).html(result);
										}
									});
						...
						{{if $item.checklist44}}
						<td style="text-align:center;">
							<a href="{{$BASE_URL}}{{$BASEFOLDER}}.download_content?file_name={{$item.checklist44}}&module_name=planning_assesment_quickreport.home">Download</a>
						...
						{{else}}
							...
							{{if $item.checklist791}}
							<td style="text-align:center;">
								<a href="{{$BASE_URL}}{{$BASEFOLDER}}.download_content?file_name={{$item.checklist791}}&module_name=planning_assesment_quickreport.home">Download</a>
								{{$item.checklist791_date}}
							</td>
							{{else}}
								<td style="background:#F00"> </td>
							{{/if}}
							...
							<td>
								<form name="recmet3" method="post" action="">
									<input type="hidden" name="nsw_cpa_email[{{$item.bs_business_id}}]" value="0" />
									<label class="switch">
										<input class="switch-input" type="checkbox"
											name="nsw_cpa_email[{{$item.bs_business_id}}]" value="1"
											onclick="this.form.submit();" {{if $item.bs_cp_emailed eq 1 }}
											checked="checked" {{/if}}>
										<span class="switch-label" data-on="Yes" data-off="No"></span>
										<span class="switch-handle"></span> <br />
									</label>
								</form>
								{{if $item.bs_cp_emailed eq 1}}
									{{$item.bs_cp_emailed_user}}<br/>
									{{$item.bs_cp_emailed_date}}<br />
								{{/if}}
							</td>

							<!-- {{if $item.checklist705}}
							<td style="text-align:center;">
								<a href="{{$BASE_URL}}{{$BASEFOLDER}}.download_content?file_name={{$item.checklist705}}&module_name=planning_assesment_quickreport.home">Download</a>
							...
							{{/if}} -->
								...
								<td data-col="property_report">
									{{$item.property_report}}
								</td>
								<td>
									...
									<script>
										function show(bid, value) {
											$.ajax({
												type: "GET",
												url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_watchlist/bsn_id/" + bid + "/value/" + value,
												success: function (result) {
								...
								<td>
									...
									<script>
										...
										function update(id, value) {
											$.ajax({
												type: "GET",
												url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_rating/bsn_id/" + id + "/value/" + value,
												success: function (result) {
												}
											});
								...
								<td style="text-align:center;">
									<a
										href="{{$BASE_URL}}{{$BASEFOLDER}}.download_content?file_name={{$item.checklist412}}&module_name=planning_assesment_quickreport.home">Download</a>
								...
								{{else}}
									...
									<td style="text-align:center;">
										<a
											href="{{$BASE_URL}}{{$BASEFOLDER}}.download_content?file_name={{$item.checklist45}}&module_name=planning_assesment_quickreport.home">Download</a>
   ...
   -->
											<a
												href="{{$BASE_URL}}{{$BASEFOLDER}}.download_content?file_name={{$item.checklist2}}&module_name=planning_assesment_quickreport.home">Download</a>
