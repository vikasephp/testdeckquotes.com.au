...
$table = new Fw_Db_Table('165_report');

$where = " WHERE 1 = 1  ";


$save_doc = $fwRequest->getParam('save_doc', '');
if($save_doc) 
{ 
		if($_FILES['document']['name'])
		{

			$docfile_12 = $_FILES['document']['name'];
			$docfile_12 =  preg_replace('/[^A-Z0-9._]/i', '_', $docfile_12);
			$temp_name_12 = $_FILES['document']['tmp_name'];
			$clData['os_doc_link'] = $docfile_12;
			
		
			upload($docfile_12, $temp_name_12);
			
			$os_id = $fwRequest->getParam('os_id', '');
				
				$table->setWhere('os_id = '.$os_id);
				if($table->rowExists())
				{
...
if($clear) {
	 ...
	 unset($_SESSION['wasrequest']);
	 unset($fwViewData['checkpass']);
	 unset($fwViewData['wasrequest']);
	 unset($_SESSION['dapassed']);
}
