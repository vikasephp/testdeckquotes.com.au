<?php
$update_escalation = $fwRequest->getParam('update_escalation', '');
if (!empty($update_escalation)) {
	...
	if ($bsn_id > 0 && ($value == 'Yes' || $value == 'No')) {
		$businessTable = new Fw_Db_Table('business');
		$businessTable->setWhere('bsn_id = ' . $bsn_id);
		$updateData = array(
			'bsn_cou_escalation_required' => $value,
			'bsn_cou_escalation_user' => $user,
			'bsn_cou_escalation_date' => $dt,
		);
		if ($value == 'Yes') {
			$updateData['bsn_cou_escalation_yes_at'] = $now;
			$updateData['bsn_cou_escalation_yes_by'] = $user_id;
		} elseif ($value == 'No') {
			$updateData['bsn_cou_escalation_no_at'] = $now;
			$updateData['bsn_cou_escalation_no_by'] = $user_id;
		}
		$businessTable->updateRow($updateData);
	}
	exit;
}
...
$bsn_cou_urgency = $fwRequest->getParam('bsn_cou_urgency', '');
...
$fwViewData['bsn_cou_urgency'] = $bsn_cou_urgency;
if ($bsn_cou_urgency):
	$where .= " AND  business.bsn_cou_urgency = '" . $bsn_cou_urgency . "' ";
	$_SESSION['bsn_cou_urgency'] = $bsn_cou_urgency;
elseif ($_SESSION['bsn_cou_urgency']):
	$where .= " AND business.bsn_cou_urgency = '" . $_SESSION['bsn_cou_urgency'] . "' ";
endif;
...
$process_commenced = $fwRequest->getParam('process_commenced', '');
if ($process_commenced) {
	$keys_4 = array_keys($process_commenced);
	$ky_4 = $keys_4[0];
	$val_4 = $process_commenced[$ky_4];

	$detail['bs_cou_process_commenced'] = $val_4;
	$detail['bs_cou_process_commenced_user'] = $user;
	$detail['bs_cou_process_commenced_date'] = $dt;
...
$project_search = $fwRequest->getParam('project_search', '');
if ($project_search):
	$where .= " AND  business.bsn_name LIKE '%" . $project_search . "%'";

	$_SESSION['project_search'] = $project_search;
	$fwViewData['project_search'] = $_SESSION['project_search'];

elseif ($_SESSION['project_search']):

	$where .= " AND business.bsn_name LIKE '%" . $_SESSION['project_search'] . "%' ";
	$fwViewData['project_search'] = $_SESSION['project_search'];
endif;
...
$query = 'SELECT ccdrql_bsn_id, MAX(ccdrql_id) AS latest_id, (SELECT ccdrql_created_by FROM cou_component_document_report_qa_log WHERE ccdrql_bsn_id = CCDRQL.ccdrql_bsn_id ORDER BY ccdrql_id DESC LIMIT 1) AS ccdrql_created_by, (SELECT ccdrql_created_at FROM cou_component_document_report_qa_log WHERE ccdrql_bsn_id = CCDRQL.ccdrql_bsn_id ORDER BY ccdrql_id DESC LIMIT 1) AS ccdrql_created_at FROM `cou_component_document_report_qa_log` AS CCDRQL GROUP BY ccdrql_bsn_id;';
$result = $fwDb->query($query);
$cou_component_document_report_qa_log_array = [];
foreach($result as $row){
	$cou_component_document_report_qa_log_array[$row['ccdrql_bsn_id']] = [
		'user' => $users_array[$row['ccdrql_created_by']],
		'date' => date('d-M-Y', strtotime($row['ccdrql_created_at'])),
	];
}

$query = 'SELECT ccdrn_bsn_id, MAX(ccdrn_id) AS latest_id, (SELECT ccdrn_notes FROM cou_component_document_report_notes WHERE ccdrn_bsn_id = CCDRN.ccdrn_bsn_id ORDER BY ccdrn_id DESC LIMIT 1) AS ccdrn_notes,(SELECT ccdrn_created_by FROM cou_component_document_report_notes WHERE ccdrn_bsn_id = CCDRN.ccdrn_bsn_id ORDER BY ccdrn_id DESC LIMIT 1) AS ccdrn_created_by, (SELECT ccdrn_created_at FROM cou_component_document_report_notes WHERE ccdrn_bsn_id = CCDRN.ccdrn_bsn_id ORDER BY ccdrn_id DESC LIMIT 1) AS ccdrn_created_at FROM `cou_component_document_report_notes` AS CCDRN GROUP BY ccdrn_bsn_id;';
$result = $fwDb->query($query);
$cou_component_document_report_notes_array = [];
foreach($result as $row){
	$cou_component_document_report_notes_array[$row['ccdrn_bsn_id']] = [
		'note' => $row['ccdrn_notes'],
		'user' => $users_array[$row['ccdrn_created_by']],
		'date' => date('d-M-Y', strtotime($row['ccdrn_created_at'])),
	];
}
...
if (!empty($clear_search)) {
	...
	unset($project_search);
	$fwViewData['project_search'] = '';
	unset($status);
	unset($_SESSION['status']);
	unset($substatus);
	unset($_SESSION['substatus']);
	unset($bsn_cou_urgency);
	unset($_SESSION['bsn_cou_urgency']);
}
...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
		...
		business_sellers.bs_cou_comp_advice, business_sellers.bs_cou_comp_user, business_sellers.bs_cou_comp_date, business_sellers.bs_cou_cust_occupancy, business_sellers.bs_cou_process_commenced, business_sellers.bs_cou_process_commenced_user, business_sellers.bs_cou_process_commenced_date,
	    ...
	    AND business.bsn_cou_escalation_required = 'Yes' 
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			$sql653 = "Select document_check_list.doc_file_name , document_check_list.doc_not_required from document_check_list
			 where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 653";
			...
			$sql318 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required  from document_check_list
			       where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 318";
			...
			$sql679 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 679";
			...
			$sql678 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 678";
			...
			$sql777 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 777";
			...
			$sql783 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 783";
			...
			$sql657 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 657";
			...
			$sql627 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 627";
			...
			$sql382 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 382";
			...
			$sql272 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 272";
			...
			$sql655 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 655";
			...
			$sql788 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 788";
			...
			$sql795 = "Select document_check_list.doc_file_name, document_check_list.doc_not_required from document_check_list
			   where document_check_list.doc_bsn_id = " . $v['bsn_id'] . " And document_check_list.doc_name_id = 795";
			...
			$sql531 = "Select  business_tasks.bt_complete, business_tasks.bt_completed_date, users.user_name from business_tasks
					   inner join users on business_tasks.bt_completed_by = users.user_id
			  		   where business_tasks.bt_bsn_id = " . $v['bsn_id'] . " And business_tasks.bt_task_id = 531";
			$data531 = $fwDb->queryOne($sql531);
			...
			$setdata2[$k]['doc_file_name_653'] = $data653['doc_file_name'];
			...
			$setdata2[$k]['doc_file_name_679'] = $data679['doc_file_name'];
			$setdata2[$k]['doc_file_name_678'] = $data678['doc_file_name'];
			$setdata2[$k]['doc_file_name_777'] = $data777['doc_file_name'];
			$setdata2[$k]['doc_file_name_783'] = $data783['doc_file_name'];
			...
			$setdata2[$k]['doc_not_required_653'] = $data653['doc_not_required'];
			...
			$setdata2[$k]['doc_file_name_318'] = $data318['doc_file_name'];
			$setdata2[$k]['doc_not_required_318'] = $data318['doc_not_required'];
			$setdata2[$k]['doc_not_required_679'] = $data679['doc_not_required'];
			$setdata2[$k]['doc_not_required_678'] = $data678['doc_not_required'];
			$setdata2[$k]['doc_not_required_777'] = $data777['doc_not_required'];
			$setdata2[$k]['doc_not_required_783'] = $data783['doc_not_required'];
			
			$setdata2[$k]['doc_file_name_657'] = $data657['doc_file_name'];
			$setdata2[$k]['doc_not_required_657'] = $data657['doc_not_required'];
			
			$setdata2[$k]['doc_file_name_627'] = $data627['doc_file_name'];
			$setdata2[$k]['doc_not_required_627'] = $data627['doc_not_required'];
			
			$setdata2[$k]['doc_file_name_382'] = $data382['doc_file_name'];
			$setdata2[$k]['doc_not_required_382'] = $data382['doc_not_required'];
			
			$setdata2[$k]['doc_file_name_272'] = $data272['doc_file_name'];
			$setdata2[$k]['doc_not_required_272'] = $data272['doc_not_required'];
			
			$setdata2[$k]['doc_file_name_655'] = $data655['doc_file_name'];
			$setdata2[$k]['doc_not_required_655'] = $data655['doc_not_required'];
			
			$setdata2[$k]['doc_file_name_788'] = $data788['doc_file_name'];
			$setdata2[$k]['doc_not_required_788'] = $data788['doc_not_required'];
			
			$setdata2[$k]['doc_file_name_795'] = $data795['doc_file_name'];
			$setdata2[$k]['doc_not_required_795'] = $data795['doc_not_required'];

			$doc_653_background = '#999';
			if ($data653['doc_file_name']) {
				$doc_653_background = '#00CC33';
			} elseif ($data653['doc_not_required'] == 0) {
				$doc_653_background = '#F00';
			}

			$setdata2[$k]['doc_653_background'] = $doc_653_background;

			$setdata2[$k]['status'] = $status;
			$setdata2[$k]['dayscount'] = $dayscount;
			
			$setdata2[$k]['bt_complete'] = $data531['bt_complete'];
			$setdata2[$k]['user_name']   = $data531['user_name'];
			$setdata2[$k]['bt_completed_date']   =  changedate_d_m_y($data531['bt_completed_date']);
			

			$cou_component_document_report_qa_log_user = $cou_component_document_report_qa_log_date = '';
			if(isset($cou_component_document_report_qa_log_array[$v['bsn_id']])) {
				$cou_component_document_report_qa_log_user = $cou_component_document_report_qa_log_array[$v['bsn_id']]['user'];
				$cou_component_document_report_qa_log_date = $cou_component_document_report_qa_log_array[$v['bsn_id']]['date'];
			}
			$setdata2[$k]['cou_component_document_report_qa_log_user'] = $cou_component_document_report_qa_log_user;
			$setdata2[$k]['cou_component_document_report_qa_log_date'] = $cou_component_document_report_qa_log_date;

			$cou_component_document_report_notes_note = $cou_component_document_report_notes_user = $cou_component_document_report_notes_date = '';
			if(isset($cou_component_document_report_notes_array[$v['bsn_id']])) {
				$cou_component_document_report_notes_note = $cou_component_document_report_notes_array[$v['bsn_id']]['note'];
				$cou_component_document_report_notes_user = $cou_component_document_report_notes_array[$v['bsn_id']]['user'];
				$cou_component_document_report_notes_date = $cou_component_document_report_notes_array[$v['bsn_id']]['date'];
			}
...
$query = 'SELECT * FROM cou_component_document_report';
$result = $fwDb->query($query);
$ccdr_ids = [];
foreach ($result as $row) {
	if (!isset($ccdr_ids[$row['ccdr_bsn_id']])) {
		$ccdr_ids[$row['ccdr_bsn_id']] = [];
	}
	$ccdr_ids[$row['ccdr_bsn_id']][$row['ccdr_doc_id']] = $row['ccdr_id'];
}

$query = 'SELECT * FROM cou_component_document_report_companies';
$result = $fwDb->query($query);
$cou_component_document_report_companies = [];
foreach ($result as $row) {
	if (!isset($cou_component_document_report_companies[$row['ccdrc_ccdr_id']])) {
		$cou_component_document_report_companies[$row['ccdrc_ccdr_id']] = [];
	}
	$cou_component_document_report_companies[$row['ccdrc_ccdr_id']][] = $row;
}
$fwViewData['cou_component_document_report_companies'] = $cou_component_document_report_companies;
// db($ccdr_ids);
// db($cou_component_document_report_companies);
...
$listsnew = [];
foreach ($setdata2 as $list) {
	// $suppliers_653 = $suppliers_261 = [];
	
	// $ccdr_id_653 = $ccdr_ids[$list['bsn_id']][653] ?? 0;
	// if($ccdr_id_653 > 0 && isset($cou_component_document_report_companies[$ccdr_id_653])) {
	// 	$suppliers_653 = $cou_component_document_report_companies[$ccdr_id_653];
	// }

	// $ccdr_id_261 = $ccdr_ids[$list['bsn_id']][261] ?? 0;
	// if($ccdr_id_261 > 0 && isset($cou_component_document_report_companies[$ccdr_id_261])) {
	// 	$suppliers_261 = $cou_component_document_report_companies[$ccdr_id_261];
	// }

	// $list['suppliers_653'] = $suppliers_653;
	// $list['suppliers_261'] = $suppliers_261;
	$list['ccdr_ids'] = $ccdr_ids[$list['bsn_id']];
...
$query = 'SELECT AVG(DATEDIFF(bsn_cou_escalation_no_at, bsn_cou_escalation_yes_at)) AS average FROM business WHERE DATEDIFF(bsn_cou_escalation_no_at, bsn_cou_escalation_yes_at) > 0 AND bsn_cou_escalation_no_at > bsn_cou_escalation_yes_at;';
$result = $fwDb->queryOne($query);
$average_time_added_minus_removed = $result['average'];

$query = "SELECT bsn_id, bsn_cou_escalation_yes_at FROM business WHERE bsn_cou_escalation_required = 'Yes'";
$result = $fwDb->query($query);
...
foreach ($result as $row) {
	...
	$yes_at = $row['bsn_cou_escalation_yes_at'];
	if (strtotime($yes_at) > 0) {
		$yes_at = date('Y-m-d', strtotime($yes_at));
		$curdate_y_m_d = date('Y-m-d');
		$escalation_days_count = daysDifference($curdate_y_m_d, $yes_at);
	}
...
$fwViewData['title'] = "COU Component Escalation Report";
...
$sqlbus = "SELECT business.bsn_address from business_sellers Inner Join bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id Inner Join business ON business_sellers.bs_business_id = business.bsn_id where business.bsn_status like '%|3|%' Group By business.bsn_id order by business_sellers.bs_cou_sub_status DESC";
