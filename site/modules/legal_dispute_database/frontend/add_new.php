...
if (!empty($submit)) {
    ...
    $sql = "SELECT bsn_id, bsn_name, bsn_address from business where bsn_sub_status = 'Open' AND bsn_name = '" . $detail['ldd_bsn_id'] . "'";
...
if ($this_id > 0) {
	...
	$sql = "SELECT bsn_id, bsn_name, bsn_address from business where bsn_sub_status = 'Open' AND bsn_id = '" . $detail['ldd_bsn_id'] . "'";
    $res = $fwDb->queryOne($sql);
