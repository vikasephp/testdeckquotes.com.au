<?php
      ...
      if( !empty($postedData['file_name']) && !empty($postedData['module_name']) )
      {
          ...
          if($postedData['module_name'] == "development_log_ephp.home")
          {
              $folder_path = "files/development_log/";
          }
          
        $filetoinclude = $_SERVER['DOCUMENT_ROOT'].'/file_upload/server/s3/S3.php';
	    include_once $filetoinclude;
        
		$bucket_name  = BUCKET_NAME_V;  $object_name  = $postedData['file_name'];  $my_file_name = $postedData['file_name'];
