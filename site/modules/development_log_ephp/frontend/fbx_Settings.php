...
// module names
$MODULE_SINGULAR = 'Development Log - EPHP';
$MODULE_PLURAL =   'Development Log - EPHP';

$fwViewData['MODULE_SINGULAR'] = $MODULE_SINGULAR;
$fwViewData['MODULE_PLURAL'] = $MODULE_PLURAL;

// module table and id
$TABLE = 'development_log_ephp';
$ID = 'de_id';
...
function upload($filename, $tmpname)
{
	...
	include_once $filetoinclude;
        ...
        try {
			...
			$s3 = new S3(ACCESS_KEY_V, SECRET_KEY_V);
       
            $flag = 0;
            
            if ($s3->putObjectFile($path . "/" . $zipfile, BUCKET_NAME_V, 'files/development_log/'.$zipfile, S3::ACL_PRIVATE)) {
...
function create_sticker($heading, $html)
		{
...
return $filepath_url;
	...
	function upload_public($filename, $tmpname)
    {
	  $path = getcwd();
    //$path = 'd10n9vq3rdfybv.cloudfront.net';
	...
	$zipfile = $filename;
        ...
        try {
            ...
            if ($s3->putObjectFile($path . "/" . $zipfile, 'accessbucketnew', 'files/development_log/'.$zipfile, S3::ACL_PUBLIC_READ)) {
...
function create_dev_pdf($heading, $html, $fname)
		{
...
//$nameoffile = str_replace(" ", "_",$fname);		
$nameoffile = 'development_log_report';
$filename = $nameoffile."_".date('d_m_Y').".pdf";
$headtext = $heading." - ".date("d/m/Y");
// create new PDF document
$pdf = new MYPDF($PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false); 
...
//$pdf->SetTitle('Answer - ".date("d/m/Y")."<br>TurnKeyStudios Project Sales');
$pdf->SetTitle("CGFB");
$pdf->SetSubject('Development Log Report');
$pdf->SetKeywords('GCON, PDF');
