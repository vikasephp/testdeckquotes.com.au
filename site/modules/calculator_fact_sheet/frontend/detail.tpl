	...
	<tr>
        ...
        <textarea cols="1000" rows="30" name="fs[doc_content]" id="editor1" class="ckeditor" style="height:670px;">{{$detail.doc_content|stripslashes}}</textarea>    
          ...
          <tr>
            ...
            {{if $detail.doc_image}} <img src="{{$BASE_URL}}files/design_fact_sheet_doc/{{$detail.doc_image}}"  height="70" />
            <a href="{{$BASE_URL}}calculator_fact_sheet.detail/doc_id/{{$detail.$ID}}/docid/{{$detail.$ID}}" onclick="javascript:if(!confirm('Are you sure want to delete the image?')) return false;" title="Delete">
...
<script type="text/javascript" language="javascript" >
    ...
    frmvalidator.addValidation("fs[doc_name]","req", "Please specify document name.");
</script>
{{/if}}
