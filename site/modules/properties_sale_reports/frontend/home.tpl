...
<script>
    function open_popup(type) {
		...
		else if (type == 'manage_stage') {
			viewurl = "{{$BASE_URL}}{{ $BASEFOLDER }}.{{ $ATTRIBUTES.manage_stage.view_fle_name }}" + "/random/" + Math.random();
		}
    ...
    <div style="display: flex; justify-content: space-between; align-items: end;">
        <div style="margin-left: 10px; text-align: left;">
            <div>
				...
				<input type="button" name="stage" value="Stage" onclick="javascript:open_popup('manage_stage');" />
            </div>
            <div>
                <strong>Filter by Stage : </strong>
                <select name="filter_stage" style="width: 175px;">
                    <option value="">Select Stage</option>
                    {{ assign var=option_col value=$ATTRIBUTES.manage_stage.option_col }}
                    {{ assign var=option_id value=$ATTRIBUTES.manage_stage.id }}
    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
				...
				<th class="topmenu" align="center" valign="middle" data-col="task_checklist">Buyer</th>
				...
				<th class="topmenu" align="center" valign="middle" data-col="brochure">Brochure</th>
        ...
        <tbody>
            ...
            <tr bgcolor="{{ cycle values=" #D3E8D6,WHITE" advance=true }}">
				...
				<td data-col="buyer">
					<a href="/{{$BASEFOLDER}}.view_project/bsn_id/{{$item.ps_bsn_id}}" onclick="localStorage.setItem('activeTab','enquiries'); openTab('enquiries'); return false;" class="set3" style="color:#FFF;" target="_blank">
						View Enquiries
					</a>
				</td>
				<td data-col="price_guarantee">
					<input type="text" 
						name="ps_price_guarantee[{{$item.$ID}}]" 
						placeholder="Enter Price Guarantee" 
						value="{{$item.ps_price_guarantee}}" 
						oninput="update_price_guarantee({{$item.$ID}}, this.value)"
						/>
						
					<script>
					function update_price_guarantee(id, value) {
						$.ajax({
							type: 'POST',
							url: "{{$BASE_URL}}{{$BASEFOLDER}}.update_price_guarantee/ps_id/" + id + "/value/" + value,
				...
				<td data-col="brochure">{{$item.ps_brochure_link}}</td>
