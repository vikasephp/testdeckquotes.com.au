...
<div align="center" style="min-height:350px; z-index:9999999;">
	<h3 class="page-title"><br />View Enquiry Notes</h3>
	<form name="detail" method="post" action="" enctype="multipart/form-data">
		<div style="float:right; margin-right:10px; background: #09F; color:#FFF !important; padding:6px; margin-bottom:5px;">
			<a href="{{$BASE_URL}}{{$BASEFOLDER}}.add_en_notes/psre_id/{{$psre_id}}" class="various" title="Edit" style="color:#FFF; text-decoration:none; font-size:14px;">Add New Enquiry Notes</a>
		</div>
		<table id="list-table" width="99%">
			...
			{{if $data}}
				{{assign var="en_count" value=1}}
				{{foreach from=$data key="keyN" item="itemN"}}
				<tr>
					<td>{{$en_count}}</td>
					<td>{{$itemN.psren_notes}}</td>
					<td>
						{{$itemN.user_name}}
					</td>
					<td>{{$itemN.psren_created_at|date_format:"%d-%m-%Y"}}</td>
					<td>
						<a href="{{$BASE_URL}}{{$BASEFOLDER}}.add_en_notes/psre_id/{{$psre_id}}/psren_id/{{$itemN.psren_id}}" class="various" title="Edit"><img style="height:20px; width:20px;" src="{{$BASE_URL}}css/admin/images/edit.png" /></a>&nbsp;&nbsp;
						<a href="{{$BASE_URL}}{{$BASEFOLDER}}.delete_en_notes/psre_id/{{$psre_id}}/psren_id/{{$itemN.psren_id}}"
				...
				{{assign var="en_count" value=$en_count+1}}
				{{/foreach}}
			{{else}}
				<tr>
