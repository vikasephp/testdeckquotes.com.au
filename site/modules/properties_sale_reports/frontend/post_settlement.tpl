<div style="float:left; text-align:left;">
	<input type="button" name="Exchange Task List Unique" value="Unique Task List" onclick="javascript:open_url('post_settlement_task_list_specific');" />
...
<div class="" style="float:right;">
	<input type="button" name="Exchange Task List" value="Task List" onclick="javascript:open_url('post_settlement_task_list');" />
...
<table id="list-table">
	...
	<tbody>
		{{if $data_psri_postsTable}}
		{{assign var="psri_post" value=1}}
		{{foreach from=$data_psri_postsTable key="key" item="item"}}
		<tr bgcolor='{{ cycle values=" #D3E8D6,WHITE" advance=true }}'>
			<td>{{$psri_post}}</td>
			<td style="display:none;">{{if $item.psrpstl_key}}{{$item.psrpstl_key}}-{{/if}}{{$item.psrpstl_uid}}</td>
			<td>{{$item.psrpstl_sequence}}</td>
			<td>
				{{if $item.psrpstl_procedure_link}}
					<a href="{{$item.psrpstl_procedure_link}}" target="_blank">Link</a>
				{{/if}}
			</td>
			<td>
				{{$item.psrpstl_task_description}}
			</td>
			<td>
				{{$item.psrpstl_person_responsible}}
			</td>
			<td>
				...
				{{foreach from=$prst_psr_complete item=item_lc}}
					{{if $item_lc.psrtc_uid_id eq $item.psrpstl_uid}}
				...
				<form name="recmet3" method="post" action="">
					<input type="hidden" name="psr_complete[{{$item.psrpstl_uid}}]" value="0">
					<input type="hidden" name="psr_complete[psrtc_key]" value="post_setl">
					<input type="hidden" name="psr_complete[bsn_id]" value="{{$bsn_id}}">
					<label class="switch">
						<input class="switch-input" type="checkbox" name="psr_complete[{{$item.psrpstl_uid}}]" value="1" onclick="this.form.submit();" {{if $completed eq 1}} checked="checked" {{/if}} />
		...
		{{assign var="psri_post" value=$psri_post+1}}
		{{/foreach}}
		{{else}}
		<tr>
