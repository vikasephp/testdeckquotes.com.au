<div style="float:left; text-align:left;">
	<input type="button" name="Exchange Task List Unique" value="Unique Task List" onclick="javascript:open_url('exchange_task_list_specific');" />
...
<div class="" style="float:right;">
	<input type="button" name="Exchange Task List" value="Task List" onclick="javascript:open_url('exchange_task_list');" />
...
<table id="list-table">
	...
	<tbody>
		{{if $data_psri_eTable}}
		{{assign var="psri_e" value=1}}
		{{foreach from=$data_psri_eTable key="key" item="item"}}
		<tr bgcolor='{{ cycle values=" #D3E8D6,WHITE" advance=true }}'>
			<td>{{$psri_e}}</td>
			<td style="display:none;">{{if $item.psretl_key}}{{$item.psretl_key}}-{{/if}}{{$item.psretl_uid}}</td>
			<td>{{$item.psretl_sequence}}</td>
			<td>
				{{if $item.psretl_procedure_link}}
					<a href="{{$item.psretl_procedure_link}}" target="_blank">Link</a>
				{{/if}}
			</td>
			<td>
				{{$item.psretl_task_description}}
			</td>
			<td>
				{{$item.psretl_person_responsible}}
			</td>
			<td>
				...
				{{foreach from=$exc_psr_complete item=item_lc}}
					{{if $item_lc.psrtc_uid_id eq $item.psretl_uid}}
				...
				<form name="recmet3" method="post" action="">
					<input type="hidden" name="psr_complete[{{$item.psretl_uid}}]" value="0">
					<input type="hidden" name="psr_complete[psrtc_key]" value="exchange">
					<input type="hidden" name="psr_complete[bsn_id]" value="{{$bsn_id}}">
					<label class="switch">
						<input class="switch-input" type="checkbox" name="psr_complete[{{$item.psretl_uid}}]" value="1" onclick="this.form.submit();" {{if $completed eq 1}} checked="checked" {{/if}} />
		...
		{{assign var="psri_e" value=$psri_e+1}}
		{{/foreach}}
		{{else}}
		<tr>
