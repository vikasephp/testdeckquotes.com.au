<div style="float:left; text-align:left;">
	<input type="button" name="Preparing for Market Task List Unique" value="Unique Task List" onclick="javascript:open_url('prep_market_task_list_specific');" />
...
<div class="" style="float:right;">
	<input type="button" name="Preparing for Market Task List" value="Task List" onclick="javascript:open_url('prep_market_task_list');" />
...
<table id="list-table">
	...
	<tbody>
		{{if $data_psri_pmTable}}
		{{assign var="psri_pm" value=1}}
		{{foreach from=$data_psri_pmTable key="key" item="item"}}
		<tr bgcolor='{{ cycle values=" #D3E8D6,WHITE" advance=true }}'>
			<td>{{$psri_pm}}</td>
			<td style="display:none;">{{if $item.psrpmtl_key}}{{$item.psrpmtl_key}}-{{/if}}{{$item.psrpmtl_uid}}</td>
			<td>{{$item.psrpmtl_sequence}}</td>
			<td>
				{{if $item.psrpmtl_procedure_link}}
					<a href="{{$item.psrpmtl_procedure_link}}" target="_blank">Link</a>
				{{/if}}
			</td>
			<td>
				{{$item.psrpmtl_task_description}}
			</td>
			<td>
				{{$item.psrpmtl_person_responsible}}
			</td>
			<td>
				...
				{{foreach from=$prmar_psr_complete item=item_lc}}
					{{if $item_lc.psrtc_uid_id eq $item.psrpmtl_uid}}
				...
				<form name="recmet3" method="post" action="">
					<input type="hidden" name="psr_complete[{{$item.psrpmtl_uid}}]" value="0">
					<input type="hidden" name="psr_complete[psrtc_key]" value="prep_mark">
					<input type="hidden" name="psr_complete[bsn_id]" value="{{$bsn_id}}">
					<label class="switch">
						<input class="switch-input" type="checkbox" name="psr_complete[{{$item.psrpmtl_uid}}]" value="1" onclick="this.form.submit();" {{if $completed eq 1}} checked="checked" {{/if}} />
		...
		{{assign var="psri_pm" value=$psri_pm+1}}
		{{/foreach}}
		{{else}}
		<tr>
