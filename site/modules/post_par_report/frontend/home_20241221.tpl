      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
       ...
       <td>
                ...
                <div style="width:100%; margin-top:15px;">
   				 ...
   				 <input type="range" name = "rating[{{$item.bsn_id}}]" value="{{$item.bsn_rating}}" min="0" max="5" data-rangeSlider>
       ...
       <td>
       {{if $item.datarm}}
        {{foreach from=$item.datarm key="key" item="item"}}
       	<a href = "{{$BASE_URL}}files/recorded_meetings/{{$item.rm_original_file}}" target="_blank">{{$item.rm_meeting_file}} <br>
