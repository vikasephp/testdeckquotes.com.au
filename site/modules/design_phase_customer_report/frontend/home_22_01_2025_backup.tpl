...
<script language="javascript">
    ...
    function add_procedure() {
        var viewurl = "{{$BASE_URL}}planning_customer_report.view_procedure" + "/random/" + Math.random();
...
<table id="list-table" class="nav-back" width="100%">
    <thead>
        <tr>
            ...
            <th class="topmenu" align="center" valign="middle" width="7%">Users</th>
            ...
            <th class="topmenu" align="center" valign="middle" width="14%">Link</th>
            ...
            <th class="topmenu" align="center" valign="middle" width="10%">Checklist</th>
        </tr>
    </thead>
    <tbody>
        ...
        <tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
            ...
            <td style="text-align: center;">
                <a id="bcust__name_{{$item.bsn_id}}_{{$item.bcust_id}}" href="{{ $item.customer_link }}" target="_blank">Link</a>
            </td>
            ...
            <td>
                <form name="checklist_form" method="post" action="">
                    ...
                    <table>
                        {{foreach from=$item.checklist key="key" item="item"}}
                        <tr>
                            <td>{{$item.dpn_unique_id}}</td>
                            <td>
                                <label class="switch">
                                    <input class="switch-input" {{if $item.checklist_status==1}} checked {{/if}}
                                        onchange="update_checklist_on_off('{{$item.dpn_id}}',this)" type="checkbox" />
                                    <span class="switch-label" data-on="Yes" data-off="No"></span>
                                    <span class="switch-handle"></span> <br />
                                </label>
                            </td>
...
<script>
    ...
    function update_date(record_id, t, bcust_id) {
        ...
        $.ajax({
            type: "POST",
            url: "{{$BASE_URL}}planning_customer_report.ajax_handler",
            data: formData,
            dataType: "json",
            encode: true,
