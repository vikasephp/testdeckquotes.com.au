...
$tableSupplier = new Fw_Db_Table('supplier_warranty');
$tableSupemail = new Fw_Db_Table('supplier_email_warranty');
...
$matsql = "SELECT contacts.* , companies.co_company_name, type_options.to_option  from contacts 
	  	  ...
	  	  where companies.co_id = " . $co_id . " AND length(cs_primary_email) > 0 order by contacts.cs_is_primary desc ";
$list = $fwDb->query($matsql);
foreach ($list as $ck => $vc) {
	$sqlc = "Select * from supplier_email_warranty where se_email = '" . $vc['cs_primary_email'] . "' and se_wa_id = " . $wa_id;
...
if (!empty($add_emails)) {
	...
	foreach ($emails as $k => $v) {
		...
		if ($v == 'on') {
			$firstName = $data['cs_first_name'];
			$lastName = $data['cs_surname'];
		...
		//{
		$tableSupemail->setWhere("se_email = '" . $emailA . "' and se_wa_id =" . $wa_id . " AND se_first_name = '$firstName' AND se_surname = '$lastName'");
		if (!$tableSupemail->rowExists()) {
			if ($v == 'on') {
				...
				// send email
				$sqlWl = "SELECT wa_id, wa_project, wa_problem from warranty_log where wa_id = " . $wa_id;
				$wlData = $fwDb->queryOne($sqlWl);
				$message = '<html><body>';
				$message .= '<p>Hi ' . $data['cs_first_name'] . '</p><br>';
				$message .= '<p>A new warranty issue has been assigned to you and your company. Please see the below details:</p>';
				$message .= '<p><strong>Warranty ID:</strong> ' . $wlData['wa_id'] . '</p>';
				$message .= '<p><strong>Project Address:</strong> ' . $wlData['wa_project'] . '</p>';
				$message .= '<p><strong>Problem:</strong> ' . $wlData['wa_problem'] . '</p><br>';
				$message .= '<p>Regards,</p>';
				$message .= '<p>CGFB Warranty Team</p>';

				$to = $emailA;
				$toname = $data['cs_first_name'] . ' ' . $data['cs_surname'];
				$subject = 'New Warranty Issue Assigned';

				//$to = 'manojsoniephp@gmail.com';
				send_email($toname, $to, 'CGFB Warranty Team', 'warranty@cgfb.com.au', $subject, $message, $attachment = '');
			}
		} elseif ($v == 'off') {
	...
	$sqlAdd = "Select se_email from supplier_email_warranty where se_wa_id = " . $wa_id;
