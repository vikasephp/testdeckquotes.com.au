...
if (!empty($submit)) {
	...
	$files = $_FILES['supp_attach'];
	foreach ($files['name'] as $index => $file) {
		$docfile_1 = $files['name'][$index];
		$file_type = $files['type'][$index];
		$datetime = trim(date('d_m_Y_H_i_'), '0');
		$docfile_1 = preg_replace('/[^A-Z0-9._]/i', '_', $docfile_1);
		$src = $files['tmp_name'][$index];
		$response = upload($docfile_1, $src, 'warranty_log.notes');
		if (!$response) {
			echo "Possible file upload attack";
		} else {
			$tableAttachment = new Fw_Db_Table('warranty_log_note_attachments');
			$attachmentData = [
				'wna_wn_id' => $wna_wn_id,
				'wna_attachment' => $docfile_1,
			];
			$res = $tableAttachment->insertRow($attachmentData);
		}
	}
	$fwViewData['opr'] = $opr;
}
