		...
		<td>
			...
			<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
				...
				<div style="float:left; margin-left:8px;">
					...
					<br />
					<strong> Rec. Number : </strong> <input type="text" name="recno" value="{{$recno}}"/>
					<br />
					...
					<strong> Require Escalation : </strong>
					...
					<select name="search_by_cgfb_responsible">
						...
						<option value="{{$item6.wc_id}}" {{if $cgfb_responsible eq $item6.wc_id}} selected="selected"
							{{/if}}>{{$item6.wc_cgfb_option}}</option>
						{{/foreach}}
					</select>
					...
					&nbsp;&nbsp;
					<span style="border:1px solid #0CF; padding:3px;">Unallocated Logs :
						{{$tot_unallocated.total_unallocated}}</span>
					&nbsp;&nbsp;
					<span style="border:1px solid #0CF; padding:3px;">Post Walkthrough Logs :
						{{$post_walkthrough_count}}</span>
					&nbsp;&nbsp;
					<span style="border:1px solid #0CF; padding:3px;">Walkthrough Logs :
						{{$walkthrough_log_count}}</span>
					<br />
					<!--<strong> Supplier Search  : </strong> <input type="text" name="supp_name" width="100" /> <input type="submit" name="supplier_o" value = "Search" />-->
					<input type="submit" name="printOP" value="Print Merge – Open and Pending"
						style="background:#00CC33;" />
					&nbsp;&nbsp;
					<input type="submit" name="printOPimgthumb" value="Print Merge – Open and Pending - Photos"
						style="background:#00CC33;" />
					<br />
				...
				<div style="float:right; text-align:right;">
					...
					<input type="button" name="priority" value="Edit Require Escalation"
						onclick="javascript:add_priority();" />
					<br />

					<div style="border:1px solid #F00; display:inline-block; padding:10px; background: #CCC;">
						<strong> Select Require Escalation : </strong>
						...
						<input type="submit" name="filter_priority" value="Filter By Require Escalation" />
						<input type="submit" name="print_filter"
							value="Print Merge For Filter by Require Escalation - Open" style="background:#F00;" />
					</div>
	...
	<table id="list-table" class="nav-back" width="100%">
		<thead>
			<tr>
				...
				<th class="topmenu" align="center" valign="middle" width="3%">&nbsp;</th>
				...
				<th class="topmenu" align="center" valign="middle" width="5%">Require Escalation</th>
				...
				<th class="topmenu" align="center" valign="middle" width="9%">Inspection Photo</th>
				...
				<th class="topmenu" align="center" valign="middle" width="4%" style="display: none;">Liability Accepted
				</th>
		...
		<tbody>
			...
			<tr {{if $item.wa_status eq 'Pending' }} bgcolor="#f7b900" {{elseif $item.wa_include_supplier eq '' }}
				...
				<td>{{$item.wa_id}}</td>
				<td>
					<!-- Always make modification in view_log_detail of warranty_log2 -->
					<a class="custom_logview" href="{{$BASE_URL}}warranty_log2.view_log_detail/wa_id/{{$item.wa_id}}"
						target="_blank">Log View</a>
				</td>
				<td>
					{{$item.project_link}}
				</td>
				...
				{{else }}
					...
					<td>
						<div style="display: flex; flex-direction: column;">
							{{assign var=id value=$item.wa_id}}
					...
					<td>
						<div style="display: flex; flex-direction: column;">
							{{assign var=id value=$item.wa_id}}
							{{assign var=sn value=1}}
							{{assign var=files value=$inspection_photos.$id}}
							{{foreach from=$files key="index" item="row"}}
								<div>
									Photo {{$sn}} : <a href="{{$row.url}}" target="_blank">Download</a>
								</div>
					...
					<td style="min-width: 400px; max-width: 400px;" class="notesContent" {{if $item.cell_red eq 1}}
						style="background:#F00;" {{/if}}>
						<div style="width: 100%; overflow: auto;">
							{{$item.notes_text}}
							<br>
							<a href="{{$BASE_URL}}warranty_log.view_notes/wa_id/{{$item.wa_id}}"
					...
					<td>
						...
						<a href="{{$BASE_URL}}warranty_log.status_approve/wa_id/{{$item.wa_id}}" class="various">{{$item.wa_status}}</a>
						{{else}}
							{{$item.wa_status}}
							{{if $item.wa_status eq 'Closed'}}
								{{if $item.wa_status_user}}
									<br/>{{$item.wa_status_user}}
								{{/if}}
								<br />
								<!-- This is comparison to fetch latest date and from now onwards we are storing date in column named wa_status_date only -->
								{{assign var="sd" value=$item.wa_status_date|default:''}}
								{{assign var="scd" value=$item.wa_status_changed_date|default:''}}
					...
					<td style="pointer-events: none;">
						<label class="switch">
							<input class="switch-input" type="checkbox" name="outstanding_invoices[{{$item.wa_id}}]"
								{{if $item.outstanding_invoices eq 1 }} checked="checked" {{/if}} disabled>
							<span class="switch-label" data-on="Yes" data-off="No"></span>
							<span class="switch-handle"></span> <br>
						</label>
					</td>
						...
						<form name="recmet3" method="post" action="">
							...
							<label class="switch">
								<input class="switch-input" type="checkbox" name="invoices_paid[{{$item.wa_id}}]"
									onclick="update_invoices_paid({{$item.wa_id}},this.checked);" {{if
									$item.wa_invoices_paid eq 1 }} checked="checked" {{/if}} />
								<span class="switch-label" data-on="Yes" data-off="No"></span>
								<span class="switch-handle"></span> <br />
							</label>
						</form>
