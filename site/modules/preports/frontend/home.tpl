...
function add_new()
 {
var viewurl = "{{$BASE_URL}}preports.detail"+ "/random/" + Math.random();	
 $(document).ready(function () {
        $.fancybox({
				'width': '98%',
				'height': '80%',
    ...
    <div >
      ...
      &nbsp; 
        ...
        <option value="0"> -- All -- </option>
         <option value="1" {{if $filters.report_type == 1}} selected="selected" {{/if}}>All Dates</option>
        <option value="2" {{if $filters.report_type == 2}} selected="selected" {{/if}}>Selected Date</option>
...
<div style="float:right; width:20%; padding:0; "><input type="button" name="Report" onclick="javascript:add_new();" value="Generate Report" disabled="disabled" /></div>
    ...
    <tbody>
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
        ...
        <td>{{$item.ur_report_name}}</td>
         <td>{{if $item.ur_type == 1}}All Dates{{else}}Selected Dates{{/if}}</td>
