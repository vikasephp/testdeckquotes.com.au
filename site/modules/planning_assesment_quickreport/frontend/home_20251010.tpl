...
<form name="mshort" action="{{$BASE_URL}}{{$XFA.home}}" method="post">
	...
	<input type="submit" value="Clear All" name="clear" /> &nbsp;&nbsp;</div>
...
<form name="mlist" method="post">
	...
	<div style="float:left;"> &nbsp; &nbsp; <strong>Search:</strong>&nbsp;&nbsp;
		...
		&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;<span style="border:1px solid #0CF; padding:3px;"> 7 Days PAR Booked : {{if $parbooked_data_weekly}}{{$parbooked_data_weekly.weekly_total}}{{/if}} </span>
		&nbsp;&nbsp;<span style="border:1px solid #0CF; padding:3px;"> YTD PAR Booked : {{if $parbooked_data_ytd}}{{$parbooked_data_ytd.ytd_total}}{{/if}}  </span>
			...
			<table id="doclist-table" class="nav-back" width="100%">
				<thead>
					<tr>
						...
						<th class="topmenu" align="center" valign="middle" width="5%" data-col="property_report">Property Report</th>
				...
				<tbody>
					...
					<tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
						...
						<td {{if $item.bs_paqr_alertoption eq 2}} style="background:#F00;" {{/if}} {{if
							$item.bs_paqr_alertoption eq 6}} style="background:#0F0;" {{/if}}>
							<select name="alert[{{$item.bsn_id}}]"
								Onchange="update_alert_option({{$item.bsn_id}},this.value)">
								{{foreach from = $alertdata key = "keya" item = "itema"}}
								<option value="{{$itema.pa_id}}" {{if $itema.pa_id eq $item.bs_paqr_alertoption}}
