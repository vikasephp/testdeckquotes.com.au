...
/*
//Filter for Meeting Occurred
$mo = $fwRequest->getParam('meetingoccur', '');
$clearmo = $fwRequest->getParam('clearmo', '');
...
if (!empty($mo) || isset($_SESSION['mo'])) {
	...
	$where = "WHERE ((bsn_sub_status = 'Open' AND bsn_planning_moveto_old <> 1) OR 
         (bsn_sub_status IN ('Hold', 'Lost') AND bsn_planning_moveto_old =  0 ) ) AND
		 business_sellers.bs_paqr_meeting_o = 1";
	$_SESSION['mo'] = 1;
}
*/
...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
	    ...
	    business_sellers.bs_qr_dm_date, business_sellers.bs_paqr_alertoption, business_sellers.bs_paqr_sqm, business_sellers.bs_paqr_meeting_o,
...
if ($plreport_not == 1 || $_SESSION['plreport_not'] == 1) {
	...
	business_sellers.bs_paqr_alertoption,  business_sellers.bs_qr_dm_user,
