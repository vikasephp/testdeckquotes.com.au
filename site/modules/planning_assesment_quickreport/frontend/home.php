...
// This is for meeting occurred button

$submit_view_meeting_occurred_records = $fwRequest->getParam('submit_view_meeting_occurred_records', null);
$view_meeting_occurred_records = $fwRequest->getParam('view_meeting_occurred_records', 'off');
if($submit_view_meeting_occurred_records) {
	$_SESSION['view_meeting_occurred_records'] = $view_meeting_occurred_records;
}
...
if (!empty($dm) || isset($_SESSION['dm'])) {
	...
	$where = "WHERE business.bsn_state <> 'NSW' AND business.bsn_type <> 27 AND ((bsn_sub_status = 'Open' AND bsn_planning_moveto_old <> 1) OR 
...
$view_meeting_occurred_records = $_SESSION['view_meeting_occurred_records'] ?? 'off';
if($view_meeting_occurred_records == 'on') {
	$where .= " AND business_sellers.bs_paqr_meeting_o = 1 ";
}
...
$fwViewData['view_meeting_occurred_records'] = $view_meeting_occurred_records;
