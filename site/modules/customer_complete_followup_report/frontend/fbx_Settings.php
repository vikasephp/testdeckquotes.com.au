...
// module names
$MODULE_SINGULAR = 'Customer Complete Followup Report';
$MODULE_PLURAL =   'Customer Complete Followup Report';

$fwViewData['MODULE_SINGULAR'] = $MODULE_SINGULAR;
$fwViewData['MODULE_PLURAL'] = $MODULE_PLURAL;

// module table and id
$TABLE = 'customer_comp_followup_report';
$ID = 'cc_id';
	...
	include_once $filetoinclude;
        ...
        try {
            if(move_uploaded_file($tmpname, $path .'/'.  basename($zipfile)))
				...
				//$s3 = new S3('AKIASLLJG4MZYZI2HUEG', 'WFdaeWkU7nR73PAQPcHyHo3MoGSm4iX11YatbKzO');

            //print_r($s3);exit;
            
            $flag = 0;
            
            if ($s3->putObjectFile($path . "/" . $zipfile, BUCKET_NAME, 'files/complete_followup/'.$zipfile, S3::ACL_PRIVATE)) {
		  //  if ($s3->putObjectFile($path . "/" . $zipfile, 'deckquote-bucket', 'files/complete_followup/'.$zipfile, S3::ACL_PRIVATE)) {		
...
function create_roofing_PDF($heading, $html,$id)
{
