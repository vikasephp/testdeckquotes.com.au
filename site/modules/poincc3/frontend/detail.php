	...
	unset($detail[$ID]);
		...
		if($_FILES['docs_2']['name'])
		{
			...
			if($this_id)
				...
				{
							$sql = "select po_id, po_unique_id  from poincc3 order by po_id DESC limit 1";
							$urnData =  $fwDb->queryOne($sql);
							
							if(empty($urnData)) {
								 $detail['po_unique_id'] = '0001'; 
								 $detail['po_urn'] = 'OS'.$detail['po_unique_id'];
						     }else { 
								$detail['po_unique_id'] =  (int)$urnData['po_unique_id'] + 1;
								
								$inr = (int)$urnData['po_unique_id'];
								$inr++;	
