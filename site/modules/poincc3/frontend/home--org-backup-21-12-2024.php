...
{
   $thisTable = new Fw_Db_Table("poincc3_merge_po_files");	$thisTable->setWhere("`is_downloaded` = 'YES'");
   $thisTable->deleteRows();
    ...
    if(is_file($zip_file_path)){ unlink($zip_file_path); } 
}

$fetch_po_r = "SELECT SQL_CALC_FOUND_ROWS null as row_name, poincc3.* FROM poincc3 WHERE 1=1 AND po_invoice_stage = ".$inv_stg." ORDER BY STR_TO_DATE(poincc3.po_invoice_date,'%d-%m-%Y') DESC";
...
{
    $zip = new ZipArchive; $fileName = 'poincc3_po_pdf_list_'.date("d-m-y").'.zip'; 
    $zip->open($fileName, ZipArchive::CREATE);  //echo "<pre>"; print_r($files_list);
    $pathdir = $_SERVER['DOCUMENT_ROOT']."/files/poincc3/po_files_list/";  $dir = opendir($pathdir); 
    ...
    if( count(glob("$pathdir/*")) > 0 )
    {
        ...
        $zip->close(); $zip_file_path = $_SERVER['DOCUMENT_ROOT'].'/site/modules/poincc3/frontend/'.$fileName;
...
/*$pdf_jurosh->addPDF('path/to/source/file.pdf', 'all', 'vertical')
  ...
  $fn = "merged_poincc3_PO_MATCH"."_".$dt.".pdf";  $fn = "just_checking.pdf";
   //$lsql = $fwRequest->getParam('sql', '');  $sqlmerge = stripslashes($lsql);
