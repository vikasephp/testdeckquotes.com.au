...
{
	var viewurl = "{{$BASE_URL}}poincc3.alert"+ "/po_id/" + po_id;	
...
{
	var viewurl = "{{$BASE_URL}}poincc3.view_procedure"+ "/random/" + Math.random();	
...
function add_new()
 {
var viewurl = "{{$BASE_URL}}poincc3.detail"+ "/random/" + Math.random();	
 $(document).ready(function () {
        $.fancybox({
				'width': '85%',
				'height': '85%',
...
function add_stage()
{
	var viewurl = "{{$BASE_URL}}poincc3.view_stage"+ "/random/" + Math.random();	
...
function view_list()
{
	var viewurl = "{{$BASE_URL}}poincc3.merged_invoice_list"+ "/random/" + Math.random();	
...
function view_approver() {
	var viewurl = "{{$BASE_URL}}poincc3.view_approver" + "/random/" + Math.random();
...
.div2 {width:2280px; height: 1100px; overflow: auto;}   
...
{{if $po_number_assign_alert eq 'true' }} <span style="color:#549933; font-weight:bold;font-size: 16px;">PO number have been assigned to the pdf's Successfully.</span> {{/if}}
<br />
...
<div class="multiselect" >
    ...
    {{foreach from=$projlist key="key" item="item"}}
        ...
        <input type="checkbox" id="one_{{$item.po_id}}" name="project[{{$item.project_number}}]" value="{{$item.project_number}}" />{{$item.project_number}}</label>
    {{/foreach}}
    </div>
</div>
...
&nbsp; <input type="button" name="stage_options" value="Approver Admin" onclick="javascript:view_approver();">
...
<input type="submit" value="Start Download and Assign PO Number" name="start_download_and_assign_po_number" {{$class8}}   id ="start_download_and_assign_po_number"/><br /><br />
...
<input type="submit" value="Download all files as zip" name="download_all_files_as_zip" id="dafaz_submitBtn"> <br /><br /> 

<input type="button" name="merge_inv_list" value="Printed/Saved Merged Report" onclick="javascript:view_list();" /> <br />
...
{{if $download eq 'true' }}
 <a href = "{{$BASE_URL}}files/merged_poincc3/{{$fn}}" target="_blank" > 
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
        ...
        <td>
            ...
            {{if $item.po_invoice_doc}}
                <a href="/poincc3.download_content?file_name={{$item.po_invoice_doc}}&module_name=poincc3.home" >Download</a> 
        ...
        <form name ="recmet2" method="post" action="">
          <select name="invoice_stage[{{$item.po_id}}]" onChange="update_stage({{$item.po_id}}, this.value)"/>
        			
                <option value="0" {{if $item.wa_priority eq -1}} selected="selected" {{/if}}>Please Select </option>
                {{foreach from = $isdetail key="key2" item="item2"}}
                <option value="{{$item2.is_id}}" {{if $item2.is_id eq $item.po_invoice_stage}} selected="selected" {{/if}}>{{$item2.is_option}}</option>
                {{/foreach}}
                     
          </select> <br />
       <span style="color: #FFF;">{{$item.po_inv_stage_user}} <br />{{$item.po_inv_stage_date}}</span>
		...
		function update_stage(id,value)
		{
			var cond = '#cond'+id;
			 ...
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}poincc3.update_stage/po_id/"+id+"/value/"+value,
...
<!--        <td>
       m
        ...
        <a href="/poincc3.download_content?file_name={{$item.po_purchase_order_doc}}&module_name=poincc3.home" >Download</a>{{/if}}
        </td>-->
        ...
        <input type="text" name="po_chartof_acc[{{$item.po_id}}]"  value="{{$item.po_chartof_acc}}"  style="width:60px;" /> 
         ...
         <td>{{if $item.po_purchase_order_no}} Yes {{else}} No {{/if}}</td>
		...
		<td>
			...
			function update_approver(id, value) {
				$.ajax({
					type: 'POST',
					url: "{{$BASE_URL}}poincc3.update_approver/po_id/" + id + "/value/" + value,
		...
		<td>
			...
			function update_approval_date(id, column, value) {
				...
				$.ajax({
					type: 'POST',
					url: "{{$BASE_URL}}poincc3.update_approval_date/po_id/" + id + "/column/" + column + "/value/" + value,
					success: function(response) {
