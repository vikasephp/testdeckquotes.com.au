...
// module names
$MODULE_SINGULAR = '165 Report 2025';
$MODULE_PLURAL =   '165 Report 2025';

$fwViewData['MODULE_SINGULAR'] = $MODULE_SINGULAR;
$fwViewData['MODULE_PLURAL'] = $MODULE_PLURAL;

// module table and id
$TABLE = '2025_165_report';
$ID = 'os2_id';
...
function upload($filename, $tmpname)
{
    ...
    try {
        // echo "$tmpname | $path | $filename"; die();
        if (move_uploaded_file($tmpname, $path . '/' .  basename($zipfile))) {
            $s3 = new S3(ACCESS_KEY, SECRET_KEY);

            $flag = 0;

            if ($s3->putObjectFile($path . "/" . $zipfile, BUCKET_NAME, 'files/uploads/' . $zipfile, S3::ACL_PRIVATE)) {
...
function create_2025_165_report($heading,$total_records, $html)
{
    ...
    public function Header(){
        ...
        $image_file = K_PATH_IMAGES.$this->header_logo_2;
        $this->Image($image_file, 15, 10, 35, '', 'PNG', '', 'T', false, 300, '', false, false, 0, false, false, false);
        
        $heading = "165 Breach Summary Report";
        $html = '<table width="40%" align="center">
                  <tr><td><h2>'.$heading.'</h2></td></tr>
                 <tr><td><h4>Total records : '.$this->total_records.'</h4></td></tr>
                 <tr><td><h4>Canberra Granny Flat Builders</h4></td></tr>
             </table>';
...
$filepath = 'files/2025_165_report/full_report'.$filename;
