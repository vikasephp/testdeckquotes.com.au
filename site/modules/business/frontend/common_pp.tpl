...
<div style="font-style:bold; text-align:left; padding:5px;">
	...
	<form name="tablename" method="post" action="">
		...
		<select name="pp[pd_checklist]" />
		<option value="0">Please Select</option>
		{{foreach from = $datachk key="key6" item="item6"}}
		<option value="{{$item6.dpn_unique_id}}" {{if $item6.dpn_unique_id eq $ppdata.pd_checklist}} selected="selected"
			{{/if}}>{{$item6.dpn_unique_id}}</option>
		{{/foreach}}
        </select>
        <br />
		...
		&nbsp; &nbsp;&nbsp;&nbsp;
		<div style="border:1px solid #9CF; display:inline-block; padding:5px; font-weight:bold;">
			LMP DC
			&nbsp; &nbsp;&nbsp;&nbsp;
			{{if $data491.doc_file_name}}
				<a href="/business.download_content?file_name={{$data491.doc_file_name}}&module_name=business.home" target="_blank">Download</a>
				&nbsp;&nbsp;&nbsp;&nbsp;
				{{$data491.doc_date_uploaded|date_format:"%e-%m-%Y"}}
				&nbsp;&nbsp;&nbsp;&nbsp;
			{{$data491.user_name}}
			{{else}}
		...
		<div style="border:1px solid #9CF; display: inline-block; padding:5px; font-weight:bold; margin-left: 5px;">
			{{ foreach from=$data20 item=row key=index }}
				LMP
				&nbsp; &nbsp;&nbsp;&nbsp;
				{{ $row.dpn_unique_id }}
				&nbsp; &nbsp;&nbsp;&nbsp;
				{{ if $row.doc_file_name }}
					<a href="/business.download_content?file_name={{$row.doc_file_name}}&module_name=business.home" target="_blank">Download</a>
					&nbsp;&nbsp;&nbsp;&nbsp;
					{{$row.doc_date_uploaded|date_format:"%e-%m-%Y"}}
					&nbsp;&nbsp;&nbsp;&nbsp;
				{{ $row.user_name }}
				{{ else }}
					No Document Uploaded
