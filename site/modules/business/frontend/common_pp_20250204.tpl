...
<table cellpadding="5" border="1" cellspacing="0" width="100%">
	<tr>
		...
		<th width="8%" data-sn="7">QA1</th>
		<th width="8%" data-sn="8">QA2</th>
		<th width="8%" data-sn="9">Uploaded to department</th>
		<th width="8%" data-sn="10">Breach List </th>
		<th width="8%" data-sn="11">Rules</th>
	</tr>
		...
		<tr {{if $item.pc_required eq 0}} style="background:#F00;" {{/if}} id="req_color{{$item.pc_id}}">
			...
			<td data-sn="8">
				<form name="recmet4" method="post" action="">
					...
					<label class="switch">
						<input class="switch-input" type="checkbox" name="qa2[{{$item.pc_id}}]"
							onclick="update_qa2({{$item.pc_id}},this.checked);" {{if $item.pc_qa2 eq 1 }}
							checked="checked" {{/if}} />
						<span class="switch-label" data-on="Yes" data-off="No"></span>
						<span class="switch-handle"></span> <br />
					</label>
				</form>
				<span id="qa2{{$item.pc_id}}">{{if $item.pc_qa2 eq 1}}{{$item.pc_qa2_user}}
					<br />{{$item.pc_qa2_date}}{{/if}} </span>
			</td>
			<td data-sn="9">
			...
			<td style="text-align:center;" data-sn="10">
				<a href="javascript:parent.plan_breach()" class="various">Link</a>
			</td>
			<td style="text-align:center;" data-sn="11">
...
<div style="display: none; position: fixed; min-height:350px; z-index:9999999; width: 90%; height: 90%; top: 30px; left: 5%; background: white;"
	...
	<a id="fancybox-close" style="display: inline;" onclick="close_plan_breach();"></a>
	...
	<div style="text-align: center; margin: 20px 0;">
		<input type="button" value="Close" onclick="close_plan_breach();" class="vsml" />
	</div>
</div>
{{/if}}
{{if !empty($iframe_msg)}}
