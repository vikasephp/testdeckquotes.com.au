...
<form name="print_qa" action="{{$BASE_URL}}{{$XFA.buyer_qa_email}}" method="post" onsubmit="target_popup(this);">
	...
	<table cellpadding="0" cellspacing="0" width="100%" id="list-table">
		...
		<tr bgcolor="#CCCCCC">
			...
			<th>Customer</th>
			<th>Question</th>
			<th>Attachment For Enquries</th>
			<th>Que. Date</th>
			<th>Answer</th>
			<th>Attachments For Answers</th>
			<th>Answer Date</th>
			<th>Business Stream</th>
			<th>Source</th>
			<th>Que User</th>
			<th>Include in&nbsp;Report <button type="button" id="selectAll" class="main"> Select All</button> </th>
		</tr>
		...
		<tr>
			<th width="5%">
				<div align="left" style="padding:0; margin:0px; display:inline;">
					...
					<a href="javascript:parent.requestqaDelete({{$item.bqa_id}});"
						onclick="javascript:if(!confirm('Are you sure you want to delete?')) return false;"
						title="Delete">
			...
			<td data-ques-attachment style="white-space: nowrap;">
			...
			<td style="white-space: nowrap;">{{$item.bqa_ans_date}}</td>
			...
			<td width="9%" class="example">
				<input type="checkbox" value="{{$item.bqa_id}}" name="qa_print[include_id][]" id="include_id" />
			</td>
			<!-- <td width="15%">
	...
	<tr>
		...
		<td align="center">
			...
			<div class='pagination'>
				...
				{{else}}
					<a class="pagination" href="javascript:requestquestionInfo({{math equation=" x - y" x=$pagenum
						y=1}})" title="Previous">&laquo; previous</a>
					{{/if}}

					{{if $pagenum != 1}}
					<span class="paginate"><a class="pagination" href="javascript:requestquestionInfo(1)"
							title="{{$page_num}} Page">1</a></span>
					{{else}}<span class='current'>1</span>
						{{/if}}{{if $pagenum != 2}}
						<span class="paginate"><a class="pagination" href="javascript:requestquestionInfo(2)"
						...
						{{else}}<span class='current'>2</span>
							...
							{{else}} <span class="paginate"><a class="pagination"
										href="javascript:requestquestionInfo({{$page_num}})"
								...
								{{else}}
									...
									{{else}}
										<a class="pagination" href="javascript:requestquestionInfo({{$page_num2}})"
										...
										{{else}}
											...
											{{else}}
												<a class="pagination"
													href="javascript:requestquestionInfo({{$lastone}})"
												...
												{{else}}
													...
													{{else}}
														<a class="pagination"
															href="javascript:requestquestionInfo({{$last}})"
														...
														{{else}}
															<a class="pagination"
																href="javascript:requestquestionInfo({{math equation=" x
