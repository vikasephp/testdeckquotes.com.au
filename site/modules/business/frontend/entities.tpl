	...
	<form name="nr" method="post" action="">
		...
		<tr {{if $item.pc_required eq 10}} style="background:#F00;" {{/if}} id="req_color{{$item.pc_id}}">
			...
			<td style="text-align:center;">
				<a
					href="javascript:requestchecklistEntitiesList('ppchecklist_entites_edit','{{$business_id}}','{{ $item.checklist_entities_referral.id }}','{{ $item.er_document }}','{{ $item.er_id }}','{{ $item_c.dpn_unique_id }}');">
			...
			<td style="text-align:center;">
				{{if !empty($item.document_check_list.doc_file_name) }}
				<a href="/business.download_content?file_name={{ $item.document_check_list.doc_file_name }}&module_name=business.detail"
...
<div style="border:1px #CCCCCC solid; padding:5px;" id="checkListIframe">
	...
	<form action="" id="checklist_entities_referral" method="POST" enctype="multipart/form-data"
		name="checklist_entities_referral45">
		...
		<input type="hidden" id="entities_referral_id" name="entities_referral_id"
			value="{{$checklist_entities_referral_id}}" />
		<input type="hidden" id="checklist_entities_referralForm" name="action"
			value="checklist_entities_referralForm" />
		<table cellpadding="0" cellspacing="0" class="table" id="upload_file_business_11">
			<tr>
				<td><strong>Utilities:</strong></td>
				<td style="background-color:#E0E6FF;padding:5px; font-size:12px; font-weight:bold;">{{$er_document}}
				</td>
			</tr>
			...
			<tr style="display: none;">
				...
				<td>
					<select name="status">
						<option {{if !empty($checklist_entities_referral) and $checklist_entities_referral.status=="" }}
							selected {{/if}} value="">Please Select</option>
						<option {{if !empty($checklist_entities_referral) and
							$checklist_entities_referral.status=="Not Required" }} selected {{/if}}
							value="Not Required">Not Required</option>
						<option {{if !empty($checklist_entities_referral) and
							$checklist_entities_referral.status=="Not Requested" }} selected {{/if}}
							value="Not Requested">Not Requested</option>
						<option {{if !empty($checklist_entities_referral) and
							$checklist_entities_referral.status=="Requested and Waiting For Entity" }} selected {{/if}}
							value="Requested and Waiting For Entity">Requested and Waiting For Entity</option>
						<option {{if !empty($checklist_entities_referral) and
							$checklist_entities_referral.status=="Passed With Conditions" }} selected {{/if}}
							value="Passed With Conditions">Passed With Conditions</option>
						<option {{if !empty($checklist_entities_referral) and
							$checklist_entities_referral.status=="Fails" }} selected {{/if}} value="Fails">Fails
						</option>
					</select>
				</td>
			</tr> -->
			...
			<tr>
				...
				<td><input type="date" name="date_applied"
						value="{{if !empty($checklist_entities_referral) }}{{ $checklist_entities_referral.date_applied }}{{/if}}"
			...
			<tr>
				...
				<td><input type="file" name="assessment_doc" style="width:700px;" />
					<span>{{if !empty($checklist_entities_referral) }} {{ $checklist_entities_referral.assessment_doc }}
						{{/if}}</span>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<input type="file" name="condition_snapshot" style="width:700px;" />
					<br>
					<span id="condition_snapshot_{{ $checklist_entities_referral.id }}">
						{{ if $checklist_entities_referral.condition_snapshot }}
							{{ $checklist_entities_referral.condition_snapshot }}
							<a href="javascript:void(0);" onclick="deleteConditionSnapshotDoc('{{ $checklist_entities_referral.id }}')" title="Delete">
...
<script>
	function deleteConditionSnapshotDoc(id) {
		if(!confirm('Are you sure you want to delete?')) return false;
		$.ajax({
			type: "GET",
			url: "{{$BASE_URL}}business.delete_condition_snapshot/id/" + id,
			success: function (result) {
				if(result == '1'){
					$('#condition_snapshot_' + id).hide();
				}
				else{
