...
try{
	...
	if ($business_tasksTable->rowExists()) {
		...
		/* foreach ($subtaskdetail as $stv) {
			// $business_tasks_subTable->setWhere('bts_bsn_id = ' . (int)$bid . ' And bts_bsst_id = ' . (int)$stv['bsst_id']);
			// $end_time = microtime(true);
			// $execution_time = ($end_time - $start_time);
			// echo "<br><br>before business_tasks_subTable rowexists Script Execution Time = " . $execution_time . " sec";
			...
			// 	// $business_tasks_subTable->insertRow([
			// 	// 	'bts_bsst_id' => (int)$stv['bsst_id'],
			// 	// 	'bts_bsn_id' => (int)$bid,
			// 	// 	'bts_task_status_id' => (int)$stv['bsst_business_status_id'],
			// 	// 	'bts_task_id' => (int)$stv['bsst_bst_id'],
			// 	// 	'bts_task_name' => $stv['bsst_task_name'] ?? '',
			// 	// ]);
			// 	$insertSQL .= '("' . (int)$stv['bsst_id'] . '", "' . (int)$bid . '", "' . (int)$stv['bsst_business_status_id'] . '", "' . (int)$stv['bsst_bst_id'] . '", "' . addslashes($stv['bsst_task_name']) . '"),';
	
			// 	$end_time = microtime(true);
			// 	$execution_time = ($end_time - $start_time);
			// 	echo "<br><br>" . $bid . " | " . $stv['bsst_id']  . " business_tasks_subTable insert Script Execution Time = " . $execution_time . " sec";
			// 	$insertCount++;
			// } else {
			// 	// $row = $business_tasks_subTable->getRow();
			// 	// // unset($std2);
			// 	// // $std2['bts_task_name'] = $stv['bsst_task_name'];
			// 	// // $business_tasks_subTable->updateRow($std2);
	
			// 	// // $business_tasks_subTable->updateRow([
			// 	// // 	'bts_task_name' => $stv['bsst_task_name'] ?? ''
			// 	// // ]);
			...
			// 	// $end_time = microtime(true);
			// 	// $execution_time = ($end_time - $start_time);
			// 	// echo "<br><br>" . $bid . " | " . $stv['bsst_id']  . " | " . $stv['bsst_task_name'] . " business_tasks_subTable update Script Execution Time = " . $execution_time . " sec";
			// 	// $updateCount++;
			// }
			// die();
		} */
		...
		// $end_time = microtime(true);
		// $execution_time = ($end_time - $start_time);
		// echo "<br><br>subtaskdetail loop Script Execution Time = " . $execution_time . " sec";

		// db($MySqliCon, '$MySqliCon');
		// db($DB_TYPE, '$DB_TYPE');
		// db($DB_NAME, '$DB_NAME');
		// db($DB_USER, '$DB_USER');
		// db($DB_PASS, '$DB_PASS');
