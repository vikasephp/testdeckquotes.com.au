...
if (!empty($postedData['action']) && $postedData['action'] == "checklist_entities_referralForm") {
    ...
    if (!empty($record_detail)) {
        $tableTask->setWhere("bsn_id = '" . $bsn_id . "' and entites_referral_id ='" . $er_id . "'");
        $l_result = $tableTask->updateRow($detail);
    ...
    if ($_FILES['condition_snapshot']['error'] == 0) {
        ...
        upload_entites_data($docfile, $tmpname);
        $condition_snapshot_detail['condition_snapshot'] = $docfile;
        $condition_snapshot_detail['condition_snapshot_upload_date'] = $current_timestamp;
        $tableTask->setWhere("bsn_id = '" . $bsn_id . "' and entites_referral_id ='" . $er_id . "'");
        $l_result = $tableTask->updateRow($condition_snapshot_detail);
    }
}
