...
$fwViewData['document_id'] = $_REQUEST['doc_id'];
...
if ($requset) {
	switch ($requset) {
		...
		case 'quotes':
			...
			if ($submit) {
				...
				for ($i = 0; $i < count($qdata['pqd_key_id']); $i++) {
					...
					if ($tablequote->rowExists()) {
						$tablequote->updateRow($qidata);
					} else {
						$tablequote->insertRow($qidata);
					}
				}
			}
			if ($qdata['TE']) {
		...
		case 'BFinance':
			$cust_id = $fwRequest->getparamget('cust_id', '');
			if ($cust_id) {
				if (send_template_email('Buyer Finance', $cust_id, '', 2)) {
					echo "set";
				} else {
					echo "error";
				}
		...
		case 'BInfo':
			$cust_id = $fwRequest->getparamget('cust_id', '');
			if ($cust_id) {
				if (send_template_email('Free Buyer Info', $cust_id, '', 2)) {
					echo "set";
				} else {
					echo "error";
				}
		...
		case 'BSurvey':
			$cust_id = $fwRequest->getparamget('cust_id', '');
			if ($cust_id) {
				if (send_template_email('Buyer Survey', $cust_id, '', 2)) {
					echo "set";
				} else {
					echo "error";
				}
		...
		case 'BQuestion':
			$cust_id = $fwRequest->getparamget('cust_id', '');
			if ($cust_id) {
				if (send_template_email('Business Question', $cust_id, '', 2)) {
					echo "set";
				} else {
					echo "error";
				}
		...
		case 'MListing':
			$cust_id = $fwRequest->getparamget('cust_id', '');
			if ($cust_id) {
				if (send_template_email('Monthly Listing', $cust_id, '', 2)) {
					echo "set";
				} else {
					echo "error";
				}
		...
		case 'RDossier':
			...
			if ($cust_id > 0) {
				echo send_all_edossier($cust_id, $bus_id, $edl_be_id);
			}
			exit;
			break;
	  ...
	  WHERE e_dossier_log.edl_bus_id = " . $bus_id . " ORDER BY e_dossier_log.edl_id DESC";
		...
		case 'addStatusTask':
			...
			$bus_id = trim($fwRequest->getParamget('bus_id', ''));
			$taskStatusId = $fwRequest->getParamget('taskStatusId', '');

			$end_time = microtime(true);
			$execution_time = ($end_time - $start_time);
			// echo "<br><br>Starting Execution Time = " . $execution_time . " sec";
			// die();

			$sql_bsn_type = "SELECT bsn_type FROM business WHERE bsn_id = $bus_id";
			$type = $fwDb->queryOne($sql_bsn_type);
			if ($type) {
				$bsn_type = $type['bsn_type'];
			}
			
			$end_time = microtime(true);
			$execution_time = ($end_time - $start_time);
			// echo "<br><br>sql_bsn_type Execution Time = " . $execution_time . " sec";
			// die();
			...
			// $end_time = microtime(true);
			// $execution_time = ($end_time - $start_time);
			// echo "<br><br>sql_checkExists Execution Time = " . $execution_time . " sec";
			// die();

			$sql_gettasks = "SELECT st.st_id, st.st_name, bst.bst_id, bst.bst_business_types, bst.bst_task_name, bst.bst_business_types_id FROM busness_status AS st LEFT JOIN busness_status_task AS bst ON st.st_id = bst.bst_business_status_id WHERE st.st_id = $taskStatusId";

			$tasksOfstatus = $fwDb->query($sql_gettasks);

			$end_time = microtime(true);
			$execution_time = ($end_time - $start_time);
			// echo "<br><br>sql_gettasks Execution Time = " . $execution_time . " sec";
			// die();

			if($taskStatusId == 4) {
				$query = 'UPDATE business SET bsn_complete_task_tracker_sync_date = "' . date('Y-m-d') . '" WHERE bsn_id = ' . $bus_id . ';';
				$fwDb->queryOne($query);
			}
			...
			// db($tasksOfstatus, 'tasksOfstatus');

			$end_time = microtime(true);
			$execution_time = ($end_time - $start_time);
			// echo "<br><br>foreach tasksOfstatus Execution Time = " . $execution_time . " sec";
			// die();

			$bt_task_id_arr = [];
			$bst_business_types_id_arr = [];
			if (count($tasksOfstatus) > 0) {
				// $bt_task_status_id_arr = [];
				// foreach ($tasksOfstatus as $k_tos => $v_tos) {
				// 	$bt_task_status_id_arr[trim($bus_id)] = trim($v_tos['st_id']);
				// }

				foreach ($tasksOfstatus as $k_tos => $v_tos) {
					$bt_task_id_arr[] = trim($v_tos['bst_id']);
					if ($v_tos['bst_business_types_id'] > 0) {
						$bst_business_types_id_arr[] = trim($v_tos['bst_business_types_id']);
					}
				}
			}
			// db($bt_task_id_arr, 'bt_task_id_arr');

			$business_tasks_notsynced = $fwDb->query("SELECT BST.bst_id FROM busness_status AS BS LEFT JOIN busness_status_task AS BST ON BS.st_id = BST.bst_business_status_id WHERE BS.st_id = $taskStatusId  AND BST.bst_id NOT IN (SELECT bt_task_id FROM business_tasks WHERE bt_bsn_id = $bus_id AND bt_task_status_id = $taskStatusId )");
			$business_tasks_query = 'INSERT INTO business_tasks (bt_bsn_id, bt_task_status_id, bt_task_id) VALUES ';
			$business_tasks_count = 0;
			foreach ($business_tasks_notsynced as $k_bts => $v_bts) {
				$business_tasks_query .= '(' . $bus_id . ', ' . $taskStatusId . ', ' . $v_bts['bst_id'] . '),';
				$business_tasks_count++;
			}

			$end_time = microtime(true);
			$execution_time = ($end_time - $start_time);
			// echo "<br><br>foreach business_tasks_notsynced Execution Time = " . $execution_time . " sec";

			if ($business_tasks_count > 0) {
				$business_tasks_query = rtrim($business_tasks_query, ',') . ';';
				mysqli_multi_query($MySqliCon, $business_tasks_query);
			}

			if (!empty($bst_business_types_id_arr)) {
				$business_task_types_notsynced = $fwDb->query("SELECT PT.pt_id, PTT.bst_id FROM project_type AS PT INNER JOIN project_types_task PTT ON PTT.bst_business_status_id = PT.pt_id WHERE PTT.bst_status = 1 AND PT.pt_id IN (" . implode(',', $bst_business_types_id_arr) . ")");

				$business_tasks_query = 'INSERT INTO business_tasks (bt_bsn_id, bt_task_status_id, bt_task_types_id, bt_task_id) VALUES ';
				$business_tasks_count = 0;
				foreach ($business_task_types_notsynced as $k_bts => $v_bts) {
					$business_tasks_query .= '(' . $bus_id . ', ' . $taskStatusId . ', ' . $v_bts['pt_id'] . ', ' . $v_bts['bst_id'] . '),';
					$business_tasks_count++;
				}
				if ($business_tasks_count > 0) {
					$business_tasks_query = rtrim($business_tasks_query, ',') . ';';
					// echo "<br><br>" . $business_tasks_query;
					mysqli_multi_query($MySqliCon, $business_tasks_query);
				}
			}
			$end_time = microtime(true);
			$execution_time = ($end_time - $start_time);
			// echo "<br><br>foreach business_task_types_notsynced Execution Time = " . $execution_time . " sec";
			...
			foreach ($subtaskdetail as $stv) {
				$bsst_id_arr[] = $stv['bsst_id'];
				$bsst_bst_id_arr[$stv['bsst_id']] = $stv['bsst_bst_id'];
				$bsst_task_name_arr[$stv['bsst_id']] = $stv['bsst_task_name'];
			}
			$business_tasks_sub_notsynced = $fwDb->query("SELECT * FROM busness_status_subtask WHERE bsst_id NOT IN (SELECT bts_bsst_id FROM business_tasks_sub WHERE bts_bsn_id = $bus_id) AND bsst_business_status_id = $taskStatusId;");
			if (!empty($business_tasks_sub_notsynced)) {
				$business_tasks_sub_query = 'INSERT INTO business_tasks_sub (bts_bsn_id, bts_bsst_id, bts_task_status_id, bts_task_id, bts_task_name) VALUES ';
				$business_tasks_sub_count = 0;
				foreach ($business_tasks_sub_notsynced as $k_bts => $v_bts) {
					$business_tasks_sub_query .= '(' . $bus_id . ', ' . $v_bts['bsst_id'] . ', ' . $taskStatusId . ', ' . $v_bts['bsst_bst_id'] . ', "' . $v_bts['bsst_task_name'] . '"),';
					$business_tasks_sub_count++;
				}
				if ($business_tasks_sub_count > 0) {
					$business_tasks_sub_query = rtrim($business_tasks_sub_query, ',') . ';';
					mysqli_multi_query($MySqliCon, $business_tasks_sub_query);
				}
			}

			$end_time = microtime(true);
			$execution_time = ($end_time - $start_time);
			// echo "<br><br>foreach business_tasks_sub_notsynced Execution Time = " . $execution_time . " sec";
			// die();
		...
		case 'getAllTasks':
			...
			// $end_time = microtime(true);
			// $execution_time = ($end_time - $start_time);
			// echo "<br><br>sql_bsn_type Script Execution Time = " . $execution_time . " sec";

			if ($type) {
				$bsn_type = $type['bsn_type'];
			}
			$sql1 = "SELECT bt.*, bs.st_name, bst.bst_id, bst.bst_task_name, bst.bst_document, bst.bst_task_doc_link,
				        bst.bst_order, bst_posid, bst.bst_auto_email_text, bst.bst_auto_sms_text, pos.p_name,
			...
			if ($cts == '0' or $cts == '1'):	$sql1 .= " AND bt.bt_complete = " . $cts;
			endif;
			if ($bt_task_status_id > 0): $sql1 .= " AND bt.bt_task_status_id = " . $bt_task_status_id;
			endif;
			...
			if ($cts == '0' || $cts == '1'):
				$sql2 .= " AND bt.bt_complete = " . $cts;
			endif;
			if ($bt_task_status_id > 0):
				$sql2 .= " AND bt.bt_task_status_id = " . $bt_task_status_id;
			endif;
			...
			if ($bus_id > 0) {
				...
				// $end_time = microtime(true);
				// $execution_time = ($end_time - $start_time);
				// echo "<br><br>userData " . count($userData) . " Script Execution Time = " . $execution_time . " sec";
				...
				if (!empty($userData)) {
					...
					if ($sql2) {
						...
						// $end_time = microtime(true);
						// $execution_time = ($end_time - $start_time);
						// echo "<br><br>all Script Execution Time = " . $execution_time . " sec";

						$bt_task_id_arr = array();
						foreach ($all as $kk => $allt) {
							$all[$kk]['bst_instruction'] = preg_replace('/[^A-Z0-9._<>\/&;]/i', ' ', $allt['bst_instruction']);
							$bt_task_id_arr[] = $allt['bt_task_id'];
						}

						$total_sub_task_sql = "SELECT bts_task_id, COUNT(*) AS tot_task FROM business_tasks_sub WHERE bts_bsn_id = " . $bus_id . " AND bts_task_id IN (" . implode(',', $bt_task_id_arr) . ") GROUP BY bts_task_id";
						$total_sub_task_res = $fwDb->query($total_sub_task_sql);

						$total_sub_task_arr = array();
						foreach ($total_sub_task_res as $tst) {
							$total_sub_task_arr[$tst['bts_task_id']] = $tst['tot_task'];
						}

						// $end_time = microtime(true);
						// $execution_time = ($end_time - $start_time);
						// echo "<br><br>total_sub_task_sql Script Execution Time = " . $execution_time . " sec";

						$comp_task_sql = "SELECT bts_task_id, COUNT(*) AS tot_task FROM business_tasks_sub WHERE bts_bsn_id = " . $bus_id . " AND bts_task_id IN (" . implode(',', $bt_task_id_arr) . ") AND bts_complete = 1 GROUP BY bts_task_id";
						$comp_task_res = $fwDb->query($comp_task_sql);

						$comp_task_arr = array();
						foreach ($comp_task_res as $ct) {
							$comp_task_arr[$ct['bts_task_id']] = $ct['tot_task'];
						}

						// $end_time = microtime(true);
						// $execution_time = ($end_time - $start_time);
						// echo "<br><br>comp_task_sql Script Execution Time = " . $execution_time . " sec";
						// die();

						// db($total_sub_task_arr, 'total_sub_task_arr');
						// db($comp_task_arr, 'comp_task_arr');
						// db($bt_task_id_arr, 'bt_task_id_arr');

						//$fwViewData['allTasks']= $fwDb->query($sql2);
						$all2 = array();
						foreach ($all as $dk => $dv) {
							// $all[$dk]['bst_instruction'] = preg_replace('/[^A-Z0-9._<>\/&;]/i', ' ', $dv['bst_instruction']);

							$dv['bst_task_doc_link'] =  explode(',', $dv['bst_task_doc_link']);
							$all2[] = $dv;
							$all2[$dk]['total_sub_task'] =  $total_sub_task_arr[$dv['bt_task_id']] ?? 0;
							$all2[$dk]['comp_task'] = $comp_task_arr[$dv['bt_task_id']] ?? 0;

							// $sql = "SELECT count(*) as tot_task FROM business_tasks_sub 
							// 		where bts_bsn_id = " . $dv['bt_bsn_id'] . " And bts_task_id = " . $dv['bt_task_id'];

							// $tot_data = $fwDb->queryOne($sql);
							// $all2[$dk]['total_sub_task'] = $tot_data['tot_task'];

							// $sql_com = "SELECT count(*) as comp_task FROM business_tasks_sub 
							// 		where bts_bsn_id = " . $dv['bt_bsn_id'] . " And bts_task_id = " . $dv['bt_task_id'] . " And bts_complete = 1 ";

							// $comp_data = $fwDb->queryOne($sql_com);
							// $all2[$dk]['comp_task'] = $comp_data['comp_task'];
						}
						...
						// $end_time = microtime(true);
						// $execution_time = ($end_time - $start_time);
						// echo "<br><br>foreach all Script Execution Time = " . $execution_time . " sec";
						// die();
						...
						$tpsql = "SELECT * FROM busness_status_task_ai_proc";
						$fwViewData['allAiProc'] = $fwDb->query($tpsql);
						// $end_time = microtime(true);
						// $execution_time = ($end_time - $start_time);
						// echo "<br><br>tpsql Script Execution Time = " . $execution_time . " sec";
						// die();
						...
						// $end_time = microtime(true);
						// $execution_time = ($end_time - $start_time);
						// echo "<br><br>tpsqlemail Script Execution Time = " . $execution_time . " sec";
						// die();
						...
						// $end_time = microtime(true);
						// $execution_time = ($end_time - $start_time);
						// echo "<br><br>tpsql_team Script Execution Time = " . $execution_time . " sec";
						// die();
					}
				}
			}
		...
		case 'getAllTasksProposal':
			...
			if ($cts == '0' or $cts == '1'):	$sql2 .= " AND bt.bt_complete = " . $cts;
			endif;
			...
			if ($bus_id > 0) {
				...
				if (!empty($userData)) {
					...
					if ($sql2) {
						...
						foreach ($all as $kk => $allt) {
							$all[$kk]['bst_instruction'] = preg_replace('/[^A-Z0-9.$_<>\/&;]/i', ' ', $allt['bst_instruction']);
							//$all[$kk]['bst_instruction'] = preg_replace('/[^A-Za-z0-9<>\-]/', ' ', $allt['bst_instruction']);	
			...
			if (count($exists) > 0) {
				...
				if (count($tasksOfstatus) > 0) {
					foreach ($tasksOfstatus as $k_tos => $v_tos) {
						...
						if ($detail) {
							$business_tasksTable->insertRow($detail);
						}
		...
		case 'getAllptypeTasks':
			...
			if ($cts == '0' or $cts == '1'):	$sql .= " AND bt_complete = " . $cts;
			endif;
			if ($bt_task_types_id > 0): $sql .= " AND bst.bst_business_status_id = " . $bt_task_types_id;
			endif;
			...
			if ($bus_id > 0) {
				...
				if (!empty($userData)) {
					...
					if ($sql2) {
						$fwViewData['allptTasks'] = $fwDb->query($sql2);
					}
				}
			}
		...
		case "DocumentCheckList":
			...
			if ($bus_id > 0) {
				$sql_print_data =  "SELECT dcl.*, adcl.*, u.user_name, (SELECT user_name FROM users WHERE user_id = dcl.doc_qr_code_by LIMIT 1) AS doc_qr_code_user, (SELECT user_name FROM users WHERE user_id = dcl.doc_quality_checked_by LIMIT 1) AS doc_quality_checked_user
				...
				for ($i = 0; $i < $count_print_data; $i++) {
					$qr_code = '<div id="qr_code_' . $print_data[$i]['doc_id'] . '" style="margin-top: 10px;">';
					if($print_data[$i]['doc_qr_code']) {
						/*$qr_code .= '<p><a href="javascript:parent.modelviewimages(\'/business.download_content?file_name=' . $print_data[$i]['doc_qr_code'] . '&module_name=business.home\')" class="action_checklist">View</a></p>';*/
						$qr_code .= '<p><a href="/business.download_content_qrcode?file_name=' . $print_data[$i]['doc_qr_code'] . '&module_name=business.home" target="_blank" class="action_checklist">Download</a></p>';
						if($print_data[$i]['doc_qr_code_user']) {
							$qr_code .= '<p>' . $print_data[$i]['doc_qr_code_user'] . '</p>';
						}						
						if(strtotime($print_data[$i]['doc_qr_code_at']) > 0) {
							$qr_code .= ('<p>' . date('d-M-Y', strtotime($print_data[$i]['doc_qr_code_at'])) . '</p>');
						}						
					}
					$qr_code .= '</div>';
					$print_data[$i]['qr_code'] = $qr_code;

					$quality_checked = '<div id="quality_checked_' . $print_data[$i]['doc_id'] . '">';
					if($print_data[$i]['doc_quality_checked']) {
						if($print_data[$i]['doc_quality_checked_user']) {
							$quality_checked .= '<p>' . $print_data[$i]['doc_quality_checked_user'] . '</p>';
						}						
						if(strtotime($print_data[$i]['doc_quality_checked_at']) > 0) {
							$quality_checked .= ('<p>' . date('d-M-Y', strtotime($print_data[$i]['doc_quality_checked_at'])) . '</p>');
						}						
					}
					$quality_checked .= '</div>';
					$print_data[$i]['quality_checked'] = $quality_checked;
		...
		case "DocumentCheckListMini":
			...
			} elseif (!empty($addList) && $addList == 'add_new') {
				if ($bus_id > 0) {
					$businessTable = new Fw_Db_Table('business');
					$businessTable->setWhere('bsn_id =' . $bus_id);
					$pt_data = $businessTable->getRow();
			...
			// sync start
			elseif (!empty($addList) && $addList == 'sync') {
				if ($bus_id > 0) {
					$businessTable = new Fw_Db_Table('business');
					$businessTable->setWhere('bsn_id =' . $bus_id);
					$pt_data = $businessTable->getRow();
			...
			if ($bus_id > 0) {
				...
				$fwViewData['dpn_add_to_er'] = $namedata_1['dpn_add_to_er'];
				...
				$fwViewData['dpn_archived'] = $namedata_1['dpn_archived'];
				...
				for ($a = 2; $a <= $ctval['dcnt_counter']; $a++) {
					...
					if (!empty($namedata_more)) {
						if ($a == 2) {
							...
							$fwViewData['dpn_add_to_er_2'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_2'] = $namedata_more['dpn_archived'];
							...
							$fwViewData['dpn_add_to_er_3'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_3'] = $namedata_more['dpn_archived'];
							...
							$fwViewData['dpn_add_to_er_4'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_4'] = $namedata_more['dpn_archived'];
							...
							$fwViewData['dpn_add_to_er_5'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_5'] = $namedata_more['dpn_archived'];
							...
							$fwViewData['dpn_add_to_er_6'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_6'] = $namedata_more['dpn_archived'];
							$fwViewData['dpn_hia_signed_6'] = $namedata_more['dpn_hia_signed'];
						} elseif ($a == 7) {
							...
							$fwViewData['dpn_add_to_er_7'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_7'] = $namedata_more['dpn_archived'];
							$fwViewData['dpn_hia_signed_7'] = $namedata_more['dpn_hia_signed'];
						} elseif ($a == 8) {
							...
							$fwViewData['dpn_add_to_er_8'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_8'] = $namedata_more['dpn_archived'];
							$fwViewData['dpn_hia_signed_8'] = $namedata_more['dpn_hia_signed'];
						} elseif ($a == 9) {
							...
							$fwViewData['dpn_add_to_er_9'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_9'] = $namedata_more['dpn_archived'];
							$fwViewData['dpn_hia_signed_9'] = $namedata_more['dpn_hia_signed'];
						} elseif ($a == 10) {
							...
							$fwViewData['dpn_add_to_er_10'] = $namedata_more['dpn_add_to_er'];
							...
							$fwViewData['dpn_archived_10'] = $namedata_more['dpn_archived'];
							$fwViewData['dpn_hia_signed_10'] = $namedata_more['dpn_hia_signed'];
						} elseif ($a == 11) {
		...
		case "checklist":
			...
			if (!empty($doc_id)) {
				...
				if (!empty($submit_iframe)) {
					...
					//	echo "<pre>";  print_r($_FILES); exit("wait we are checking here...");
					...
					if ($_FILES['image']['error'] == 0) {
                        ...
                        $dcfi = explode(".",$docfile);
						
						
						$last = array_pop($dcfi);
						$dcfi = array(implode('.', $dcfi), $last);
						
						$dcfn = $dcfi[0].date("d_m_Y_h_i_sa").".".$dcfi[1]; 
						$docfile = $dcfn;
		...
		case "checklistmini":
			...
			if (!empty($doc_id)) {
				...
				if (!empty($submit_iframe)) {
					...
					$sqlm = "select doc_bsn_id, doc_name_id, doc_checklist_number from document_check_list_mini where doc_id = " . $doc_id;
					$lmpData = $fwDb->queryOne($sqlm);

					$lmpDetail['lm_bsn_id'] = $lmpData['doc_bsn_id'];
					$lmpDetail['lm_doc_file_name'] = $doc['doc_file_name'];
					$lmpDetail['lm_checklist_number'] = $lmpData['doc_checklist_number'];
					$lmpDetail['lm_doc_date_uploaded'] = $doc['doc_date_uploaded'];

					if ($lmpData['doc_name_id'] == 20) {
						$lmpTable = new Fw_Db_Table('latest_merge_plan');
						$idid = $lmpTable->insertRow($lmpDetail);
					}
		...
		case "adddoc_checklistmini":
			...
			$fwViewData['adddoc_checklistmini_bus_id'] = $_REQUEST['bus_id'];
		...
		case 'documents':
			...
			if (!empty($subDocument)) {
				...
				if (!$data['bd_upload_dossier']) {
					$data['bd_upload_dossier'] = $data['bd_upload_dossier'] = 0;
				}
		...
		case 'addspecialcondition':
			...
			if ($tableid > 0) {
				$sql = "SELECT * FROM bus_special_condition 
						WHERE bus_special_condition.sp_id = $tableid ";

				$detail = $fwDb->queryOne($sql);
				$fwViewData['detail'] = $detail;
			}
			$special_conditionTable = new Fw_Db_Table('bus_special_condition');
			$spDocument = $fwRequest->getParam('spDocument', '');

			if (!empty($spDocument)) {
				$data = $fwRequest->getParam('data', array());

				$data['sp_date'] = date('d-m-Y');
				$data['sp_who'] = $_SESSION['user']['user_name'];

				$word = $_FILES['spword']['name'];
				$pdf  = $_FILES['sppdf']['name'];


				if ($_FILES['spword']['error'] == 0) {
					$docfile = $_FILES['spword']['name'];
					$file_type = $_FILES['spword']['type'];

					$docfile = preg_replace('/[^A-Z0-9._]/i', '_', $docfile);
					$src = $_FILES['spword']['tmp_name'];
					upload_special_condition($docfile, $src);

					//unset($data['bd_doc_name']);
					$data['sp_word'] = $docfile;
				...
				if ($_FILES['sppdf']['error'] == 0) {
					$docfile2 = $_FILES['sppdf']['name'];
					$file_type = $_FILES['sppdf']['type'];

					$docfile2 = preg_replace('/[^A-Z0-9._]/i', '_', $docfile2);
					$src2 = $_FILES['sppdf']['tmp_name'];
					upload_special_condition($docfile2, $src2);

					//unset($data['bd_doc_name']);
					$data['sp_pdf'] = $docfile2;
				...
				if (empty($file_exist_error)) {
					...
					if (!empty($data['sp_id']) and $data['sp_id'] > 0) {
						$sp_id = $data['sp_id'];
						unset($data['sp_id']);
						$special_conditionTable->setWhere("sp_id = $sp_id");
						$special_conditionTable->updateRow($data);
						$success_msg_sp = "Document Updated Successfully.";
					} else {
						$data['sp_date'] = $_SESSION['user']['user_id'];
						$data['sp_date'] = date('Y-m-d H:i');

						unset($data['sp_id']);

						$special_conditionTable->insertRow($data);
						$success_msg_sp = "Document Added Successfully.";
					}

					$fwViewData['success_msg_sp'] = $success_msg_sp;
					$fwViewData['showspIframe'] = '0';
					//}
					//else
					//{
					//$fwViewData['file_exist'] = "Document is mandatory. Please select the file.";
		...
		case 'addvariations':
			$fwViewData['showvariationsIframe'] = 1;
			$bus_id = $fwRequest->getParamget('bus_id', '');
			$divid = $fwRequest->getParamget('divid', '');
			$tableid = $fwRequest->getParamget('tableid', '');
			$fwViewData['bus_id'] = $bus_id;
			...
			if ($tableid > 0) {
				$sql = "SELECT * FROM bus_variations WHERE bus_variations.va_id = $tableid ";

				$detail = $fwDb->queryOne($sql);
				$fwViewData['detail'] = $detail;
			}
			$special_conditionTable = new Fw_Db_Table('bus_variations');
			$spDocument = $fwRequest->getParam('spDocument', '');

			if (!empty($spDocument)) {
				$data = $fwRequest->getParam('data', array());

				$data['va_date'] = date('d-m-Y');
				$data['va_who'] = $_SESSION['user']['user_name'];

				$word = $_FILES['vaword']['name'];
				$pdf  = $_FILES['vapdf']['name'];


				if($_FILES['vaword']['error'] == 0)
				{
					$docfile = $_FILES['vaword']['name'];
					$file_type = $_FILES['vaword']['type'];
					
					$docfile = preg_replace('/[^A-Z0-9._]/i', '_', $docfile);
					$src = $_FILES['vaword']['tmp_name'];
					upload_variations($docfile, $src);
		
					$data['va_word'] = $docfile;
				...
				if ($_FILES['vapdf']['error'] == 0) {
					$docfile2 = $_FILES['vapdf']['name'];
					$file_type = $_FILES['vapdf']['type'];

					$docfile2 = preg_replace('/[^A-Z0-9._]/i', '_', $docfile2);
					$src2 = $_FILES['vapdf']['tmp_name'];
					upload_variations($docfile2, $src2);

					$data['va_pdf'] = $docfile2;
				...
				if ($_FILES['vasigned']['error'] == 0) {
					$docfile3 = $_FILES['vasigned']['name'];
					$file_type = $_FILES['vasigned']['type'];

					$docfile3 = preg_replace('/[^A-Z0-9._]/i', '_', $docfile3);
					$src3 = $_FILES['vasigned']['tmp_name'];
					upload_variations($docfile3, $src3);

					$data['va_signed'] = $docfile3;
					$data['va_signed_date'] = date('d-m-Y');
				...
				if (empty($file_exist_error)) {
					...
					if (!empty($data['va_id']) and $data['va_id'] > 0) {
						$va_id = $data['va_id'];
						unset($data['va_id']);
						$special_conditionTable->setWhere("va_id = $va_id");
						$special_conditionTable->updateRow($data);
						$success_msg_var = "Document Updated Successfully.";
					} else {
						$data['va_date'] = $_SESSION['user']['user_id'];
						$data['va_date'] = date('Y-m-d H:i');

						unset($data['va_id']);

						$special_conditionTable->insertRow($data);
						$success_msg_var = "Document Added Successfully.";
					}

					$fwViewData['success_msg_var'] = $success_msg_var;
					$fwViewData['showvariationsIframe'] = '0';
				} else {
					$fwViewData['detail'] = $data;
					$fwViewData['file_exist'] = "A file already exists with this filename. Please Rename the file and Upload it again.";
					$fwViewData['showvariationsIframe'] = 1;
		...
		case 'adddesignplan':
			$fwViewData['showdesignplanIframe'] = 1;
			$bus_id = $fwRequest->getParamget('bus_id', '');
			$divid = $fwRequest->getParamget('divid', '');
			$tableid = $fwRequest->getParamget('tableid', '');
			$fwViewData['bus_id'] = $bus_id;
			...
			if ($tableid > 0) {
				$sql = "SELECT * FROM bus_designplan WHERE bus_designplan.dp_id = $tableid ";

				$detail = $fwDb->queryOne($sql);
				$fwViewData['detail'] = $detail;
			}
			$desig_planTable = new Fw_Db_Table('bus_designplan');
			$spDocument = $fwRequest->getParam('spDocument', '');

			if (!empty($spDocument)) {
				$data = $fwRequest->getParam('data', array());
				$pdf  = $_FILES['dppdf']['name'];

				if ($_FILES['dppdf']['error'] == 0) {
					$docfile2 = $_FILES['dppdf']['name'];
					$file_type = $_FILES['dppdf']['type'];

					$docfile2 = preg_replace('/[^A-Z0-9._]/i', '_', $docfile2);
					$src2 = $_FILES['dppdf']['tmp_name'];
					upload_designplan($docfile2, $src2);

					$data['dp_pdf_file'] = $docfile2;
				...
				if (empty($file_exist_error)) {
					...
					if (!empty($data['dp_id']) and $data['dp_id'] > 0) {
						$dp_id = $data['dp_id'];
						unset($data['dp_id']);
						$desig_planTable->setWhere("dp_id = $dp_id");
						$desig_planTable->updateRow($data);
						$success_msg_dp = "Document Updated Successfully.";
					} else {

						$data['dp_date_uploaded'] = date('d-m-Y');

						unset($data['dp_id']);

						$desig_planTable->insertRow($data);
						$success_msg_dp = "Document Added Successfully.";
					}

					$fwViewData['success_msg_dp'] = $success_msg_dp;
					$fwViewData['showdesignplanIframe'] = '0';
				} else {
					$fwViewData['detail'] = $data;
					$fwViewData['file_exist'] = "A file already exists with this filename. Please Rename the file and Upload it again.";
					$fwViewData['showdesignplanIframe'] = 1;
		...
		case 'addlmp':
			...
			if ($tableid > 0) {
				$sql = "SELECT * FROM bus_lmp WHERE bus_lmp.lmp_id = $tableid ";

				$detail = $fwDb->queryOne($sql);
				$fwViewData['detail'] = $detail;
			}
			$lmpTable = new Fw_Db_Table('bus_lmp');
			$lmpDocument = $fwRequest->getParam('lmpDocument', '');

			if (!empty($lmpDocument)) {
				$data = $fwRequest->getParam('data', array());
				$pdf  = $_FILES['lmppdf']['name'];

				if ($_FILES['lmppdf']['error'] == 0) {
					$docfile2 = $_FILES['lmppdf']['name'];
					$file_type = $_FILES['lmppdf']['type'];

					$docfile2 = preg_replace('/[^A-Z0-9._]/i', '_', $docfile2);
					$src2 = $_FILES['lmppdf']['tmp_name'];
					upload_lmp($docfile2, $src2);

					$data['lmp_doc_name'] = $docfile2;
				...
				if (empty($file_exist_error)) {
					...
					if (!empty($data['lmp_id']) and $data['lmp_id'] > 0) {
						$lmp_id = $data['lmp_id'];
						unset($data['lmp_id']);
						$lmpTable->setWhere("lmp_id = $lmp_id");
						$lmpTable->updateRow($data);
						$success_msg_dp = "Document Updated Successfully.";
					} else {

						//$data['lmp_date_uploaded'] = date('d-m-Y');

						unset($data['lmp_id']);

						$lmpTable->insertRow($data);
						$success_msg_dp = "Document Added Successfully.";
					}

					$fwViewData['success_msg_lmp'] = $success_msg_dp;
					$fwViewData['showlmpIframe'] = '0';
				} else {
					$fwViewData['detail'] = $data;
					$fwViewData['file_exist'] = "A file already exists with this filename. Please Rename the file and Upload it again.";
					$fwViewData['showlmpIframe'] = 1;
		...
		case 'addmm':
			...
			if ($tableid > 0) {
				$sql = "SELECT * FROM bus_mm WHERE bus_mm.mm_id = $tableid ";

				$detail = $fwDb->queryOne($sql);
				$fwViewData['detail'] = $detail;
			}
			$mmTable = new Fw_Db_Table('bus_mm');
			$mmDocument = $fwRequest->getParam('mmDocument', '');

			if (!empty($mmDocument)) {
				$data = $fwRequest->getParam('data', array());
				$pdf  = $_FILES['mmpdf']['name'];

				if ($_FILES['mmpdf']['error'] == 0) {
					$docfile2 = $_FILES['mmpdf']['name'];
					$file_type = $_FILES['mmpdf']['type'];

					$docfile2 = preg_replace('/[^A-Z0-9._]/i', '_', $docfile2);
					$src2 = $_FILES['mmpdf']['tmp_name'];
					upload_mm($docfile2, $src2);

					$data['mm_meeting_file'] = $docfile2;
				...
				if (empty($file_exist_error)) {
					...
					if (!empty($data['mm_id']) and $data['mm_id'] > 0) {
						$mm_id = $data['mm_id'];
						unset($data['mm_id']);
						$mmTable->setWhere("mm_id = $mm_id");
						$mmTable->updateRow($data);
						$success_msg_mm = "Document Updated Successfully.";
					} else {
						unset($data['mm_id']);

						$mmTable->insertRow($data);
						$success_msg_mm = "Document Added Successfully.";
					}

					$fwViewData['success_msg_mm'] = $success_msg_mm;
					$fwViewData['showmmIframe'] = '0';
				} else {
					$fwViewData['detail'] = $data;
					$fwViewData['file_exist'] = "A file already exists with this filename. Please Rename the file and Upload it again.";
					$fwViewData['showmmIframe'] = 1;
		...
		case 'showspecial':
			...
			if ($bus_id > 0) {
				$sql = "SELECT * from bus_special_condition where sp_bsn_id = " . $bus_id . " ORDER BY sp_id DESC";
				...
				if (!empty($userData)) {
					...
					$sql2 =  $sql . " " . $max;
					if ($sql2) {
						$newsp_detail = $fwDb->query($sql2);
					}



					$fwViewData['special_data'] = $newsp_detail;
				}
				/*************** End Pagnation ********************/
				$fwViewData['special_data_show'] = "Show";
				$fwViewData['divid'] = $divid;
			}
			break;
		...
		case 'showvariations':
			...
			if ($bus_id > 0) {
				$sql = "SELECT * from bus_variations where va_bsn_id = " . $bus_id . " ORDER BY va_id DESC";
				...
				if (!empty($userData)) {
					...
					$fwViewData['variations_data'] = $newsp_detail;
				}
				/*************** End Pagnation ********************/
				$fwViewData['variations_data_show'] = "Show";
				$fwViewData['divid'] = $divid;
			}
			break;
		...
		case 'showdesignplan':
			...
			if ($bus_id > 0) {
				$sql = "SELECT * from bus_designplan where dp_bsn_id = " . $bus_id . " ORDER BY dp_id DESC";
				...
				if (!empty($userData)) {
					...
					$fwViewData['designplan_data'] = $newsp_detail;
				}
				/*************** End Pagnation ********************/
				$fwViewData['designplan_data_show'] = "Show";
				$fwViewData['divid'] = $divid;
			}
			break;
		...
		case 'showlmpplan':
			...
			if ($bus_id > 0) {
				$sql = "SELECT * from bus_lmp where lmp_bsn_id = " . $bus_id . " ORDER BY lmp_id DESC";
				...
				if (!empty($userData)) {
					...
					$fwViewData['lmp_data'] = $newsp_detail;
				}
				/*************** End Pagnation ********************/
				$fwViewData['lmp_data_show'] = "Show";
				$fwViewData['divid'] = $divid;
			}
			break;
		...
		case 'showmm':
			...
			if ($bus_id > 0) {
				$sql = "SELECT * from bus_mm where mm_bsn_id = " . $bus_id . " ORDER BY mm_id DESC";
				...
				if (!empty($userData)) {
					...
					$sql2 =  $sql . " " . $max;
					if ($sql2) {
						$mm_detail = $fwDb->query($sql2);
					}



					$fwViewData['mm_data'] = $mm_detail;
				}
				/*************** End Pagnation ********************/
				$fwViewData['mm_data_show'] = "Show";
				$fwViewData['divid'] = $divid;
			}
			break;
		...
		case 'delete_document_sp':
			$tableid = $fwRequest->getParamget('tableid', '');
			$id = $fwRequest->getParamget('id', '');

			$sql = "DELETE FROM bus_special_condition WHERE sp_id = $tableid";
			$temp = $fwDb->queryOne($sql);
		...
		case 'delete_document_var':
			$tableid = $fwRequest->getParamget('tableid', '');
			$id = $fwRequest->getParamget('id', '');

			$sql = "DELETE FROM bus_variations WHERE va_id = $tableid";
			$temp = $fwDb->queryOne($sql);
		...
		case 'delete_document_dp':
			$tableid = $fwRequest->getParamget('tableid', '');
			$id = $fwRequest->getParamget('id', '');

			$sql = "DELETE FROM bus_designplan WHERE dp_id = $tableid";
			$temp = $fwDb->queryOne($sql);
		...
		case 'padocuments':
			...
			if (!empty($subDocument)) {
				$data = $fwRequest->getParam('data', array());
				if (!$data['bpd_upload_dossier']) {
					$data['bpd_upload_dossier'] = $data['bpd_upload_dossier'] = 0;
				}
		...
		case 'cdocuments':
			...
			if ($cdata1 > 0 and empty($subDocument) or ($cdata1 == 0 or empty($cdata1))) {
				...
				if ($cdata == 1) {
					$type = "Document check list A";
				} else if ($cdata == 2) {
					$type = "Document check list B";
				} else if ($cdata == 3) {
					$type = "Document check list C";
				} else if ($cdata == 4) {
					$type = "Document check list D";
				}
				...
				if (count($checkdocs) > 0) {
					foreach ($checkdocs as $key => $val) {
						if ($val['bd_doc_name']) {
							...
							$docsname .= " >> <a href='https://deckquote.s3.amazonaws.com/files/business_documents/" . $val['bd_doc_name'] . "'
 title='https://deckquote.s3.amazonaws.com/files/business_documents/" . $val['bd_doc_name'] . "'>" . $val['bd_doc_name'] . "</a>";
						}
					}
				}
		...
		case 'followupslist':
			...
			if (!empty($userData)) {
				...
				if ($sql2) {
					$fwViewData['listsalesfollowup'] = $fwDb->query($sql2);
				}
			}
		...
		case 'followups':
			...
			if ($submit) {
				...
				unset($detail['sf_id']);
				if ($detail['sf_date']): $detail['sf_date'] = changedate_y_m_d($detail['sf_date']);
				endif;
				if ($this_id > 0) {
