...
if($bsn_id > 0 && isset($params['productpage'])) {
	...
	foreach ($prog_val as $ppk2 => $ppv2) {
		...
		$v66 = preg_replace('/[^\d.]/', '', $ppv2['ppd_inv_value']);
		if (empty($v66)) {
			$v66 = 0;
		}
		$total_inv_val = $total_inv_val + $v66;
	}
	$fwViewData['total_value'] = $totaled;
	$fwViewData['total_value_hia'] = $totaled_hia;
	$fwViewData['total_inv_val'] = $total_inv_val;
	...
	if (!empty($submitpp)) {
		...
		foreach ($detailpp as $kprm => $vpr) {
			...
			$prdetail['ppd_inv_value'] = $vpr['ppd_inv_value'];
		...
		foreach ($prog_pay as $pk => $pv) {
			...
			$ppd_hia_value = preg_replace('/[^\d.]/', '', $pv['ppd_hia_value']);
			if (empty($ppd_hia_value)) {
				$ppd_hia_value = 0;
			}

			$ppd_value = preg_replace('/[^\d.]/', '', $pv['ppd_value']);
			if (empty($ppd_value)) {
				$ppd_value = 0;
			}

			$prog_pay[$pk]['amount_owed'] = number_format($ppd_hia_value - ($ppd_value), 2);
			...
			if ($pv['ppd_stage'] == 'Planning Approvals Submitted'){
				if ( trim($pv['ppd_invoice_sent']) != '' && trim($pv['ppd_inv_due_date']) != '' ) {

					$pasdetail = array(
						'ppd_inv_value' => $pv['ppd_hia_value']
					);

					$prog_pay[$pk]['ppd_inv_value'] = $pv['ppd_hia_value'];
					
					$tablepas = new Fw_Db_Table('progress_payment_detail');
					$tablepas->setWhere("ppd_id = " . $pv['ppd_id']);
					if ($tablepas->rowExists()) {
						$tablepas->updateRow($pasdetail);
					}
	...
	/*Phone Log Count*/
	if ($bsn_id) {
		$phone_sql = "SELECT count(*) AS total_call from phone_log WHERE ph_bsn_id = " . intval($bsn_id);
		$phone_data = $fwDb->queryOne($phone_sql);
		$phone_count = (int)$phone_data['total_call'];
		
		$phone_await_sql = 'SELECT count(*) AS total_await from phone_log WHERE ph_actioned Like "%Awaiting Action%" and ph_bsn_id = ' . intval($bsn_id);
		$phone_await_data = $fwDb->queryOne($phone_await_sql);
		$phone_await_count = (int)$phone_await_data['total_await'];
		
		$fwViewData['phone_count'] = $phone_count;
		$fwViewData['phone_await_count'] = $phone_await_count;
	}
...
if ($bsn_id > 0){
	//$sql_checklist = "SELECT dpn_unique_id FROM document_renovation_name where dpn_hia_signed = 1 and dpn_bsn_id = ".$bsn_id;
	$sql_checklist = "SELECT dpn_unique_id FROM document_proposal_name WHERE dpn_hia_signed = 1 AND dpn_bsn_id = ".$bsn_id." UNION ALL SELECT dpn_unique_id FROM document_renovation_name WHERE dpn_hia_signed = 1 AND dpn_bsn_id = ".$bsn_id;
	//echo $sql_checklist;
	$checklist_number = $fwDb->query($sql_checklist);
	$fwViewData['checklist_number'] = $checklist_number;
	//echo"<pre>"; print_r($checklist_number); exit('checking');
}
