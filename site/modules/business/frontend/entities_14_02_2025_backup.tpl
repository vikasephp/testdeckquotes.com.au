			...
			<td style="text-align:center;">
                ...
                {{else}}
                    {{ if $item.er_document eq 'Evo – Electrical' }}
                        {{ $item.document_status.dpn_electric_ref }}
                    {{ elseif $item.er_document eq 'Icon – Water' }}
                        {{ $item.document_status.dpn_water_ref }}
                    {{ elseif $item.er_document eq 'TCCS – Stormwater' }}
                        {{ $item.document_status.dpn_storm_ref }}
                    {{ elseif $item.er_document eq 'Gas' }}
                        {{ $item.document_status.dpn_gas_ref }}
                    {{ elseif $item.er_document eq 'Heritage' }}
                        {{ $item.document_status.dpn_heritage_ref }}
                    {{/if}}
                {{/if}}
			</td>
		...
		<table cellpadding="0" cellspacing="0" class="table" id="upload_file_business_11">
			...
			<tr>
				...
				<td><input type="file" name="condition_snapshot" style="width:700px;" />
					<span>{{if !empty($checklist_entities_referral) }}
						{{ $checklist_entities_referral.condition_snapshot }} {{/if}}</span>
				</td>
			</tr>
