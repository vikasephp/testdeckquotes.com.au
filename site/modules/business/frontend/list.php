...
if (!empty($print)) {
	...
	if ($_SESSION['last_sql']) {
		...
		$t_task_arr = [];
		$t_task_res = $fwDb->query("SELECT bt_bsn_id, count(bt_id) as t_task FROM business_tasks WHERE bt_bsn_id IN (" . implode(",", $bsn_id_arr) . ") GROUP BY bt_bsn_id");
		foreach ($t_task_res as $k => $v) {
			$t_task_arr[$v['bt_bsn_id']] = $v['t_task'];
		}

		$compt_task_arr = [];
		$compt_task_res = $fwDb->query("SELECT bt_bsn_id, count(bt_id) as compt_task FROM business_tasks WHERE bt_bsn_id IN (" . implode(",", $bsn_id_arr) . ") AND bt_complete = 1 GROUP BY bt_bsn_id");
		foreach ($compt_task_res as $k => $v) {
			$compt_task_arr[$v['bt_bsn_id']] = $v['compt_task'];
		}

		$doc_task_arr = [];
		$doc_task_res = $fwDb->query("SELECT doc_bsn_id, count(doc_id) as doc_task FROM document_check_list WHERE doc_bsn_id IN (" . implode(",", $bsn_id_arr) . ") GROUP BY doc_bsn_id");
		foreach ($doc_task_res as $k => $v) {
			$doc_task_arr[$v['doc_bsn_id']] = $v['doc_task'];
		}

		$compdoc_task_arr = [];
		$compdoc_task_res = $fwDb->query("SELECT doc_bsn_id, count(doc_id) as compdoc_task FROM document_check_list WHERE doc_bsn_id IN (" . implode(",", $bsn_id_arr) . ") AND doc_file_name != ' ' GROUP BY doc_bsn_id");
		foreach ($compdoc_task_res as $k => $v) {
			$compdoc_task_arr[$v['doc_bsn_id']] = $v['compdoc_task'];
		}

		$sellers_arr = [];
		$sellers_res = $fwDb->query("SELECT business_sellers.*, bus_customers.bcust_id, bus_customers.bcust_fname, bus_customers.bcust_lname FROM business_sellers LEFT JOIN bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id WHERE business_sellers.bs_business_id IN (" . implode(",", $bsn_id_arr) . ")");
		foreach ($sellers_res as $k => $v) {
			if(!isset($sellers_arr[$v['bs_business_id']])) {
				$sellers_arr[$v['bs_business_id']] = [];
			}
			$sellers_arr[$v['bs_business_id']][] = $v['bcust_fname'] . " " . $v['bcust_lname'];
		}
		$data_wsexport = [];
		foreach ($exportdata as $k => $v) {
			$bid = $v['bsn_id'];
			$v['task_cnt'] = ($compt_task_arr[$bid] ?? 0) . " / " . ($t_task_arr[$bid] ?? 0);
			$v['doc_cnt'] = ($compdoc_task_arr[$bid] ?? 0) . " / " . ($doc_task_arr[$bid] ?? 0);
			$sellers = '';
			if(!empty($sellers_arr[$bid])) {
				$sellers = implode(",", $sellers_arr[$bid]);
			}
			$v['sellers'] = $sellers;
			...
			$v['statusdays'] = strtotime($v['bsn_status_date']) > 0 ? daysDifference(date('Y-m-d'), $v['bsn_status_date']) : '-';
			$v['systemdays'] = strtotime($v['bsn_status_sys_date']) > 0 ? daysDifference(date('Y-m-d'), $v['bsn_status_sys_date']) : '-';
			$data_wsexport[] = $v;
		}
	}
...
if ($export > 0) {
	...
	if ($_SESSION['last_sql']) {
		...
		foreach ($exportdata as $k => $v) {
			...
			$v['bsn_name'] = !empty($v['bsn_name']) ? $v['bsn_name'] : '-';
			$v['bsn_description'] = !empty($v['bsn_description']) ? $v['bsn_description'] : '-';
			$v['bsn_suburb'] = !empty($v['bsn_suburb']) ? $v['bsn_suburb'] : '-';
			$v['bsn_cd_ask_price_from'] = !empty($v['bsn_cd_ask_price_from']) ? $v['bsn_cd_ask_price_from'] : '-';
			$v['bsn_cd_ask_price_to'] = !empty($v['bsn_cd_ask_price_to']) ? $v['bsn_cd_ask_price_to'] : '-';
			$v['st_name'] = !empty($v['st_name']) ? $v['st_name'] : '-';
			$v['task_cnt'] = !empty($v['task_cnt']) ? $v['task_cnt'] : '-';
			$v['doc_cnt'] = !empty($v['doc_cnt']) ? $v['doc_cnt'] : '-';
			$v['sellers'] = !empty($v['sellers']) ? $v['sellers'] : '-';
			$v['systemdays'] = !empty($v['systemdays']) ? $v['systemdays'] : '-';
			$v['statusdays'] = !empty($v['statusdays']) ? $v['statusdays'] : '-';
			$data_wsexport[] = $v;
		}
		...
		foreach ($data_wsexport as $k => $v) {
			foreach ($v as $k2 => $v2) {
				if (in_array($k2, $fields)) {
					$csv_output .= stripslashes(trim(preg_replace('/\s+/', ' ', $v2))) . "\t";
				}
			}
			$csv_output .= "\n";
		}
...
if (!empty($submit)) {
	...
	// echo "<br><br>Start Script Execution Time = " . $execution_time . " sec";
	...
	$end_time = microtime(true);
	$execution_time = ($end_time - $start_time);
	// echo "<br><br>Finished where and order Script Execution Time = " . $execution_time . " sec";
	// die();
	...
	if (empty($_SESSION['last_sql'])) {
		if ($strings) {
			...
			$sql = "SELECT DISTINCT business.bsn_id AS distinct_id, business.*, busness_status.*, project_type.*, business_sellers.*, bus_customers.bcust_id, bus_customers.bcust_fname, bus_customers.bcust_lname FROM business 
			...
			$sql = "SELECT DISTINCT business.bsn_id AS distinct_id, business.*, busness_status.st_name, project_type.pt_name FROM  business LEFT JOIN busness_status ON business.bsn_status = busness_status.st_id LEFT JOIN project_type ON business.bsn_type = project_type.pt_id WHERE business.bsn_name != '' " . $order;
...
if ($sql) {
	...
	$execution_time = ($end_time - $start_time);
	// echo "<br><br>$sql";
	// echo "<br><br>Finished sql Script Execution Time = " . $execution_time . " sec";
	// die();
	...
	// $execution_time = ($end_time - $start_time);
	// echo "<br><br>$sql2";
	// echo "<br><br>Finished sql2 Script Execution Time = " . $execution_time . " sec";
...
if (!empty($data)) {
	...
	if ($data) {
		...
		// $end_time = microtime(true);
		// $execution_time = ($end_time - $start_time);
		// echo "<br><br>data foreach Script Execution Time = " . $execution_time . " sec";
		// die();
		...
		// $end_time = microtime(true);
		// $execution_time = ($end_time - $start_time);
		// echo "<br><br>t_task_res foreach Script Execution Time = " . $execution_time . " sec";
		...
		// $end_time = microtime(true);
		// $execution_time = ($end_time - $start_time);
		// echo "<br><br>compt_task_res foreach Script Execution Time = " . $execution_time . " sec";
		...
		// $end_time = microtime(true);
		// $execution_time = ($end_time - $start_time);
		// echo "<br><br>doc_task_res foreach Script Execution Time = " . $execution_time . " sec";
		...
		// $end_time = microtime(true);
		// $execution_time = ($end_time - $start_time);
		// echo "<br><br>compdoc_task_res foreach Script Execution Time = " . $execution_time . " sec";
		...
		// $end_time = microtime(true);
		// $execution_time = ($end_time - $start_time);
		// echo "<br><br>sellers_res foreach Script Execution Time = " . $execution_time . " sec";

		// db($bsn_id_arr, 'bsn_id_arr'); die();
		...
		foreach ($data as $k => $v) {
			...
			$sellers = '';
			if(!empty($sellers_arr[$bid])) {
				foreach ($sellers_arr[$bid] as $k1 => $v1) {
					$sellers .= $v1 . "<br>";
				}
			}
			$v['sellers'] = $sellers;
