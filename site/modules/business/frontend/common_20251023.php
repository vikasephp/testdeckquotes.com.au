	...
	switch ($requset) {
	  ...
	  WHERE e_dossier_log.edl_bus_id = " . $bus_id . " ORDER BY e_dossier_log.edl_id DESC";
		...
		case 'addStatusTask':
			...
			/* $business_tasks_notsynced = $fwDb->query("SELECT * FROM business_tasks WHERE bt_bsn_id = $bus_id AND bt_task_status_id = $taskStatusId AND bt_task_id NOT IN (" . implode(',', $bt_task_id_arr) . ")");
			// db($business_tasks_notsynced, 'business_tasks_notsynced');
			$business_tasks_query = 'INSERT INTO business_tasks (bt_bsn_id, bt_task_status_id, bt_task_id) VALUES ';
			$business_tasks_count = 0;
			foreach ($business_tasks_notsynced as $k_bts => $v_bts) {
				$business_tasks_query .= '(' . $bus_id . ', ' . $taskStatusId . ', ' . $v_bts['bt_task_id'] . '),';
				$business_tasks_count++;
			}
			...
			if($business_tasks_count > 0){
				$business_tasks_query = rtrim($business_tasks_query, ',') . ';';
				echo "<br><br>business_tasks_query = " . $business_tasks_query;
				// mysqli_multi_query($MySqliCon, $business_tasks_query);
			} */
