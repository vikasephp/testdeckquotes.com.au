...
if (!empty($submit)) {
	...
	if ($detail['bsn_cd_ask_price_from'] and empty($detail['bsn_cd_ask_price_to'])) {
	...
	if (empty($detail['bsn_cd_ask_price_from']) and $detail['bsn_cd_ask_price_to']) {
		$string .= "business.bsn_cd_ask_price_to <= '" . trim($detail['bsn_cd_ask_price_to']) . "' AND ";
	}

	if ($detail['bsn_cd_ask_price_from'] and $detail['bsn_cd_ask_price_to']) {
...
if ($sql) {
    ...
    $total_rows = 0 ; $total_rows_data = $fwDb->query($sql);
    if(!empty($total_rows_data)){ $total_rows = count($total_rows_data); }
    if($total_rows == 0){ $fwViewData['search_msg'] = "No projects were found."; }
	if (!(isset($pagenum))) {
		$pagenum = 1;
	}elseif ($pagenum <= 1) {
		$pagenum = 1;
