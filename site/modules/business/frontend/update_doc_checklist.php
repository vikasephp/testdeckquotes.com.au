<?php		
	...
	if($action_name == "")
	{
    	...
    	{
    	    $postedData = $_POST;
    	    $doc_id = isset($postedData['doc']) ? $postedData['doc']['doc_id'] : $doc_id;
    	        $checklistTableTask = new Fw_Db_Table('checklist_entities_referral');
    		...
    		$er_id = 0;  // set dafault value
			$entities_get_data = "SELECT er_id,er_dc_uid FROM `entities_referral` WHERE er_dc_uid = '".$doc_data['doc_name_id']."'";
            $entities_referral_record = $fwDb->queryOne($entities_get_data);
            if (!empty($entities_referral_record)) {
                $er_id = $entities_referral_record['er_id'];
                
                $checklist_data = "SELECT * FROM `checklist_entities_referral` WHERE bsn_id = '".$doc_data['doc_bsn_id']."' AND entites_referral_id = '".$er_id."'";
                $checklist_record = $fwDb->queryOne($checklist_data);
                if (empty($checklist_record)) {
                    $checklistDetail['user_id'] = $_SESSION['user']['user_id'];
                    $checklistDetail['bsn_id'] = $doc_data['doc_bsn_id'];
                    $checklistDetail['entites_referral_id'] = $er_id;
                    $checklistTableTask->insertRow($checklistDetail);
                }
            }
    		...
    		$submit_iframe = $fwRequest -> getParam('submit_iframe','');
    			...
    			$doc = $fwRequest -> getParam('doc','');
					...
					if (!empty($doc['doc_live_link'])) {
						$doc['doc_live_link_username'] = $_SESSION['user']['user_name'];
						//$doc['doc_live_link_username'] = $_SESSION['user']['user_id'];
    					$doc['doc_live_link_date'] = date('Y-m-d');
					}
    				...
    				$checklistDetail = [
                        'assessment_doc' => $docfile,
                        'user_id' => $_SESSION['user']['user_id'],
                        'assessment_upload_doc_date' => date('Y-m-d H:i'),
                        'date_applied' => date('Y-m-d')
                    ];
					
					$checklistTableTask->setWhere("bsn_id = '".$doc_data['doc_bsn_id']."' and entites_referral_id ='".$er_id."'");
                    $checklistTableTask->updateRow($checklistDetail);
