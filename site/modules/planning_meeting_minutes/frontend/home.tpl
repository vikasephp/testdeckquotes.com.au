...
function add_new()
 {
var viewurl = "{{$BASE_URL}}planning_meeting_minutes.detail"+ "/random/" + Math.random();	
 $(document).ready(function () {
        $.fancybox({
				'width': '78%',
				'height': '80%',
...
<input type="text" name="keyword" value="{{$keyword}}" style="width:400px;" placeholder="Search by document name"/>
&nbsp;&nbsp;&nbsp;&nbsp;
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="34%">Download Word/Excel</th>
    ...
    <form name="mlist" method="post">
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
        <td>{{$item.mm_uminid}}.</td>
        <td>{{$item.mm_doc_name}}</td>
        <td>{{$item.mm_date_uploaded}}</td>
        <td class="tooltip">{{if $item.mm_doc_pdf}}
        <!--<a href="https://deckquote.s3.amazonaws.com/files/meeting_minutes_doc/{{$item.mm_doc_pdf}}?dummy={{$ran}}" target="_blank">-->
        <a href="/planning_meeting_minutes.download_content?file_name={{$item.mm_doc_pdf}}&module_name=planning_meeting_minutes.home" target="_blank">
        {{$item.mm_doc_pdf|truncate:70:"...":true}}</a>
      <span class="tooltiptext">{{$item.mm_doc_pdf|stripslashes}}</span> {{/if}}</td>
        <td class="tooltip">{{if $item.mm_doc_word}}
        <!--<a href="https://deckquote.s3.amazonaws.com/files/meeting_minutes_doc/{{$item.mm_doc_word}}?dummy={{$ran}}"> -->
         <a href="/planning_meeting_minutes.download_content?file_name={{$item.mm_doc_word}}&module_name=planning_meeting_minutes.home" target="_blank">
        {{$item.mm_doc_word|truncate:70:"...":true}} </a>
         <span class="tooltiptext">{{$item.mm_doc_word|stripslashes}}</span> {{/if}}
        </td>
