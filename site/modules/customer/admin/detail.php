...
if(empty($detail['bcust_misc_logged_user'])){$detail['bcust_misc_logged_user'] = $user_name;}
   ...
   $table_id = (int)$detail[$ID];
	...
	if(!empty($table_id)){$table->setWhere($ID." = ".$table_id);$table->updateRow($detail);
	
	if(!empty($detail['user_password']) && !empty($detail['conf_password']) && $detail['user_password'] == $detail['conf_password'])
		{
		$data['user_password'] = md5($detail['user_password']);
		
		$tableUser->setWhere("user_id = ".$detail['bcust_user_id']);
		$tableUser->updateRow($data);
		}
	}else{$table->insertRow($detail);}
