...
<form name="search" action="{{$XFA.list}}" method="post">
        <thead>
            <tr>	<th align="center" valign="middle" width="15%">First Name:&nbsp;</th><td><input type="text" class="lrg" name="{{$TABLE}}[bcust_fname]" value="{{if $detail.bcust_fname}}{{$detail.bcust_fname}}{{/if}}" /> </td>    
            		<th align="center" valign="middle" width="10%"> Last Name:&nbsp;</th><td><input type="text" class="lrg" name="{{$TABLE}}[bcust_lname]" value="{{if $detail.bcust_lname}}{{$detail.bcust_lname}}{{/if}}"/> </td>  
            </tr>
            <tr>	<th align="center" valign="middle" width="15%">Email:&nbsp;</th><td><input type="text" class="lrg" name="{{$TABLE}}[bcust_misc_email1]" value="{{if $detail.bcust_misc_email1}}{{$detail.bcust_misc_email1}}{{/if}}"/> </td>    
            		<th align="center" valign="middle" width="10%"> Phone:&nbsp;</th><td><input type="text" class="lrg" name="{{$TABLE}}[phones]" value="{{if $detail.phones}}{{$detail.phones}}{{/if}}"/> </td>  
            </tr>
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
             <td><a href="{{$BASE_URL}}{{$XFA.detail}}/{{$ID}}/{{$item.$ID}}">
                ...
                <td>{{if $item.ul_date}} {{$item.ul_date|date_format:"%d-%m-%Y"}}{{else}} Not Logged{{/if}}</td>
                <td> {{$item.cs_name}}</td>
