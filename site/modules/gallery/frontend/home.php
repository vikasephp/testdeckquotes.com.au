...
if($_SESSION['sort_order']){$order = "ORDER BY ".$_SESSION['sort_order'];}else{$order = "ORDER BY CONVERT (".$TABLE.".bi_title USING latin2) ASC";}
...
if(!empty($submit))
	...
	else{$sql = "SELECT ".$TABLE.".*, users.user_name FROM ".$TABLE." LEFT JOIN users ON users.user_id = ".$TABLE.".bi_author ".$order;}
				...
				if($doc_files)
					{
						foreach($doc_files as $kf => $vf)
								{
								if($vf['bid_type']){$vf['iconpath'] = get_icon_path($vf['bid_type'], $vf['bid_name'], $Document_path);}
								$filedata[] = $vf;
								}
					}
