    ...
    <table id="doclist-table" class="nav-back" width="100%">
        <thead>
         <tr>	
                    ...
                    <th class="topmenu" align="center" valign="middle" width="280">Download Images</th>
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
            	<td>{{$countn}}.</td>
                <td>{{$item.bi_title}}</td>
                <td>{{$item.bi_summery|truncate:200:"...":true}}</td>
               <td>{{$item.bi_bus_streem}}</td>
               <td>{{if $item.bi_access == 1}}All Access{{else}}Users{{/if}}</td>
                <td>{{$item.user_name}}</td>
                <td>{{if $item.bi_last_update !== '0000-00-00 00:00:00'}}{{$item.bi_last_update|date_format:"%d/%m/%Y %H:%M"}}{{else}}
                  {{$item.bi_insert_date|date_format:"%d/%m/%Y %H:%M"}}{{/if}}</td>
                 <td>
                 ...
                 {{foreach from=$item.doc_files key="keyfiles" item="itemfiles"}}
                 	...
                 	<td>
                    <!--<a href="{{$BASE_URL}}{{$FILE_PATH}}files/{{$document_path}}/{{$itemfiles.bid_name}}" target="_blank" title="Start download">-->
                    <a href="/gallery.download_content?file_name={{$itemfiles.bid_name}}&module_name=gallery.home" title="Start download" >
                    Download</a>
                    
                    
                    <br />
                   <!-- <a href="{{$BASE_URL}}{{$FILE_PATH}}files/{{$document_path}}/{{$itemfiles.bid_name}}" title="{{$itemfiles.bid_name}}" target="_blank">-->
                   <!-- <a href="/gallery.download_content?file_name={{$itemfiles.bid_name}}&module_name=gallery.home" title="Start download" >
                    {{$itemfiles.bid_name|truncate:"9"}}</a>-->
                    </td>
                 {{if $count % 4 == 0}}</tr><tr>
                 {{/if}}
                 {{counter}}
