...
<h3 class="page-title">{{$title}}</h3>
 ...
 <input type="hidden" name="{{$TABLE}}[{{$ID}}]" value="{{$detail.$ID}}" id="dh_id"/>
     <table id="viewdoc-table" width="100%" cellpadding="0" cellspacing="0">
        <tr>
            <th > Name:</th>
            <td> <input type="text" name="{{$TABLE}}[bi_title]" value="{{$detail.bi_title}}" style="width:300px;"/></td>
        </tr>
        <tr>
            <th> Summary :</th>
            <td><textarea rows="8" cols="60" name="{{$TABLE}}[bi_summery]">{{$detail.bi_summery}}</textarea> </td>
        </tr>   
         <tr>
            <th> Business Stream:</th>
            <td> <input type="text" name="{{$TABLE}}[bi_bus_streem]" value="{{$detail.bi_bus_streem}}" size="35" /></td>
        </tr>
        <tr>
            <th > Access:</th>
            <td> <select name="{{$TABLE}}[bi_access]" class="lrg" style="width:220px;">
                        <option value="1" {{if $detail.bi_access == 1}} selected="selected" {{/if}}> All Users</option>
                        <option value="2" {{if $detail.bi_access == 2}} selected="selected" {{/if}}> Users</option>    	    
  				 </select>
            </td>
        </tr>
        <tr>
...
<script type="text/javascript" language="javascript" >
    ...
    frmvalidator.addValidation("{{$TABLE}}[bi_title]","req", "Please specify title.");
</script>
{{/if}}