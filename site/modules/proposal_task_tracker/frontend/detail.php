...
$tableDesignInterfacePricing = new Fw_Db_Table('design_interface_pricing');
...
else
{
	...
	if($tableDesignInterfacePricing->rowExists())
	{
 		 //$sqldip = "select * from design_interface_pricing where di_id = ".$this_id;
		 $sqldip = "SELECT design_interface_pricing.* , users.user_name FROM design_interface_pricing
                    LEFT JOIN users ON design_interface_pricing.dip_user = users.user_id
                    WHERE di_id = ".$this_id. " Order By design_interface_pricing.dip_order ";
	 ...
	 $sqldt1 = "select * from design_interface_pricing where di_id = ".$this_id . " AND dip_component like '%Rounded Total Price Total%' ";
	 ...
	 $sqlgp = "select * from design_interface_pricing where di_id = ".$this_id . "  AND dip_component = 'GP Value'";
     $gpDetail = $fwDb->queryOne($sqlgp);
     $fwViewData['gp_value'] = $gpDetail['dip_value_entered'];
 
	
	 $sqlteam = "select * from design_interface_edropbox where didb_di_id = ".$this_id . " AND didb_foldername like '%Team%' ";
     $teamDetail = $fwDb->queryOne($sqlteam);
     $fwViewData['ms_team'] =  str_replace("!",'/',$teamDetail['didb_filepath']);
     //str_replace("!",'/',$di_dropbox[$dk]['didb_filepath']);


	$tabledipMaster = new Fw_Db_Table('design_interface_pricing_master');
...
//							$docfile =   $_FILES['design_img']['name'];
//							$file_type = $_FILES['design_img']['type'];
//							$datetime = trim(date('d_m_Y_H_i_'),'0');
//							$docfile = preg_replace('/[^A-Z0-9._]/i', '_', $docfile);
//							$src = $_FILES['design_img']['tmp_name'];
//							$destination = BASE_DIR.'files/design_interface/'.$docfile;
//																
...
//							{
//								 $detailDesign['dd_di_id']= $des_id;
//								 $detailDesign['di_design_image']= $docfile;
