...
{
	var viewurl = "{{$BASE_URL}}street_sign_construction.show-notes-content"+ "/ss_id/" + ssid;	
...
function add_new()
 {
var viewurl = "{{$BASE_URL}}street_sign_construction.detail"+ "/random/" + Math.random();	
 $(document).ready(function () {
        $.fancybox({
				'width': '95%',
				'height': '90%',
...
{
	var viewurl = "{{$BASE_URL}}street_sign_construction.view_procedure"+ "/random/" + Math.random();	
...
function add_type()
{
	var viewurl = "{{$BASE_URL}}street_sign_construction.view_type"+ "/random/" + Math.random();	
	$(document).ready(function () {
...
function add_action()
{
	var viewurl = "{{$BASE_URL}}street_sign_construction.view_action"+ "/random/" + Math.random();	
	$(document).ready(function () {
...
{
	var viewurl = "{{$BASE_URL}}street_sign_construction.view_intensity"+ "/random/" + Math.random();	
	$(document).ready(function () {
...
{
	var viewurl = "{{$BASE_URL}}street_sign_construction.view_banner_action"+ "/random/" + Math.random();	
	$(document).ready(function () {
    ...
    <!--<form name="mlist" method="post">-->
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
		...
		function update_intensity(id,value)
		{
			 ...
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign_construction.update_intensity/ss_id/"+id+"/value/"+value,
		...
		function update_sign_action(id,value)
		{
			 ...
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign_construction.update_signaction/ss_id/"+id+"/value/"+value,
					   success: function(result){
						   $(tana).html(result);
					}
				});
		...
		function update_banner_action(id,value)
		{
			 ...
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign_construction.update_banner_action/ss_id/"+id+"/value/"+value,
					   success: function(result){
						   $(tana2).html(result);
					}
				});
		...
		function update_sign_type(id,value)
		{
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}street_sign_construction.update_signtype/ss_id/"+id+"/value/"+value,
					   success: function(result){
					}
				});
		}
         ...
         <a href="/street_sign_construction.download_content?file_name={{$item.ss_actmapi_image}}&module_name=street_sign_construction.home" >Download</a><br />
            {{$item.ss_actmapi_image_date}}
            {{/if}}
         </td>
         
         <td> {{if $item.ss_street_sign_image}}
         <a href="/street_sign_construction.download_content?file_name={{$item.ss_street_sign_image}}&module_name=street_sign_construction.home" >Download</a>
        <!-- <a href="https://deckquote.s3.amazonaws.com/files/street_sign_construction/{{$item.ss_street_sign_image}}" target="_blank">Download</a> --><br />
            {{$item.ss_sign_image_date}}
            {{/if}}
         </td>
         <td>{{if $item.ss_banner_image}}
         <a href="/street_sign_construction.download_content?file_name={{$item.ss_banner_image}}&module_name=street_sign_construction.home" >Download</a>
        <!-- <a href="https://deckquote.s3.amazonaws.com/files/street_sign_construction/{{$item.ss_banner_image}}" target="_blank">Download</a> --><br />
  ...
  <!--          <input type="button" name="View Notes" value="View  Notes" onclick="javascript:show_notes({{$item.ss_id}});" style="width:90px; height:20px; font-size:11px; line-height:17px !important; padding:0px !important" />-->
	   ...
	   function update_notes(id,value)
		...
		//alert(id);	
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}street_sign_construction.update_notes/ss_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
