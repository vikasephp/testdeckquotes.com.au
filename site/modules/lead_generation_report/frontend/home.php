...
elseif($_SESSION['st_date']):
	...
	$where .= " AND STR_TO_DATE(business.bsn_status_sys_date , '%Y-%m-%d' ) > CURDATE( ) - INTERVAL " .$_SESSION['st_date']. " DAY ";	
	    ...
	    $dt1 = changedate_y_m_d($fwRequest->getParam('dt_date1', ''));
		$dt2 = changedate_y_m_d($fwRequest->getParam('dt_date2', ''));
		
		if(strtotime($dt1) and strtotime($dt2)) {
			$where .= " AND business.bsn_status_sys_date BETWEEN '$dt1' AND '$dt2' ";
			$_SESSION['dt1'] = $dt1;
			$_SESSION['dt2'] = $dt2;
		}
		else if(strtotime($_SESSION['dt1']) and strtotime($_SESSION['dt2'])) {	
			$where .= " AND STR_TO_DATE(business.bsn_status_sys_date, '%Y-%m-%d') BETWEEN
		                STR_TO_DATE('".$_SESSION['dt1']."','%Y-%m-%d') AND STR_TO_DATE('".$_SESSION['dt2']."','%Y-%m-%d') ";
		}
...
//Weekly & YTD data for leads, On Hold Status and Lost Status
$lead_weekly = "SELECT COUNT(*) AS weekly_total FROM business WHERE bsn_status_sys_date >= CURDATE() - INTERVAL 7 DAY";
...
$onhold_weekly = "SELECT COUNT(*) AS weekly_total FROM business WHERE bsn_status_sys_date >= DATE_FORMAT(CURDATE(), '%Y-07-01') AND bsn_sub_status = 'Hold' ";
$onhold_data_weekly = $fwDb->queryOne($onhold_weekly);
$fwViewData['onhold_data_weekly'] = $onhold_data_weekly;

$onhold_ytd = "SELECT COUNT(*) AS ytd_total FROM business WHERE bsn_status_sys_date >= IF(MONTH(CURDATE()) >= 7, DATE_FORMAT(CURDATE(), '%Y-07-01'), DATE_FORMAT(CURDATE() - INTERVAL 1 YEAR, '%Y-07-01')) AND bsn_sub_status = 'Hold' ";
$onhold_data_ytd = $fwDb->queryOne($onhold_ytd);
$fwViewData['onhold_data_ytd'] = $onhold_data_ytd;

$lost_weekly = "SELECT COUNT(*) AS weekly_total FROM business WHERE bsn_status_sys_date >= DATE_FORMAT(CURDATE(), '%Y-07-01') AND bsn_sub_status = 'Lost' ";
$lost_data_weekly = $fwDb->queryOne($lost_weekly);
$fwViewData['lost_data_weekly'] = $lost_data_weekly;

$lost_ytd = "SELECT COUNT(*) AS ytd_total FROM business WHERE bsn_status_sys_date >= IF(MONTH(CURDATE()) >= 7, DATE_FORMAT(CURDATE(), '%Y-07-01'), DATE_FORMAT(CURDATE() - INTERVAL 1 YEAR, '%Y-07-01')) AND bsn_sub_status = 'Lost' ";
$lost_data_ytd = $fwDb->queryOne($lost_ytd);
$fwViewData['lost_data_ytd'] = $lost_data_ytd;
...
$objPHPExcel = new PHPExcel();
       ...
       // $excel = $this->get('phpexcel')->createPHPExcelObject();
        ...
        $excel->getActiveSheet()->setTitle('Total_Enquiry');
...
$srrow2 = $row-1;
        $dataseriesLabels1 = array(
            new \PHPExcel_Chart_DataSeriesValues('String', 'Total_Enquiry!$B$1', NULL, 1), //  Rainfall
        );


	    $xAxisTickValues = array(
            new \PHPExcel_Chart_DataSeriesValues('String', 'Total_Enquiry!$A$2:$A$'.$srrow, NULL, 12), //  Jan to Dec
        );

  		
		  $dataSeriesValues1 = array(
            new \PHPExcel_Chart_DataSeriesValues('Number', 'Total_Enquiry!$B$2:$B$'.$srrow, NULL, 12),
        );
	  ...
	  // Start second chart
        ...
        $excel->getActiveSheet()->setTitle('Average_Budget');
		$excel->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
	    ...
	    $excel->setActiveSheetIndex(1)
            ->setCellValue('A1', 'Enquiry Source')
            ->setCellValue('B1', 'Average Budget')
			;	
		...
		{
			$excel->getActiveSheet()
						->setCellValue('A'.$r, $v2['soe_name'])
						->setCellValue('B'.$r, round($v2['number'],0))
						;	
			$r= $r+1;				
		}
	   ...
	   $dataseriesLabels12 = array(
            new \PHPExcel_Chart_DataSeriesValues('String', 'Average_Budget!$B$1', NULL, 1), //  Rainfall
        );


	    $xAxisTickValues2 = array(
            new \PHPExcel_Chart_DataSeriesValues('String', 'Average_Budget!$A$2:$A$'.$mrow2, NULL, 12), //  Jan to Dec
        );

  		
		  $dataSeriesValues2 = array(
            new \PHPExcel_Chart_DataSeriesValues('Number', 'Average_Budget!$B$2:$B$'.$mrow2, NULL, 12),
        );
		...
		$titleY2 = new \PHPExcel_Chart_Title('Average Budget');
		...
		foreach($setdata2 as $k=>$v)
		{
			...
			$excel->getActiveSheet()
						...
						->setCellValue('B'.$row, $v['bsn_status_sys_date'])
						->setCellValue('C'.$row, $v['bsn_address'])
						->setCellValue('D'.$row, $v['bsn_sub_status'])
						->setCellValue('E'.$row, $data_1['soe_name'])
						->setCellValue('F'.$row, $yn)
