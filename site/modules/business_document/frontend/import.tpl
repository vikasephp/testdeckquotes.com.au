		...
		<div class="bd-header">
			<div class="bd-title-block">
				...
				<div class="bd-title-text">
					...
					<p>Pull known deckquotes / GCONS URLs into the Central URL Table in one go.</p>
				</div>
			</div>
				...
				<table class="bd-table bd-table-sm">
					...
					<tbody>
						...
						<tr>
							...
							<td><span class="bd-path" style="color:#1d4ed8; text-decoration:underline; cursor:pointer;" title="{{$item.path|escape:'html'}}" onclick="window.open('{{$item.path|escape:'javascript'}}');">{{$item.path|escape:'html'}}</span></td>
							<td>
								{{if $item.already}}
