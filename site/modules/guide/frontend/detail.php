...
$tabledoc = new Fw_Db_Table('bes_guide_docs');
...
if(!empty($submit))
{
	...
	if($_FILES['docs'])
		...
		$file_type = $_FILES['docs']['type'];
				...
				$docs['bgd_name'] = $docfile;
				$docs['bgd_type'] = $file_type;
				$docs['bgd_size'] = $_FILES['docs']['size'];
				$docs['bgd_insert_date'] = date('Y-m-d H:i');
				
			//	$destination = BASE_DIR.'files/cgfb_guide/'.$docfile;
...
if($docfile){ $tabledoc->setWhere("bgd_name = '".$docfile."'");
