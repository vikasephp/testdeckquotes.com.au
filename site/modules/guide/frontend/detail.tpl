...
<script type="text/javascript">
	    ...
	    var geturl= "{{$BASE_URL}}guide.doclist?bgd_dh_id="; // The server-side script
		var getdelurl= "{{$BASE_URL}}guide.docdelete?bgd_id="; // The server-side script
...
<div align="center">
  ...
  <form name="detail" method="post" action="{{$XFA.detail}}" enctype="multipart/form-data">
    <input type="hidden" name="bm_listing[bg_id]" value="{{$detail.$ID}}" id="dh_id"/>
    <table id="viewdoc-table" width="100%" cellpadding="0" cellspacing="0">
      <tr>
        <th> Name:</th>
        <td><input type="text" name="bm_listing[bg_title]" value="{{$detail.bg_title}}" style="width:300px;"/></td>
      </tr>
      <tr>
        <th> Summary :</th>
        <td><textarea rows="8" cols="60" name="bm_listing[bg_summery]">{{$detail.bg_summery}}</textarea>
        </td>
      </tr>
      <tr>
        <th> Business Stream:</th>
        <td><input type="text" name="bm_listing[bg_bus_streem]" value="{{$detail.bg_bus_streem}}" size="35" /></td>
      </tr>
      <tr>
        <th > Access:</th>
        <td><select name="bm_listing[bg_access]" class="lrg" style="width:220px;">
            <option value="1" {{if $detail.bg_access == 1}} selected="selected" {{/if}}> All Users</option>
            <option value="2" {{if $detail.bg_access == 2}} selected="selected" {{/if}}> Users</option>
          </select>
        </td>
      </tr>
      <tr>
...
<script type="text/javascript" language="javascript" >
    ...
    frmvalidator.addValidation("bm_listing[bg_title]","req", "Please specify title.");
</script>
{{/if}}