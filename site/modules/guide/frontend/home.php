...
if($short)
	...
	switch ($short)
			{
				case 'lc':
				$_SESSION['sort_order'] = $TABLE.".bg_title DESC";
				break;
				
				case 'bl':
				$_SESSION['sort_order'] = $TABLE.".bg_title ASC";
				break;
			}
	}
...
if(!empty($submit))
	...
	else{$sql = "SELECT ".$TABLE.".*, users.user_name FROM ".$TABLE." LEFT JOIN users ON users.user_id = ".$TABLE.".bg_author ".$order;}
...
foreach($setdata as $k => $v)
		{
		$docid = $v['bg_id'];
		$sqldocs = "SELECT bes_guide_docs.* FROM bes_guide_docs WHERE bes_guide_docs.bgd_dh_id = ".$docid;
		$doc_files = $fwDb->query($sqldocs);
				if($doc_files)
					{
						foreach($doc_files as $kf => $vf)
								{
								if($vf['bgd_type']){$vf['iconpath'] = get_icon_path($vf['bgd_type'], $vf['bgd_name'], 'bes_guide');}
								$filedata[] = $vf;
								}
					}
