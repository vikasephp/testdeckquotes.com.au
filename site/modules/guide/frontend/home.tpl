...
function add_new()
 {
var viewurl = "{{$BASE_URL}}guide.detail"+ "/random/" + Math.random();	
 $(document).ready(function () {
        $.fancybox({
				'width': '58%',
				'height': '70%',
    ...
    <table id="doclist-table" class="nav-back" width="100%">
        ...
        {{foreach from=$list key="key" item="item"}}
            <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
            	<td>{{$countn}}.</td>
                <td>{{$item.bg_title}}</td>
                <td>{{$item.bg_summery|truncate:200:"...":true}}</td>
               <td>{{$item.bg_bus_streem}}</td>
               <td>{{if $item.bg_access == 1}}All Access{{else}}Users{{/if}}</td>
                ...
                <td>{{if $item.bg_last_update !== '0000-00-00 00:00:00'}}{{$item.bg_last_update|date_format:"%d/%m/%Y %H:%M"}}{{else}}
                 ...
                 {{foreach from=$item.doc_files key="keyfiles" item="itemfiles"}}
                    ...
                    <li><img src="{{$itemfiles.iconpath}}" width="20" /> <br />
                      <a  class="shofilemenu">{{$itemfiles.bgd_name|truncate:"9"}}</a>
                      <ul class="small-menu">
                        <li> {{if $itemfiles.bgd_docs_status == 0}}
                        
                 <!--       <a href="{{$BASE_URL}}{{$FILE_PATH}}files/cgfb_guide/{{$itemfiles.bgd_name}}" title="{{$itemfiles.bgd_name}}" target="_blank" style="color:#FFFFFF;">-->
                    <a href="/guide.download_content?file_name={{$itemfiles.bgd_name}}&module_name=guide.home" target="_blank"/>    
                        {{else}}<a href="{{$BASE_URL}}dochub.open/fileid/{{$itemfiles.bgd_docs_path}}"  class="shofile" style="color:#FFFFFF;" >{{/if}}View/Edit</a>
                    ...
                    <!--    <a href="{{$BASE_URL}}{{$FILE_PATH}}files/cgfb_guide/{{$itemfiles.bgd_name}}" title="{{$itemfiles.bgd_name}}" target="_blank" style="color:#FFFFFF;">-->
                  <a href="/guide.download_content?file_name={{$itemfiles.bgd_name}}&module_name=guide.home" target="_blank"/>         
                        {{else}}<a href="{{$BASE_URL}}dochub.open/download/1/fileid/{{$itemfiles.bgd_docs_path}}"  title="Start Download" style="color:#FFFFFF;">
                        {{/if}}Download</a>
     ...
     <a href="{{$BASE_URL}}{{$XFA.delete}}/{{$ID}}/{{$item.$ID}}" onclick="javascript:if(!confirm('Are you sure want to delete the document? Because it will delete all the document related to this listing.')) return false;" title="Delete"><img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/delete.png"/></a>
  <!--   {{/if}}-->
     </td>
            </tr>
