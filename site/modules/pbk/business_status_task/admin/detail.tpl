...
<option value="{{$item.st_id}}" {{if $detail.bst_business_status_id == $item.st_id OR $current_bst_business_status_id == $item.st_id}} selected="selected" {{/if}}>{{$item.st_name}}</option>
                    {{/foreach}} 
                </select>
              </td>
          </tr>
           <tr>
                ...
                <td width="80%">
              <select name="data[bst_business_types_id]" class="lrg">
                	<option value="">-- Select --</option>
                	{{foreach from=$busness_types item=item key=key}}
<option value="{{$item.pt_id}}" {{if $detail.bst_business_types_id == $item.pt_id OR $current_bst_business_status_id == $item.pt_id}} selected="selected" {{/if}}>{{$item.pt_name}}</option>
                    {{/foreach}} 
                </select>
              </td>
          </tr>
