...
$tableStaff = new Fw_Db_Table('quote_tracking_resp_staff');
$submit = $fwRequest->getParam('subAddDetail', '');
$co_id = $fwRequest->getParam('co_id', 0);
$fwViewData['co_id'] = $co_id;
$qt_id = $fwRequest->getParam('qt_id', '');
	...
	$matsql = "SELECT contacts.* , companies.co_company_name, type_options.to_option  from contacts 
	 ...
	 foreach($list as $ck=>$vc)
	 {
		...
		$sqlc = "Select * from quote_tracking_resp_staff  where ir_email = '".$vc['cs_primary_email']."' and ir_qt_id = ".$qt_id;
...
if(!empty($add_emails))
{
	...
	foreach($emails as $k => $v)
	{
		...
		if($v =='on') {
		 ...
		 $tableStaff->setWhere("ir_email = '".$email."' and ir_qt_id =".$qt_id);
	 	 if(!$tableStaff->rowExists())
	         {
			if($v == 'on') {
				$irDetail['ir_qt_id'] = $qt_id;
	 ...
	 $detail['qt_responsible_staff'] = $poseml2;
	 $table->setWhere("qt_id = ".$qt_id);
