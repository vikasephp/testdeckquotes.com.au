...
window.location.href = "{{$BASE_URL}}quote_tracking_report.view_status_delivery";
</script>
{{/if}}
...
<table id="list-table" width="100%">
  ...
  <tr>
     ...
     <td><select  name="task[qs_color]">
       ...
       <option value= "#0000f7" {{if $detail.qs_color eq '#0000f7'}} selected="selected" {{/if}}>Blue</option>
       <option value= "#eec017" {{if $detail.qs_color eq '#eec017'}} selected="selected" {{/if}}>Gold</option>
       <option value= "#7d007d" {{if $detail.qs_color eq '#7d007d'}} selected="selected" {{/if}}>Purple</option>
       </select>
