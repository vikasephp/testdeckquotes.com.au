 ...
 function add_procedure()
{
 ...
 function add_status()
{
	var viewurl = "{{$BASE_URL}}quote_tracking_report.view_status_po"+ "/random/" + Math.random();	
...
.wrapper2{ }
.div1 {width:6590px; height: 20px; }
.div2 {width:6590px;  overflow: auto;}   
...
<input type="button" name="back_qtr" value="Delivery" onclick="javascript:delivery();"/>
    ...
    <form name="mlist" method="post">
     ...
     <input type="text" name="start_onsite_date[{{$item.qt_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo1_{{$item.qt_id}}" value="{{$item.bsn_starting_onsite_date}}" onfocus = "update_onsite({{$item.bsn_id}}, this.value);" /> 
       ...
       <option value="">Please Select</option>
        {{foreach from=$pustdetail key="key100" item="item100"}}
           <option  {{if $item100.qs_name eq $item.qt_wall_roof}} selected="selected" {{/if}}>{{$item100.qs_name}}</option>
        {{/foreach}}
      </select>  
        ...
        function update_status(id,value,colname)
		{
			 ...
			 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}quote_tracking_report.update_po_status/bsn_id/"+id+"/value/"+value+"/col/"+colname,
				   success: function(result){
