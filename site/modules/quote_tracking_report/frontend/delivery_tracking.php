...
$table = new Fw_Db_Table('quote_tracking_report_delivery');
...
if($keyword):
$where .= " AND  quote_tracking_report_delivery.qt_project_address LIKE '%".$keyword."%'";
...
$where .= " AND quote_tracking_report_delivery.qt_project_address LIKE '%".$_SESSION['keyword']."%' ";	
...
//$sql = "SELECT quote_tracking_report_delivery.*  from business_sellers 
//	    Inner Join bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id 
//	    Inner Join quote_tracking_report_delivery ON business_sellers.bs_business_id = quote_tracking_report_delivery.qt_bsn_id  
...
function color_set($k) {
		...
		$fwDb = new Fw_Db();
		$sqlcc = "select qs_color from quote_tracking_status_delivery where qs_name = '".$k."'" ;
