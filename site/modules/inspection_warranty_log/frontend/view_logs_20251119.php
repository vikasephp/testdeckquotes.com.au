...
$query = 'SELECT WL.wa_id, WL.wa_problem, WL.wa_wlis_id, WL.wa_inspection_proposed_fees, WL.wa_inspection_acknowledgement_statement FROM warranty_log AS WL INNER JOIN business AS B ON B.bsn_name = WL.wa_project WHERE B.bsn_id = ' . $bsn_id . ' AND WL.wa_inspection_required = "Yes" AND WL.wa_inspection_required = "Yes" AND WL.wa_status = "Open" ORDER BY wa_id ASC;';
