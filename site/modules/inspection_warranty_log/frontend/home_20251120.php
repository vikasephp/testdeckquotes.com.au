...
$mainQuery = 'SELECT B.bsn_name, B.bsn_id, BC.bcust_fname, BC.bcust_lname, BC.bcust_misc_email1, BC.bcust_misc_moble FROM business AS B INNER JOIN business_sellers AS BS ON BS.bs_business_id = B.bsn_id INNER JOIN bus_customers AS BC ON BC.bcust_id = BS.bs_customers_id LEFT JOIN warranty_log AS WL ON B.bsn_name = WL.wa_project WHERE WL.wa_is_inspection_fee_required = 1 AND WL.wa_status = "Open" GROUP BY B.bsn_name';
