...
foreach($records as $index => $row) {
    ...
    // db($row['reschedule_status']);
    // db((int)$row['reschedule_status']);

    if($row['wa_wlis_id'] == 8 && $row['reschedule_status'] === 0) {
        $records[$index]['inspection_status'] .= '<br><small>(temporarily pending client confirmation)</small>';
    }
    ...
    $wa_is_inspection_fee_required_completed_date = '';
    if($row['wa_is_inspection_fee_required_inspection_completed']) {
        $wa_is_inspection_fee_required_completed_date = date('d-M-Y', strtotime($row['wa_is_inspection_fee_required_inspection_completed_at']));
    }
    $records[$index]['wa_is_inspection_fee_required_completed_date'] = $wa_is_inspection_fee_required_completed_date;
