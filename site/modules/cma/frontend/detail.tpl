...
<script type="text/javascript">
		...
		var getdelurl= "{{$BASE_URL}}{{$XFA.docdelete}}?btdl_docs_id="; // The server-side script
   		var getseturl = "{{$BASE_URL}}{{$XFA.setting}}?request="; // The server-side script
	function show_wait()	
				{
...
function handleHttpsetResponse() {   
        ...
        function set_default(id, dhd_dh_id) { 
		if(!confirm('Are you sure want to set it as default image?')) return false;
    		var docurl = getseturl + "setdefault&dhd_dh_id=" + dhd_dh_id+"&id=" + id+"&random=" + Math.random();
			http.open("GET", docurl, true);
