...
$matsql = "SELECT construction_alert_report.*  FROM construction_alert_report 
           ...
           where car_supplier_update.cu_alert_resolved = 1 and construction_alert_report.car_approve = '' " .$where ." GROUP BY construction_alert_report.car_id "  .$ord;
...
//echo "<pre>"; echo count($listsnew); exit('Checking');
//Code for Merge Print 
...
if(!empty($clear_database_for_print_all_files)) {
 	...
 	$thisTable = new Fw_Db_Table("construction_alert_pending_report_manage_report");	$thisTable->setWhere("`id` != ''");
   $thisTable->deleteRows();
    // loop through the files one by one
    	  
    $files_list = glob($_SERVER['DOCUMENT_ROOT']."/reports/car_content/reportlist/*.*");
    ...
    $file_absolute_path = $_SERVER['DOCUMENT_ROOT'].'/reports/car_content/reportlist/merge_report/*.*';
     $files_list = glob($file_absolute_path);
    foreach($files_list as $file){
        // check if is a file and not sub-directory
        if(is_file($file)){
            // delete file
...
if(!empty($download_all_files_report))
{
   require_once($_SERVER['DOCUMENT_ROOT'].'/PDFMerger-master/PDFMerger.php'); 	$pdf = new PDFMerger; 
          ...
          $fn = "construction_alert_pending_report"."_".time().".pdf";  
    	//  $po_pdf_newfile = $_SERVER['DOCUMENT_ROOT'].'/reports/'.$v['file_name']; 
         
        foreach(glob($_SERVER['DOCUMENT_ROOT'].'/reports/car_content/reportlist/*.*') as $filename){
     		$pdf->addPDF($_SERVER['DOCUMENT_ROOT'].'/reports/car_content/reportlist/'.basename($filename));
    	  }
    	  
    	  $file_absolute_path = $_SERVER['DOCUMENT_ROOT'].'/reports/car_content/reportlist/merge_report/'.$fn;
        $pdf->merge('file', $file_absolute_path);
...
$fwViewData['total_records_of_print_reports'] = $total_records = count($listsnew);

if(!empty($submit))
{
 	
 	$total_counted = 0;  $is_record_exist = 0;
 	$printed_r_sql = "SELECT record_id FROM `construction_alert_pending_report_manage_report` WHERE is_printed = 1"; $total_printed_ids = [];
	...
	$construction_alert_merge_files = new Fw_Db_Table('construction_alert_pending_report_manage_report');
	
	//echo count($listsnew); //print_r($listsnew); // exit;
//	echo "<pre>"; print_r($listsnew); exit('checked');
	foreach($listsnew as $lk => $lv)
	{
		if(!in_array($lv["car_id"], $total_printed_ids))
        {
            ...
            $insertData = ["record_id"=> $lv["car_id"] , "is_printed"=> 1];
            
            $construction_alert_merge_files->insertRow($insertData);
    		
    		if ($total_counted > 25){ break; } $total_counted++;
        }
	}
	$html .= '</table>';
	//echo "<pre>"; print_r($html); exit('checked');
	 if($is_record_exist > 0){ create_car2($heading,$html); }
}

  
if(!empty($submit)){ $fwViewData['total_records_of_print_reports'] = $total_records = count($listsnew); }
    
    $printed_r_sql = "SELECT record_id FROM `construction_alert_pending_report_manage_report` WHERE is_printed = 1";
    if(!empty($printed_r_sql)){ $listsnew = $fwDb->query($printed_r_sql); 
      $fwViewData['total_printed_records'] = $total_printed_records = $total_records = count($listsnew); 
    }
