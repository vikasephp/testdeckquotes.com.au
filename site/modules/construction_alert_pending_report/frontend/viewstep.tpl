...
<td>{{counter}}</td>
<td><input type="hidden" name="step[{{$item.apd_id}}][apd_step_desc]" value="{{$item.apd_step_desc}}" />
    <input type="hidden" name="step[{{$item.apd_id}}][apd_car_id]" value="{{$car_id}}" />
    <input type="hidden" name="step[{{$item.apd_id}}][apd_id]" value="{{$item.apd_id}}" />
{{$item.apd_step_desc}}</td>
<td>
<datalist id='contact'>
           {{foreach from=$contactdetail key="key2" item="item2"}}
            <option value="{{$item2.cs_first_name}} {{$item2.cs_surname}} - {{$item2.cs_primary_email}}" >
           {{$item2.co_company_name}} - {{$item2.cs_first_name}} {{$item2.cs_surname}} - {{$item2.cs_primary_email}}
            </option>
            {{/foreach}}
  	 </datalist>
