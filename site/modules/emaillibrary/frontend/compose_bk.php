...
$tableEmailLibrary->setOrderBy('eml_code'.' ASC');
...
//$tableContact = new Fw_Db_Table('contact_list');
//$tableContact->setOrderBy('cl_contact_name'.' ASC');
//$fwViewData['contactdetail'] =  $tableContact->getRows();


$tableContact = new Fw_Db_Table('contacts');
$tableContact->setOrderBy('cs_first_name'.' ASC');
$fwViewData['contactdetail'] =  $tableContact->getRows();
...
//$submit = $fwRequest->getParam('eml_template', '');
...
if(!empty($submit))
{
	...
	if($fwViewData['p_c_name']==0) { $fwViewData['p_c_name'] =''; }
	$prda = explode("|",$prda);
	$bsnid =  $prda[0];
...
if(!empty($submit2))
{
	...
	//$conarr = explode(",",$detail['to_contact_list']);

//
//	if(!empty($arr1) and !empty($detail['to2'])) {
//		$toData = array_merge($arr2,$arr1);
//	}
//	
//	if(!empty($toData)) {
//		db("Test1");	
...
//	exit;
	...
	if(!empty($detail['bsn_document1'])) {	
				...
				if($v)
					...
					$finalatta = "https://deckquote.s3.amazonaws.com/files/document_check_list_files/".$data[0];
					$file = "https://deckquote.s3.amazonaws.com/files/document_check_list_files/".$data[0];
	...
	if($_FILES['image1']['name'])
	{
		...
		$destination3 = BASE_DIR.'files/email_attachment/'.$file_name3;
		move_uploaded_file($src3, $destination3);
		$imagename1 =  BASE_URL.FILE_PATH."files/email_attachment/".$file_name3 ;
	}
	...
	if($detail['eml_ie_email'] == 'External')
	{
			...
			if(!empty($to_2)) 
			{
				 ...
				 $fwViewData['email_sent_ok'] = "Email has been sent successfully ";
					...
					if(!empty($custdata['bcust_misc_email1'])) {
						$logDetail['elog_to'] = $custdata['bcust_misc_email1'];
					}
					
					if(!empty($custdata['bcust_misc_email2'])) {
						$logDetail['elog_to_secondary'] = $custdata['bcust_misc_email2'];
					}
					...
					if(!empty($templogattach)) { $tedoc .= implode(", ",$templogattach); }
					if(!empty($templogattach1)) { $tedoc .= implode(", ",$templogattach1); }
...
if(!empty($submit3))
{
	...
	if(!empty($doclogattach)) { $draftDetail['ed_proj_docs'] = implode(", ",$doclogattach); }
	if(!empty($templogattach)) { $draftDetail['ed_template_docs'] = implode(", ",$templogattach); }
