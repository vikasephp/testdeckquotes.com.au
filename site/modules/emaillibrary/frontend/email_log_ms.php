...
if ($tdsql) {
	...
	$query_count = $fwDb->query("SELECT COUNT('elog_eml_code') as total_records FROM email_log " . $where);
...
foreach ($setdata as $k => $v) {
	...
	if($setdata[$k]['custom_id']) {
				foreach ($d1 as $k1 => $v1) {
				$setdata[$k]['elog_doc_attached_new'] .= "<a class='elog_doc_attached_new' href='/central_letter_database.download_content?file_name=" . trim($v1) . "&module_name=central_letter_database.home' >" . $v1 . "</a><br>";
			}

			$d2 = explode(",", $v['elog_temp_attached']);

	        foreach ($d2 as $k2 => $v2) {
			$setdata[$k]['elog_temp_attached_new'] .= "<a class='elog_doc_attached_new' href='/central_letter_database.download_content?file_name=" . trim($v2) . "&module_name=central_letter_database.home' >" . $v2 . "</a><br>";
			}

			$setdata[$k]['elog_image_new'] .= "<a class='elog_doc_attached_new' href='/central_letter_database.download_content?file_name=" . trim($v['elog_image']) . "&module_name=central_letter_database.home' >" . $v['elog_image'] . "</a><br>";
			$setdata[$k]['elog_attach1_new'] .= "<a class='elog_doc_attached_new' href='/central_letter_database.download_content?file_name=" . trim($v['elog_attachment1']) . "&module_name=central_letter_database.home' >" . $v['elog_attachment1'] . "</a><br>";
			$setdata[$k]['elog_attach2_new'] .= "<a class='elog_doc_attached_new' href='/central_letter_database.download_content?file_name=" . trim($v['elog_attachment2']) . "&module_name=central_letter_database.home' >" . $v['elog_attachment2'] . "</a><br>";
			$setdata[$k]['elog_attach3_new'] .= "<a class='elog_doc_attached_new' href='/central_letter_database.download_content?file_name=" . trim($v['elog_attachment3']) . "&module_name=central_letter_database.home' >" . $v['elog_attachment3'] . "</a><br>";

	    
	} else {
		    foreach ($d1 as $k1 => $v1) {
				$setdata[$k]['elog_doc_attached_new'] .= "<a class='elog_doc_attached_new' href='/emaillibrary.download_content?file_name=" . trim($v1) . "&module_name=emaillibrary.email_log_ms.document_check_list_files' >" . $v1 . "</a><br>";
			}
