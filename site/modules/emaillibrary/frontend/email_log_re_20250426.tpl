...
&nbsp;&nbsp;<input type="submit" name="clear" value="Clear Search">
  ...
  <table id="doclist-table" class="nav-back" width="99%">
    ...
    <thead>
      ...
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="23%">All Attachment</th>
  ...
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr> {{if $list}}
      ...
      <td align="center"><div class='pagination'> {{if $pagenum == 1}} <span class='disabled'><< previous</span> {{else}} <a class="pagination" href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/{{math equation="x - y" x=$pagenum y=1}}" title="Previous"><< previous</a> {{/if}}

          

          {{if $pagenum != 1}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/1" title="{{$page_num}} Page">1</a></span> {{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/2" title="{{$page_num}} Page">2</a></span> {{else}}<span class='current'>2</span> {{/if}}
          ...
          {{else}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/{{$page_num}}" title="{{$page_num}} Page">{{$page_num}}</a></span> {{/if}}
          ...
          {{if $page_num2 == $pagenum OR $page_num2 <= 0}} <span class='current'> {{$page_num2}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/{{$page_num2}}" title="{{$page_num2}} Page">{{$page_num2}}</a> {{/if}}
          ...
          {{if $pagenum == $lastone }} <span class='current'>{{$lastone}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/{{$lastone}}" title="{{$lastone}} Page">{{$lastone}}</a> {{/if}}
          ...
          {{if $pagenum == $last}} <span class='current'>{{$last}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/{{$last}}" title="{{$last}} Page">{{$last}}</a> {{/if}}

          {{/if}}

          {{if $pagenum == $last}} <span class='disabled'>next >></span> {{else}} <a class="pagination" href="{{$BASE_URL}}emaillibrary.email_log_re/pagenum/{{math equation="x + y" x=$pagenum y=1}}" title="Next">next >></a></div>
