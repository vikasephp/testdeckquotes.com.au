...
{
	var viewurl = "{{$BASE_URL}}emaillibrary.show-email-content"+ "/eid/" + eid;	
...
<input type="text" name="{{$TABLE}}[keyword]" value="{{$keyword}}" placeholder="Enter Email Code or Email Subject For Search" style="width:400px;"/>
&nbsp;&nbsp;&nbsp;&nbsp;
<input type="submit" value="Search" name="search"  />
Sort By :
<input type="submit" value="eid" name="eid"  />
<input type="submit" value="Email Code" name="ecode"  />
<input type="submit" value="Category" name="ecategory"  />
<input type="submit" value="Email Type" name="etype"  />
<input type="submit" value="Business Stream" name="ebusstream"  />
<input type="submit" value="Owner" name="owner"  />
<input type="submit" value="Old Id" name="oldid"  />
<input type="submit" value="Link Uid" name="linkuid"  />
<input type="submit" value="Status" name="status"  />
 </div>
</form>

<div style="float:right;">
 <!--<input type="button" name="compose" value="Compose" onclick="document.location.href='{{$BASE_URL}}emaillibrary.compose'" />-->
 
    
 <input type="button" name="Add New " value="Create New Template" onclick="javascript:add_new();" /> </div>
</td></tr></table>
<div style="float:left; width:100%;">
  ...
  <table id="doclist-table" class="nav-back" width="99%">
    ...
    <thead>
      ...
      <tr>
        <th class="topmenu" align="center" valign="middle" width="3%">EID.</th>
        <th class="topmenu" align="center" valign="middle" width="7%">Email Code</th>
        <th class="topmenu" align="center" valign="middle" width="7%">Email Category</th>
        <th class="topmenu" align="center" valign="middle" width="7%">Email Type </th>
        <th class="topmenu" align="center" valign="middle" width="14%">Email Subject </th>
        <th class="topmenu" align="center" valign="middle" width="8%">Business Stream </th> 
        <th class="topmenu" align="center" valign="middle" width="5%">Link UID</th>
        <th class="topmenu" align="center" valign="middle" width="8%">Email Content </th>
      <!--  <th class="topmenu" align="center" valign="middle" width="23%">Template Docs </th>-->
        <th class="topmenu" align="center" valign="middle" width="5%">Old Id </th>
    ...
    <form name="mlist" method="post">
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
        ...
        <td>{{$item.eml_id}}</td>
	    <td>{{$item.eml_code}}</td>
        <td>{{$item.eml_ie_email}}</td>
        <td>{{$item.eml_email_type}}</td>
        <td>{{$item.eml_subject}}</td>
        <td>{{$item.eml_business_stream}}</td>
        <td>{{$item.eml_link_uid}}</td>
        <td>
        <input type="button" name="View Content" value="View Content" onclick="javascript:show_email_conent({{$item.eml_id}});" style="width:90px; height:18px; font-size:11px; line-height:17px !important; padding:0px !important" />
        </td>
        
   <!--     <td>{{$item.eml_temp_docs}}</td>-->
        
        <td>{{$item.eml_old_id}}</td>
		<td>{{$item.eml_status}}</td>
		...
		<a href="{{$BASE_URL}}emaillibrary.compose/{{$ID}}/{{$item.$ID}}" title="Compose">
         <img style="height: 20px;; width:20px" src="{{$BASE_URL}}css/admin/images/compose.png"/></a>
