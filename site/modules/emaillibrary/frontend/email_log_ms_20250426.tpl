...
&nbsp;&nbsp;<input type="submit" name="clear" value="Clear Search">
  ...
  <table id="doclist-table" class="nav-back" width="99%">
	...
	<td>{{$item.elog_eml_code}}
        ...
        <td>{{$item.elog_to}} {{if $item.elog_to_secondary}} <br /> {{$item.elog_to_secondary}} {{/if}} 
        {{if $item.elog_to_email3}} <br /> {{$item.elog_to_email3}} {{/if}}
        {{if $item.elog_to_email4}} <br /> {{$item.elog_to_email4}} {{/if}}
        {{if $item.elog_to_email5}} <br /> {{$item.elog_to_email5}} {{/if}}
        </td>
  ...
  <table width="90%" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr> {{if $list}}
      ...
      <td align="center"><div class='pagination'> {{if $pagenum == 1}} <span class='disabled'><< previous</span> {{else}} <a class="pagination" href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/{{math equation="x - y" x=$pagenum y=1}}" title="Previous"><< previous</a> {{/if}}

          

          {{if $pagenum != 1}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/1" title="{{$page_num}} Page">1</a></span> {{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/2" title="{{$page_num}} Page">2</a></span> {{else}}<span class='current'>2</span> {{/if}}
          ...
          {{else}} <span class="paginate"><a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/{{$page_num}}" title="{{$page_num}} Page">{{$page_num}}</a></span> {{/if}}
          ...
          {{if $page_num2 == $pagenum OR $page_num2 <= 0}} <span class='current'> {{$page_num2}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/{{$page_num2}}" title="{{$page_num2}} Page">{{$page_num2}}</a> {{/if}}
          ...
          {{if $pagenum == $lastone }} <span class='current'>{{$lastone}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/{{$lastone}}" title="{{$lastone}} Page">{{$lastone}}</a> {{/if}}
          ...
          {{if $pagenum == $last}} <span class='current'>{{$last}}</span> {{else}} <a class="pagination"  href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/{{$last}}" title="{{$last}} Page">{{$last}}</a> {{/if}}

          {{/if}}

          {{if $pagenum == $last}} <span class='disabled'>next >></span> {{else}} <a class="pagination" href="{{$BASE_URL}}emaillibrary.email_log_ms/pagenum/{{math equation="x + y" x=$pagenum y=1}}" title="Next">next >></a></div>
