...
$where = "WHERE elog_eml_code like 'RE%' ";
$TABLE1 = 'email_log';
...
if ($tdsql) {
	...
	$page_rows = 50;
	$rows =  $total_records = count($records_data);
...
foreach ($setdata as $k => $v) {
	...
	// db($setdata[$k]['elog_to']);
	// db(json_decode($setdata[$k]['elog_to']));
	// exit;
	...
	foreach ($d1 as $k1 => $v1) {
		$setdata[$k]['elog_doc_attached_new'] .= "<a href='/emaillibrary.download_content?file_name=" . trim($v1) . "&module_name=emaillibrary.email_log_re' >" . $v1 . "</a><br>";
	}
