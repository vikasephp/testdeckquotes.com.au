...
<script type="text/javascript">
	...
	function show_log_email_content(lid) {
		var viewurl = "{{$BASE_URL}}emaillibrary.show-log-email-content/elog_id/" + lid + "/emailfailure/1";
...
<h3 class="page-title">Email Failure Log</h3>
...
<div style="float:left; width:100%;">
	...
	<form name="mshort" action="" method="post">
		<div style="float:left; margin-left:2%;">
			<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<strong>Search by Email Code:</strong>&nbsp;&nbsp;</span>
	...
	<table id="doclist-table" class="nav-back" width="99%">
		...
		<thead>
			...
			<tr>
				...
				<th class="topmenu" align="center" valign="middle" width="10%">Reason of Failure</th>
		...
		<tbody>
			...
			<form name="mlist" method="post">
				...
				<tr bgcolor="{{cycle values='#D3E8D6,WHITE' advance=true}}">
					...
					<td>
						<a href="{{$BASE_URL}}emaillibrary.deleteflog/elog_id/{{$item.elog_id}}"
				...
				<div class='pagination'> {{if $pagenum == 1}} <span class='disabled'>
						<< previous</span>
							{{else}} <a class="pagination"
									href="{{$BASE_URL}}emaillibrary.email_failure_log/pagenum/{{math equation=" x - y"
										...
										{{else}}<span class='current'>1</span> {{/if}}{{if $pagenum != 2}} <span
											...
											{{else}}<span class='current'>2</span> {{/if}}
												...
												{{else}} <span class="paginate"><a class="pagination"
															href="{{$BASE_URL}}emaillibrary.email_failure_log/pagenum/{{$page_num}}"
													...
													{{else}}
														...
														{{else}} <a class="pagination"
																href="{{$BASE_URL}}emaillibrary.email_failure_log/pagenum/{{$page_num2}}"
															...
															{{else}}
																...
																{{else}} <a class="pagination"
																		href="{{$BASE_URL}}emaillibrary.email_failure_log/pagenum/{{$lastone}}"
																	...
																	{{else}}
																		...
																		{{else}} <a class="pagination"
																				href="{{$BASE_URL}}emaillibrary.email_failure_log/pagenum/{{$last}}"
																			...
																			{{else}} <a class="pagination"
																					href="{{$BASE_URL}}emaillibrary.email_failure_log/pagenum/{{math equation="
																					x + y" x=$pagenum y=1}}"
