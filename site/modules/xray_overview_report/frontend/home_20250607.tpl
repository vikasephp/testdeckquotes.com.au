...
.bn {padding:3px !important; font-size:12px !important;
 text-transform:capitalize !important ; background:none !important;
 border:none !important; color:#09C !important; font-weight:bold !important; } 
</style>
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="7%">Planning Approvals Submitted- Invoiced</th>
        <th class="topmenu" align="center" valign="middle" width="7%">Planning Approvals Submitted - Paid</th>
        
        <th class="topmenu" align="center" valign="middle" width="7%">Floor Systems Complete - Invoiced </th>
        <th class="topmenu" align="center" valign="middle" width="7%">Floor Systems Complete - Paid</th>
        
         <th class="topmenu" align="center" valign="middle" width="7%">Wall and Roof Frames Complete - Invoiced</th>
         <th class="topmenu" align="center" valign="middle" width="7%">Wall and Roof Frames Complete - Paid</th>
    ...
    <tbody>
      ...
      <!--{{assign var=countn value= $numstartvounter}}-->
       ...
       {{foreach from=$list key="key" item="item"}}
        ...
        <option value="{{$item1.sb_id}}" {{if $item1.sb_id eq $item.bsn_xor_sbc}} selected="selected" {{/if}}>{{$item1.sb_option}}</option>
        {{/foreach}}
        </select>
        ...
        <a href="{{$BASE_URL}}xray_overview_report.notes/bsn_id/{{$item.bsn_id}}" class="various">Add/View Notes</a>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="hide[{{$item.bs_business_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bs_xor_hide eq 1 }} checked="checked" {{/if}} />
        ...
        {{if $item.ppd_invoice_sent}}Yes Invoiced {{/if}}</td>
        
        <td>{{$item.ppd_value_2}}</td> 
      
        <td {{if $item.ppd_invoice_sent_3}} style="background: #0C6;" {{/if}}>{{$item.ppd_hia_value_3}}<br />
        {{if $item.ppd_invoice_sent_3}}Yes Invoiced {{/if}}</td>
        ...
        <td {{if $item.ppd_invoice_sent_5}} style="background: #0C6;" {{/if}}>{{$item.ppd_hia_value_5}}<br />
        {{if $item.ppd_invoice_sent_5}}Yes Invoiced {{/if}} </td>
        <td>{{$item.ppd_value_5}} </td>
          
        <td {{if $item.ppd_invoice_sent_6}} style="background: #0C6;" {{/if}}>{{$item.ppd_hia_value_6}}<br />
         {{if $item.ppd_invoice_sent_6}}Yes Invoiced {{/if}} </td>
        <td>{{$item.ppd_value_6}} </td>    
       
        <td {{if $item.ppd_invoice_sent_7}} style="background: #0C6;" {{/if}}>{{$item.ppd_hia_value_7}}<br />
        {{if $item.ppd_invoice_sent_7}}Yes Invoiced {{/if}} </td>
        <td>{{$item.ppd_value_7}} </td>
          
        <td {{if $item.ppd_invoice_sent_8}} style="background: #0C6;" {{/if}}>{{$item.ppd_hia_value_8}}<br />
       {{if $item.ppd_invoice_sent_8}}Yes Invoiced {{/if}} </td>
