...
.wrapper2{ }
.div1 {width:4990px; height: 20px; }
.div2 {width:4990px;  overflow: auto;}   
...
<h3 class="page-title">Renovation Design Report - ACTIVE</h3>
<br />
...
<input type="button" value="Export To XLS" onclick="document.location.href='{{$BASE_URL}}design_report_renovation.home_active/export/1'" /> &nbsp;
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="3%">Renovation Checklist Number</th>
	...
	<th class="topmenu" align="center" valign="middle" width="7%">Sign off sent date</th>
        ...
        <th class="topmenu" align="center" valign="middle" width="4%">Total<br />Checklists</th>
	...
	function update_rating(id,value)
		{
		 ...
		 $.ajax({
					   type: "GET",
					   url: "{{$BASE_URL}}design_report_renovation.update_rating/dpn_unique_id/"+id+"/value/"+value,
        ...
        <td>
      <!--  <input type="checkbox"  {{if $item.bsn_address_same eq 1}} checked="checked" {{/if}} /> -->
        
       <input name="same" type="checkbox" {{if $item.bsn_address_same eq 1}} checked="checked" {{/if}}  onclick="update_same({{$item.bsn_id}},this.checked)" />
        <script>
...
--> <input type="text" name="desagr[{{$item.chknoData}}]" value="{{$item.dpn_dr_deagree}}" onkeyup = "update_da_currency({{$item.chknoData|ltrim:'0'}},this.value)" style="width:50px;"/>
	   ...
	   function update_da_currency(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_design_agree_curr/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
       ...
       <script>
        ...
        function update_building_file(id,value)
		{
		 ...
		 $.ajax({
					   type: "GET",
					   url: "{{$BASE_URL}}design_report_renovation.update_building_file/dpn_unique_id/"+id+"/value/"+value,
						   success: function(result){
					   }
					 });
...
--> 
 <input type="text" name="dpn_dr_surveydt[{{$item.chknoData}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo33_{{$item.chknoData}}" value="{{$item.dpn_dr_surveydt}}" onfocus ="update_two({{$item.chknoData|ltrim:'0'}}, this.value);"  /> 
		...
		function update_survey(id,value)
		{
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}design_report_renovation.update_survey/cs_id/"+id+"/value/"+value,
					   success: function(result){
					}
				});
		}
		...
		function update_two(id,value)
		{
			$.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}design_report_renovation.update_survey/cs_id2/"+id+"/value2/"+value,
					   success: function(result){
					}
				});
	...
	Date :<input type="text" name="cdate[{{$item.chknoData}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo44_{{$item.chknoData}}{{$item.bsn_id}}" value="{{$item.dpn_dr_sur_comp_date}}"  onfocus = "update_three({{$item.chknoData|ltrim:'0'}}, this.value);" /> 
       <!-- <input type="submit" value="Save" name="survey_details" class="set" />-->
        ...
        function update_three(id,value)
		{
			...
			$.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}design_report_renovation.update_survey/cno/"+id+"/dt/"+value,
					   success: function(result){
					}
				});
		}
	 ...
	 function update_surveyor(id,value)
		{
			...
			$.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}design_report_renovation.update_survey/cnos/"+id+"/sur/"+value,
					   success: function(result){
					}
				});
		}
	...
	function update_who(id,value)
		{
			$.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}design_report_renovation.update_survey/cnow/"+id+"/who/"+value,
					   success: function(result){
					}
				});
		}	
      ...
      <a href="/design_report_renovation.download_content_dc?file_name={{$item.survey_uploaded}}&module_name=design_report_renovation.home_active" target="_blank">Link</a>
    ...
    <!--  <a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.checklist179_file}}" target="_blank">Link</a>-->
      <a href="/design_report_renovation.download_content_dc?file_name={{$item.checklist179_file}}&module_name=design_report_renovation.home_active" target="_blank">Link</a>
      ...
      {{/if}}
       ...
       {{else}}
    	   <td>
       {{/if}}  
       <a href="{{$BASE_URL}}design_report_renovation.txtmsg/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
       <img src="{{$BASE_URL}}images/textmsgyellow.png" /></a> &nbsp;       
		...
		function update_sur_imp(id,value)
		{
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}design_report_renovation.update_sur_imp/dpn_unique_id/"+id+"/value/"+value,
					   success: function(result){
					}
				});
		}
       ...
       <script>
		...
		function rr_update(id,value)
		{
			 $.ajax({
				   type: "GET",
				   url: "{{$BASE_URL}}design_report_renovation.update_renovation_report/dpn_unique_id/"+id+"/value/"+value,
					   success: function(result){
					}
				});
		}
...
<input class="switch-input" type="checkbox"  name="renoonly[{{$item.chknoData}}]" value="1"  onclick="this.form.submit();" {{if $item.dpn_dr_renoonly  eq 1 }} checked="checked" {{/if}} />
       ...
       <script>
        ...
        function update_project_type(id,value)
		{
		 ...
		 $.ajax({
					   type: "GET",
					   url: "{{$BASE_URL}}design_report_renovation.update_project_type/dpn_unique_id/"+id+"/value/"+value,
						   success: function(result){
					   }
					 });
      ...
      <td> 
       ...
       <script>
        ...
        function udt(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_design_type/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
       ...
       <input type="text" name="dpn_dr_first_meeting_date[{{$item.chknoData}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo1_{{$item.chknoData}}" value="{{$item.dpn_dr_first_meeting_date}}" onfocus = "update_fmd({{$item.chknoData|ltrim:'0'}}, this.value);" /> 
        ...
        function update_fmd(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_survey/fmd_id/"+id+"/fmd_value/"+value,
				   success: function(result){
			   }
			 });
		}
       ...
       <td> {{$item.designer}}</td> 
       <td>{{$item.osarchi}} </td>
       <td> {{if $item.checklist482_doc}}
      <!-- <a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.checklist482_doc}}" target="_blank">Download</a>-->
      <a href="/design_report_renovation.download_content_dc?file_name={{$item.checklist482_doc}}&module_name=design_report_renovation.home_active" target="_blank">Download</a>
       
       {{/if}}
       {{$item.checklist482}}
        </td>
        <td>{{if $item.file_name_54}}
       <!--<a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.file_name_54}}" target="_blank">Download</a>-->
       <a href="/design_report_renovation.download_content_dc?file_name={{$item.file_name_54}}&module_name=design_report_renovation.home_active" target="_blank">Download</a>
       
       {{/if}}
        {{$item.checklist54}} </td>
       
      
 <td>  <input type="text" name="whereare[{{$item.chknoData}}]" value="{{$item.dpn_dr_wherewe}}" onkeyup = "update_where_are({{$item.chknoData|ltrim:'0'}},this.value)"/>
	   ...
	   function update_where_are(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_where_we_are/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
 ...
 <script>
		...
		function update2(id,value)
		{
		 ...
		 $.ajax({
					   type: "GET",
					   url: "{{$BASE_URL}}design_report_renovation.update_per2/dpn_unique_id/"+id+"/value/"+value,
						   success: function(result){
					   }
					 });
	...
	function update_letter_text2(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_letter_text2/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
	...
	function update_sqm2(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_sqm2/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}	
...
<!--       <td>
      m
      <a href="{{$BASE_URL}}design_report_renovation.txtmsg_ec/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
	...
	function update_estimate_cost2(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_estimate_cost2/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
        ...
        <td> 
          ...
          <a href="{{$BASE_URL}}design_report_renovation.txtmsg_lsv/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
	...
	function update_likely_value2(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_likely_value2/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
       ...
       <a href="{{$BASE_URL}}design_report_renovation.txtmsg_erc/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
	   ...
	   function update_reno_cost(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_renovation_cost/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
...
<!--       <td> 
 m
  <a href="{{$BASE_URL}}design_report_renovation.txtmsg_re/dpn_unique_id/{{$item.chknoData|ltrim:'0'}}" class="various">
	   ...
	   function update_reno_extn2(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_reno_extn/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
	   ...
	   function update_alerts(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_alerts/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
	   ...
	   function update_tar_sign(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_survey/cno_t/"+id+"/value_t/"+value,
				   success: function(result){
			   }
			 });
		}
 ...
 -->   <input type="text" name="cust_design_hours[{{$item.chknoData}}]" value="{{$item.dpn_dr_cust_des_hour}}" onkeyup = "update_cdh({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
  ...
  -->  
     <input type="text" name="total_hours[{{$item.chknoData}}]" value="{{$item.dpn_dr_total_hours}}" onkeyup = "update_tot_hours({{$item.chknoData|ltrim:'0'}},this.value)" style="width:40px"/>
	   ...
	   function update_sign_off_date(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_survey/sign_off_id/"+id+"/value_so/"+value,
				   success: function(result){
			   }
			 });
		}
       ...
       <a href="{{$BASE_URL}}design_report_renovation.colorbox/chk_no/{{$item.chknoData}}" class="various">Color</a>
       <br /><br />{{$item.dpn_dr_tl_user}}<br />{{$item.dpn_dr_tl_date}}<br />
        <a href="{{$BASE_URL}}design_report_renovation.reason/chk_no/{{$item.chknoData}}" class="various">Read</a>
       </td>
       ...
       {{else }}
       <td>
       <a href="{{$BASE_URL}}design_report_renovation.colorbox/chk_no/{{$item.chknoData}}" class="various">Color</a></td>
       {{/if}}
       ...
       <td> {{if $item.file_lmp}}
      <!-- <a href="https://deckquote.s3.amazonaws.com/files/document_check_list_files/{{$item.file_lmp}}" target="_blank">Download</a>-->
       <a href="/design_report_renovation.download_content_dc?file_name={{$item.file_lmp}}&module_name=design_report_renovation.home_active" target="_blank">Download</a>
      ...
      <form name="pw" method="post" enctype="multipart/form-data"> 
        ...
        <a href="/design_report_renovation.download_content?file_name={{$item.dpn_dr_latest_letter}}&module_name=design_report_renovation.home_active" target="_blank">Download</a>
       
        <a href="{{$BASE_URL}}design_report_renovation.delete_w/dpn_unique_id/{{$item.chknoData}}" onclick="javascript:if(!confirm('Are you sure want to delete?')) return false;" title="Delete"><img src="{{$BASE_URL}}/images/delete_icon.png" width="20" /></a>
	   ...
	   function update_last_audit(id,value)
		{
		 ...
		 $.ajax({
			   type: "GET",
			   url: "{{$BASE_URL}}design_report_renovation.update_last_audit/dpn_unique_id/"+id+"/value/"+value,
				   success: function(result){
			   }
			 });
		}
