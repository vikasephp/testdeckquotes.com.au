...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		$qry = "SELECT doc_bsn_id, doc_date_uploaded from document_check_list where doc_name_id = 45";
		$res = $fwDb->query($qry);
		$data45 = [];
		foreach($res as $row) {
		...
		$qry = "SELECT doc_date_uploaded, doc_bsn_id, doc_checklist_number FROM document_check_list_renovation WHERE doc_name_id = 8";
		$res = $fwDb->query($qry);
		$cpData = [];
		foreach($res as $row) {
		...
		$qry = "SELECT doc_date_uploaded, doc_bsn_id, doc_checklist_number FROM document_check_list_renovation WHERE doc_name_id = 5";
		$res = $fwDb->query($qry);
		$data54 = [];
		foreach($res as $row) {
		...
		$qry = "SELECT prd_bsn_id, prd_name from person_responsible_detail where prd_position = 'Designer'";
		$res = $fwDb->query($qry);
		$dataDesig = [];
		foreach($res as $row) {
		...
		$qry = "SELECT prd_bsn_id, prd_name from person_responsible_detail where prd_position like '%Overseas Architect%'";
		$res = $fwDb->query($qry);
		$dataOsa = [];
		foreach($res as $row) {
		...
		$qry = "SELECT dpn_bsn_id, dcnt_counter from document_check_list_renovation_count";
		$res = $fwDb->query($qry);
		$datapc = [];
		foreach($res as $row) {
			$datapc[$row['dpn_bsn_id']] = $row;
		}

		$qry = "SELECT doc_date_uploaded, doc_bsn_id, doc_checklist_number FROM document_check_list_renovation WHERE doc_name_id = 3";
		$res = $fwDb->query($qry);
		$mpData = [];
		foreach($res as $row) {
		...
		$qry = "SELECT doc_bsn_id, doc_file_name, doc_date_uploaded from document_check_list where doc_name_id = 72";
		$res = $fwDb->query($qry);
		$suData = [];
		foreach($res as $row) {
			$suData[$row['dpn_bsn_id']] = $row;
		}

		$qry = "SELECT bt_bsn_id, bt_completed_date from business_tasks where bt_task_id = 303";
		$res = $fwDb->query($qry);
		$data303 = [];
		foreach($res as $row) {
		...
		$qry = "SELECT doc_bsn_id, doc_file_name from document_check_list where doc_name_id = 458";
		$res = $fwDb->query($qry);
		$data_lcl = [];
		foreach($res as $row) {
		...
		$qry = "SELECT doc_bsn_id, doc_file_name, doc_date_uploaded from document_check_list where doc_name_id = 180";
		$res = $fwDb->query($qry);
		$data_cwg = [];
		foreach($res as $row) {
		...
		foreach ($setdata2 as $k2 => $v) {
			...
			// $sqlcp = "SELECT doc_date_uploaded FROM document_check_list_renovation 
		    //       WHERE doc_bsn_id = " . $v['dpn_bsn_id'] . " AND doc_name_id = 8 and doc_checklist_number = " . $v['dpn_proposal_number'];
			...
			// $sql54 = "SELECT doc_date_uploaded, doc_bsn_id, doc_checklist_number FROM document_check_list_renovation 
			...
			// $sqlmp = "SELECT doc_date_uploaded, doc_bsn_id, doc_checklist_number, doc_file_name FROM document_check_list_renovation 
			...
			$setdata2[$k]['checklist53'] =  strtotime($cpData[$v['dpn_bsn_id']][$v['dpn_proposal_number']]['doc_date_uploaded']) > 0 ? changedate_d_m_y($cpData[$v['dpn_bsn_id']][$v['dpn_proposal_number']]['doc_date_uploaded']) : '';
			$setdata2[$k]['checklist54'] =  strtotime($data54[$v['dpn_bsn_id']][$v['dpn_proposal_number']]['doc_date_uploaded']) > 0 ? changedate_d_m_y($data54[$v['dpn_bsn_id']][$v['dpn_proposal_number']]['doc_date_uploaded']) : '';
			...
			$setdata2[$k]['lmpData'] = $mpData[$v['dpn_bsn_id']][$v['dpn_proposal_number']];
			$setdata2[$k]['bsn_address_same'] = $v['bsn_address_same'];
			$setdata2[$k]['survey_uploaded'] = $suData[$v['dpn_bsn_id']]['doc_file_name'] ?? '';
			$setdata2[$k]['tasklist303'] =  strtotime($data303[$v['dpn_bsn_id']]['bt_completed_date']) > 0 ? changedate_d_m_y($data303[$v['dpn_bsn_id']]['bt_completed_date']) : '';
			...
			$setdata2[$k]['checklist179_date'] = strtotime($data_cwg[$v['dpn_bsn_id']]['bt_completed_date']) > 0 ? changedate_d_m_y($data_cwg[$v['dpn_bsn_id']]['bt_completed_date']) : '';
			$setdata2[$k]['bsn_showin_cpr'] = $busData[$v['dpn_bsn_id']]['bsn_showin_cpr'];
			$setdata2[$k]['dayscount'] = $dayscount;
			$setdata2[$k]['checklist45'] =  strtotime($data45[$v['dpn_bsn_id']]['doc_date_uploaded']) > 0 ? changedate_d_m_y($data45[$v['dpn_bsn_id']]['doc_date_uploaded']) : '';
