...
<form name="mlist" method="post" enctype="multipart/form-data">
    ...
    <div style="float: right; text-align: right;">
        <div>
            <input type="button" value="Export To XLS" onclick="document.location.href='{{$BASE_URL}}{{ $BASEFOLDER }}.home/export/1'" />
        </div>
        <div>
        ...
        <tbody>
            ...
            <tr bgcolor="{{ cycle values=" #D3E8D6,WHITE" advance=true }}">
                ...
                <td data-col="letter_type">
                    <select name="{{$TABLE}}[cld_letter_type_id]" style="width: auto;" onchange="updateRecord('{{ $item.$ID }}', 'cld_letter_type_id', this.value)">
                        <option value="0">Please Select</option>
                        {{ assign var=option_col value=$ATTRIBUTES.type.option_col }}
                        {{ assign var=option_id value=$ATTRIBUTES.type.id }}
                        {{ foreach from=$typedata key="index" item="row" }}
                        <option value="{{ $row.$option_id }}" {{ if $row.$option_id eq $item.cld_letter_type_id }}selected{{ /if }}>{{ $row.$option_col }}</option>
                        {{ /foreach }}
                    </select>
                </td>
                <td data-col="date_uploaded">
                    ...
                    {{ /if }}
                    <div>
                        <button class="update_elemnt_date" data-id="" type="button" style="white-space: nowrap;">Update Date</button>
                        <input style="display: none;" onchange="updateRecord('{{ $item.$ID }}', 'cld_date_uploaded', this.value)" type="date" />
                    </div> 
                </td>
                ...
                <td data-col="status">
                    <select name="{{$TABLE}}[cld_status_id]" style="width: auto;" onchange="updateRecord('{{ $item.$ID }}', 'cld_status_id', this.value)">
                        <option value="0">Please Select</option>
                        ...
                        <option value="{{ $row.$option_id }}" {{ if $row.$option_id eq $item.cld_status_id }}selected{{ /if }}>{{ $row.$option_col }}</option>
                        {{ /foreach }}
                    </select>
                </td>
                ...
                <td data-col="additinal_file">
                    <form name="cal" method="post" enctype="multipart/form-data"> 
                        ...
                        <input type= "submit" name="save_central_letter_database_files" title="Save" value="Upload" class="set2">
                    </form>
