...
// function to send email
function send_email_bk($to_name, $to, $from_name, $from, $subject, $html, $attachment = '')
{
	require_once(LIB_DIR . '/PEAR/PEAR/Mail.php');
	require_once(LIB_DIR . '/PEAR/PEAR/Mail/mime.php');
...
function send_email($to_name, $to, $from_name, $from, $subject, $html, $attachment = '')
{
	...
	try {
		...
		//  $mail->Host       = "mail.deckquotes.com.au";      // sets GMAIL as the SMTP server
		...
		// $mail->Username   = "deckquotes@deckquotes.com.au";  // GMAIL username
		// $mail->Password   = "CTb7q9pYE5DzF5ey";
		...
		$mail->AddBCC("emailbackup@cgfb.com.au", "Backup Email");
		// $mail->AddCC("supportmanager@cgfb.com.au", "Backup Email");
...
function send_email_letter($to_name, $to, $cc, $from_name, $from, $subject, $html, $attachment = '')
{
	...
	if ($attachment) {
		...
		$newpathattach = unserialize($attachment);
		// if (is_array($attached)) {
		// 	foreach ($attached as $k => $v) {
		// 		$npath = str_replace(BASE_URL, "", $v);
	...
	try {
		...
		foreach ($ccArray as $key => $value) {
			...
			if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
				$mail->AddCC($email, $to_name);
			} elseif (!empty($email)) {
				//echo "Invalid CC email address: $email<br>";
			}
		}
		$mail->AddBCC("emailbackup@cgfb.com.au", "Backup Email");
		$mail->Subject = $subject;
...
function email_tracking($toname, $email, $fromname, $from, $subject, $message, $attachment, $type)
{
	...
	if (empty($etdata['et_user_id'])) {
		$etdata['et_user_id'] = 1;
	}
	if (empty($etdata['et_user_name'])) {
		$etdata['et_user_name'] = "Auto";
	}
	$etdata['et_type'] = $type;
...
$fwViewData['email_temp_vars'] = array(
...
$fwViewData['email_temp_vars_buyers'] = array(
	'First name' => '{{customerfname}}',
...
$fwViewData['custom_dcl'] = array(
	'1' => 'Document Check List - A',
	'2' => 'Document Check List - B',
	'3' => 'Document Check List - C',
	'4' => 'Document Check List - D'
);


$fwViewData['docs_folders'] = array(
	'TODO_LIST_DOCS' =>
	array(
...
//$fwViewData['Decking_types'] = array(1 => '84mm - 90mm', 2 => '135mm', 3 => '65mm'); 	
...
function create_open_PDF($heading, $html, $fname)
{
...
function send_template_email($tempname, $custid, $busid, $email_section)
{
	...
	if ($email_template_data['et_attachment']) {
		...
		foreach ($attachdata as $k => $v) {
			if ($v) {
				$fdata[] = BASE_URL . FILE_PATH . "files/template_files/" . $v;
			}
		}
...
function send_general_disclamer_reminder($fname, $lname, $email)
{
	...
	if ($email_template_data) {
		...
		if ($email_template_data['et_attachment']) {
			$attach = unserialize($email_template_data['et_attachment']);
			foreach ($attach as $k => $v) {
				$attachment[] = BASE_URL . FILE_PATH . 'files/template_files/' . $v;
			}
		}
...
function Convert_Project_Daily_Task_Summary($reporttype, $project_type = 0, $project_status = 0, $gen_user_id = 0, $deffdate = 0)
{
	...
	if ($project_type > 0) {
		$ptwhere = " AND business.bsn_type = " . $project_type;
	}
	if ($project_status > 0) {
		$ptwhere .= " AND business.bsn_status = " . $project_status;
	}
	if ($gen_user_id > 0) {
		$ptwhere .= " AND business.bsn_sales_user_id = " . $gen_user_id;
	}
	if ($deffdate > 0) {
		$ptwhere .= " AND business.bsn_status_sys_date BETWEEN CURDATE() - INTERVAL {$deffdate} DAY AND CURDATE() ";
	}
	...
	foreach ($qaData as $k1 => $v1) {
		...
		if ($numrows == 8 && !empty($rowdata)) {
			$rowdatas[] = $rowheader . $rowdata . '</table>';
			$rowdata = '';
			$numrows = 0;
		}
		if ($v1['booked'] && $v1['ComApp']): $Comappdays = daysDifference($v1['ComApp'], $v1['booked']);
		endif;
		if ($v1['ComPro'] && $v1['ComApp']): $Comprodays = daysDifference($v1['ComPro'], $v1['ComApp']);
		endif;
...
function Preparation_and_Planning_Daily_Task_Summary($reporttype, $project_type = 0, $project_status = 0, $gen_user_id = 0, $deffdate = 0)
{
	...
	foreach ($qaData as $k1 => $v1) {
		...
		if ($v1['bsn_readyforsale'] == 1):
			if ($v1['bsn_approval_type'] == 1) {
				$bsn_approval_type = "Building Approval";
			} elseif ($v1['bsn_approval_type'] == 2) {
				$bsn_approval_type = "Development Approval";
			}
		endif;
		$bsn_name = $v1['bsn_name'];
		if ($numrows == 8 && !empty($rowdata)) {
		...
		if ($v1['booked'] && $v1['ComPro']): $Comprodays = daysDifference($v1['ComPro'], $v1['booked']);
		endif;
		if ($v1['pm_5per_amt'] && $v1['pm_5per_amt'] > 0): $pm_5per_amt = "Yes";
		endif;
		$rowdata .= '<tr><td>' . $v1['pt_name'] . ' - ' . $v1['bsn_sub_status'] . '</td>
				<!--<td>' . $v1['bsn_address'] . " " . $v1['bsn_suburb'] . " " . $v1['bsn_state'] . ",<br> " . $v1['bsn_pcode'] . '</td>-->
				<td><a href="' . BASE_URL . 'business.detail/bsn_id/' . $bsn_id . '">' . $v1['bsn_name'] . '</a></td>
...
function Construction_Report_Summary($reporttype, $project_type = 0, $project_status = 0, $gen_user_id = 0, $deffdate = 0)
{
	...
	foreach ($qaData as $k1 => $v1) {
		...
		if ($v1['pm_45per_amt'] && $v1['pm_45per_amt'] > 0): $pm_45per_amt = "Yes";
		endif;
		$rowdata .= '<tr><td>' . $v1['pt_name'] . ' - ' . $v1['bsn_sub_status'] . '</td>
				<td><a href="' . BASE_URL . 'business.detail/bsn_id/' . $bsn_id . '">' . $v1['bsn_name'] . '</a></td>
				<td>' . $v1['totaltasks'] . '/' . $v1['compltotal'] . '</td>
...
function Construction_Complete_Report($reporttype, $project_type = 0, $project_status = 0, $gen_user_id = 0, $deffdate = 0)
{
	...
	foreach ($qaData as $k1 => $v1) {
		...
		if ($v1['pm_50per_amt'] && $v1['pm_50per_amt'] > 0): $pm_50per_amt = "Yes";
		endif;
		$rowdata .= '<tr><td>' . $v1['pt_name'] . ' - ' . $v1['bsn_sub_status'] . '</td>
				<td><a href="' . BASE_URL . 'business.detail/bsn_id/' . $bsn_id . '">' . $v1['bsn_name'] . '</a></td>
				<td>' . $v1['totaltasks'] . '/' . $v1['compltotal'] . '</td>
...
function upload_central($filename, $tmpname, $folder)
{
	...
	try {
		if (move_uploaded_file($tmpname, $path . '/' .  basename($zipfile))) {
			...
			if ($s3->putObjectFile($path . "/" . $zipfile, BUCKET_NAME, 'files/' . $folder . '/' . $zipfile, S3::ACL_PRIVATE)) {
