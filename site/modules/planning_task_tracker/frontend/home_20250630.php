...
$qry = "SELECT project_planning_checklist.pc_bsn_id from project_planning_checklist
...
$datot = [];
foreach ($res as $row) {
	$datot[$row['pc_bsn_id']] = $row;
}

$qry = "SELECT project_planning_checklist.pc_bsn_id, planning_project_checklist_admin.ppc_id
	from project_planning_checklist
	left join planning_project_checklist_admin on project_planning_checklist.pc_doc_id = planning_project_checklist_admin.ppc_id
	where planning_project_checklist_admin.ppc_required = 'DA' AND project_planning_checklist.pc_required = 1 and length(TRIM(project_planning_checklist.pc_live_link)) > 0";
$res = $fwDb->query($qry);
$datotll = [];
foreach ($res as $row) {
	$datotll[$row['pc_bsn_id']] = $row;
}
...
$batot = [];
foreach ($res as $row) {
	$batot[$row['pc_bsn_id']] = $row;
}

$qry = "SELECT project_planning_checklist.pc_bsn_id from project_planning_checklist
	left join planning_project_checklist_admin on  project_planning_checklist.pc_doc_id = planning_project_checklist_admin.ppc_id
	where planning_project_checklist_admin.ppc_required = 'BA' AND project_planning_checklist.pc_required = 1 and length(TRIM(project_planning_checklist.pc_live_link)) > 0";
$res = $fwDb->query($qry);
$batotll = $res;
foreach ($res as $row) {
	$batotll[$row['pc_bsn_id']] = $row;
}
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			$totalDA = sizeof($datot[$v['bs_business_id']]);
			...
			$totalDAll = sizeof($datotll[$v['bs_business_id']]);
			...
			$totalBA = sizeof($batot[$v['bs_business_id']]);
			...
			$totalBAll = sizeof($batotll[$v['bs_business_id']]);
