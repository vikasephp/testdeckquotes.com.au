...
&nbsp;<input type= "submit" name="amberlight" title="Save" value="Filter Amber Light Projects"> &nbsp; &nbsp;-->
			...
			<input type="text" name="searchkey" style="width:200px;" />
		...
		<table id="doclist-table" class="nav-back" width="100%">
				<thead>
					<tr>
						...
						<th class="topmenu" align="center" valign="middle" style="width:150px;" data-col="alert">Alert
						</th>
						...
						<th class="topmenu" align="center" valign="middle" style="width:5px;">Updated Weekly Letter</th>
				...
				<tbody>
					...
					<tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
							...
							<form name="recmet" method="post" action="">
								...
								<label class="switch">
									<input class="switch-input" type="checkbox" name="hide[{{$item.bsn_id}}]" value="1"
										onclick="this.form.submit();" {{if $item.bs_ptt_hide eq 1 }} checked="checked"
										{{/if}} />
						...
						>
							<a href="{{$BASE_URL}}planning_task_tracker.add_alert/bsn_id/{{ $item.bsn_id }}" class="various" title="Add" style="text-decoration: none;">
							...
							<br>
							<a href="{{$BASE_URL}}planning_task_tracker.view_alert/bsn_id/{{ $item.bsn_id }}" class="various" title="Read">Read</a>
						</td>
						...
						<td>
							<select name="letter[{{$item.bsn_id}}]"
								Onchange="update_letter({{$item.bsn_id}}, this.value)">
								{{foreach from=$letterData key="key2" item="item2"}}
								<option value="{{$item2.lt_code}}" {{if $item2.lt_code eq $item.bsn_ptt_letter_text}}
									selected="selected" {{/if}}>{{$item2.lt_code}}</option>
								{{/foreach}}
							</select>
							<div id="usr2{{$item.bsn_id}}">
								{{$item.bsn_ptt_letter_text_date}}<br />{{$item.bsn_ptt_letter_text_user}}
							</div>
							<script>
             ...
             {{if $item.bs_ptt_last_spoken}} {{$item.bs_ptt_last_spoken_date}} <br /> {{$item.bs_ptt_last_spoken_user}} {{/if}}      
						...
						<td>
							{{if $item.weekly_update_456}}
							 <a href="{{$BASE_URL}}planning_task_tracker.download_content?file_name={{$item.weekly_update_456}}&module_name=planning_task_tracker.home" title="{{$item.weekly_update_456}}" target="_blank">Download</a>
							 {{/if}} 
