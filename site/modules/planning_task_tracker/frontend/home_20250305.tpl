...
.wrapper2{ }
.div1 {width:11680px; height: 20px; }
.div2 {width:11990px;  overflow: auto;}  
...
.ent {padding:3px; font-weight:bold ; margin-bottom:4px; border-radius:5px; color:#06F;}
</style>
...
<div style="float:left; text-align: left; margin-left:10px;  text-align:left; display:grid; grid-template-columns: 1fr 1fr;">
...
&nbsp;<input type="button" name="Add New" value="Project Type Admin" onclick="javascript:add_project_type();" /> 
<br />
...
<br />
&nbsp;<select name="bs_ptt_sub_status_filter"   Onchange = "update_sub_status({{$item.bs_business_id}},this.value)"/>
...
<strong>Latest Meeting Agenda :</strong> <input type="text" name="meeting_agenda" value="{{$ma_ma_doc}}" style="width:600px;" />
...
<tr>
<td>Red</td>
<td><input type= "submit" name="redlight" title="Save" value="{{$dataRed}}" style="background:#F00 !important;"></td>
</tr>
<tr>
<td>Ambar</td>
<td><input type= "submit" name="amberlight" title="Save" value="{{$dataAmbar}}" style="background: #F90 !important;"></td>
</tr>
<tr>
<td>Green</td>
<td><input type= "submit" name="greenlight" title="Save" value="{{$dataGreen}}" style="background: #0C0 !important;"></td>
</tr>
</table> <br />
 ...
 <div style="border:1px solid black;width:auto;height:auto;overflow-y:hidden;overflow-x:scroll;">
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    ...
    <!--     <th class="topmenu" align="center" valign="middle" style="width:110px;">Number Of Projects</th>-->
        ...
        <th class="topmenu" align="center" valign="middle" style="width:50px;">Add To Post HIA Report</th>
        <th class="topmenu" align="center" valign="middle" style="width:50px;">Operational Plans Implemented</th>
        ...
        <th class="topmenu" align="center" valign="middle" style="width:10px;">Latest Merged Plas OP</th> 
...
<!--       <td> 
       ...
       <td>
  <a href='{{$BASE_URL}}planning_task_tracker.view_project/bsn_id/{{$item.bsn_id}}' target="_blank" class="set3" style="color:#FFF;" />View Project
 </td>
      
       <td> <form name="tana" method="post" action="">
        <select name="bs_ptt_sub_status[{{$item.bs_business_id}}]"   Onchange = "this.form.submit();"/>
      	 {{foreach from=$sub_status key="key" item="item2"}}
       ...
       <td>{{$item.substatus_days}}</td>
        <td>
        <input type="button" name="Add New" value="Add To Post Hia"  class= "set2" style="width:95px !important;" onclick="javascript:add_phr({{$item.bsn_id}});" /> <br />
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="opi[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bsn_ptt_opi  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       
       {{if  $item.bsn_ptt_opi eq 1}} {{$item.bsn_ptt_opi_date}}<br />{{$item.bsn_ptt_opi_user}}{{/if}}
       ...
       <form name ="recmet3" method="post" action="">
     	<input type="hidden" name="opi[{{$item.bsn_id}}]" value="0" />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="opi[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bsn_ptt_override  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="plan_plans[{{$item.bsn_id}}]" value="1"  onclick="update_plan_plans({{$item.bsn_id}},this.checked);" {{if $item.bsn_ptt_planning_plans  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
      <div id="pp{{$item.bsn_id}}">  {{if  $item.bsn_ptt_planning_plans eq 1}} {{$item.bsn_ptt_pp_user}}<br />{{$item.bsn_ptt_pp_date}}{{/if}}</div>
      ...
      <td> 
       ...
       {{foreach from = $item.datawa key="key27" item="item27"}}
        ...
        <div class="ent" {{if $item27.dpn_water_ref eq 'Fails' }} style="background: #F00;" {{elseif $item27.dpn_water_ref eq 'Requested and Waiting For Entity'}} style="background: #FF0;" {{elseif $item27.dpn_water_ref eq 'Not Requested'}} style="background:#0CF;" {{elseif $item27.dpn_water_ref eq 'Passed With Conditions'}} style="background: #3F0;" {{else}} style="background: #FFF;" {{/if}}>
        
     {{if $item.doc_file_name_627}}
      <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name_627}}&module_name=planning_task_tracker.home" target="_blank" style="text-decoration: none;">Water_{{$item27.dpn_unique_id}}</a>
       {{else}}
     ...
     {{foreach from = $item.datawa key="key28" item="item28"}}
      ...
      <div class="ent"  {{if $item28.dpn_electric_ref eq 'Fails' }} style="background: #F00;" {{elseif $item28.dpn_electric_ref eq 'Requested and Waiting For Entity'}} style="background: #FF0;" {{elseif $item28.dpn_electric_ref eq 'Not Requested'}} style="background:#0CF;" {{elseif $item28.dpn_electric_ref eq 'Passed With Conditions'}} style="background: #3F0;" {{else}} style="background: #FFF;" {{/if}}>
      {{if $item28.doc_file_name_628}}
      <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name_628}}&module_name=planning_task_tracker.home" target="_blank" style="text-decoration: none;">Electric_{{$item28.dpn_unique_id}}</a>
     ...
     {{/foreach}}
       ...
       {{if $item29.dpn_gas_ref ne 'Not Required' }}
        ...
        <div class="ent"  {{if $item29.dpn_gas_ref eq 'Fails' }} style="background: #F00;" {{elseif $item29.dpn_gas_ref eq 'Requested and Waiting For Entity'}} style="background: #FF0;" {{elseif $item29.dpn_gas_ref eq 'Not Requested'}} style="background:#0CF;" {{elseif $item29.dpn_gas_ref eq 'Passed With Conditions'}} style="background: #3F0;" {{else}} style="background: #FFF;" {{/if}}>
         {{if $item.doc_file_name_629}}
        <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name_629}}&module_name=planning_task_tracker.home" target="_blank" style="text-decoration: none;">Gas_{{$item29.dpn_unique_id}}</a>
       ...
       {{/foreach}}
        ...
        <div class="ent"  {{if $item30.dpn_storm_ref eq 'Fails' }} style="background: #F00;" {{elseif $item30.dpn_storm_ref eq 'Requested and Waiting For Entity'}} style="background: #FF0;" {{elseif $item30.dpn_storm_ref eq 'Not Requested'}} style="background:#0CF;" {{elseif $item30.dpn_storm_ref eq 'Passed With Conditions'}} style="background: #3F0;" {{else}} style="background: #FFF;" {{/if}}>
        {{if $item.doc_file_name_630}}
        <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name_630}}&module_name=planning_task_tracker.home" target="_blank" style="text-decoration: none;">Storm_{{$item30.dpn_unique_id}}</a>
       ...
       {{/if}}
        ...
        <div class="ent"  {{if $item31.dpn_heritage_ref eq 'Fails' }} style="background: #F00;" {{elseif $item31.dpn_heritage_ref eq 'Requested and Waiting For Entity'}} style="background: #FF0;" {{elseif $item31.dpn_heritage_ref eq 'Not Requested'}} style="background:#0CF;" {{elseif $item31.dpn_heritage_ref eq 'Passed With Conditions'}} style="background: #3F0;" {{else}} style="background: #FFF;" {{/if}}>
        {{if $item.doc_file_name_716}}
        <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name_716}}&module_name=planning_task_tracker.home" target="_blank" style="text-decoration: none;">Heritage_{{$item31.dpn_unique_id}}</a>
      ...
      {{/foreach}}
        ...
        <div class="ent"  {{if $item32.dpn_heritage_ref eq 'Fails' }} style="background: #F00;" {{elseif $item32.dpn_heritage_ref eq 'Requested and Waiting For Entity'}} style="background: #FF0;" {{elseif $item32.dpn_heritage_ref eq 'Not Requested'}} style="background:#0CF;" {{elseif $item32.dpn_heritage_ref eq 'Passed With Conditions'}} style="background: #3F0;" {{else}} style="background: #FFF;" {{/if}}>
        {{if $item.doc_file_name_717}}
        <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name_717}}&module_name=planning_task_tracker.home" target="_blank" style="text-decoration: none;">Tree_{{$item32.dpn_unique_id}}</a>
      ...
      {{/foreach}}   
       ...
       {{if $item.doc_file_name_491}} 
            <a href="/planning_task_tracker.download_content?file_name={{$item.doc_file_name_491}}&module_name=planning_task_tracker.home" target="_blank">
	 ...
	 <script>
        ...
        <td><form name="mlist_o" method="post" action="" >
        ...
        {{if $item.bsn_orginal_csbd}} {{$item.bsn_original_csbd_date}} <br /> {{$item.bsn_original_csbd_user}} {{/if}}      
          </td>
         <td><form name="mlist_l" method="post" action="" >
             ...
             {{if $item.bsn_latest_csbd}} {{$item.bsn_latest_csbd_date}} <br /> {{$item.bsn_latest_csbd_user}} {{/if}}      
...
<!--         <td> m
       ...
       <input type="file" name="pr_letter" /><br />
        ...
        <a href="/planning_task_tracker.download_content_wl?file_name={{$item.bc_ptt_letter}}&module_name=planning_task_tracker.home" target="_blank">
        Download</a>
        </div> {{/if}}
      ...
      <a href="{{$BASE_URL}}planning_task_tracker.reason/bsn_id/{{$item.bsn_id}}" class="various">Read</a>
       ...
       <td>
        ...
        <select name="plans[{{$item.bs_business_id}}]" Onchange = "update_pc({{$item.bs_business_id}},this.value)" >
          
           <option value="0">Please Select</option>   
          {{foreach from=$pcData key="key3" item="item3"}}
          <option value="{{$item3.pc_id}}"    {{if $item3.pc_id eq $item.bs_ptt_pc_option}} selected="selected" {{/if}}>{{$item3.pc_option}}</option>
          {{/foreach}}
          </select>
         <div id="usr3{{$item.bs_business_id}}">{{$item.bs_ptt_pc_user}}<br />{{$item.bs_ptt_pc_date}} </div>
	 <script>
