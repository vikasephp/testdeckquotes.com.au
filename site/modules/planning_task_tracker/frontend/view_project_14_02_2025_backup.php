	   ...
	   // echo "<pre>"; print_r($entities_referral_result); echo "</pre>";
	    foreach ($entities_referral_result as $key => $value) {
	       ...
	       $sql2 = "SELECT * FROM `checklist_entities_referral` where checklist_entities_referral.bsn_id= '".$bsn_id."' and entites_referral_id = '".$value['er_id']."'" ;
            ...
            $sql4 = "SELECT `dpn_heritage_ref`,`dpn_water_ref`,`dpn_electric_ref`,`dpn_gas_ref`,`dpn_storm_ref` FROM `document_proposal_name` where dpn_bsn_id = '".$bsn_id."' limit 1";
            $result4 = $fwDb->query($sql4);
            if(!empty($result4)) {
                $entities_referral_result[$key]['document_status'] = $result4[0] ;
            } else {
                $entities_referral_result[$key]['document_status'] = $result4 ;
            }
