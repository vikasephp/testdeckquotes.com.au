...
if (!empty($unhidden)) {
	unset($_SESSION['ptt_show_hidden']);
	unset($_SESSION['unhide']);
}

$unhide = $fwRequest->getParam('unhide', '');
if (!empty($unhide)) {
	$_SESSION['ptt_show_hidden'] = 1;
}
if (!empty($_SESSION['ptt_show_hidden'])) {
	$pttShowHidden = true;
}

$pttHideClause = $pttShowHidden ? "business_sellers.bs_ptt_hide = 1" : "business_sellers.bs_ptt_hide = 0";
$where = " WHERE " . $pttHideClause . " ";
...
if (!empty($add_task)) {
	...
	$sqlta =  "SELECT  business_sellers.bs_business_id, business.bsn_id
			  ...
			  person_responsible_detail.prd_prm_id = 8 where business.bsn_status like '%|2|%' 
...
if (!empty($redlight)) {
	$where = "WHERE business_sellers.bs_traffic_light = 3 AND " . $pttHideClause . " ";
}

$amberlight = $fwRequest->getParam('amberlight', '');
if (!empty($amberlight)) {
	$where = "WHERE business_sellers.bs_traffic_light = 2 AND " . $pttHideClause . " ";
}


$greenlight = $fwRequest->getParam('greenlight', '');
if (!empty($greenlight)) {
	$where = "WHERE business_sellers.bs_traffic_light = 1 AND " . $pttHideClause . " ";
}
...
if (!empty($filter_ss)) {
	...
	$where = " WHERE business_sellers.bs_ptt_sub_status = " . $ss . " AND " . $pttHideClause . " ";
	$fwViewData['ss'] = $ss;
}
...
$sql = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
		...
		business_sellers.bs_ptt_atop_user, business_sellers.bs_ptt_atop_date, business_sellers.bs_ptt_hide
		from business_sellers 		
		Inner Join bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id 
		Inner Join business ON business_sellers.bs_business_id = business.bsn_id  
		Right JOIN person_responsible_detail ON  business_sellers.bs_business_id = person_responsible_detail.prd_bsn_id And 
...
$sqlRed = "SELECT  count(*) as green_total from business_sellers 		
		...
		Inner Join business ON business_sellers.bs_business_id = business.bsn_id  
		Right JOIN person_responsible_detail ON  business_sellers.bs_business_id = person_responsible_detail.prd_bsn_id And 
		person_responsible_detail.prd_prm_id = 8 WHERE 1=1  And  business.bsn_status like '%|2|%' and business_sellers.bs_traffic_light = 3
		Group By business.bsn_id 
		Order By business_sellers.bs_ptt_sub_status_order ASC   ";
...
$sqlAmbar = "SELECT  count(*) as green_total from business_sellers 		
		...
		Inner Join business ON business_sellers.bs_business_id = business.bsn_id  
		Right JOIN person_responsible_detail ON  business_sellers.bs_business_id = person_responsible_detail.prd_bsn_id And 
		person_responsible_detail.prd_prm_id = 8 WHERE 1=1  And  business.bsn_status like '%|2|%' and business_sellers.bs_traffic_light = 2
		Group By business.bsn_id 
		Order By business_sellers.bs_ptt_sub_status_order ASC ";
...
$sqlGreen = "SELECT  count(*) as green_total from business_sellers 		
		...
		Inner Join business ON business_sellers.bs_business_id = business.bsn_id  
		Right JOIN person_responsible_detail ON  business_sellers.bs_business_id = person_responsible_detail.prd_bsn_id And 
		person_responsible_detail.prd_prm_id = 8 WHERE 1=1  And  business.bsn_status like '%|2|%' and business_sellers.bs_traffic_light = 1
		Group By business.bsn_id 
		Order By business_sellers.bs_ptt_sub_status_order ASC";
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata2 as $k => $v) {
			...
			$setdata2[$k]['bs_ptt_hide'] = $v['bs_ptt_hide'];
...
if ($export > 0) {
	...
	foreach ($setdata2 as $k => $v) {
		...
		$objPHPExcel->getActiveSheet()
			...
			->setCellValue('H' . $row, $v['bsn_ptt_letter_text']);
	...
	header('Content-Disposition: attachment;filename="planning_task_tracker.xls"');
