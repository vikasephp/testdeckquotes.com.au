...
<input type="button" name="priority" value="Project Management" onclick="javascript:proj_management();" /> 
...
<input type="button" name="proc_panel" value="Responsible Position Panel" onclick="javascript:staff_included();" /> 
<input type="button" name="liability" value="Liability Options" onclick="javascript:add_liability();" /> 
<input type="button" name="checklist" value="Checklist Admin" onclick="javascript:add_checklist();" /> 
...
<input type="button" name="cgfb" value="Why Not Responsible Admin" onclick="javascript:add_why_not();" />
...
<select name="warranty_type">
        ...
        <option value="{{$item4.wt_type_option}}" {{if $item4.wt_type_option eq $item.wa_type}} selected="selected" {{/if}}>{{$item4.wt_type_option}}</option>
        {{/foreach}}
</select>
<br />
...
<select name="warranty_status">
        <option value="">Please Select</option>
        <option value="Open" {{if $warranty_status eq "Open" }} selected="selected" {{/if}}>Open</option>
        <option value="Closed" {{if $warranty_status eq "Closed" }} selected="selected" {{/if}}>Closed</option>
        <option value="Pending" {{if $warranty_status eq "Pending" }} selected="selected" {{/if}}>Pending</option>
...
<select name="priority"  >
        ...
        <option value="{{$item7.pr_id}}" {{if $item7.pr_id eq $search_by_priority}} selected="selected" {{/if}}>{{$item7.pr_priority}}</option>
        {{/foreach}}
</select>
...
<select name="search_by_rs"  >
                ...
                <option value="{{$item5.cs_position}}" {{if $res_staff eq $item5.cs_position}} selected="selected" {{/if}}>{{$item5.cs_position}}</option>
                {{/foreach}}
</select>
...
<input type="radio" value="2" name="oc" {{if $oc eq 2}} checked="checked" {{/if}} /> Closed &nbsp; &nbsp
<input type="submit" name="sevenday" value = "7 Day Report" /><br />
</div>
<input type="submit" name="print" value = "Print Merge Of Search result open and closed" /> 
...
&nbsp;&nbsp; <span style="border:1px solid #0CF; padding:3px;">90 Day + Logs : {{$tot_ninety.ninety}} </span><br />
<!--<strong> Supplier Search  : </strong> <input type="text" name="supp_name" width="100" /> <input type="submit" name="supplier_o" value = "Search" />--> 
<input type="submit" name="printOP" value = "Print Merge Of Search result open and Pending" style="background:#00CC33;" /> <br />
...
<input type="submit" name="print_filter" value = "Print Merge For Filter by Priority - Open" style="background:#F00;" /> 
</div>
...
<div id="">
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
                   ...
                   <th class="topmenu" align="center" valign="middle" width="4%">Liability Accepted</th> 
                   <th class="topmenu" align="center" valign="middle" width="4%">Who's Paying</th> 
 ...
 {{foreach from=$list key="key" item="item"}}
    ...
    <tr {{if $item.wa_status eq 'Pending'}} bgcolor="#f7b900" {{elseif $item.wa_include_supplier eq  ''}}  bgcolor="#F00" {{else}} bgcolor="#FFFFFF" {{/if}}>
        ...
        <a href="{{$BASE_URL}}warranty_log.log_history/wa_project/{{$item.wa_project}}" class="various">Log History</a>
      ...
      <td>
       <select name="{{$TABLE}}[wa_area]"  Onchange = "update_cgfb({{$item.wa_id}},this.value)">
                <option value="-1" {{if $item.wa_area eq -1}} selected="selected" {{/if}}>Please Select </option>
                {{foreach from = $cgfbdetail key="key6" item="item6"}}
                <option value="{{$item6.wc_id}}" {{if $item6.wc_id eq $item.wa_cgfb_resp}} selected="selected" {{/if}}>{{$item6.wc_cgfb_option}}
                </option>
                 {{/foreach}}
        </select>
      ...
      <td> 
      <select name="{{$TABLE}}[wa_why_not]" class="myselect" Onchange = "update_why_not({{$item.wa_id}},this.value)">
                <option value="0" {{if $item.wa_why_not eq 0}} selected="selected" {{/if}}>Please Select </option>
                {{foreach from = $whynotdetail key="key7" item="item7"}}
                <option value="{{$item7.ww_id}}" {{if $item7.ww_id eq $item.wa_why_not}} selected="selected" {{/if}}>{{$item7.ww_option}}
                </option>
                {{/foreach}}
        </select>
        ...
        <a href ="{{$BASE_URL}}warranty_log.include_suppliers/wa_id/{{$item.wa_id}}" class="various">Include Trades</a><br /><br />
       ...
       {{else }}
       <td>
       <a href="{{$BASE_URL}}warranty_log.colorbox/wa_id/{{$item.wa_id}}" class="various">Color</a></td>
       {{/if}}
   
        
        <td style="text-align:center;">
        {{$item.task_name}}<br />
        <a href="{{$BASE_URL}}warranty_log.view_checklist_status/wa_id/{{$item.wa_id}}" class="various">{{$item.tot_cd}}/{{$tot_chk}}</a></td>
        <td>
         <select name="{{$TABLE}}[wa_priority]"  Onchange = "update_priority({{$item.wa_id}},this.value)">
                <option value="-1" {{if $item.wa_priority eq -1}} selected="selected" {{/if}}>Please Select </option>
                {{foreach from = $prdetail key="key4" item="item4"}}
                <option value="{{$item4.pr_id}}" {{if $item4.pr_id eq $item.wa_priority}} selected="selected" {{/if}}>{{$item4.pr_priority}}</option>
                {{/foreach}}
        </select>
        ...
        <select name="{{$TABLE}}[wa_type]"  Onchange = "update_type({{$item.wa_id}},this.value)">
                <option value='NA' {{if $item.wa_type eq 'NA'}} selected="selected" {{/if}}>Please Select </option>
                {{foreach from = $typedetail key="key4" item="item4"}}
                <option value="{{$item4.wt_type_option}}" {{if $item4.wt_type_option eq $item.wa_type}} selected="selected" {{/if}}>{{$item4.wt_type_option}}
                </option>
                 {{/foreach}}
        </select>
		...
		function update_type(id,value)
		{
         ...
         <select name="{{$TABLE}}[wa_area]"  Onchange = "update_area({{$item.wa_id}},this.value)">
                ...
                <option value="{{$item5.wa_id}}" {{if $item5.wa_id eq $item.wa_area}} selected="selected" {{/if}}>{{$item5.wa_area_option}}
                </option>
                 {{/foreach}}
        </select>
...
" target="_blank">Download</a>
        ...
        {{$item.wa_resp_staff}}
         <a href ="{{$BASE_URL}}warranty_log.select_staff/wa_id/{{$item.wa_id}}" class="various">Select Staff</a>
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="variation[{{$item.wa_id}}]"   onclick="update_variation({{$item.wa_id}},this.checked);" {{if $item.wa_variation eq 1 }} checked="checked" {{/if}} />
      ...
      <script>
        ...
        <td>
      <!--  <input name="same" type="checkbox" {{if $item.wa_lia_acc eq 1}} checked="checked" {{/if}}  onclick="update_la({{$item.wa_id}},this.checked)" />-->
         <select name="{{$TABLE}}[wa_lia_acc]"  Onchange = "update_la({{$item.wa_id}},this.value)">
                ...
                <option value="{{$item5.wl_id}}" {{if $item5.wl_id eq $item.wa_lia_acc}} selected="selected" {{/if}}>{{$item5.wl_liability_option}}
                </option>
                 {{/foreach}}
        </select>
       ...
       <td>
        ...
        <a href ="{{$BASE_URL}}warranty_log.include_suppliers_whos/wa_id/{{$item.wa_id}}" class="various">Include Trades</a><br /><br />
       </td>
       
<td><input type="button" name="Alert" value="Hot Button" onclick="javascript:hot_button({{$item.bs_business_id}},{{$item.bs_customers_id}},{{$item.wa_id}});" style="width:90px; height:20px; font-size:11px; line-height:17px !important; padding:0px !important" /></td>
        ...
        <td> <a href="{{$BASE_URL}}{{$XFA.detail}}/{{$ID}}/{{$item.$ID}}" class="various" title="Edit"> 
             ...
             <input type="submit" name="single_log" style="background-image:url({{$BASE_URL}}css/admin/images/print.png); width:20px; height:20px;" value=" " >
             </form>
        </td>
    </tr>  
  {{/foreach}}
 ...
 {{if $notfound}}<div style="font-size:18px; color:#F00; padding-top:10px;" ><strong>{{$notfound}}</strong></div>{{/if}}   
