<?php
$fwMainView = 'file:' . getcwd() . '/view_staff_alert.tpl';

$submit = $fwRequest->getParam('subAddDetail', '');
$ir_email = $fwRequest->getParam('ir_email', '');
$fwViewData['ic_id'] = $ic_id;
		
	$sql2 = "select * from warranty_log where wa_resp_staff like '%".$ir_email."%' and wa_status = 'Open'";
	
	$alertdata = $fwDb->query($sql2);
	

foreach($alertdata as $k=>$v)
	{
	
		$sql = 'SELECT  business_sellers.bs_business_id, bus_customers.bcust_misc_email1, bus_customers.bcust_fname, 
			bus_customers.bcust_lname, business.bsn_id, business.bsn_name from business_sellers 		
			Inner Join bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id 
			Inner Join business ON business_sellers.bs_business_id = business.bsn_id 
			WHERE business.bsn_name LIKE "%'.$v['wa_project'].'%"  group by bsn_id' ;
	
		  $data = $fwDb->queryOne($sql);
		  $toname = $data['bcust_fname']. " ". $data['bcust_lname'];
		  
		  $alertdata[$k]['contact_info'] = $toname;
		  
		  
		  $x = explode("<br>",$v['wa_include_supplier']);

		$incsup='';
		foreach($x as $k2=>$v2)
		{
			preg_match('~>\K[^<>]*(?=<)~',  $v2,$str);
			$incsup = [];
			$incsup[] = $str[0];	
		}
	
		$alertdata[$k]['wa_include_supplier'] = implode("<br>",$incsup) ;
			
		$sqlto = "Select pr_priority from warranty_log_priority where pr_id = ".$v['wa_priority'];;
		$a = $fwDb->queryOne($sqlto); 
		
		$alertdata[$k]['wa_priority'] = $a['pr_priority'];	
	}

$fwViewData['alertdata'] = $alertdata;
