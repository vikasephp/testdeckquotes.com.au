...
<div align="center" style="min-height:350px; padding-top: 40px;">
    ...
    <table cellpadding="0" cellspacing="0" class="table" id="list-table" style="margin-bottom: 30px; width: 800px;">
		...
		<tbody>
			...
			<tr>
				<td>Customer Signed Warranty and Lifetime Workmanship Guarantee – Docusign Live Link</td>
				<td>
					{{if $file_778}}
						<a href="/warranty_log2.download_content?file_name={{$file_778}}&module_name=business.home"
							class="action_checklist" title="{{$file_778}}"
							target="_blank" style="display: block;">Download</a>
							{{if $date_uploaded_778}} - {{$date_uploaded_778|date_format:"%d-%m-%Y"}}{{/if}}
					{{/if}}
				</td>
			</tr>
			<tr>
				...
				<td>
					{{if $file_779}}
						<a href="/warranty_log2.download_content?file_name={{$file_779}}&module_name=business.home"
							class="action_checklist" title="{{$file_779}}"
							target="_blank" style="display: block;">Download</a>
							{{if $date_uploaded_779}} - {{$date_uploaded_779|date_format:"%d-%m-%Y"}}{{/if}}
					{{/if}}
				</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td>
					<div style="width: 200px;">
						<select name="{{$TABLE}}[wa_why_not]" class="myselect" onchange="update_why_not({{$data_log.wa_id}},this.value)" style="width: 100%;">
							...
							<option value="{{$item7.ww_id}}" {{if $item7.ww_id eq $data_log.wa_why_not}} selected="selected"{{/if}}>
								{{$item7.ww_option}}
							</option>
							{{/foreach}}
						</select>
						<div id="why{{$item.wa_id}}" style="display: flex; justify-content: space-between;">
							<span>{{$data_log.wa_why_not_user}}</span>
							<span>{{$data_log.wa_why_not_date}}</span>
						</div>
						<script>
			...
			<tr>
				<td>Signed Warranty Agreement</td>
				<td>TBD</td>
			</tr>
		<tbody>
    </table>
