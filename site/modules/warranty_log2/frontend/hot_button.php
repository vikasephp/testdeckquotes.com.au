...
$wap = $fwRequest->getParam('wap', '');

if(!empty($wap)) {

	
	$sqlw = "select wa_incl_supp_email  from warranty_log where wa_id = ".$wa_id;
	$dataw = $fwDb->queryOne($sqlw);
	
	$dataem = explode(",",$dataw['wa_incl_supp_email']);
	
	$fromname = "CGFB Warranty Team";
	$from = "warranty@cgfb.com.au";
	$subject = "Warranty Acceptance Agreement";
	
	foreach($dataem as $k=>$v) 
	{
	    if(!empty($v)) {
   	    $sqlco = "select companies.co_company_name from companies
	            inner join contacts on companies.co_id = contacts.cs_company 
				where contacts.cs_primary_email = '".$v."'";
			
	 	$codata = $fwDb->queryOne($sqlco);
		
		$link = "https://www.deckquotes.com.au/wap/wap.htm/?wa_id=".$wa_id."&comp=".$codata['co_company_name']."&eml=".$v;
		
		$toname = $codata['co_company_name'];
		$to = $codatap['cs_primary_email'];
		
		//$to = "manojsoniephp@gmail.com";
		
		$html  = '<p>'.$toname.'</p><br>';
		$html .= '<p>Here is your Warranty Acceptance Agreement form link. Please click on the link and fill up the form<p><br>';
		$html .= '<p><a href="'.$link.'" target="_blank">Warranty Acceptance Agreement Form </a><p><br>';
		$html .= "<p>Yours Sincerely,</p>";
		$html .= "<p>CGFB Warranty Team</p>";
		send_email($toname,$to, $fromname, $from, $subject, $html, $attachmentsend='');
		$fwViewData['msgwap'] = "Emails Has Been Successfully Sent";
		}
	}
}