...
foreach($setdata as $k => $v)
		{
		$docid = $v['dh_id'];
		$sqldocs = "SELECT document_ohs_docs.* FROM document_ohs_docs WHERE document_ohs_docs.dhd_dh_id = ".$docid;
		$doc_files = $fwDb->query($sqldocs);
				if($doc_files)
					{
						foreach($doc_files as $kf => $vf)
								{
								if($vf['dhd_type']){$vf['iconpath'] = get_icon_path($vf['dhd_type'], $vf['dhd_name'], 'template_ohs_docs');}
								$filedata[] = $vf;
								}
					}		
