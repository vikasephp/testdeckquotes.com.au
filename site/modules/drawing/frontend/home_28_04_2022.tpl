...
<tbody>
 ...
 <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
    <td>{{counter}}</td>
    <td><div align="center"><a href="{{$BASE_URL}}files/drawing_designs/{{$item.filename|default:'no_image.jpg'}}" class="fancybox">
    <img src="{{$item.iconpath}}" /></a><br /><br />
    <span>{{$item.title}}</span> {{if $item.filename}} 
    <a href="{{$BASE_URL}}files/drawing_designs/{{$item.filename}}" title="Start Download" target="_blank">Download</a>{{/if}}</div>
    </td>
