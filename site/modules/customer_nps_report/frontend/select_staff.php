...
$tableStaff = new Fw_Db_Table('include_resp_staff_nps');
$submit = $fwRequest->getParam('subAddDetail', '');
$cn_id = $fwRequest->getParam('cn_id', '');
	...
	$matsql = "SELECT contacts.* , companies.co_company_name, type_options.to_option  from contacts 
	 ...
	 foreach($list as $ck=>$vc)
	 {
		...
		$sqlc = "Select * from include_resp_staff_nps where ir_email = '".$vc['cs_primary_email']."' and ir_cn_id = ".$cn_id;
   ...
   if(!empty($add_emails))
   {
	...
	foreach($emails as $k => $v)
	{
		...
		if($v =='on') {
		 ...
		 $tableStaff->setWhere("ir_email = '".$email."' and ir_cn_id =".$cn_id);
	 	 if(!$tableStaff->rowExists())
	         {
			if($v == 'on') {
				$irDetail['ir_cn_id'] = $cn_id;
	 ...
	 $detail['cn_resp_staff'] = $poseml2;
	 $table->setWhere("cn_id = ".$cn_id);
