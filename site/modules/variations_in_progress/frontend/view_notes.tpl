...
<div align="center" style="min-height:350px; z-index:9999999;">
	...
	<form name="detail" method="post" action="" enctype="multipart/form-data">
		<div style="float:right; margin-right:10px; background: #09F; color:#FFF !important; padding:6px; margin-bottom:5px;">
			<a href="{{$BASE_URL}}{{ $BASEFOLDER }}.add_notes/vipn_va_id/{{$vipn_va_id}}" class="various" title="Edit" style="color:#FFF; text-decoration:none; font-size:14px;">Add New Notes</a>
		</div>
		<table id="list-table" width="99%">
			...
			<tr>
				<td>{{$counter}}</td>
				<td>{{$item.vipn_notes}}</td>
				<td>
					{{$item.vipn_added_by}}
				</td>
				<td>{{$item.vipn_added_date|date_format:"%d-%m-%Y"}}</td>
				<td>
					<a href="{{$BASE_URL}}{{$BASEFOLDER}}.add_notes/vipn_id/{{$item.vipn_id}}/vipn_va_id/{{$vipn_va_id}}"
								class="various" title="Edit">
								<img style="height:20px; width:20px;" src="{{$BASE_URL}}css/admin/images/edit.png" /></a>&nbsp;&nbsp;
					<a href="{{$BASE_URL}}{{$BASEFOLDER}}.delete_notes/vipn_id/{{$item.vipn_id}}/vipn_va_id/{{$vipn_va_id}}"
