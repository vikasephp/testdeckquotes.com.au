...
$flter_data_query = $fwRequest->getParam('flter_data_query', '');

if(!empty($flter_data_query)){
   
   	$where ='';
	$sqlvar = 1; 
	$status = explode("-", $flter_data_query);
	$data=''; $flter_data_query = "";
//	echo "<pre>"; print_r($status); echo "</pre>";
	...
	if(!empty($status)){ $flter_data_query = implode("-", $status); ;  }
...
if (!(isset($pagenum)))
    ...
    $fwViewData['flter_data_query'] = 	$flter_data_query;	
	...
	{  
		...
		foreach($bsdata as $k2 => $v2)
		{
			...
			foreach($setdata as $k => $v)
			{
				...
				$setdata2[$a]['who'] = $v['bsn_proj_salu_who'];
			}
