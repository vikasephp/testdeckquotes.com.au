  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="10%">Project Status</th>
        <th class="topmenu" align="center" valign="middle" width="10%">Total HIA Value</th>
        <th class="topmenu" align="center" valign="middle" width="10%">Paid Value</th>
        <th class="topmenu" align="center" valign="middle" width="10%">Last Audited</th>
        <th class="topmenu" align="center" valign="middle" width="10%">Add To Deposit Report</th>
        
        <th class="topmenu" align="center" valign="middle" width="12%">Move To Old</th>
...
<input class="switch-input" type="checkbox"  name="last_audit[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bs_xry_last_audit eq 1 }} checked="checked" {{/if}} />
       ...
       {{if $item.bs_xry_last_audit eq 1}} {{$item.bs_xry_last_audit_user}} {{$item.bs_xry_last_audit_date}} {{/if}}
        ...
        <form name ="recmet5" method="post" action="">
     	<input type="hidden" name="add_to_deposite[{{$item.bsn_id}}]" value="0" />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="add_to_deposite[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bs_xry_add_to_deposite eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form> 
       {{if $item.bs_xry_add_to_deposite eq 1}} {{$item.bs_xray_deposite_user}}<br /> {{$item.bs_xray_deposite_date}} {{/if}}        
        </td>
...
<input class="switch-input" type="checkbox"  name="move[{{$item.bsn_id}}]" value="1"  onclick="this.form.submit();" {{if $item.bs_xray_old eq 1 }} checked="checked" {{/if}} />
      ...
      {{if $item.bs_xray_old eq 1}} {{$item.bs_xray_move_user}}<br /> {{$item.bs_xray_move_user}} {{/if}}
         </td>
