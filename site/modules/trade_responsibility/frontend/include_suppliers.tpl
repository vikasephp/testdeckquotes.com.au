...
window.location.href = "{{$BASE_URL}}trade_responsibility.detail/wa_id/{{$wa_id}}";
</script>
{{/if}}
...
<tr>
	{{assign var = countn value = 1}}
       ...
       <td>
<input type="checkbox" name="supplier[{{$item.co_id}}]" {{foreach from=$incsup key="key2" item="item2"}} {{if $item2 eq $item.co_company_name}} checked="checked" {{/if}} {{/foreach}} />
