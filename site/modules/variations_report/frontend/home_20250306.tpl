...
&nbsp; &nbsp;<strong> Filter By Financial: </strong> 
 ...
 <input type="button" value="Export To XLS" onclick="document.location.href='{{$BASE_URL}}variations_report.home/export/1'" /> &nbsp;
 </div>   
</form>
...
<input class="switch-input" type="checkbox"  name="financial[{{$item.va_id}}]" value="1"  onclick="this.form.submit();" {{if $item.va_financial eq 1 }} checked="checked" {{/if}} />
    ...
    <td style="text-align:center;">{{if $item.va_pdf}}
    <!--<a href="https://deckquote.s3.amazonaws.com/files/variations_doc/{{$item.va_pdf}}" target="_blank">Link</a>-->
    <a href="/variations_report.download_content?file_name={{$item.va_pdf}}&module_name=variations_report.home
" target="_blank">Link</a>
    ...
    <form name ="recmet4" method="post" action="">
          <select name="plans[{{$item.va_id}}]" onChange="update_plans({{$item.va_id}}, this.value)"/>
                ...
                <option value="{{$item5.vp_id}}" {{if $item5.vp_id eq $item.va_plans}} selected="selected" {{/if}}>{{$item5.vp_option}}</option>
                {{/foreach}}
    ...
    <td style="text-align:center;">{{if $item.va_signed}}
    <!--<a href="https://deckquote.s3.amazonaws.com/files/variations_doc/{{$item.va_signed}}" target="_blank">Link</a>-->
    <a href="/variations_report.download_content?file_name={{$item.va_signed}}&module_name=variations_report.home
" target="_blank">Link</a>
    {{/if}}</td>
    
   <td><input type="text" name="vardate[{{$item.va_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo_{{$item.va_id}}" value="{{$item.va_variation_date}}" onfocus ="update_var_date({{$item.va_id}}, this.value);"  />
    ...
    <td>
     <form name ="recmet4" method="post" action="">
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="update_inv_created[{{$item.va_id}}]" value="1"  onclick="this.form.submit();" {{if $item.va_inv_created eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
    ...
    <td> <input type="text" name="value_var" value="{{$item.va_values}}"  onkeyup="update_value({{$item.va_id}}, this.value);"  /> 
    ...
    <td>
     <form name ="recmet4" method="post" action="">
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="linked_project[{{$item.va_id}}]" value="1"  onclick="this.form.submit();" {{if $item.va_link_project  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
      {{if $item.va_link_project  eq 1 }} {{$item.va_link_project_user}}<br />{{$item.va_link_project_date}} {{/if}}
    </td>
    ...
    <td>
     <form name ="recmet4" method="post" action="">
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="linked_pass[{{$item.va_id}}]" value="1"  onclick="this.form.submit();" {{if $item.va_link_pr_pass  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
      {{if $item.va_link_pr_pass  eq 1 }} {{$item.va_link_pr_pass_user}}<br />{{$item.va_link_pr_pass_date}} {{/if}}
    </td>
  ...
  <td>
   ...
   <form name ="recmet2" method="post" action="">
          <select name="status[{{$item.va_id}}]" onChange="update_status({{$item.va_id}}, this.value)"/>
                <option value="0" {{if $item.va_status eq 0}} selected="selected" {{/if}}>Please Select </option>
                {{foreach from = $statusData key="key3" item="item3"}}
                <option value="{{$item3.st_option}}" {{if $item3.st_option eq $item.va_status}} selected="selected" {{/if}}>{{$item3.st_option}}</option>
                {{/foreach}}
    ...
    <td> 
     ...
     <form name ="recmet5" method="post" action="">
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="schedule[{{$item.va_id}}]" value="1"  onclick="this.form.submit();" {{if $item.va_schedule_up  eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form> 
