...
if(!empty($update_link)){
	...
	$table_toplink->setWhere("tl_slug = 'design_interface'");
...
if($ser_dn):
	$where .= "AND ".$TABLE.".di_design_number  = ".$ser_dn;
	$_SESSION['ser_dn'] = $ser_dn;
	$fwViewData['ser_dn']=$_SESSION['ser_dn'];
elseif($_SESSION['ser_dn']):
	$where .= "AND ".$TABLE.".di_design_number = ".$_SESSION['ser_dn'];
	$fwViewData['ser_dn']=$_SESSION['ser_dn'];
endif;
...
foreach($listsnew as $k => $v)
{
	...
	$sql2 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$v['di_id']. " AND dip_component = 'GP Value'";
	...
	$sql3 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$v['di_id']. " AND dip_component = 'Rounded Connector Value'";
...
if($export > 0)
	...
	$sqlexport = "Select design_interface.*, design_interface_edropbox.didb_filepath from design_interface 
		   Left join design_interface_edropbox on design_interface.di_id = design_interface_edropbox.didb_di_id
		   AND  design_interface_edropbox.didb_foldername like '%Team%'
		   ".$where. " ORDER BY  design_interface.di_design_number  ASC";
...
foreach($exportData as $k=>$v)
{
	$objPHPExcel->getActiveSheet()
				...
				->setCellValue('E'.$row, $v['di_cgfb_design_link'])
				...
				->setCellValue('N'.$row, 'http://www.deckquotes.com.au/files/design_interface/'. $v['di_master_project_calculator'])
				->setCellValue('O'.$row, 'http://www.deckquotes.com.au/files/design_interface/'. $v['di_proposal'])
				->setCellValue('P'.$row, 'http://www.deckquotes.com.au/files/design_interface/'. $v['di_brochure'])
				->setCellValue('Q'.$row, 'http://www.deckquotes.com.au/files/design_interface/'. $v['di_full_brochure'])
				->setCellValue('R'.$row, 'http://www.deckquotes.com.au/files/design_interface/'. $v['di_operations_calculator'])
...
if(!empty($di_id)) {
	...
	$sqlp = "select * from design_interface where di_id =".$di_id;
	$diData = $fwDb->queryOne($sqlp);	
	
	$sql1 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$di_id. " AND dip_component = 'Rounded Total Price Total'";
	$data1 = $fwDb->queryOne($sql1);
	
	$sql4 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$di_id. " AND dip_component = 'Rounded Build Cost with GP'";
	$data4 = $fwDb->queryOne($sql4);
	
	$sql5 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$di_id. " AND dip_component = 'Rounded Siteworks Price'";
	$data5 = $fwDb->queryOne($sql5);
	
	$sql6 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$di_id. " AND dip_component = 'Rounded Planning Price'";
	$data6 = $fwDb->queryOne($sql6);
	
	$sql3 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$di_id. " AND dip_component = 'Rounded Connector Value'";
	$data3 = $fwDb->queryOne($sql3);
	
	$sql2 = "select design_interface_pricing.* from design_interface_pricing
	         where design_interface_pricing.di_id = " .$di_id. " AND dip_component = 'GP Value'";
	$data2 = $fwDb->queryOne($sql2);
	...
	if($diData['di_master_project_calculator']) {
		...
		$html .= '<td><a href="https://deckquote.s3.amazonaws.com/files/design_interface/'.$mpc.'">Download</a></td>';
	} else {
		$html .= "<td></td>";	
	}
	...
	if($diData['di_proposal']) {
		...
		$html .= '<td><a href="https://deckquote.s3.amazonaws.com/files/design_interface/'.$pro.'">Download</a></td>';
	} else {
		$html .= "<td></td>";	
	}
	...
	if($diData['di_brochure']) {
		...
		$html .= '<td><a href="https://deckquote.s3.amazonaws.com/files/design_interface/'.$broc.'">Download</a></td>';
	} else {
		$html .= "<td></td>";	
	}
	...
	if($diData['di_full_brochure']) {
		...
		$html .= '<td><a href="https://deckquote.s3.amazonaws.com/files/design_interface/'.$fb.'">Download</a></td>';
	} else {
		$html .= "<td></td>";	
	}
	...
	if($diData['di_design_summary']) {
		...
		$html .= '<td><a href="https://deckquote.s3.amazonaws.com/files/design_interface/'.$ds.'">Download</a></td>';
	} else {
		$html .= "<td></td>";	
	}
	...
	if($diData['di_operations_calculator']) {
		...
		$html .= '<td><a href="https://deckquote.s3.amazonaws.com/files/design_interface/'.$oc.'">Download</a></td>';
	} else {
		$html .= "<td></td>";	
	}
	...
	if($diData['di_msteam_link']) {
		...
		$html .= '<td><a href="https://deckquote.s3.amazonaws.com/files/design_interface/'.$team.'">Download</a></td>';
	} else {
		$html .= "<td></td>";	
	}
	...
	$tablefeature = new Fw_Db_Table('design_interface_features');
	...
	$tableDesignInterDesignes = new Fw_Db_Table('design_interface_designs');
	...
	foreach($designDetail as $kd => $vd)
	{
	  if($vd['di_design_image']) {
		  ...
		  if($extension !== 'pdf') {
	    $img2 =  '<img src="https://deckquote.s3.amazonaws.com/files/design_interface/'.$vd['di_design_image'].'" width="225" height="225">';
	    $html .= '<td>'.$img2.'</td>';
	    $m++;
	    if($m == 4) {
		$html .= '</tr><tr>';
	...
	//create_di_report($heading, $html, $fname);
	  ...
	  header("Content-Disposition: attachment;Filename=design_interface.doc");    
	  echo "<html>";
...
$sqlmr = "Select design_interface.di_design_number, design_interface.di_qr_code , design_interface.di_id
          from design_interface  where di_active = 'Yes' order by design_interface.di_design_number "; 
...
foreach($mrData as $k=>$v)
{
	...
	$tablelegend = new Fw_Db_Table('design_interface_bfpl');
...
$html .= '</body></html>';
	//create_merge_report('Design Interface Merge Report', $html, $fname='');
          ...
          header("Content-type: application/vnd.ms-word.doc");
	  header("Content-Disposition: attachment;Filename=design_merge.doc");    
	  echo "<html>";
	  echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=Windows-1252\">";
	  echo "<body>";
	  echo "<div style='font-family:calibri;'>";
