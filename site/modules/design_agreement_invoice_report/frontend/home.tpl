...
<form name="mlist" method="post" enctype="multipart/form-data">
	...
	<div style="float:left; text-align:left;">
		...
		&nbsp; &nbsp;<input type="button" name="payment_options" value="Payment Admin"
			onclick="javascript:add_payment();" />
		<br />

		&nbsp; &nbsp; <strong>Search Address: </strong>
		<input type="text" name="address" value="{{$address}}" style="width:250px;" placeholder="Search for Address" />
		...
		<input type="text" name="client" value="{{$client}}" style="width:250px;" placeholder="Search for First Name" />
		<input type="submit" value="Search" name="search" /> &nbsp;<input type="submit" name="clear_search"
			value="Clear Search" />
		<br />
		...
		<option value="{{$item2.st_id}}" {{if $item2.st_id eq $invoice_status}} selected="selected" {{/if}}>
			{{$item2.st_option}}</option>
		{{/foreach}}
		</select>
		...
		<option value="{{$item4.dp_id}}" {{if $item4.dp_id eq $paytment_status}} selected="selected" {{/if}}>
			{{$item4.dp_option}}</option>
		{{/foreach}}
		</select>
		&nbsp; &nbsp;<input type="submit" value="Filter" name="filter" />
		<input type="submit" value="Clear Filter" name="clear" />
	...
	<div style="float:right;">
		...
		<input type="button" value="Export To XLS"
			onclick="document.location.href='{{$BASE_URL}}design_agreement_invoice_report.home/export/1'" />
	</div>
		...
		<table id="doclist-table" class="nav-back" width="100%">
			<thead>
				<tr>
					...
					<th class="topmenu" align="center" valign="middle" width="5%">Signed Design <br />Agreement Link
					</th>
			...
			<tbody>
				...
				<tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
					...
					<td><strong>{{$item.bcust_fname}} {{$item.bcust_lname}}</strong><br />
						<a href="{{$item.link}}" target="_blank">{{$item.bsn_name}}</a>
					</td>
					<td>{{$item.bsn_sub_status}}</td>
					<td>
						{{if $item.doc_file_name_214}}
						<a href="/design_agreement_invoice_report.download_content?file_name={{$item.doc_file_name_214}}&module_name=design_agreement_invoice_report.home"
							target="_blank">Download</a>
						{{/if}}
					</td>
					<td><input type="text" name="da_sent_date" value="{{$item.bsn_dair_agree_sent}}"
							onfocus="update_sent_date({{$item.bsn_id}},this.value)"
					...
					<td>
						...
						<a href="/design_agreement_invoice_report.download_content?file_name={{$item.doc_file_name}}&module_name=design_agreement_invoice_report.home"
							target="_blank">Download</a>
						{{/if}}
					</td>
					...
					<td>
						<input type="text" name="dt_of_dasd" value="{{$item.bsn_dair_dasd}}"
							onfocus="update_date_dasd({{$item.bsn_id}},this.value)"
							class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo11_{{$item.bsn_id}}"
							style="width:60px;" />
					...
					<td><input type="text" name="dt_of_pdd" value="{{$item.bsn_dair_pdd}}"
							onfocus="update_date_pdd({{$item.bsn_id}},this.value)"
					...
					<td>
						<form name="recmet2" method="post" action="">
							...
							<option value="0" {{if $item.sp_www eq -1}} selected="selected" {{/if}}>Please Select
							</option>
							{{foreach from = $typedata key="key3" item="item3"}}
							<option value="{{$item3.dt_id}}" {{if $item3.dt_id eq $item.bsn_dair_type}}
								selected="selected" {{/if}}>{{$item3.dt_option}}</option>
							{{/foreach}}
							</select>
						</form>
					...
					<td>
						<form name="recmet2" method="post" action="">
							...
							{{foreach from = $stdata key="key2" item="item2"}}
							<option value="{{$item2.st_id}}" {{if $item2.st_id eq $item.bsn_dair_inv_status}}
					...
					<td>{{ if $item.bsn_dair_invoice}}<br />
						<a href="/design_agreement_invoice_report.download_content_inv?file_name={{$item.bsn_dair_invoice}}&module_name=design_agreement_invoice_report.home"
							target="_blank">{{$item.bsn_dair_invoice_num}}</a>
					...
					<td>
						<form name="amt" method="post" action="">
							<input type="text" name="amount[{{$item.bsn_id}}]" value="{{$item.bsn_dair_amount}}"
								onkeyup="update_amount({{$item.bsn_id}},this.value)" style="width:80px;" />
					...
					<td>
						...
						<form name="pay_date" method="post" action="">
							<input type="text" name="payment[{{$item.bsn_id}}]" value="{{$item.bsn_dair_pay_date}}"
								id="demo3_{{$item.bsn_id}}" class="w16em dateformat-d-ds-m-ds-Y dtpic"
								onfocus="update_pay_date({{$item.bsn_id}},this.value)" />
					...
					<td>
						<form name="recmet2" method="post" action="">
							...
							<option value="{{$item3.dp_id}}" {{if $item3.dp_id eq $item.bsn_dair_pay_status}}
								selected="selected" {{/if}}>{{$item3.dp_option}}</option>
							{{/foreach}}
					...
					<td {{if $item.bsn_dair_notes}} style="background:#F00;" {{/if}}><a
							href="{{$BASE_URL}}design_agreement_invoice_report.notes/bsn_id/{{$item.bsn_id}}"
							class="various">Notes</a></td>
						...
						<form name="recmet3" method="post" action="">
							...
							<label class="switch">
								<input class="switch-input" type="checkbox" name="hide[{{$item.bsn_id}}]" value="1"
									onclick="this.form.submit();" {{if $item.bsn_dair_hide eq 1 }} checked="checked"
									{{/if}} />
