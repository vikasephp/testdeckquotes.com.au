...
if($bsn_id > 0) {
    ...
    $where = 'WHERE BST.bst_business_status_id = 5 AND BT.bt_bsn_id = ' . $bsn_id . ' AND (bt_completed_date IS NULL OR bt_completed_date = "0000-00-00 00:00:00" OR bt_complete = 0)';
    ...
    foreach($busness_status_task as $row) {
        ...
        $final_list[] = [
            ...
            'not_required_at' => (strtotime($row['bt_task_not_required_at']) > 0 ? date('d-M-Y', strtotime($row['bt_task_not_required_at'])) : ''),
            'not_required_by' => ($users[$row['bt_task_not_required_by']] ?? ''),
            'uid' => $row['bst_task_id'],
            'seq' => $row['bst_order'],
