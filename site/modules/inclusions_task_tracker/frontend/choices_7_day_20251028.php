...
$sql =
    'SELECT
        ...
        (SELECT tccl_changed_per FROM tracker_choices_changed_log WHERE tccl_bsn_id = TCCL.tccl_bsn_id ORDER BY tccl_id DESC LIMIT 1) AS latest_per,
        (SELECT tccl_changed_per FROM tracker_choices_changed_log WHERE tccl_bsn_id = TCCL.tccl_bsn_id AND tccl_id < (SELECT tccl_id FROM tracker_choices_changed_log WHERE tccl_bsn_id = TCCL.tccl_bsn_id ORDER BY tccl_id DESC LIMIT 1) ORDER BY tccl_id DESC LIMIT 1) AS last_per,
        (SELECT tccl_changed_per FROM tracker_choices_changed_log WHERE tccl_bsn_id = TCCL.tccl_bsn_id AND tccl_changed_at BETWEEN "' . $lastWeekStartDate . '" AND "' . $lastWeekEndDate . '" ORDER BY tccl_id DESC LIMIT 1) AS last_week_per,
        (SELECT tccl_changed_per FROM tracker_choices_changed_log WHERE tccl_bsn_id = TCCL.tccl_bsn_id AND tccl_changed_at BETWEEN "' . $thisWeekStartDate . '" AND "' . $thisWeekEndDate . '" ORDER BY tccl_id DESC LIMIT 1) AS this_week_per
    FROM
...
foreach ($result as $row) {
    ...
    } elseif (!empty($row['last_per']) && !empty($this_week_per)) {
        $difference_per = $this_week_per - $row['last_per'];
...
foreach ($projects as $bsn_id => $project) {
    $record = $tracker_choices_changed_log[$bsn_id] ?? [];
    $last_week_per = $this_week_per = $difference_per = '';
    if (!empty($record['last_week_per'])) {
        $last_week_per = $record['last_week_per'];
    }
    if (!empty($record['this_week_per'])) {
        $this_week_per = $record['this_week_per'];
    }
    if (!empty($record['difference_per'])) {
        $difference_per = $record['difference_per'];
    }
