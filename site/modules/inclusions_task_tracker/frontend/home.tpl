...
<script type="text/javascript">
	...
	function project_type_admin() {
		var viewurl = "{{$BASE_URL}}inclusions_task_tracker.view_projecttype" + "/random/" + Math.random();
		$(document).ready(function () {
			$.fancybox({
...
<script>
	...
	function onsite(bsn_id) {
		var viewurl = "{{$BASE_URL}}inclusions_task_tracker.onsite";
		window.open(viewurl, "_blank");
		return false;
		$(document).ready(function () {
			$.fancybox({
		...
		<div style="margin-left: 10px; text-align: left;">
			<div style="display: inline-block;">
				...
				<div style="margin-bottom: 5px;">
					...
					<input type="button" name="project_type" value="Project Type Admin" onclick="javascript:project_type_admin();" />
					<input type="submit" value="Add Tasks" name="add_task" />
					<input type="button" name="" value="% choices 7 day" onclick="choices_7_day();" />
					<input type="button" name="" value="Onsite" onclick="onsite();" />
				</div>
				<div>
			...
			<table id="doclist-table" class="nav-back" width="100%">
				<thead>
					<tr>
						...
						<th class="topmenu" align="center" valign="middle" style="min-width: 60px;">Project type</th>
						...
						<th class="topmenu" align="center" valign="middle" style="width:40px;">Inclusions Package</th>
						<th class="topmenu" align="center" valign="middle" style="width:40px;">Constuction Package</th>
						<th class="topmenu" align="center" valign="middle" style="width:10px;">Document Click<br />Link</th>
						<th class="topmenu" align="center" valign="middle" style="width:40px;">WWW</th>
						...
						<th class="topmenu" align="center" valign="middle" style="min-width: 60px;">Where the data comes from</th>
						<th class="topmenu" align="center" valign="middle" style="width:110px;">Live Weekly Letter</th>
				...
				<tbody>
					...
					<tr bgcolor="{{cycle values=" #D3E8D6,WHITE" advance=true}}">
						...
						<td>
							<!--{{if $item.data_bsn_type}}{{$item.data_bsn_type}}{{/if}}-->
							<select name="proj_type" Onchange="update_itt_proj_type({{$item.bsn_id}},this.value)" style="width: 180px;">
								<option value="">Please Select </option>
								{{foreach from=$project_type key="keyt" item="itemt"}}
								<option value="{{$itemt.itt_pt_id}}" {{if $itemt.itt_pt_id eq $item.bsn_itt_proj_type}}
									selected="selected" {{/if}}>{{$itemt.itt_pt_option}}</option>
								{{/foreach}}
							</select>
							
							<script>
							function update_itt_proj_type(id, value) {

									$.ajax({
										type: "GET",
										url: "{{$BASE_URL}}inclusions_task_tracker.update_itt_proj_type/bsn_id/" + id + "/value/" + value,
										success: function (result) {
										}
									});
								}
						...
						<td>
							{{if $item.doc_283}}
							<a href="/inclusions_task_tracker.download_content?file_name={{$item.doc_283}}&module_name=business.home">Download</a>
							{{/if}}
						</td>
						<td>
							{{if $item.doc_142}}
							<a href="/inclusions_task_tracker.download_content?file_name={{$item.doc_142}}&module_name=business.home">Download</a>
							{{/if}}
						</td>
						<td style="text-align:center;"><a href="{{$item.link2}}" target="blank">Link</a></td>
						<td data-col="www">
							{{if $item.bsn_id}}
							<a href="https://planningapprovalscanberra.com.au/project/design-phase/{{$item.bsn_id}}" target="_blank">Link</td>
							{{/if}}
						</td>
						...
						<td data-col="latest_onsite_date">
							...
							<input type="text" class="w16em dtpic" id="demo3_{{$item.bsn_id}}" value="{{$item.bsn_starting_onsite_date}}" readonly /><br/>
							{{if $item.bsn_starting_onsite_date}} {{$item.bsn_onsite_user}} {{/if}}
							{{if $item.data_onsiteDate_updatedOn}}
								<br/>{{$item.data_onsiteDate_updatedOn|date_format:"%d-%m-%Y"}}
							{{/if}}
						</td>
						<!-- Not Needed Anymore
						<td>
						...
						<td>{{$item.wheredatacomesfrom}}</td>
						<td>
							{{if $item.doc_457}}
							<a href="/inclusions_task_tracker.download_content?file_name={{$item.doc_457}}&module_name=business.home">Download</a>
							{{/if}}
						</td>
						<td>
							<p>
