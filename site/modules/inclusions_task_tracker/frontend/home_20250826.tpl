...
.wrapper2{ }
.div1 {width:7980px; height: 20px; }
.div2 {width:7980px;  overflow: auto;}  
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    ...
    <tbody>
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
       ...
       {{else }}
       <td>
       <a href="{{$BASE_URL}}inclusions_task_tracker.colorbox/bsn_id/{{$item.bsn_id}}" class="various">Color</a></td>
       {{/if}}
       ...
       <select name="letter[{{$item.bs_customers_id}}]" Onchange = "update_letter({{$item.bs_customers_id}},this.value)" >
          ...
          <option value="{{$item2.lt_code}}"    {{if $item2.lt_code eq $item.bcust_itt_letter_text}} selected="selected" {{/if}}>{{$item2.lt_code}}</option>
          {{/foreach}}
          </select>
	...
	function update(id,value)
	{
	 ...
	 $.ajax({
      ...
      <td> 
         ...
         <input type="text" name="bsn_incl_cc_date[{{$item.bsn_id}}]"  class="w16em dateformat-d-ds-m-ds-Y dtpic" id="demo3_{{$item.bsn_id}}" value="{{$item.bsn_incl_cc_date}}"  /> 
      ...
      <td> </td>
      <td> <form name="mlist123" method="post" enctype="multipart/form-data"> 
        ...
        <div style="text-align:center;">
         <!--<a href="{{$BASE_URL}}files/uploads/{{$item.bc_incl_letter}}" target="_blank">Download</a>-->
          <a href="/inclusions_task_tracker.download_content?file_name={{$item.bc_incl_letter}}&module_name=inclusions_task_tracker.home" target="_blank"/>Download</a>
         </div> {{/if}}
