...
$query = 'SELECT DISTINCT prd_name FROM person_responsible_detail WHERE prd_prm_id = 10 AND prd_name IN ("Elisa Sko", "Leon Barrett") ORDER BY prd_name ASC;';
$result = $fwDb->query($query);
...
$sql = "SELECT business.bsn_inclusions_task_tracker_notes, business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname, bus_customers.bcust_misc_email1,bus_customers.bcust_misc_email2,bus_customers.bcust_misc_moble,bus_customers.bcust_misc_business, bus_customers.bcust_misc_home,business.bsn_advert_text,  business.bsn_id, business.bsn_name,  business.bsn_sub_status, business.bsn_status_sys_date, business.bsn_address, person_responsible_detail.prd_name, business.bsn_cp_incl_choices, 	business.bsn_per_complete, incl_tracker_docs.it_weekly_doc, incl_tracker_docs.it_uploaded_date, business.bsn_incl_cc_date, business.bsn_starting_onsite_date, business.bsn_onsite_user, business.bsn_itt_proj_type, business_sellers.bc_incl_letter, business_sellers.bs_traffic_light_incl, bus_customers.bcust_itt_letter_text, bus_customers.bcust_itt_letter_text_by, bus_customers.bcust_itt_letter_text_at, business_sellers.bs_tl_user_incl, business_sellers.bs_tl_date_incl
	from business_sellers 		
	Inner Join bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id 
	Inner Join business ON business_sellers.bs_business_id = business.bsn_id  
	Right JOIN person_responsible_detail ON  business_sellers.bs_business_id = person_responsible_detail.prd_bsn_id And 
...
if (!empty($userData)) {
	...
	if ($sql2) {
		...
		foreach ($setdata as $k => $v) {
			...
			if($inclusions_task_tracker[$v['bsn_id']]['itt_renovation_calculator_link'] != '') {
				$renovation_calculator = '<a href="' . $inclusions_task_tracker[$v['bsn_id']]['itt_renovation_calculator_link'] . '" target="_blank">View</a><br><br>';
			}
			$renovation_calculator .= '<a href="' . BASE_URL . 'inclusions_task_tracker.update_renovation_calculator_link/bsn_id/' . $v['bsn_id'] . '" class="various">Edit</a>';
			$setdata2[$k]['renovation_calculator'] = $renovation_calculator;
			
			$wheredatacomesfrom = '';
			if($inclusions_task_tracker[$v['bsn_id']]['itt_where_data_comes_from'] != '') {
				$wheredatacomesfrom = '<a href="' . $inclusions_task_tracker[$v['bsn_id']]['itt_where_data_comes_from'] . '" target="_blank">View</a><br><br>';
			}
			$wheredatacomesfrom .= '<a href="' . BASE_URL . 'inclusions_task_tracker.wheredatacomesfrom_link/bsn_id/' . $v['bsn_id'] . '" class="various">Edit</a>';
			$setdata2[$k]['wheredatacomesfrom'] = $wheredatacomesfrom;
			...
			$sql_142 = "SELECT doc_file_name FROM document_check_list where doc_bsn_id = " . $v['bsn_id'] . " and doc_name_id = 142";
			$data_142 = $fwDb->queryOne($sql_142);
			$setdata2[$k]['doc_142'] = $data_142['doc_file_name'];
			
			$sql_283 = "SELECT doc_file_name FROM document_check_list where doc_bsn_id = " . $v['bsn_id'] . " and doc_name_id = 283";
			$data_283 = $fwDb->queryOne($sql_283);
			$setdata2[$k]['doc_283'] = $data_283['doc_file_name'];
			
			$sql_457 = "SELECT doc_file_name FROM document_check_list where doc_bsn_id = " . $v['bsn_id'] . " and doc_name_id = 457";
			$data_457 = $fwDb->queryOne($sql_457);
			$setdata2[$k]['doc_457'] = $data_457['doc_file_name'];
			
			$sql_bsn_type = "SELECT project_type.pt_name FROM business Left Join project_type on project_type.pt_id = business.bsn_type where business.bsn_id = 3486";
			$data_bsn_type = $fwDb->queryOne($sql_bsn_type);
			$setdata2[$k]['data_bsn_type'] = $data_bsn_type['pt_name'];
			...
			$setdata2[$k]['bsn_itt_proj_type'] = $v['bsn_itt_proj_type'];
		}
...
$sql_project_type = "Select * from inclusions_task_tracker_project_type";
$fwViewData['project_type'] = $fwDb->query($sql_project_type);
