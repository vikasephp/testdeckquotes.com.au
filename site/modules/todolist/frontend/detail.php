...
if(!empty($submit))
{
	...
	if($_FILES['docs']['name'])
		...
		$insertFile = insertFile($drive, $docfile, $docfile, $parentId, $file_type, $_FILES['docs']['tmp_name']);
									$docs['btdl_docs_path']= $insertFile['id'];
									$docs['btdl_docs_name']= $docfile;							
									$docs['btdl_docs_type']= $file_type;
									$docs['btdl_docs_status']= 1;						
			}
			if($this_id)
				{
...
if($docs['btdl_docs_path']){ $tabledoc->insertRow($docs); }
if($this_id){	
