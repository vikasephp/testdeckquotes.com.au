...
if(!empty($submit))
	...
	LEFT JOIN users as resusers ON resusers.user_id = ".$TABLE.".btdl_responcible_id WHERE ".$TABLE.".btdl_status =1 ".$order;}
				...
				if($doc_files)
					{
						foreach($doc_files as $kf => $vf)
								{
								if($vf['btdl_docs_type']){$vf['iconpath'] = get_icon_path($vf['btdl_docs_type'], $vf['btdl_docs_name']);}
								$filedata[] = $vf;
								}
					}
