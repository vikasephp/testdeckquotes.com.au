...
<table  width="100%">
  <tr>
    <td><div style="float:left;">
        <input type="button" name="Show Archives" value="Show Archives" onclick="document.location.href='{{$BASE_URL}}todolist.archives'" />
      </div>
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    ...
    <form name="mlist" method="post">
      ...
      <td><span style="color:#FF0000;">{{if $item.btdl_urgency == 1}}Extreme {{elseif $item.btdl_urgency == 2}} High {{elseif $item.btdl_urgency == 3}} Moderate {{elseif $item.btdl_urgency == 4}}Low{{/if}}</span>
        ...
        <td>{{if $item.btdl_due_date == '0000-00-00 00:00:00' OR $item.btdl_due_date == '0000-00-00' }}NA{{else}}{{$item.btdl_due_date|date_format:"%d/%m/%Y"}}{{/if}}</td>
        ...
        <td><div style="float:left;">
            <table cellpadding="0" cellspacing="5" id="doc_list">
              <tr>{{counter start=1 print=false assign="count"}}
                ...
                <td width="34">{{if $itemfiles.btdl_docs_status == 0}}<a href="{{$BASE_URL}}{{$FILE_PATH}}files/business_todo_documents/{{$itemfiles.btdl_docs_path}}" target="_blank" title="Start download">{{else}}<a href="{{$BASE_URL}}{{$XFA.open}}/fileid/{{$itemfiles.btdl_docs_path}}" title="View/Edit" class="shofile">{{/if}}<img src="{{$itemfiles.iconpath}}" width="20" /></a><br />{{if $itemfiles.btdl_docs_status == 0}}<a href="{{$BASE_URL}}{{$FILE_PATH}}files/business_todo_documents/{{$itemfiles.btdl_docs_path}}" target="_blank" title="Start download">{{else}}<a href="{{$BASE_URL}}{{$XFA.open}}/fileid/{{$itemfiles.btdl_docs_path}}" title="View/Edit" class="shofile">{{/if}}{{$itemfiles.btdl_docs_name|truncate:"9"}}</a></td>
                 {{if $count % 3 == 0}}</tr>
              <tr> {{/if}}
                 {{counter}}
                 {{/foreach}} </tr>
          ...
          <div style="float:right;">
            <table>
              <tr>
                <td valign="top">
            <a href="{{$BASE_URL}}{{$XFA.upload}}/{{$ID}}/{{$item.$ID}}" title="Quick Add New" class="addfile"><img src="{{$BASE_URL}}images/add_files.png" width="20" /></a><br />
            <a href="{{$BASE_URL}}{{$XFA.upload}}/{{$ID}}/{{$item.$ID}}" title="Quick Add New" class="addfile">New</a></td>
              </tr>
            </table>
          </div></td>
        <td valign="top"><a href="{{$BASE_URL}}{{$XFA.detail}}/{{$ID}}/{{$item.$ID}}" class="various" title="Edit"> 
