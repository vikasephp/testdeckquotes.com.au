...
$sql_2 = "SELECT  business_sellers.bs_business_id, business_sellers.bs_customers_id, bus_customers.bcust_fname, bus_customers.bcust_lname,
         bus_customers.bcust_misc_email1, business.bsn_id, business.bsn_name from business_sellers 		
		 Inner Join bus_customers ON business_sellers.bs_customers_id = bus_customers.bcust_id 
		 Inner Join business ON business_sellers.bs_business_id = business.bsn_id where business.bsn_id = ".$bsn_id;

$projdata =  $fwDb->queryOne($sql_2);
$fwViewData['bsn_name'] = $projdata['bsn_name'];
$fwViewData['bcust_misc_email1'] = $projdata['bcust_misc_email1'];
...
if(!empty($bsn_id))
{
	...
	$fwViewData['subject'] = stripslashes($emldata['eml_subject']) .' - '.stripslashes($projdata['bsn_name']) . ' ('. $emldata['eml_code']. ')';
		...
		if(!empty($detail['to2']))
		{
			...
			foreach($todetail as $vto)
			{
				...
				{	
					...
					if(!empty($doclogattach)) { $logDetail2['elog_doc_attached'] = implode(", ",$doclogattach); }
