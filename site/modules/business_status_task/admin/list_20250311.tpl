...
Select Position Responsible
	<select name="pos" onchange="javascript:submit_form2();" >
    	...
    	{{foreach from=$positions item=item2 key=key2}}
        	<option value="{{$item2.p_id}}" {{if $pos_id == $item2.p_id}} selected="selected" {{/if}}>{{$item2.p_name}}</option>
        {{/foreach}}
    </select>
</form>
</div>
    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
              ...
              <th class="topmenu_list"  valign="middle" width="5%"><div align="center">Status</div></th>   
