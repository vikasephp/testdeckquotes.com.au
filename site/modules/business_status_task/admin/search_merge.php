...
if(!empty($data['bst_business_status_id'])) { $bystatus = " and bst_business_status_id = ". $data['bst_business_status_id']; } 
if(!empty($data['bst_task_id'])) { $bytid = " and bst_task_id = ". $data['bst_task_id']; } 
$wh = " where 1 ";
$where = $wh . $bypr . $bystatus . $bytid;

$sql = " Select  busness_status_task.*, positions.p_name, busness_status.st_name from  busness_status_task
         Inner join positions on  busness_status_task.bst_posid = positions.p_id 
		 Inner join busness_status on busness_status_task.bst_business_status_id = busness_status.st_id
		 " . $where . " Order by busness_status_task.bst_order ";
