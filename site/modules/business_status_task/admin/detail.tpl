...
<div align="center">
    ...
    <form name="detail" method="post" action="" enctype="multipart/form-data">
        <table id="list-table" width="90%">
            ...
            <tr>
                ...
                <td width="80%">
                    <select name="data[bst_business_status_id]" class="lrg">
                        ...
                        <option value="{{$item.st_id}}" {{if $detail.bst_business_status_id==$item.st_id OR
                            $current_bst_business_status_id==$item.st_id}} selected="selected" {{/if}}>{{$item.st_name}}
                        </option>{{/foreach}}
                    </select>
                </td>
            </tr>
            <tr>
                <th width="20%">Assign Project Types to add tasks list:</th>
                <td width="80%">
                    <div class="bsearch_scroll_div1">
                        <ul style="margin: 0; padding: 0;">
                            {{foreach from=$busness_types item=item key=key}}
                            <li>{{$item.pt_name}}
                                <div><input type="radio" name="data[bst_business_types][{{$item.pt_id}}]" value="1"
                                        {{foreach from=$detail.bst_business_types item="item1" key="key1" }} {{if
                                        $key1==$item.pt_id && $item1==1}} checked="checked" {{/if}}
                                        {{/foreach}} />&nbsp;Yes&nbsp;
                                    &nbsp;<input type="radio" name="data[bst_business_types][{{$item.pt_id}}]" value="0"
                                        {{foreach from=$detail.bst_business_types item="item1" key="key1" }} {{if
                                        $key1==$item.pt_id && $item1==0}} checked="checked" {{/if}}
                                        {{/foreach}} />&nbsp;No</div>
                            </li>
                            {{/foreach}}
                        </ul>
            ...
            <tr>
                <th>Task Name:</th>
                <td><input type="text" name="data[bst_task_name]" value="{{$detail.bst_task_name}}" class="xxlrg" />
                </td>
            </tr>
            <tr>
                <th>What Text:</th>
                <td>
                    <table id="none-table22">
                        <tr>
                            <td><textarea cols="106" name="data[bst_what_text]" rows="7" class="tinymce">{{$detail.bst_what_text|stripslashes}}</textarea></td>
                        </tr>
                    </table>
                </td>
            </tr>
            <tr>
                <th>Planning Order: (Only for Planning Approval)</th>
                <td><input type="text" name="data[bst_planning_order]" value="{{$detail.bst_planning_order}}" class="xxlrg" />
                </td>
            </tr>
            <tr>
                <th>Instructions:</th>
                <td>
                    <table id="none-table22">
                        <tr>
                            <td><textarea cols="106" name="data[bst_instruction]" rows="7"
                                    class="tinymce">{{$detail.bst_instruction|stripslashes}}</textarea></td>
                        </tr>
                    </table>
                </td>
            </tr>
        ...
        <td>
            ...
            <tr>
                ...
                <th><a href="javascript:appendRowTaskLink2({{$detail.bst_id}})" value="Add New">Add New MSTEAM
                        Procedure</a>:</th>
                <td>
                    ...
                    <div id="lptask2">
                        ...
                        <span id="lpt{{$item9.btp_id}}">
                            <a href="javascript:delete_link_proc_link({{$item9.btp_id}})" value="Delete Task Procedure"
                                title="Delete">
                                <img style="margin-top:5px;" src="{{$BASE_URL}}css/admin/images/deletecross.png" /></a>
                        ...
                        <datalist id='docstask2'>
                            ...
                            <option value="DID{{$item4.dh_uid}} - {{$item4.dh_title}}">DID{{$item4.dh_uid}} -
                                {{$item4.dh_title}} </option>
                            {{/foreach}}
                        </datalist>
            ...
            <tr>
                ...
                <td>
                    <select name="data[bst_status]" class="lrg">
                        ...
                        <option value="0" {{if $detail.bst_status=='0' }} selected="selected" {{/if}}>Deactivate
                        </option>
                    </select>
                </td>
            </tr>
            <tr>
                <th>Task UID:</th>
                <td><input type="text" name="data[bst_task_id]"
                        value="{{if $detail.bst_task_id != 0}}{{$detail.bst_task_id}}{{else}}{{$detail.bst_id}}{{/if}}"
                        class="lrg" readonly="readonly" />
            ...
            <tr>
                ...
                <td width="80%">
                    <select name="data[bst_posid]" class="lrg">
                        ...
                        <option value="{{$item.p_id}}" {{if $detail.bst_posid==$item.p_id}} selected="selected" {{/if}}>
                            {{$item.p_name}}</option>
                        {{/foreach}}
                    </select>
                </td>
            ...
            <tr>
                <th>Video Link</th>
                <td><input type="text" name="data[bst_video_link]" value="{{$detail.bst_video_link}}" class="xxlrg" />
                </td>
            </tr>
            ...
            <tr>
                <th>Auto Email </th>
                <td><input type="radio" name="data[bst_auto_email_text]" value="1" {{if $detail.bst_auto_email_text eq 1
                        }} checked="checked" {{/if}} /> Yes &nbsp; &nbsp; &nbsp;
                    <input type="radio" name="data[bst_auto_email_text]" value="0" {{if $detail.bst_auto_email_text eq 0
                        }} checked="checked" {{/if}} /> No
                    &nbsp; &nbsp;
                    <a href="{{$BASE_URL}}business_status_taskAdmin.auto_email/bst_id/{{$detail.bst_id}}"
                        target="_blank">Add/Update Email Text</a>
                </td>
            </tr>

            <tr>
                <th>Auto Text</th>
                <td><input type="radio" name="data[bst_auto_sms_text]" value="1" {{if $detail.bst_auto_sms_text eq 1 }}
                        checked="checked" {{/if}} /> Yes &nbsp; &nbsp; &nbsp;
                    <input type="radio" name="data[bst_auto_sms_text]" value="0" {{if $detail.bst_auto_sms_text eq 0 }}
                        checked="checked" {{/if}} /> No
                    &nbsp; &nbsp;
                    <a href="{{$BASE_URL}}business_status_taskAdmin.auto_msg/bst_id/{{$detail.bst_id}}"
                        target="_blank">Add/Update Auto Message</a>
                </td>
            </tr>
            <tr>
                <th>Order</th>
            ...
            <tr>
                <td colspan="2" align="center">
                    <h3 class="page-title">Sub Tasks</h3>
                </td>
            </tr>
            ...
            <tr>
                <td colspan="2">
                    ...
                    <div id="div">
                        ...
                        <a href="{{$BASE_URL}}business_status_taskAdmin.delete_subtask/bsst_id/{{$item2.bsst_id}}/bst_id/{{$bst_id}}"
                            title="Delete">
                            <img src="{{$BASE_URL}}css/admin/images/deletecross.png" /></a>
                        <input type="text" value="{{$item2.bsst_task_name}}" size='100' id="st{{$item2.bsst_id}}"
                            onkeyup="update_subtask({{$item2.bsst_id}});" />
                        ...
                        <div id="lp{{$item2.bsst_id}}">
                            ...
                            <div
                                style="background: #FFC; width:78%; padding:5px; border:1px solid #6FF; margin-left:20px;">
                                ...
                                <span id="lpm{{$itemlp.bstlp_id}}">
                                    <a href="javascript:delete_link_proc_subtask({{$itemlp.bstlp_id}})"
                                        value="Delete Procedure" title="Delete">
                                        <img style="margin-top:5px;"
                                            src="{{$BASE_URL}}css/admin/images/deletecross.png" /></a>
                                    <a href="{{$BASE_URL}}files/template_documents_hub/{{$itemlp.bstlp_doc_name}}"
                                        style="color:#06C;">{{$itemlp.bstlp_option_text}}</a> &nbsp;
                                    <br />
                                </span>
                                {{/if}}
                                {{/foreach}}
                                ...
                                <span id="lpm2{{$itemlp.bstlp_id}}">
                                    <a href="javascript:delete_link_proc_subtask_ms({{$itemlp.bstlp_id}})"
                                        value="Delete Procedure" title="Delete">
                                        <img style="margin-top:5px;"
                                            src="{{$BASE_URL}}css/admin/images/deletecross.png" /></a>
                                    <a href="{{$BASE_URL}}files/template_documents_hub/{{$itemlp.bstlp_doc_name}}"
                                ...
                                <a href="javascript:appendRowLp({{$item2.bsst_id}})" value="Add New"
                                    title="Add New SubTask">
                                    <img style="margin-top:5px;" src="{{$BASE_URL}}css/admin/images/addnew.png" /></a>
                                <br />

                                Click Below + to add Msteam Link <br />
                                <a href="javascript:appendRowLpsub({{$item2.bsst_id}})" value="Add New"
                                    title="Add New SubTask">
                                    <img style="margin-top:5px;" src="{{$BASE_URL}}css/admin/images/addnew.png" /></a>
                                <br />
                            </div>
                            ...
                            <datalist id='docs'>
                                {{foreach from=$template_docdetail key="key3" item="item3"}}
                                <option value="{{$item3.dhd_name2}} - {{$item3.dh_title}} - Procedure">
                                    {{$item3.dhd_name2}} - {{$item3.dh_title}} - Procedure </option>
                                {{/foreach}}
                            </datalist>
            ...
            <td> <input type="text" name="data[bst_last_audit]" value="{{$detail.bst_last_audit}}"
                    class="w16em dateformat-d-ds-m-ds-Y" id="demo4" maxlength="25" />
                ...
                <tr>
                    <td colspan="2" style="text-align:center;">
                        ...
                        <input type="button" name="btnCancelDetail" value="Cancel"
                            onclick="document.location.href='{{$BASE_URL}}{{$XFA.list}}'" class="vsml" />
                        {{if $detail.bst_id}}<input type="submit" name="btnassignTasks" value="Assign Tasks"
                            class="lrg" />{{/if}}
                    </td>
                </tr>
        </table>
