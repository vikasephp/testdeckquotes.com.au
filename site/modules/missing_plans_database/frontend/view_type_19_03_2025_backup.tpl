...
<table id="list-table" width="99%">
    ...
    <tr>
        ...
        <td>{{$item.mpd_type}}</td>
        <td>
            <a href="{{$BASE_URL}}{{ $BASEFOLDER }}.add_type/mpd_id/{{$item.mpd_id}}" class="various" title="Edit">
                <img style="height:20px; width:20px;" src="{{$BASE_URL}}css/admin/images/edit.png" /></a>
            <a href="{{$BASE_URL}}{{ $BASEFOLDER }}.delete_type/mpd_id/{{$item.mpd_id}}" onclick="javascript:if(!confirm('Are you sure want to delete this Type?')) return false;" title="Delete">
...
<input type="button" name="btnCancelDetail" value="Close" onclick="javascript:closepop();" class="vsml" />
	...
	<script type="text/javascript" language="javascript" >
        var frmvalidator  = new Validator("detail");
			...
			frmvalidator.addValidation("missing_plans_database_type[al_item]","req", "Please specify item.");
