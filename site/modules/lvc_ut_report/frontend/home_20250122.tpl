...
&nbsp; &nbsp;<input type="button" name="lvc_type_options" value="LVC Type Admin" onclick="javascript:add_lvc_type();" /> 
    ...
    <tr bgcolor="#FFFFFF">
        ...
        <form name ="recmet2" method="post" action="">
          <select name="{{$TABLE}}[lv_type]" onChange="update_type({{$item.lv_id}}, this.value)">
    		 ...
    		 <option value="{{$item2.lt_id}}" {{if $item2.lt_id eq $item.lv_type}} selected="selected" {{/if}}>{{$item2.lt_option}}</option>
    		 {{/foreach}}
          </select>
        </form>
       ...
       <td>
        <form name ="recmet2" method="post" action="">
          <select name="lvc_type[{{$item.lv_id}}]" onChange="update_lvc_type({{$item.lv_id}}, this.value)"/>
        			
                <option value="0" {{if $item.sp_www eq -1}} selected="selected" {{/if}}>Please Select </option>
                {{foreach from = $lvcdetail key="key3" item="item3"}}
                <option value="{{$item3.lv_id}}" {{if $item3.lv_id eq $item.lv_lvc_type}} selected="selected" {{/if}}>{{$item3.lv_option}}</option>
                {{/foreach}}
        ...
        <td>{{$item.lv_fee }} </td>
         ...
         {{if $item.doc_file_name}} 
            <a href="/lvc_ut_report.download_content?file_name={{$item.doc_file_name}}&module_name=lvc_ut_report.home" target="_blank">
         Download</a>{{/if}}
         </td>
        
        <td>{{if $item.doc_file_name_707}} 
            <a href="/lvc_ut_report.download_content?file_name={{$item.doc_file_name_707}}&module_name=lvc_ut_report.home" target="_blank">
         Download</a>{{/if}} </td>
        
        <td> <form name ="recmet3" method="post" action="">
     	<input type="hidden" name="emailed[{{$item.lv_id}}]" value="0" />
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="emailed[{{$item.lv_id}}]" value="1"  onclick="this.form.submit();" {{if $item.lv_emailed_client eq 1 }} checked="checked" {{/if}} />
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="courier[{{$item.lv_id}}]" value="1"  onclick="this.form.submit();" {{if $item.lv_courier_client eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
  	...
  	<label class="switch">
<input class="switch-input" type="checkbox"  name="hide[{{$item.lv_id}}]" value="1"  onclick="this.form.submit();" {{if $item.lv_hide eq 1 }} checked="checked" {{/if}} />
        <span class="switch-label" data-on="Yes" data-off="No"></span> 
        <span class="switch-handle"></span> <br />
       </label> 
       </form>  
