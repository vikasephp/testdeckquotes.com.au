...
window.location.href = "{{$BASE_URL}}partners_agreements.viewtype";
</script>
{{/if}}
...
<table id="list-table" width="100%">
    <input type="hidden" name="type[it_id]" value="{{$detail.it_id}}" />
   ...
   {{if $error}} <tr><th colspan="2" style="color:#FF0000;">{{$error}}</th></tr>{{/if}}
    ...
    <tr>
     <th>Invoice Type </th>
     <td>
          <input type="text"  name="type[it_type]" style ="width:700px" value="{{$detail.it_type}}" />
      </td>
    </tr>
