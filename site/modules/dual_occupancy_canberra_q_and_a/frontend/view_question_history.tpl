...
<div align="center" style="min-height:350px; z-index:9999999;">
	<h3 class="page-title"><br />View Question History</h3>
	<form name="detail" method="post" action="" enctype="multipart/form-data">
		<table id="list-table" width="99%">
			...
			{{if $data}}
				...
				<tr>
					<td>{{$ques_count}}</td>
					<td>{{$itemN.docqh_question}}</td>
					<td>
						{{$itemN.docqh_question_updated_by}}
					</td>
					<td>{{$itemN.docqh_question_updated_at|date_format:"%d-%m-%Y"}}</td>
				</tr>
				{{assign var="ques_count" value=$ques_count+1}}
				{{/foreach}}
			{{else}}
				<tr>
					<td colspan="5" style="text-align:center;">No Question History Available</td>
				</tr>
			{{/if}}
		</table>
		<br>
