    ...
    <table id="list-table" class="nav-back" width="100%">
        <thead>
            <tr>
				...
				<th class="topmenu" align="center" valign="middle" width="10%">Slug</th>
...
<tbody>
 {{foreach from=$list key="key" item="item"}}
 	 ...
 	 <tr {{if $item.docqa_qed eq 1 }} bgcolor='{{cycle values="#D3E8D6,WHITE" advance=true}}' {{else}} bgcolor="#FF0000" {{/if}}>
		...
		<td>{{if $item.docqa_public_rank gt 0}}{{$item.docqa_public_rank}}{{/if}}</td>
		<td>
			{{if $item.docqa_slug}}
		...
		<td style="color:#000;">
			<div class="show-read-more">{{$item.docqa_question}}</div>
		...
		<td style="color:#000;">
			<div class="show-read-more">{{$item.docqa_answer}}</div>
