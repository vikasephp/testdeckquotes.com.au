...
{
	var viewurl = "{{$BASE_URL}}project_plans_qa_checklist_report.view_procedure"+ "/random/" + Math.random();	
  ...
  <table id="doclist-table" class="nav-back" width="100%">
    <thead>
      <tr>
        ...
        <th class="topmenu" align="center" valign="middle" width="15%">Checklist Number</th>
        <th class="topmenu" align="center" valign="middle" width="15%">User </th>
        <th class="topmenu" align="center" valign="middle" width="3%">Download</th>
    ...
    <tbody>
      ...
      <tr bgcolor="{{cycle values="#D3E8D6,WHITE" advance=true}}">
        ...
        <td>{{$item.qa_cno}}</td>
        <td>{{$item.qa_user}}</td>
        <td style="text-align:center;" ><a href="{{$BASE_URL}}{{$XFA.home}}/word/{{$item.qa_cno}}/bsn_id/{{$item.bsn_id}}" title="Generate Word Report"> 
             <img  src="{{$BASE_URL}}css/admin/images/word.png"/></a></td>
        <td>      
        
        <form name ="recmet" method="post" action="">
        <input type="hidden" name="hide[{{$item.qa_id}}]" value="0" />
  	<label class="switch">
        <input class="switch-input" type="checkbox"  name="hide[{{$item.qa_id}}]" value="1"  onclick="this.form.submit();" {{if $item.qa_hide eq 1 }} checked="checked" {{/if}} />
        ...
        <td><a href="{{$BASE_URL}}project_plans_qa_checklist_report.report_edit/cno/{{$item.qa_cno}}/bsn_id/{{$item.bsn_id}}"  title="Edit"> 
